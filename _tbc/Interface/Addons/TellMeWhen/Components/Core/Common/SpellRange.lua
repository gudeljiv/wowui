...
if C_Spell.EnableSpellRangeCheck then
    -- WoW 11.1.5+
    SpellRange:RegisterEvent("SPELL_RANGE_CHECK_UPDATE")
    ...
    SpellRange:SetScript("OnEvent", function(self, event, spellIdentifier, inRange, checksRange)
        if event == "SPELL_RANGE_CHECK_UPDATE" then
            CachedRange[spellIdentifier] = { inRange, checksRange }
    ...
    function SpellRange.HasRangeEvents(spell)
        if not EnabledSpells[spell] then
            EnabledSpells[spell] = true
            C_Spell.EnableSpellRangeCheck(spell, true)
        end
        return true
    end

elseif C_ActionBar and C_ActionBar.EnableActionRangeCheck then
