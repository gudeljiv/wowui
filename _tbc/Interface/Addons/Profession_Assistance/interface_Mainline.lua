...
-- Minimap stuff --
-------------------
function ProfessionAssistance_OnAddonCompartmentClick(addonName, buttonName)
	PA_Panel:Show();
end
...
PA_InterfaceFrame:SetScript("OnEvent", function(self,event,...)
	...
	local createTab = function(parent, index, name)
		local tab = CreateFrame("Button", "$parentTab"..index, parent, "TabSystemButtonTemplate");
		tab.id = index;
		if index == 1 then tab:SetPoint("TOPLEFT",5,24); else tab:SetPoint("LEFT", "$parentTab"..index-1, "RIGHT", -5, 0); end
		tab:SetScript("OnClick", function(self) setPanel(self:GetParent(), self.id); end);
