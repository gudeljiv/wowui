...
local function Add_Waylaid_Supplies_Tooltip(tooltip, itemID)
	...
	if crateData then
		...
		if reputationAmounts then
			...
			local reputationStatus = (itemLevel < threshold) and "\124cFFFF0000No longer grants rep with "
				or colorCode .. "Will grant reputation with "

			-- Checks the player's faction to display the Supply Crate faction
			local factionName = (UnitFactionGroup("player") == "Horde") and "Durotar Supply and Logistics"
				or "Azeroth Commerce Authority"
...
local function DisplayAssociatedItemTooltip(tooltip, fillItem, waylaidCrates)
	...
	if itemLink then
		...
		if TSM_API then
			...
			tooltip:AddDoubleLine(
				"\124cFFADD8E6Min Buyout: " .. GetMoneyString(minBuyout) .. " each.",
				"\124cFFADD8E6" .. GetMoneyString(filledMinBuyout) .. " filled."
			)
			tooltip:AddDoubleLine(
				"\124cFFADD8E6Market Value: " .. GetMoneyString(marketValue) .. " each.",
				"\124cFFADD8E6" .. GetMoneyString(filledMarketValue) .. " filled."
			)
		...
		if C_AddOns.IsAddOnLoaded("Auctionator") then
			...
			if auctionatorMinBuyout > 0 then
				...
				tooltip:AddDoubleLine(
					"\124cFFFFD700Min Buyout: " .. GetMoneyString(auctionatorMinBuyout) .. " each.",
					"\124cFFFFD700" .. GetMoneyString(filledAuctionatorMinBuyout) .. " filled."
				)
			else
				tooltip:AddLine("\124cFFFFA500Auctionator prices for " .. itemName)
