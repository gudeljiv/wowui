...
ItemRack.BracketInfo = {
	["TOP"] = { 36, 12, 0.25, 0.75, 0, 0.25 }, -- bracket construction info
	["BOTTOM"] = { 36, 12, 0.25, 0.75, 0.75, 1 }, -- cx,cy,left,right,top,bottom
	["LEFT"] = { 12, 36, 0, 0.25, 0.25, 0.75 },
	["RIGHT"] = { 12, 36, 0.75, 1, 0.25, 0.75 },
	["TOPLEFT"] = { 12, 12, 0, 0.25, 0, 0.25 },
	["TOPRIGHT"] = { 12, 12, 0.75, 1, 0, 0.25 },
	["BOTTOMLEFT"] = { 12, 12, 0, 0.25, 0.75, 1 },
	["BOTTOMRIGHT"] = { 12, 12, 0.75, 1, 0.75, 1 },
}
...
function ItemRack.InitButtons()
	...
	ItemRack.CreateTimer("ButtonsDocking", ItemRack.ButtonsDocking, 0.2, 1) -- (repeat) on while buttons docking
	ItemRack.CreateTimer("MenuDocking", ItemRack.MenuDocking, 0.2, 1) -- (repeat) on while menu docking
	...
	ItemRack.CreateTimer("ReflectClickedUpdate", ItemRack.ReflectClickedUpdate, 0.2, 1)
...
function ItemRack.UpdateDisableAltClick()
	if not InCombatLockdown() then
		for i = 0, 19 do
			_G["ItemRackButton" .. i]:SetAttribute(
				"alt-slot*",
...
function ItemRack.AddButton(id)
	...
	if ItemRack.NewAnchor and ItemRackUser.Buttons[ItemRack.NewAnchor] then
		...
		button:SetPoint(
			"LEFT",
			"ItemRackButton" .. ItemRack.NewAnchor,
			"RIGHT",
			dockinfo.xoff * (ItemRackUser.ButtonSpacing or 4),
			dockinfo.yoff * (ItemRackUser.ButtonSpacing or 4)
		)
	else
...
function ItemRack.ButtonsDocking()
	...
	for i in pairs(ItemRackUser.Buttons) do
		...
		if
			near(button:GetLeft(), dock:GetRight())
			and (near(button:GetTop(), dock:GetTop()) or near(button:GetBottom(), dock:GetBottom()))
			and ItemRack.LegalDock(buttonID, i)
		then
			ItemRack.MoveBracket("Main", "LEFT", button, "TOPLEFT")
			ItemRack.MoveBracket("Menu", "RIGHT", dock, "TOPRIGHT")
			ItemRack.Docking.Side = "LEFT"
		elseif
			near(button:GetRight(), dock:GetLeft())
			and (near(button:GetTop(), dock:GetTop()) or near(button:GetBottom(), dock:GetBottom()))
			and ItemRack.LegalDock(buttonID, i)
		then
		...
		elseif
			near(button:GetTop(), dock:GetBottom())
			and (near(button:GetLeft(), dock:GetLeft()) or near(button:GetRight(), dock:GetRight()))
			and ItemRack.LegalDock(buttonID, i)
		then
		...
		elseif
			near(button:GetBottom(), dock:GetTop())
			and (near(button:GetLeft(), dock:GetLeft()) or near(button:GetRight(), dock:GetRight()))
			and ItemRack.LegalDock(buttonID, i)
		then
...
function ItemRack.StopMovingButton(self)
	...
	if ItemRack.Docking.Side then
		...
		ItemRack.ButtonMoving:SetPoint(
			ItemRack.Docking.Side,
			"ItemRackButton" .. ItemRack.Docking.To,
			ItemRack.OppositeSide[ItemRack.Docking.Side],
			dockinfo.xoff * (ItemRackUser.ButtonSpacing or 4),
			dockinfo.yoff * (ItemRackUser.ButtonSpacing or 4)
		)
...
function ItemRack.ConstructLayout()
		...
		if ItemRackUser.Buttons[i].needsDrawn and not ItemRackUser.Buttons[i].DockTo then
			...
			if ItemRackUser.Buttons[i].Left then
				button:SetPoint(
					...
					"UIParent",
					"BOTTOMLEFT",
					ItemRackUser.Buttons[i].Left,
					ItemRackUser.Buttons[i].Top
				)
			else
				button:SetPoint("CENTER", "UIParent", "CENTER")
			end
	...
	while not done do
		...
		for i in pairs(ItemRackUser.Buttons) do
			if
				ItemRackUser.Buttons[i].needsDrawn
				and not ItemRackUser.Buttons[ItemRackUser.Buttons[i].DockTo].needsDrawn
			then -- if this button's DockTo is already drawn
				--				button = ItemRack.CreateButton(ItemRackUser.Buttons[i].name,i,ItemRackUser.Buttons[i].type)
				button = _G["ItemRackButton" .. i]
				ItemRackUser.Buttons[i].needsDrawn = nil
				button:ClearAllPoints()
				...
				button:SetPoint(
					ItemRackUser.Buttons[i].Side,
					"ItemRackButton" .. ItemRackUser.Buttons[i].DockTo,
					ItemRack.OppositeSide[ItemRackUser.Buttons[i].Side],
					dockinfo.xoff * (ItemRackUser.ButtonSpacing or 4),
					dockinfo.yoff * (ItemRackUser.ButtonSpacing or 4)
				)
				button:Show()
...
function ItemRack.OnEnterButton(self)
	...
	if
		ItemRack.IsTimerActive("ButtonsDocking")
		or (not IsShiftKeyDown() and ItemRackSettings.MenuOnShift == "ON")
		or ItemRackSettings.MenuOnRight == "ON"
	then
...
function ItemRack.WriteCooldown(where, start, duration)
	...
	elseif cooldown < 3 and not where:GetText() then
		...
		where:SetText(
			(cooldown < (ItemRackSettings.Cooldown90 == "ON" and 90 or 60) and math.floor(cooldown + 0.5) .. " s")
				or (cooldown < 3600 and math.ceil(cooldown / 60) .. " m")
				or math.ceil(cooldown / 3600) .. " h"
		)
	end
end
