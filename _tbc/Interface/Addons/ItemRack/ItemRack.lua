...
-- Hack for TBC Anniversary Pre-Patch (C_AddOns compatibility)
...
_G[addonName] = addon
-- Classic / TBC compatibility layer for container APIs (Anniversary clients vary)
-- Some clients have C_Container but NOT the old global GetContainer* functions.
-- ItemRack expects the old globals, so we recreate them if missing.

-- Ensure C_Container exists (if you already added this above, keep only one copy)
...
-- Recreate missing GLOBALS using C_Container (for ItemRack / older addons)
if C_Container and not GetContainerNumSlots then
	function GetContainerNumSlots(bag)
		return C_Container.GetContainerNumSlots(bag)
	end
end

if C_Container and not GetContainerItemLink then
	function GetContainerItemLink(bag, slot)
		return C_Container.GetContainerItemLink(bag, slot)
	end
end

if C_Container and not GetContainerItemInfo then
	function GetContainerItemInfo(bag, slot)
		...
		-- Convert retail-style info table into classic-style multiple returns
		...
		return texture, count, locked, quality, readable, lootable, link, filtered, noValue, itemID
	end
end

-- Classic / TBC compatibility for Retail APIs
if not C_Container then
	C_Container = {}
...
ItemRackUser = {
	...
	MenuScale = 0.85, -- scale of the menu in relation to docked buttons
...
ItemRack.TooltipInfo = {
	...
	{
		"ItemRackButtonMenuOptions",
		"Options",
		"Open Options window to change settings, configure sets or auto queues.",
	},
	...
	},
	{
		...
		"Hide",
		"Hide this set in menus. (Equivalent of Alt+clicking the set in the menu)",
	},
	{
	...
	},
	{
		...
		"Bind Key to Set",
		"This will let you bind a key or key combination to equip a set.",
	},
...
do
	...
	function ItemRack.ListSetsHavingItem(tooltip, id, exact)
		...
		for name in pairs(data) do
			tooltip:AddDoubleLine("ItemRack Set: ", name, 0, 0.6, 1, 0, 0.6, 1)
			data[name] = nil
		end
		tooltip:Show()
	end
...
function ItemRack.InitCore()
	...
	ItemRack.CreateTimer("MenuMouseover", ItemRack.MenuMouseover, 0.25, 1)
	...
	ItemRack.CreateTimer("LocksChanged", ItemRack.LocksChanged, 0.2)
	ItemRack.CreateTimer("DelayedCombatQueue", ItemRack.DelayedCombatQueue, 0.1)
...
function ItemRack.GetIRString(inputString, baseid, regular)
	return string.match(
		inputString or "",
		(
			baseid and (regular and ItemRack.iSPatternBaseIDFromRegular or ItemRack.iSPatternBaseIDFromIR)
			or ItemRack.iSPatternRegularToIR
		)
	) or 0
end
...
function ItemRack.PlayerCanWear(invslot, bag, slot)
	...
	for i = 2, ItemRackTooltip:NumLines() do
		...
		if
			(ItemRack.IsRed("Left" .. i) or ItemRack.IsRed("Right" .. i))
			and not string.find(txt, ItemRack.DURABILITY_PATTERN)
			and not string.match(txt, ItemRack.REQUIRES_PATTERN)
		then
			return nil
		end
	end
...
function ItemRack.BuildMenu(id, menuInclude, masqueGroup)
	...
	local showButtonMenu = (ItemRackButtonMenu and ItemRack.menuMovable)
		and (IsAltKeyDown() or ItemRackUser.Locked == "OFF")
	...
	if id < 20 then
		...
		for i = 0, 4 do
			for j = 1, GetContainerNumSlots(i) do
				...
				if
					ItemRack.SlotInfo[id][equipSlot]
					and ItemRack.PlayerCanWear(id, i, j)
					and (ItemRackSettings.HideTradables == "OFF" or ItemRack.IsSoulbound(i, j))
				then
	...
	if #ItemRack.Menu < 1 then
		...
		local col, row, xpos, ypos =
			0, 0, ItemRack.DockInfo[ItemRack.currentDock].xstart, ItemRack.DockInfo[ItemRack.currentDock].ystart
		local max_cols = 1
		local button, icon
		...
		for i = 1, #ItemRack.Menu do
			...
			if icon then
				...
				if
					IsAltKeyDown()
					and ItemRackSettings.AllowHidden == "ON"
					and IsAltKeyDown()
					and ItemRack.IsHidden(ItemRack.Menu[i])
				then
		...
		for i = 1, #ItemRack.Menu do
			...
			if ItemRack.menuOpen == 20 then
				...
				if missing == 0 then
					border:SetVertexColor(1, 0.1, 0.1)
					border:Show()
				elseif missing == 1 then
					border:SetVertexColor(0.3, 0.5, 1)
					border:Show()
				end
			else
...
function ItemRack.MenuMouseover()
	...
	if
		MouseIsOver(ItemRackMenuFrame)
		or IsShiftKeyDown()
		or (frame and frameName and frameVisible and IRmouseOverFrame)
	then
	...
	elseif ItemRack.menuOpen < 20 then
		if ItemRack.BankOpen then
			...
			if
				...
				and ItemRack.menuOpen <= 14
				and ItemRackSettings.TrinketMenuMode == "ON"
				and ItemRackUser.Buttons[13]
				and ItemRackUser.Buttons[14]
			then
...
function ItemRack.EquipItemByID(id, slot)
	...
	if
		ItemRack.NowCasting
		or (not ItemRack.SlotInfo[slot].swappable and (UnitAffectingCombat("player") or ItemRack.IsPlayerReallyDead()))
	then
			...
			if b then
				...
				if not isLocked and not IsInventoryItemLocked(slot) then
					...
					if
						...
						or (ItemRack.HasTitansGrip and not ItemRack.NoTitansGrip[select(
							7,
							GetItemInfo(GetContainerItemLink(b, s))
						) or ""])
						or not GetInventoryItemLink("player", 17)
					then
...
function ItemRack.UpdateCombatQueue()
	...
	for i in pairs(ItemRackUser.Buttons) do
		...
		elseif ItemRack.GetQueuesEnabled()[i] then
			...
			queue:SetAlpha(ItemRackUser.EnableQueues == "ON" and 1 or 0.5)
			queue:Show()
		elseif i ~= 20 then
			queue:Hide()
		end
...
function ItemRack.OnTooltip(self, line1, line2)
	if ItemRackSettings.ShowTooltips == "ON" then
		...
		if line1 then
			...
			GameTooltip:AddLine(line2, 0.8, 0.8, 0.8, 1)
			GameTooltip:Show()
			return
		else
			...
			for i = 1, #ItemRack.TooltipInfo do
				if ItemRack.TooltipInfo[i][1] == name and ItemRack.TooltipInfo[i][2] then
					...
					GameTooltip:AddLine(ItemRack.TooltipInfo[i][3], 0.8, 0.8, 0.8, 1)
					GameTooltip:Show()
					return
				end
			end
			...
			for i = 0, 19 do
				if set[i] then
					...
					if itemName then
						...
						GameTooltip:AddLine(
							"|cFFFFFFFF" .. ItemRack.SlotInfo[i].real .. ": |c" .. itemColor .. itemName
						)
					end
				end
			end
...
function ItemRack.Notify(msg)
	...
	if SCT_Display then -- send via SCT if it exists
		SCT_Display(msg, { r = 0.2, g = 0.7, b = 0.9 })
	elseif SHOW_COMBAT_TEXT == "1" then
		CombatText_AddMessage(msg, CombatText_StandardScroll, 0.2, 0.7, 0.9) -- or default UI's SCT
	else
...
function PaperDollItemSlotButton_OnEnter(self)
	...
	if
		ItemRack.menuDockedTo ~= self:GetName()
		and (ItemRackSettings.MenuOnShift == "OFF" or IsShiftKeyDown())
		and ItemRackSettings.CharacterSheetMenus == "ON"
	then
...
function ItemRack.MinimapOnEnter(tooltip)
	...
	tooltip:AddLine("Left click: Select a set", 0.8, 0.8, 0.8, 1)
	tooltip:AddLine("Right click: Open options", 0.8, 0.8, 0.8, 1)
	tooltip:AddLine("Alt left click: Show hidden sets", 0.8, 0.8, 0.8, 1)
	tooltip:AddLine("Alt right click: Toggle events", 0.8, 0.8, 0.8, 1)
	tooltip:AddLine("Shift click: Unequip this set", 0.8, 0.8, 0.8, 1)
end
...
function ItemRack.ReflectLock(override)
	...
	if ItemRackUser.Locked == "ON" or override then
		...
		ItemRackMenuFrame:SetBackdropBorderColor(0.3, 0.3, 0.3, 1)
