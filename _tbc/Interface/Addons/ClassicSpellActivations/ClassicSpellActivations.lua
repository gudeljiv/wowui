...
local function FindAura(unit, spellID, filter)
	for i = 1, 100 do
		local name,
			...
			auraSpellID = UnitAura(unit, i, filter)
		if not name then
			return nil
		end
...
function f:COMBAT_LOG_EVENT_UNFILTERED(event)
	local timestamp,
		...
		arg3,
		arg4,
		arg5 = CombatLogGetCurrentEventInfo()
