...
local function isKnown(merchantIndex)
	...
	for i = lines, 1, -1 do
		local text = _G["WTWarlockTomeScanningTooltipTextLeft" .. i]:GetText()
...
local function colorKnown(merchantButton, itemButton)
	colorItem(merchantButton, 0.5, 0, 0, itemButton, 0.5, 0, 0)
end
local function colorIgnored(merchantButton, itemButton)
	colorItem(merchantButton, 0.55, 0.55, 0, itemButton, 0.45, 0.45, 0)
end
...
local function updateMerchantFrame()
	...
	if C_AddOns.IsAddOnLoaded("GrimoireKeeper") then
		return
	end
	...
	if not tomeMerchantIds[npcId] then
		return
	end
	...
	for i = 1, MERCHANT_ITEMS_PER_PAGE do
		...
		if index <= numMerchantItems then
			...
			if wt.SayaadTomes[merchantItemID] then
				...
				if
					not wt:IsPetAbilityLearned(succubusKey)
					and petMatches
					and engPet == "Succubus"
					and isKnown(index)
				then
					hasUpdate = hasUpdate or wt:SetPetAbilityStatus(succubusKey, true)
				elseif
					not wt:IsPetAbilityLearned(incubusKey)
					and petMatches
					and engPet == "Incubus"
					and isKnown(index)
				then
				...
				if succubusKnown then
					succubusState = true
				elseif succubusIgnored then
					succubusState = false
				end
				local incubusState = nil
				if incubusKnown then
					incubusState = true
				elseif incubusIgnored then
					incubusState = false
				end
				...
				elseif (succubusKnown and engPet == "Succubus") or (incubusKnown and engPet == "Incubus") then
					...
					colorItem(merchantButton, 0.75, 0, 0, itemButton, 0.75, 0, 0)
				end
				local succubusKey, incubusKey = sayaadKeys(merchantItemID)
...
hooksecurefunc(GameTooltip, "SetMerchantItem", function(tt, index)
	local merchantItemID = GetMerchantItemID(index)
	if not wt.TomeIds[merchantItemID] then
		return
	end
	if wt.SayaadTomes[merchantItemID] then
		local succubusKey, incubusKey = sayaadKeys(merchantItemID)
		...
		if wt:IsPetAbilityLearned(merchantItemID) then
			tt:AddLine(
				RED_FONT_COLOR:WrapTextInColorCode(spaced(tomes[merchantItemID].localFamily, status(merchantItemID)))
			)
		elseif ignoreStore:IsIgnored(merchantItemID) then
			tt:AddLine(
				LIGHTYELLOW_FONT_COLOR:WrapTextInColorCode(
