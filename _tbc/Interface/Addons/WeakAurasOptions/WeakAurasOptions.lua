...
function OptionsPrivate.ConfirmDelete(toDelete, parents)
	if toDelete then
		local warningForm =
			L["You are about to delete %d aura(s). |cFFFF0000This cannot be undone!|r Would you like to continue?"]
...
function OptionsPrivate.SortDisplayButtons(filter, overrideReset, id)
	...
	if frame.loadedButton:GetExpanded() then
		table.sort(topLevelLoadedAuras, function(a, b)
			return a:lower() < b:lower()
		end)
	...
	if frame.unloadedButton:GetExpanded() then
		table.sort(topLevelUnloadedAuras, function(a, b)
			return a:lower() < b:lower()
		end)
...
function OptionsPrivate.PickDisplayMultipleShift(target)
	if frame.pickedDisplay then
		...
		if first and first ~= target then
			...
			if
				firstData.parent == targetData.parent
				and not targetData.controlledChildren
				and not firstData.controlledChildren
			then
						...
						if button.type == "WeakAurasDisplayButton" then
							...
							if data and (data.id == target or data.id == first) then
								...
								for i = index + 1, #frame.buttonsScroll.children do
									...
									if
										currentData
										and not currentData.parent
										and not currentData.controlledChildren
									then
...
local BaseDynamicTextCodes = {
	trigger = {
		{ type = "mini", name = "p", desc = L["Progress - The remaining time of a timer, or a non-timer value"] },
		{
			type = "mini",
			name = "t",
			desc = L["Total - The maximum duration of a timer, or a maximum non-timer value"],
		},
		{
			type = "mini",
			name = "n",
			desc = L["Name - The name of the display (usually an aura name), or the display's ID if there is no dynamic name"],
		},
	...
	global = {
		{
			type = "mini",
			name = "c",
			desc = L["Custom - Allows you to define a custom Lua function that returns a list of string values. %c1 will be replaced by the first value returned, %c2 by the second, etc."],
		},
...
function OptionsPrivate.UpdateTextReplacements(frame, data)
	...
	for triggerNum, triggerProps in pairs(props) do
		if next(triggerProps) then
			...
			table.insert(
				sortedProps,
				{ type = "header", triggerNum = triggerNum, name = OptionsPrivate.GetTriggerTitle(data, triggerNum) }
			)
	...
	for i, prop in ipairs(sortedProps) do
		if prop.type == "header" then
			...
			if (prop.type == "mini" or prop.type == "marker") and prop.type ~= lastType then
	...
	if targetId then
		...
		if target then
			...
			if group then
				...
				if
					(regionType == "group" or regionType == "dynamicgroup")
					and group.data.regionType == "dynamicgroup"
				then
					return
				end
			...
			error(
				string.format(
					"Calling 'WeakAuras.NewAura' with invalid groupId %s. Reload your UI to fix the display list.",
					targetId
				)
			)
		end
...
function OptionsPrivate.AddTextFormatOption(
	input,
	withHeader,
	get,
	addOption,
		...
		if not seenSymbols[symbol] then
			...
			-- No special options for these
				...
				if OptionsPrivate.Private.format_types[selectedFormat] then
					OptionsPrivate.Private.format_types[selectedFormat].AddOptions(
						symbol,
						hidden,
						addOption,
						get,
