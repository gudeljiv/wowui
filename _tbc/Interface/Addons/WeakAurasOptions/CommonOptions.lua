...
local function addCollapsibleHeader(options, key, input, order, isGroupTab)
	...
	local titleWidth = WeakAuras.doubleWidth - (hasAdd and 0.15 or 0) - (hasDelete and 0.15 or 0) - (hasUp and 0.15 or 0) - (hasDown and 0.15 or 0) - (hasDuplicate and 0.15 or 0) - (hasApplyTemplate and 0.15 or 0)
...
local function PositionOptions(id, data, _, hideWidthHeight, disableSelfPoint, group)
	...
	OptionsPrivate.commonOptions.AddCodeOption(
		...
		72.1,
		function()
			return not (data.anchorFrameType == 'CUSTOM' and not IsParentDynamicGroup())
		end,
		{'customAnchor'},
		false,
		{setOnParent = group}
...
local function BorderOptions(id, data, showBackDropOptions, hiddenFunc, order)
	local borderOptions = {
		...
		borderEdge = {
			...
			hidden = function()
				return hiddenFunc and hiddenFunc() or not data.border
			end
		},
		borderBackdrop = {
			type = 'select',
		...
		borderInFront = {
			...
			order = order + 0.8,
			hidden = function()
				return hiddenFunc and hiddenFunc() or not data.border or not showBackDropOptions
			end
		},
		backdropColor = {
			type = 'color',
