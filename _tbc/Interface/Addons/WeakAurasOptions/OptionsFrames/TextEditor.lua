...
if not WeakAurasSaved.editor_tab_spaces then
	WeakAurasSaved.editor_tab_spaces = 4
end
if not WeakAurasSaved.editor_font_size then
	WeakAurasSaved.editor_font_size = 12
end -- set default font size if missing
...
local function ConstructTextEditor(frame)
	...
	local function UpdateSnippets(frame)
		...
		table.sort(savedSnippets, function(a, b)
	...
	filterInput:SetScript("OnTextChanged", function(self)
		...
		APISearchCTimer = C_Timer.NewTimer(APISearchTextChangeDelay, function()
	...
	apiSearchScroll.scrollframe:SetScript("OnScrollRangeChanged", function(frame)
		frame.obj:DoLayout()
	end)
	...
	local function APISearch(word)
		...
		for _, systemInfo in ipairs(APIDocumentation.systems) do
			...
			local systemMatch = nsName
				and #nsName >= 4
				and systemInfo.Namespace
				and systemInfo.Namespace:lower():match(nsName)
	...
	makeAPISearch = function(apiToSearchFor)
		...
		if not apiToSearchFor or #apiToSearchFor < 4 then
			...
			if lastSearch == apiToSearchFor then
				return
			end
	...
	-- Toggle the side bar on click
	apiSearchButton:SetScript("OnClick", function()
	...
	editor.editBox:HookScript("OnKeyDown", function(self, key)
	...
	editor.editBox:HookScript("OnTextChanged", function(self, userInput)
		...
		if not str or str:trim() == "" or editor.combinedText == true then
			...
			if self.group.enclose then
				func, errorString = OptionsPrivate.Private.LoadFunction(
					"return function() " .. str .. "\n end",
					self.group.data.id,
					true
				)
	...
	function group.Open(self, data, path, enclose, multipath, reloadOptions, setOnParent, url, validator)
		...
		editor.editBox.timeMachine = {}
		editor.editBox.timeMachinePos = 1
		editor.editBox:SetScript("OnEscapePressed", function()
