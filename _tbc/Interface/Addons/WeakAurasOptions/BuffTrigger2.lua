...
local function CreateNameOptions(aura_options, data, trigger, size, isExactSpellId, isIgnoreList, prefix, baseOrder, useKey, optionKey, name, desc, inverse)
	...
	for i = 1, size do
		...
		aura_options[prefix .. i] = {
			...
			hidden = hiddenFunction,
			get = function(info)
				return trigger[optionKey] and trigger[optionKey][i]
			end,
			set = function(info, v)
...
local function GetBuffTriggerOptions(data, triggernum)
	...
	local aura_options = {
		...
		warnSpecifcUnit = {
			...
			order = 10.4,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit == 'member' and WeakAuras.UntrackableUnit(trigger.specificUnit))
			end
		},
		useDebuffType = {
			type = 'toggle',
		...
		debuffClass = {
			...
			order = 11.3,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and trigger.use_debuffClass)
			end,
		...
		debuffClassSpace = {
			...
			order = 11.4,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and not trigger.use_debuffClass)
			end
		},
		useName = {
			type = 'toggle',
		...
		useIgnoreNameSpace = {
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and not trigger.useIgnoreName and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger))
			end
		},
		...
		useIgnoreExactSpellIddSpace = {
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and not trigger.useIgnoreExactSpellId and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger))
			end
		},
		useNamePattern = {
			type = 'toggle',
		...
		stacksOperator = {
			...
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and trigger.useStacks)
			end,
			get = function()
		...
		useStacksSpace = {
			...
			order = 60.3,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and not trigger.useStacks)
			end
		},
		useRem = {
			type = 'toggle',
		...
		remOperator = {
			...
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and trigger.useRem)
			end,
			get = function()
		...
		useRemSpace = {
			...
			order = 61.3,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and not trigger.useRem)
			end
		},
		useTotal = {
			type = 'toggle',
		...
		totalOperator = {
			...
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and trigger.useTotal)
			end,
			get = function()
		...
		useTotalSpace = {
			...
			order = 61.7,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and not trigger.useTotal)
			end
		},
		fetchRole = {
			type = 'toggle',
			...
			width = WeakAuras.doubleWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi') or WeakAuras.IsClassic() or WeakAuras.IsBCC()
			end
		},
		fetchRaidMark = {
			type = 'toggle',
		...
		fetchTooltip = {
			...
			width = WeakAuras.doubleWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and not IsSingleMissing(trigger))
			end
		},
		use_tooltip = {
			type = 'toggle',
			...
			order = 62.1,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and trigger.fetchTooltip)
			end
		},
		use_tooltipSpace = {
			type = 'description',
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and not trigger.use_tooltip and trigger.fetchTooltip)
			end
		},
		tooltip_operator = {
			type = 'select',
			...
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and trigger.use_tooltip and trigger.fetchTooltip)
			end,
			values = OptionsPrivate.Private.string_operator_types
		},
		tooltip = {
		...
		tooltipValueNumber = {
			...
			order = 63.2,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and trigger.use_tooltipValue and trigger.fetchTooltip)
			end,
		...
		use_tooltipValueSpace = {
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit ~= 'multi' and CanHaveMatchCheck(trigger) and not trigger.use_tooltipValue and trigger.fetchTooltip)
			end
		},
		...
		use_isBossDebuff = {
			...
			name = function(input)
				...
				if value == nil then
					return L['Is Boss Debuff']
				elseif value == false then
					return '|cFFFF0000 ' .. L['Negator'] .. ' ' .. L['Is Boss Debuff']
				else
					return '|cFF00FF00' .. L['Is Boss Debuff']
				end
			end,
			width = WeakAuras.doubleWidth,
			order = 64.1,
			...
			set = function(info, v)
				if v then
					...
					local value = trigger.use_isBossDebuff
					if value == false then
						trigger.use_isBossDebuff = nil
					else
						trigger.use_isBossDebuff = false
					end
				end
				WeakAuras.Add(data)
			end
		...
		use_castByPlayer = {
			...
			name = function(input)
				...
				if value == nil then
					return L['Cast by Players']
				elseif value == false then
					return '|cFFFF0000 ' .. L['Negator'] .. ' ' .. L['Cast by Players']
				else
					return '|cFF00FF00' .. L['Cast by Player Character']
				end
			end,
			...
			set = function(info, v)
				if v then
					...
					local value = trigger.use_castByPlayer
					if value == false then
						trigger.use_castByPlayer = nil
					else
						trigger.use_castByPlayer = false
					end
				end
				WeakAuras.Add(data)
			end
		...
		includePets = {
			...
			order = 66.15,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and trigger.use_includePets)
			end
		},
		includePetsSpace = {
			type = 'description',
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and not trigger.use_includePets)
			end
		},
		useGroupRole = {
			type = 'toggle',
			...
			order = 67.1,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party')) or WeakAuras.IsClassic() or WeakAuras.IsBCC()
			end
		},
		group_role = {
			type = 'multiselect',
		...
		raid_role = {
			...
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and trigger.useRaidRole)
			end,
			order = 67.2
		},
		raid_roleSpace = {
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and not trigger.useRaidRole)
			end
		},
		useArenaSpec = {
			type = 'toggle',
		...
		ignoreSelf = {
			...
			width = WeakAuras.doubleWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party' or trigger.unit == 'nameplate'))
			end
		},
		useClass = {
			type = 'toggle',
		...
		class = {
			...
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and trigger.useClass)
			end,
			order = 68.2
		},
		classSpace = {
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and not trigger.useClass)
			end
		},
		useHostility = {
			type = 'toggle',
		...
		hostility = {
			...
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit == 'nameplate' and trigger.useHostility)
			end,
			order = 68.5
		},
		hostilitySpace = {
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and trigger.unit == 'nameplate' and not trigger.useHostility)
			end
		},
		ignoreDead = {
			type = 'toggle',
		...
		unitName = {
			...
			order = 69.2,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and trigger.useUnitName)
			end
		},
		unitNameSpace = {
			type = 'description',
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and (trigger.unit == 'group' or trigger.unit == 'raid' or trigger.unit == 'party') and not trigger.useUnitName)
			end
		},
		useGroup_count = {
			type = 'toggle',
		...
		useGroup_countSpace = {
			...
			width = WeakAuras.normalWidth,
			hidden = function()
				return not (trigger.type == 'aura2' and IsGroupTrigger(trigger) and not trigger.useGroup_count)
			end
		},
		group_countOperator = {
			type = 'select',
			...
			values = OptionsPrivate.Private.operator_types,
			hidden = function()
				return not (trigger.type == 'aura2' and IsGroupTrigger(trigger) and trigger.useGroup_count)
			end,
			get = function()
