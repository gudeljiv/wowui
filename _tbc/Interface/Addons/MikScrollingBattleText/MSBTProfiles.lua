...
local differential_mt = {
	__index = function(t, k)
		return differentialMap[t][k]
	end,
}
...
local function CreateClassSettingsTable(class)
	...
	if not RAID_CLASS_COLORS[class] then
		return { disabled = true, colorR = 1, colorG = 1, colorB = 1 }
	end

	-- Return a table using the default class color.
	return {
		colorR = RAID_CLASS_COLORS[class].r,
		colorG = RAID_CLASS_COLORS[class].g,
		colorB = RAID_CLASS_COLORS[class].b,
	}
end
	...
	masterProfile = {
		...
		triggers = {
			...
			MSBT_TRIGGER_CLEARCASTING = {
				...
				mainEvents = "SPELL_AURA_APPLIED{skillName;;eq;;"
					.. SPELL_CLEARCASTING
					.. ";;recipientAffiliation;;eq;;"
					.. FLAG_YOU
					.. "}",
			},
			...
			MSBT_TRIGGER_EXECUTE = {
				...
				fontSize = 26,
				classes = "WARRIOR",
				mainEvents = "UNIT_HEALTH{unitID;;eq;;target;;threshold;;lt;;20;;unitReaction;;eq;;"
					.. REACTION_HOSTILE
					.. "}",
			...
			MSBT_TRIGGER_REVENGE = {
				...
				fontSize = 26,
				classes = "WARRIOR",
				mainEvents = "GENERIC_MISSED{recipientAffiliation;;eq;;"
					.. FLAG_YOU
					.. ";;missType;;eq;;BLOCK}&&GENERIC_MISSED{recipientAffiliation;;eq;;"
					.. FLAG_YOU
					.. ";;missType;;eq;;DODGE}&&GENERIC_MISSED{recipientAffiliation;;eq;;"
					.. FLAG_YOU
					.. ";;missType;;eq;;PARRY}",
				exceptions = "warriorStance;;ne;;2;;unavailableSkill;;eq;;"
					.. SPELL_REVENGE
					.. ";;recentlyFired;;lt;;2",
		...
		absorb = {
			...
			colorG = 1,
			colorB = 0,
			trailer = string_gsub(string_gsub(ABSORB_TRAILER, "%((.+)%)", "<%1>"), "%%d", "%%a"),
		},
		block = {
			colorR = 0.5,
			colorG = 0,
			colorB = 1,
			trailer = string_gsub(string_gsub(BLOCK_TRAILER, "%((.+)%)", "<%1>"), "%%d", "%%a"),
		},
		resist = {
			colorR = 0.5,
			colorG = 0,
			colorB = 0.5,
			trailer = string_gsub(string_gsub(RESIST_TRAILER, "%((.+)%)", "<%1>"), "%%d", "%%a"),
		},
	...
	masterProfile = {
		...
		triggers = {
			...
			MSBT_TRIGGER_ELUSIVE_BREW = {
				...
				mainEvents = "SPELL_AURA_APPLIED{skillName;;eq;;"
					.. SPELL_ELUSIVE_BREW
					.. ";;amount;;eq;;5;;recipientAffiliation;;eq;;"
					.. FLAG_YOU
					.. "}&&"
					.. "SPELL_AURA_APPLIED{skillName;;eq;;"
					.. SPELL_ELUSIVE_BREW
					.. ";;amount;;eq;;10;;recipientAffiliation;;eq;;"
					.. FLAG_YOU
					.. "}&&"
					.. "SPELL_AURA_APPLIED{skillName;;eq;;"
					.. SPELL_ELUSIVE_BREW
					.. ";;amount;;eq;;15;;recipientAffiliation;;eq;;"
					.. FLAG_YOU
					.. "}",
			},
			MSBT_TRIGGER_EXECUTE = {
			...
			MSBT_TRIGGER_LAVA_SURGE = {
				...
				fontSize = 26,
				classes = "SHAMAN",
				mainEvents = "SPELL_CAST_SUCCESS{sourceAffiliation;;eq;;"
					.. FLAG_YOU
					.. ";;skillID;;eq;;"
					.. SPELLID_LAVA_SURGE
					.. "}",
			...
			MSBT_TRIGGER_VICTORY_RUSH = {
				...
				classes = "WARRIOR",
				mainEvents = "PARTY_KILL{sourceAffiliation;;eq;;" .. FLAG_YOU .. "}",
				exceptions = "unavailableSkill;;eq;;"
					.. SPELL_VICTORY_RUSH
					.. ";;trivialTarget;;eq;;true;;recentlyFired;;lt;;2",
				iconSkill = SPELLID_VICTORY_RUSH,
			},
			--[[MSBT_TRIGGER_VITAL_MISTS = {
				colorR			= 0.5,
...
local function ShowOptions()
	...
	if C_AddOns.IsAddOnLoaded(optionsName) then
...
local function SetOption(optionPath, optionName, optionValue, optionDefault)
	...
	if optionPath then
		SplitString(optionPath, "%.", pathTable)
	end
	...
	if optionValue ~= masterOption then
		needsOverride = true
	end
	...
	if optionValue == nil and masterOption then
		optionValue = false
	end
...
local function LoadUsedFonts()
	...
	local usedFonts = {}
	if currentProfile.normalFontName then
		usedFonts[currentProfile.normalFontName] = true
	end
	if currentProfile.critFontName then
		usedFonts[currentProfile.critFontName] = true
	end
	...
	if currentProfile.scrollAreas then
		for saKey, saSettings in pairs(currentProfile.scrollAreas) do
			if saSettings.normalFontName then
				usedFonts[saSettings.normalFontName] = true
			end
			if saSettings.critFontName then
				usedFonts[saSettings.critFontName] = true
			end
		end
	end
	...
	if currentProfile.events then
		for eventName, eventSettings in pairs(currentProfile.events) do
			if eventSettings.fontName then
				usedFonts[eventSettings.fontName] = true
			end
		end
	end
	...
	if currentProfile.triggers then
		for triggerName, triggerSettings in pairs(currentProfile.triggers) do
			if type(triggerSettings) == "table" then
				if triggerSettings.fontName then
					usedFonts[triggerSettings.fontName] = true
				end
			end
		end
	end
	...
	for fontName in pairs(usedFonts) do
		MikSBT.Animations.LoadFont(fontName)
	end
end
...
local function CopyProfile(srcProfileName, destProfileName)
	...
	if not destProfileName or destProfileName == "" then
		return
	end
...
local function ResetProfile(profileName, showOutput)
	...
	if not profileName then
		profileName = savedVariablesPerChar.currentProfileName
	end
	...
	elseif event == "VARIABLES_LOADED" then
		...
		if CUSTOM_CLASS_COLORS then
			...
			if CUSTOM_CLASS_COLORS.RegisterCallback then
				CUSTOM_CLASS_COLORS:RegisterCallback(UpdateCustomClassColors)
			end
		end
