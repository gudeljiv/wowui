...
M.event =
	setmetatable(
	{},
	{__metatable = false, __newindex = function(_, k, v)
			aux_events[k](v)
		end}
)
		...
		function(_, event, arg1, ...)
			if event == 'ADDON_LOADED' then
				...
				elseif arg1 == 'Blizzard_AuctionUI' then
					for _, f in ipairs(handlers3) do
						f(arg1, ...)
					end
				end
			elseif event == 'PLAYER_LOGIN' then
				for _, f in ipairs(handlers2) do
					f(arg1, ...)
				end
				sort(
					account_data.auctionable_items,
					function(a, b)
						return strlen(a) < strlen(b) or (strlen(a) == strlen(b) and a < b)
					end
				)
				print('loaded - /aux')
			else
...
do
	...
	function on_tab_click(i)
		do
			(index and get_tab().CLOSE or pass)()
		end
		index = i
		do
			(index and get_tab().OPEN or pass)()
		end
	end
end
