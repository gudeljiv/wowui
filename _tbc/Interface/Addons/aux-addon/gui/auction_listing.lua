...
local methods = {
	...
	OnClick = function(self, button)
		if IsControlKeyDown() or IsShiftKeyDown() then
			...
			do
				(self.rt.handlers.OnClick or pass)(self, button)
			end
		end
	end,
	...
	UpdateRowInfo = function(self)
		...
		self.rowInfo.single_item =
			aux.all(
			records,
			function(record)
				return record.item_key == records[1].item_key
			end
		)
	...
	SetRowInfo = function(self, rowIndex, record, totalAuctions, totalPlayerAuctions, indented, expandable, expandKey)
		if rowIndex <= 0 or rowIndex > #self.rows then
			return
		end
