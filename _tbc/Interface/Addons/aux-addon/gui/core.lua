...
function M.editbox(parent)
	...
	editbox:SetScript(
		...
		function(self)
			...
			do
				(self.focus_loss or pass)(self)
			end
		end
	)
	editbox:SetScript(
		'OnTextChanged',
		function(self, is_user_input)
			do
				(self.change or pass)(self, is_user_input)
			end
	...
	function M.dropdown(parent, text_height)
		...
		function set_index(new_index)
			...
			if new_index ~= index then
				index = new_index
				do
					(editbox.selection_change or pass)()
				end
			end
		end
