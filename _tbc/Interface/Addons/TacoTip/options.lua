...
frame:SetScript("OnShow", function(frame)
	...
	local function newCheckbox(name, label, description, onClick)
		local check =
			CreateFrame("CheckButton", "TacoTipOptCheckBox" .. name, frame, "InterfaceOptionsCheckButtonTemplate")
	...
	local function newRadioButton(name, label, description, onClick)
		local check = CreateFrame(
			"CheckButton",
			"TacoTipOptRadioButton" .. name,
			frame,
			"InterfaceOptionsCheckButtonTemplate, UIRadioButtonTemplate"
		)
		check:SetScript("OnClick", function(self)
	...
	options.exampleTooltipHealthBar =
		CreateFrame("StatusBar", "TacoTipOptExampleTooltipStatusBar", options.exampleTooltip)
	...
	options.exampleTooltipPowerBar =
		CreateFrame("StatusBar", "TacoTipOptExampleTooltipPowerBar", options.exampleTooltip)
	...
	local function showExampleTooltip()
		...
		options.exampleTooltip:AddLine(
			string.format(
				"|cFF%02x%02x%02xKebabstorm%s %s%s|r",
				name_r * 255,
				name_g * 255,
				name_b * 255,
				title,
				(TacoTipConfig.show_team and (HORDE_ICON .. " ") or ""),
		...
		if TacoTipConfig.show_guild_name then
			if TacoTipConfig.show_guild_rank then
				if TacoTipConfig.guild_rank_alt_style then
					...
					options.exampleTooltip:AddLine(
						string.format("|cFF40FB40" .. L["FORMAT_GUILD_RANK_1"] .. "|r", "Officer", "Drunken Wrath")
					)
				end
			else
		...
		local wide_style = (
			TacoTipConfig.tip_style == 1
			or ((TacoTipConfig.tip_style == 2 or TacoTipConfig.tip_style == 4) and IsModifierKeyDown())
		)
		...
		if TacoTipConfig.show_target then
			if wide_style then
				options.exampleTooltip:AddDoubleLine(
					L["Target"] .. ":",
					L["None"],
					NORMAL_FONT_COLOR.r,
					NORMAL_FONT_COLOR.g,
		...
		if TacoTipConfig.show_talents then
			if wide_style then
				options.exampleTooltip:AddDoubleLine(
					L["Talents"] .. ":",
					CI:GetSpecializationName("ROGUE", 1, true) .. " [51/18/2]",
					NORMAL_FONT_COLOR.r,
					NORMAL_FONT_COLOR.g,
					NORMAL_FONT_COLOR.b,
				...
				options.exampleTooltip:AddDoubleLine(
					" ",
					CI:GetSpecializationName("ROGUE", 3, true) .. " [14/3/54]",
					NORMAL_FONT_COLOR.r,
					NORMAL_FONT_COLOR.g,
					NORMAL_FONT_COLOR.b,
					GRAY_FONT_COLOR.r,
				...
				options.exampleTooltip:AddLine(
					L["Talents"] .. ":|cFFFFFFFF " .. CI:GetSpecializationName("ROGUE", 1, true) .. " [51/18/2]"
				)
			end
		end
		local miniText = ""
		...
		if isPawnLoaded and TacoTipConfig.show_pawn_player then
			...
			if wide_style then
				options.exampleTooltip:AddDoubleLine(
					string.format("Pawn: %s1234.56|r", specColor),
					string.format("%s(%s)|r", specColor, CI:GetSpecializationName("ROGUE", 1, true)),
					1,
					1,
					1,
					1,
			...
			elseif mini_style then
				...
				options.exampleTooltip:AddLine(
					string.format("Pawn: %s1234.56 (%s)|r", specColor, CI:GetSpecializationName("ROGUE", 1, true)),
					1,
					1,
					1
				)
	...
	options.exampleTooltip:SetScript("OnEvent", function()
		showExampleTooltip()
	end)
	...
	options.uberTips = newCheckbox("UberTips", L["Enhanced Tooltips"], L["TEXT_OPT_UBERTIPS"], function(self, value)
	...
	options.styleChoice = newDropDown("StyleChoice", dropdown_values, function(value)
	...
	local function getConfig()
		...
		options.pawnScorePlayer.label:SetText(
			isPawnLoaded and "PawnScore" or "PawnScore (" .. L["requires Pawn"] .. ")"
		)
		...
		options.lockCharacterInfoPosition:SetDisabled(
			not (TacoTipConfig.show_gs_character or TacoTipConfig.show_avg_ilvl)
		)
