...
L.EnableAddonCompartment = "블리자드 애드온 보관함 통합 사용"
L.ResetMinimapBtn = "미니맵 버튼 위치 초기화."
L.AddonCompartmentReloadMsg = "애드온 보관함 변경 사항은 UI 재시작이 필요합니다. 적용하려면 /reload 를 사용하세요."
...
L.ConfigCache = "캐시"
L.ConfigCacheHeader = "BagSync 아이템 캐시 및 제한 설정입니다."
L.ConfigCacheHowTitle = "캐시 작동 방식"
L.ConfigCacheHowText_1 = "로그인 시 아이템 정보가 완전히 제공되지 않습니다. |cFFB19CD9GetItemInfo|r 또는 |cFFB19CD9C_Item.GetItemInfo|r 같은 API는 해당 아이템이 세션에 캐시되기 전에는 nil(또는 없음)을 반환합니다."
L.ConfigCacheHowText_2 = "BagSync는 누락된 아이템 데이터를 작은 배치로 요청하고 클라이언트가 받기를 기다립니다(이 과정에서 GET_ITEM_INFO_RECEIVED가 발생)."
L.ConfigCacheHowText_3 = "제한을 통해 캐시가 채워지는 동안 성능을 안정적으로 유지하여 검색과 툴팁이 점차 완성됩니다."
L.ConfigCacheRatesTitle = "현재 속도"
L.ConfigCacheSpeedTitle = "캐시 속도"
L.ConfigCacheSpeedLabel = "제한"
L.ConfigCacheSpeedHelp = "|cFFB19CD9느림|r은 기본값이며 성능에 가장 좋지만, 검색이 완전히 캐시되기까지 더 오래 걸립니다.\n|cFFB19CD9중간|r은 더 빠르지만 로그인 시 작은 스파이크가 있을 수 있습니다.\n|cFFB19CD9빠름|r은 가장 강하며 로그인 시 랙 스파이크가 발생할 수 있습니다.\n|cFFFF4D4D사용 안 함|r은 |cFFFF4D4D권장하지 않음|r (|cFFFFFFFF검색 창이 열려 있거나 검색 중일 때 BagSync는 |cFFB19CD9빠름|r을 사용합니다.|r)"
L.CacheSpeedSlow = "느림 (백그라운드 + 램프)"
L.CacheSpeedMedium = "중간"
L.CacheSpeedFast = "빠름"
L.CacheSpeedDisabled = "사용 안 함 (백그라운드 캐시 없음)"
L.CacheSpeedRampIntro = "느림 모드는 %d초마다 증가합니다."
L.CacheSpeedRampLine = "단계 %d: %.2fs마다 %d개 (~초당 %d개)."
L.CacheSpeedSingleLine = "%.2fs마다 %d개 (~초당 %d개)."
L.CacheSpeedDisabledSummary = "백그라운드 캐시는 비활성화됩니다. BagSync를 열면 빠름을 사용하며 검색도 빠름으로 실행됩니다."
L.ConfigMain = "일반"
...
L.ConfigFont = "글꼴"
L.ConfigExtTooltipAnchor = "GameTooltip 위치"
L.ConfigExtTipPositionSettings = "외부 툴팁 위치 설정"
L.ConfigExtTipCustomAnchorEnable = "사용자 지정 앵커 위치 사용"
L.ConfigExtTipCustomAnchorLocation = "사용자 지정 위치"
L.ConfigExtTipCustomAnchorShow = "ExtTip 앵커 표시"
L.ExtTipCustomAnchor_TopLeft = "왼쪽 위"
L.ExtTipCustomAnchor_TopRight = "오른쪽 위"
L.ExtTipCustomAnchor_BottomLeft = "왼쪽 아래"
L.ExtTipCustomAnchor_BottomRight = "오른쪽 아래"
L.ExtTipCustomAnchor_Center = "가운데"
L.ExtTipCustomAnchor_CenterTop = "가운데 위"
L.ExtTipCustomAnchor_CenterBottom = "가운데 아래"
L.ExtTipCustomAnchor_UseAnchor = "ExtTip 앵커 사용"
L.ExtTipAnchorLabel = "BagSync ExtTip 앵커\n\n(우클릭으로 위치 저장)"
...
L.ExtTooltipAnchor_Bottom = "아래"
L.ExtTooltipAnchor_Left = "왼쪽"
L.ExtTooltipAnchor_Right = "오른쪽"
L.ExtTipNoticeTitle = "외부 툴팁(ExtTip) 알림"
L.ExtTipNoticeText = "|cFFFFFFFFBlizzard의 |cFFFF0000Secret Values|r|cFFFFFFFF 때문에 ExtTip 위치가 막힐 수 있습니다. BagSync는 보호 장치를 추가했지만, 안전한 tooltip을 찾지 못하면 카운트가 |cFFFF0000GameTooltip|r|cFFFFFFFF에 표시됩니다. 자세한 내용은 FAQ의 |cFFFF0000'Secret Values'|r|cFFFFFFFF 항목을 확인하세요.|r"
...
L.CachingItemData = "아이템 데이터 캐싱 중... (%d개 남음)"
...
L.FAQ_Question_8 = "비밀 값 오류와 블리자드 애드온 제한"
L.FAQ_Question_8_p1 = [[
블리자드의 새로운 |cFFDF2B2B"비밀 값"|r 시스템은 애드온이 전투 관련 일부 데이터를 |cFF99CC33표시|r할 수는 있지만 |cFFDF2B2B안전하게 검사하거나 계산할 수는 없도록|r 설계되어 있습니다. 오염된 코드 경로에서는 산술 연산, 비교, 그리고 이 값들에 대한 일부 인덱싱까지 즉시 Lua 오류를 일으킵니다. 이 값들은 비밀 속성/앵커를 통해 프레임과 툴팁을 "감염"시킬 수도 있어, |cFF99CC33같은 툴팁|r이 나중에 비밀 좌표를 반환하기 시작할 수 있습니다. 이는 그 툴팁을 건드린 애드온이 처음에 비밀 값을 만든 것이 아니더라도 발생합니다. 따라서 BagSync를 가리키는 스택 트레이스가 BagSync가 비밀 값을 도입했음을 |cFFDF2B2B의미하지는|r 않으며, 이미 오염된 객체를 다음으로 건드린 애드온이 BagSync였다는 뜻인 경우가 많습니다.

이는 BagSync만의 문제가 아닙니다. 블리자드 기본 UI도 서드파티 애드온이 없어도 동일한 "attempt to perform arithmetic on a secret value" 오류(예: MoneyFrame / GameTooltip)를 발생시키고 있습니다. 다른 애드온들도 툴팁과 머니 프레임의 비밀 값으로 인해 발생하는 |cFF99CC33동일한 오류 유형|r에 대한 긴급 수정이나 가드를 이미 배포 중입니다. |cFFDF2B2B요약하면: 이는 광범위한 플랫폼 문제이지, BagSync만의 고유한 버그가 아닙니다|r.

저는 가능한 한 많은 안전장치와 방어적 체크를 추가하려고 |cFFDF2B2B시도했습니다|r, BagSync가 이런 오류를 일으킬 가능성을 줄이려 했지만, 블리자드는 혼합 애드온 환경에서 비밀 값을 |cFFDF2B2B추적하고 통제하기 매우 어렵게|r 만들어 두었습니다. 이 시스템은 애드온이 값을 신뢰성 있게 검사하거나 |cFF99CC33어디서|r 시작됐는지조차 확인하지 못하게 적극적으로 막습니다. 그래서 이런 오류는 재현하기가 어렵고, 오류가 터질 때에만 보이는 경우가 많은 것입니다.
]]
L.BagSyncInfoWindow = [[
