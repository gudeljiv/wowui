...
function Layout:CheckMedia()
	mediaPath[SML.MediaType.STATUSBAR] = loadMedia(
		SML.MediaType.STATUSBAR,
		ShadowUF.db.profile.bars.texture,
		"Interface\\AddOns\\ShadowedUnitFrames\\media\\textures\\Aluminium"
	)
	mediaPath[SML.MediaType.FONT] = loadMedia(
		SML.MediaType.FONT,
		ShadowUF.db.profile.font.name,
		"Interface\\AddOns\\ShadowedUnitFrames\\media\\fonts\\Myriad Condensed Web.ttf"
	)
	mediaPath[SML.MediaType.BACKGROUND] = loadMedia(
		SML.MediaType.BACKGROUND,
		ShadowUF.db.profile.backdrop.backgroundTexture,
...
function Layout:Load(frame)
	...
	for _, module in pairs(ShadowUF.moduleOrder) do
		if
			frame.visibility[module.moduleKey]
			and ShadowUF.db.profile.units[frame.unitType][module.moduleKey]
			and ShadowUF.db.profile.units[frame.unitType][module.moduleKey].secure
			and module:SecureLockable()
		then
...
SML:Register(
	SML.MediaType.STATUSBAR,
	"Minimalist",
	"Interface\\AddOns\\ShadowedUnitFrames\\media\\textures\\Minimalist"
)
...
function Layout:ReverseDirection(key)
	return key == "LEFT" and "RIGHT"
		or key == "RIGHT" and "LEFT"
		or key == "TOP" and "BOTTOM"
		or key == "BOTTOM" and "TOP"
end
...
function Layout:AnchorFrame(parent, frame, config)
	...
	local relativePoint = config.relativePoint and config.relativePoint ~= "" and config.relativePoint
		or preDefRelative[config.anchorPoint]
		or "CENTER"
...
function Layout:SetupFrame(frame, config)
	...
	frame:SetBackdropColor(
		backdrop.backgroundColor.r,
		backdrop.backgroundColor.g,
		backdrop.backgroundColor.b,
		backdrop.backgroundColor.a
	)
	frame:SetBackdropBorderColor(
		backdrop.borderColor.r,
		backdrop.borderColor.g,
		backdrop.borderColor.b,
		backdrop.borderColor.a
	)
...
function Layout:SetupBars(frame, config)
	for _, module in pairs(ShadowUF.modules) do
		...
		if widget and (module.moduleHasBar or config[key] and config[key].isBar) then
			...
			if
				(widget:IsShown() or (not frame[key].visibilityManaged and module.defaultVisibility == false))
				and widget.SetStatusBarTexture
			then
			...
			if widget.background then
				if config[key].background or config[key].invert then
					...
					widget.background.overrideColor = ShadowUF.db.profile.bars.backgroundColor
						or config[key].backgroundColor

					if widget.background.overrideColor then
						widget.background:SetVertexColor(
							widget.background.overrideColor.r,
							widget.background.overrideColor.g,
							widget.background.overrideColor.b,
							ShadowUF.db.profile.bars.backgroundAlpha
						)
					end
				else
...
function Layout:SetupFontString(fontString, extraSize)
	...
	if
		ShadowUF.db.profile.font.shadowColor
		and ShadowUF.db.profile.font.shadowX
		and ShadowUF.db.profile.font.shadowY
	then
		fontString:SetShadowColor(
			ShadowUF.db.profile.font.shadowColor.r,
			ShadowUF.db.profile.font.shadowColor.g,
			ShadowUF.db.profile.font.shadowColor.b,
			ShadowUF.db.profile.font.shadowColor.a
		)
...
function Layout:InitFontString(parent, frame, id, config, blockID)
	...
	fontString:SetTextColor(
		ShadowUF.db.profile.font.color.r,
		ShadowUF.db.profile.font.color.g,
		ShadowUF.db.profile.font.color.b,
		ShadowUF.db.profile.font.color.a
	)
...
function Layout:SetupText(frame, config)
	...
	for id, row in pairs(config.text) do
		...
		if
			parent
			and (ShadowUF.modules[module].defaultVisibility == false or parent:IsShown())
			and row.enabled
			and row.text ~= ""
		then
...
function Layout:PositionWidgets(frame, config)
	...
	for key, module in pairs(ShadowUF.modules) do
		...
		if
			(module.moduleHasBar or config[key] and config[key].isBar)
			and frame[key]
			and frame[key]:IsShown()
			and config[key].height > 0
		then
			...
			if
				not frame.visibility.portrait
				or config.portrait.isBar
				or config[key].order < config.portrait.fullBefore
				or config[key].order > config.portrait.fullAfter
			then
				hasFullSize = true
	...
	for id, key in pairs(barOrder) do
		...
		if
			not portraitAnchor
			and config[key]
			and config[key].order
			and config.portrait
			and config.portrait.fullBefore
			and config[key].order >= config.portrait.fullBefore
		then
			portraitAnchor = bar
		end
