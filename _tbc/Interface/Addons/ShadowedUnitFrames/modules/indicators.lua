...
function Indicators:UpdateClass(frame)
	if not frame.indicators.class or not frame.indicators.class.enabled then
		return
	end
...
function Indicators:UpdatePhase(frame)
	if not frame.indicators.phase or not frame.indicators.phase.enabled then
		return
	end
...
function Indicators:UpdateResurrect(frame)
	if not frame.indicators.resurrect or not frame.indicators.resurrect.enabled then
		return
	end
...
function Indicators:UpdateMasterLoot(frame)
	if not frame.indicators.masterLoot or not frame.indicators.masterLoot.enabled then
		return
	end
	...
	elseif
		(partyID and partyID == 0 and UnitIsUnit(frame.unit, "player"))
		or (partyID and partyID > 0 and UnitIsUnit(frame.unit, ShadowUF.partyUnits[partyID]))
		or (raidID and raidID > 0 and UnitIsUnit(frame.unit, ShadowUF.raidUnits[raidID]))
	then
...
function Indicators:UpdateRaidTarget(frame)
	if not frame.indicators.raidTarget or not frame.indicators.raidTarget.enabled then
		return
	end
...
function Indicators:UpdateLeader(frame)
	if not frame.indicators.leader or not frame.indicators.leader.enabled then
		return
	end
...
function Indicators:UpdatePVPFlag(frame)
	if not frame.indicators.pvp or not frame.indicators.pvp.enabled then
		return
	end
...
function Indicators:UpdateStatus(frame)
	if not frame.indicators.status or not frame.indicators.status.enabled then
		return
	end
...
function Indicators:UpdateReadyCheck(frame, event)
	if not frame.indicators.ready or not frame.indicators.ready.enabled then
		return
	end

	-- We're done, and should fade it out if it's shown
	if event == "READY_CHECK_FINISHED" then
		if not frame.indicators.ready:IsShown() then
			return
		end
...
function Indicators:UpdateHappiness(frame)
	if not frame.indicators.happiness or not frame.indicators.happiness.enabled then
		return
	end
...
function Indicators:OnLayoutApplied(frame, config)
	if frame.visibility.indicators then
		...
		for _, key in pairs(self.list) do
			...
			if
				indicator
				and config.indicators[key]
				and config.indicators[key].enabled
				and config.indicators[key].size
			then
