...
function addon:CheckCastModifiers(unitID, cast)
	...
	if cast.unitGUID == self.PLAYER_GUID then
		return
	end -- modifiers already taken into account with CastingInfo()
	if unaffectedCastModsSpells[cast.spellID] then
		return
	end
...
function addon:DeleteCast(unitGUID, isInterrupted, skipDeleteCache, isCastComplete, noFadeOut)
	if not unitGUID then
		return
	end -- may be nil when called from OnUpdate script (rare)
...
function addon:CastPushback(unitGUID)
	local cast = activeTimers[unitGUID]
	if not cast or cast.hasPushbackImmuneModifier then
		return
	end
	if pushbackBlacklist[cast.spellName] then
		return
	end
...
function addon:UNIT_AURA()
	...
	if auraRows == TargetFrame.auraRows then
		return
	end
	...
	elseif eventType == 'SPELL_CAST_SUCCESS' then
		...
		if channelCast then
			...
			if cast and (spellName == ARCANE_MISSILES or spellName == ARCANE_MISSILE) then
				...
				if cast.spellName == ARCANE_MISSILES or cast.spellName == ARCANE_MISSILE then
					return
				end
			end
		...
		if missType == 'IMMUNE' and playerInterrupts[spellName] then
			...
			if bit_band(dstFlags, COMBATLOG_OBJECT_CONTROL_PLAYER) <= 0 then -- dest unit is not a player
				if bit_band(srcFlags, COMBATLOG_OBJECT_CONTROL_PLAYER) > 0 then -- source unit is player
					...
					if npcCastUninterruptibleCache[npcID .. cast.spellName] then
						return
					end -- already added
