...
function GBB.FilterDungeon(dungeon, isHeroic, isRaid)
	...
	return GBB.DBChar['FilterDungeon' .. dungeon] and (isRaid or ((GBB.DBChar['HeroicOnly'] == false or isHeroic) and (GBB.DBChar['NormalOnly'] == false or isHeroic == false))) and (GBB.DBChar.FilterLevel == false or inLevelRange)
end
...
function GBB.CreateTagListLOC(loc)
	...
	for id, tag in pairs(GBB.suffixTagsLoc[loc]) do
		...
		if tContains(GBB.suffixTags, tag) == false then
...
function GBB.Init()
	...
	-- Initalize options
	if not GroupBulletinBoardDB then
		GroupBulletinBoardDB = {}
	end -- fresh DB
	if not GroupBulletinBoardDBChar then
	...
	if not GBB.DB.MinimapButton then
		GBB.DB.MinimapButton = {}
	end
	if not GBB.DB.Custom then
		GBB.DB.Custom = {}
	end
	if not GBB.DB.CustomLocales then
		GBB.DB.CustomLocales = {}
	end
	if not GBB.DB.CustomLocalesDungeon then
		GBB.DB.CustomLocalesDungeon = {}
	end
	if not GBB.DB.FontSize then
		GBB.DB.FontSize = 'GameFontNormal'
	end
	if not GBB.DB.DisplayLFG then
		GBB.DB.DisplayLFG = false
	end
	GBB.DB.Server = nil -- old settings

	if GBB.DB.OnDebug == nil then
		GBB.DB.OnDebug = false
	end
	...
	GBB.Tool.SlashCommand(
		{'/gbb', '/groupbulletinboard'},
		{
			{
				'notify',
				'',
	...
	if GBB.DB.AdditionalInfo and name then
		...
		if info ~= '' then
			...
			if class and class ~= '' then
				txt = '|Hplayer:' .. name .. '|h' .. GBB.Tool.IconClass[class] .. '|c' .. GBB.Tool.ClassColor[class].colorStr .. name .. '|r' .. symbol .. '|h'
			else
