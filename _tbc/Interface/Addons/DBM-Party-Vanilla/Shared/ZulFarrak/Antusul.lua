local mod	= DBM:NewMod(484, "DBM-Party-Vanilla", DBM:IsPostCata() and 15 or 20, 241)
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpell(15982) and args:IsSrcTypeHostile() then
		timerHealingWaveAllyCD:Start()
		if self:CheckInterruptFilter(args.sourceGUID, false, true) then
			specWarnHealingWaveAlly:Show(args.sourceName)
			specWarnHealingWaveAlly:Play("kickcast")
		end
	elseif args:IsSpell(8376) and args:IsSrcTypeHostile() then
		warningEarthgrabTotem:Show()
	elseif args:IsSpell(4971) and args:IsSrcTypeHostile() then
		warningHealingWard:Show()
	end
end

function mod:SPELL_CAST_SUCCESS(args)
	if args:IsSpell(11894) then
