...
local function customStatusTex(statusFrame, textString, value, valueMin, valueMax)
	...
	valueDisplay =
		((value > 1e3 and value < 1e5 and format('%1.3f', value / k)) or (value >= 1e5 and value < 1e6 and format('%1.0f K', value / k)) or (value >= 1e6 and value < 1e9 and format('%1.2f M', value / m)) or
		(value >= 1e9 and format('%1.1f M', value / m)) or
		value)

	valueMaxDisplay =
		((valueMax > 1e3 and valueMax < 1e5 and format('%1.3f', valueMax / k)) or (valueMax >= 1e5 and valueMax < 1e6 and format('%1.0f K', valueMax / k)) or
		(valueMax >= 1e6 and valueMax < 1e9 and format('%1.2f M', valueMax / m)) or
		(valueMax >= 1e9 and format('%1.1f M', valueMax / m)) or
		valueMax)
		...
		for i, v in pairs(
			{
				...
				PlayerFrameManaBar.LeftText,
				PlayerFrameManaBar.RightText,
		...
		for i, v in pairs(
			{
				...
				FocusFrameManaBar.LeftText,
				FocusFrameManaBar.RightText,
