...
-- Check for AdiButtonAuras - use modern API
local isAddOnLoaded = C_AddOns and C_AddOns.IsAddOnLoaded or IsAddOnLoaded
if isAddOnLoaded and isAddOnLoaded("AdiButtonAuras") then
...
hooksecurefunc("MultiActionBar_Update", CheckFakeStatus)
 ...
 -- Disable bottom right bar checkbox during combat - Updated for Midnight
...
eventWatcher:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_REGEN_DISABLED" then
        -- Modern UI doesn't use InterfaceOptionsActionBarsPanelBottomRight
        -- This is handled differently in modern WoW
...
local function CreateBar(id)
    local bottomRightShown = false
    if GetActionBarToggles then
        bottomRightShown = select(2, GetActionBarToggles())
    end
    
    local bar = CreateFrame("Frame", "FakeMultiBarBottomRight"..id, MainMenuBar or UIParent, "SecureHandlerStateTemplate")
    
    -- Modern WoW may not have MainMenuBarArtFrame
