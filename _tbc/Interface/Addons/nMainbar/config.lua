...
nMainbar.Config = {
    ...
    color = {   -- Red, Green, Blue, Alpha
        ...
        Mouseover = MakeColor(1.0, 1.0, 1.0, 1.0),  -- Mouseover-Rand (Standard: Textur-Originalfarbe)
    ...
    -- Default mouseover color (for reset button) - white = no tint = original texture color
...
configFrame:SetScript("OnEvent", function(self, event, addonName)
    if event == "ADDON_LOADED" and addonName == "nMainbar" then
        ...
        if nMainbarConfig then
            ...
            -- Restore saved mouseover color
            -- Version guard: reset if saved color was the old wrong gold default
            if nMainbarConfig.mouseoverColor then
                local mc = nMainbarConfig.mouseoverColor
                -- Old wrong default was gold (r=1, g≈0.82, b=0) - reset those to white
                local wasOldGold = (mc.r == 1.0 and mc.g ~= 1.0 and mc.b == 0.0)
                if not wasOldGold then
                    nMainbar.Config.color.Mouseover = MakeColor(mc.r, mc.g, mc.b, mc.a)
                end
                -- else: keep the new white default
            end
        end
...
function nMainbar.SaveConfig()
    ...
    -- Save mouseover color separately (user-configurable)
