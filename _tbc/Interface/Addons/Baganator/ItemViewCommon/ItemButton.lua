...
function addonTable.ItemButtonUtil.UpdateSettings()
	if not registered then
		...
		addonTable.CallbackRegistry:RegisterCallback("PluginsUpdated", function()
			addonTable.ItemButtonUtil.UpdateSettings()
			addonTable.CallbackRegistry:TriggerEvent(
				"RefreshStateChange",
				{ [addonTable.Constants.RefreshReason.ItemWidgets] = true }
			)
		end)
	end
	itemCallbacks = {}
		...
		table.insert(itemCallbacks, function(self)
			...
			if self.BGR.tooltipInfo then
				for _, row in ipairs(self.BGR.tooltipInfo.lines) do
					if
						row.leftColor.r == 1
							and row.leftColor.g < 0.2
							and row.leftColor.b < 0.2
							and row.leftText ~= ITEM_SCRAPABLE_NOT
						or row.rightColor
							and row.rightColor.r == 1
							and row.rightColor.g < 0.2
							and row.rightColor.b < 0.2
					then
								...
								local function queueFunc()
									...
									if itemButton.BGR == BGR and itemButton:IsShown() then
										...
										if
											itemButton.BGR.guid
											and (
												not C_Item.DoesItemExist(itemButton.BGR.itemLocation)
												or itemButton.BGR.guid
													~= C_Item.GetItemGUID(itemButton.BGR.itemLocation)
											)
										then
...
local function ApplyNewItemAnimation(self, quality)
	...
	local isNewItem = addonTable.NewItems:IsNewItem(self:GetParent():GetID(), self:GetID())
		and addonTable.Config.Get(addonTable.Config.Options.NEW_ITEMS_FLASHING)
...
local function GetItemContextMatch(self)
	if self.BGR and self.BGR.itemID and self.BGR.itemLocation and C_Item.DoesItemExist(self.BGR.itemLocation) then
		...
		if
			addonTable.Constants.IsRetail
			and bankFrame
			and bankFrame.currentTab.isLive
			and bankFrame.Warband:IsVisible()
		then
		...
		elseif
			addonTable.Constants.IsRetail
			and addonTable.Compatibility.Context.MailInfo
			and addonTable.Compatibility.Context.SendMail
		then
			if not C_Item.IsItemDataCachedByID(self.BGR.itemID) then
				C_Item.RequestLoadItemDataByID(self.BGR.itemID)
				needsData = true
			else
				return not self.BGR.isBound
					or C_Bank.IsItemAllowedInBankType(Enum.BankType.Account, self.BGR.itemLocation)
			end
		elseif addonTable.Compatibility.Context.Merchant then
			return not self.BGR.hasNoValue
				or (C_Item.DoesItemExist(self.BGR.itemLocation) and C_Item.CanBeRefunded(self.BGR.itemLocation))
		elseif addonTable.Compatibility.Context.GuildBanker then
			if not C_Item.IsItemDataCachedByID(self.BGR.itemID) then
				...
				return not self.BGR.isBound
					and (
						not addonTable.Constants.IsRetail or not C_Item.IsBoundToAccountUntilEquip(
							self.BGR.itemLocation
						)
					)
			end
...
function BaganatorRetailLiveContainerItemButtonMixin:MyOnLoad()
	...
	self.GetItemContextMatchResult = function()
		local result = (
			not iconSettings.contextFading
			or ItemButtonUtil.GetItemContextMatchResultForItem({
						bagID = self:GetBagID(),
						slotIndex = self:GetID(),
					})
					~= ItemButtonUtil.ItemContextMatchResult.Mismatch
				and GetItemContextMatch(self)
		)
		if self.BGR then
...
function BaganatorRetailLiveContainerItemButtonMixin:PreClickHook(mouseButton)
	if
		...
		and BankPanel:IsShown()
		and tIndexOf(Syndicator.Constants.AllBagIndexes, self:GetParent():GetID()) ~= nil
		and self.BGR.itemLocation
		and C_Item.DoesItemExist(self.BGR.itemLocation)
		and C_Bank.IsItemAllowedInBankType(BankPanel:GetActiveBankType(), self.BGR.itemLocation)
	then
...
function BaganatorRetailLiveContainerItemButtonMixin:PostUpdateItemContextOverlay()
	if self.widgetContainer then
		SetWidgetsAlpha(
			self,
			not self.ItemContextOverlay:IsShown() and (self.BGR == nil or self.BGR.matchesSearch ~= false)
		)
	end
end
...
function BaganatorRetailLiveContainerItemButtonMixin:SetItemDetails(cacheData)
	...
	GetInfo(self, cacheData, function()
		self.BGR.tooltipGetter = function()
			return C_TooltipInfo.GetBagItem(self:GetBagID(), self:GetID())
		end
		local itemLocation = { bagID = self:GetParent():GetID(), slotIndex = self:GetID() }
		if C_Item.DoesItemExist(itemLocation) then
			self.BGR.guid = C_Item.GetItemGUID(itemLocation)
			self.BGR.setInfo =
				addonTable.ItemViewCommon.GetEquipmentSetInfo(itemLocation, self.BGR.guid, self.BGR.itemLink)
...
function BaganatorRetailLiveGuildItemButtonMixin:SetItemDetails(cacheData, tabIndex)
	...
	GetInfo(self, cacheData, function()
		self.BGR.tooltipGetter = function()
			return C_TooltipInfo.GetGuildBankItem(tabIndex, self:GetID())
		end
	end, function()
...
function BaganatorRetailLiveGuildItemButtonMixin:ClearNewItem() end
...
function BaganatorClassicCachedItemButtonMixin:OnClick()
	if IsModifiedClick("CHATLINK") then
		addonTable.Utilities.ChatInsertLink(self.BGR.itemLink)
	elseif IsModifiedClick("DRESSUP") then
		return DressUpItemLink(self.BGR.itemLink)
			or DressUpBattlePetLink(self.BGR.itemLink)
			or DressUpMountLink(self.BGR.itemLink)
	elseif IsAltKeyDown() then
		addonTable.CallbackRegistry:TriggerEvent("HighlightSimilarItems", self.BGR.itemLink)
	end
end
...
function BaganatorClassicLiveGuildItemButtonMixin:ClearNewItem() end
