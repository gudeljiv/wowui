...
TimeManagerClockButton:SetScript("OnEnter", function(self)
	...
	if savedInstances > 0 then
		for index = 1, savedInstances do
			local instanceName, _, _, _, locked, _, _, isRaid, _, difficultyName, maxBosses, defeatedBosses =
				GetSavedInstanceInfo(index)

			if locked then
				instanceLockouts[index] =
					{ name = instanceName, difficulty = difficultyName, defeated = defeatedBosses, total = maxBosses }
			end
		end
		...
		if next(instanceLockouts) ~= nil then
			...
			for i, saved in ipairs(instanceLockouts) do
				...
				GameTooltip:AddDoubleLine(
					saved.name .. " |cffffffff" .. saved.difficulty .. "|r",
					saved.defeated .. "/" .. saved.total,
					1.0,
					0.82,
					0.0,
					unpack(bossColor)
