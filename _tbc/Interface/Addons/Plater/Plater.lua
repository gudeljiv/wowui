...
if not DF then
	print(
		"|cFFFFAA00Plater: framework not found, if you just installed or updated the addon, please restart your client.|r"
	)
	return
end
...
local IS_WOW_PROJECT_CLASSIC_WRATH = IS_WOW_PROJECT_NOT_MAINLINE
	and ClassicExpansionAtLeast
	and LE_EXPANSION_WRATH_OF_THE_LICH_KING
	and ClassicExpansionAtLeast(LE_EXPANSION_WRATH_OF_THE_LICH_KING)
...
local IS_WOW_PROJECT_CLASSIC_MOP = IS_WOW_PROJECT_NOT_MAINLINE
	and ClassicExpansionAtLeast
	and LE_EXPANSION_MISTS_OF_PANDARIA
	and ClassicExpansionAtLeast(LE_EXPANSION_MISTS_OF_PANDARIA)
...
---@type plater
local Plater = DF:CreateAddOn(
	"Plater",
	"PlaterDB",
	PLATER_DEFAULT_SETTINGS,
	InterfaceOptionsFrame
		and { --options table --TODO: DISABLED FOR DRAGONFLIGHT FOR NOW!
...
Plater.fullVersionInfo = Plater.versionString
	.. " - DF v"
	.. select(2, LibStub:GetLibrary("DetailsFramework-1.0"))
	.. " - "
	.. GetBuildInfo()
...
function Plater.UpdateSettingsCache() --private
	...
	Plater.ActorTypeSettingsCache[ACTORTYPE_FRIENDLY_PLAYER] =
		DF.table.copy({}, namePlateConfig[ACTORTYPE_FRIENDLY_PLAYER])
		...
		local databroker = LDB:NewDataObject("Plater", {
			...
			OnClick = function(self, button)
				...
				elseif button == "RightButton" then
					...
					end, true, nil, nil, "Start profiling", nil, true)
					GameCooltip:AddIcon(
					...
					end, true, nil, nil, "Stop profiling", nil, true)
					GameCooltip:AddIcon([[Interface\Addons\Plater\media\square_64]], 1, 1, 14, 14, 0, 1, 0, 1, "blue")
					GameCooltip:AddMenu(1, function()
						Plater.ShowPerfData()
					end, true, nil, nil, "Show profiling data", nil, true)
					...
					GameCooltip:AddIcon(
						[[Interface\Buttons\UI-Panel-HideButton-Disabled]],
						1,
						1,
						14,
						14,
					...
					GameCooltip:SetWallpaper(
						1,
						[[Interface\SPELLBOOK\Spellbook-Page-1]],
						{ 0.6, 0.1, 0.64453125, 0 },
						{ 0.8, 0.8, 0.8, 0.2 },
						true
					)
...
function Plater.GetHealthCutoffValue(getOnly)
	...
	if IS_WOW_PROJECT_MAINLINE then
		...
		local isTalentLearned = function(nodeID)
			...
			return nodeInfo and nodeInfo.entryIDsWithCommittedRanks and nodeInfo.entryIDsWithCommittedRanks[1] and true
				or false
		end
...
function Plater.CheckRange(plateFrame, onAdded)
	...
	elseif
		(not DB_USE_ALPHA_FRIENDLIES and plateFrame[MEMBER_REACTION] >= 5)
		or (not DB_USE_ALPHA_ENEMIES and plateFrame[MEMBER_REACTION] < 5)
		or (not DB_USE_RANGE_CHECK and not DB_USE_NON_TARGETS_ALPHA)
	then
		unitFrame:SetAlpha(nameplateAlpha)
		unitFrame.healthBar:SetAlpha(1)
		if not castBarFade then
	...
	if plateFrame[MEMBER_REACTION] < 5 then
		...
		rangeChecker = Plater.RangeCheckFunctionEnemy
			or LibRangeCheck:GetHarmMaxChecker(Plater.RangeCheckRangeEnemy or 40, true)
		...
		rangeChecker = Plater.RangeCheckFunctionFriendly
			or LibRangeCheck:GetFriendMaxChecker(Plater.RangeCheckRangeFriendly or 40, true)
	...
	if
		(DB_USE_NON_TARGETS_ALPHA and Plater.PlayerHasTargetNonSelf)
		or (DB_USE_FOCUS_TARGET_ALPHA and Plater.PlayerHasFocusTargetNonSelf)
	then
	...
	if DB_USE_RANGE_CHECK and rangeChecker then
		...
		if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(isInRange) then
			...
			unitFrame:SetAlphaFromBoolean(
				isInRange,
				nameplateAlpha * inRangeAlpha * (notTheTarget and overallRangeCheckAlpha or 1),
				(unitIsTarget and inRangeAlpha or overallRangeCheckAlpha)
					* nameplateAlpha
					* (notTheTarget and alphaMultiplier or 1)
			)
			healthBar:SetAlphaFromBoolean(
				isInRange,
				inRangeAlpha * (notTheTarget and healthBar_rangeCheckAlpha or 1),
				(unitIsTarget and inRangeAlpha or healthBar_rangeCheckAlpha) * (notTheTarget and alphaMultiplier or 1)
			)
			if not castBarFade then
				castBar:SetAlphaFromBoolean(
					isInRange,
					inRangeAlpha * (notTheTarget and castBar_rangeCheckAlpha or 1),
					(unitIsTarget and inRangeAlpha or castBar_rangeCheckAlpha) * (notTheTarget and alphaMultiplier or 1)
				)
			end
			...
			buffFrame1:SetAlphaFromBoolean(
				isInRange,
				inRangeAlpha * (notTheTarget and buffFrames_rangeCheckAlpha or 1),
				(unitIsTarget and inRangeAlpha or buffFrames_rangeCheckAlpha) * (notTheTarget and alphaMultiplier or 1)
			)
			buffFrame2:SetAlphaFromBoolean(
				isInRange,
				inRangeAlpha * (notTheTarget and buffFrames_rangeCheckAlpha or 1),
				(unitIsTarget and inRangeAlpha or buffFrames_rangeCheckAlpha) * (notTheTarget and alphaMultiplier or 1)
			)
		...
		elseif isInRange then
			...
			if not IS_WOW_PROJECT_MIDNIGHT then
				powerBar:SetAlpha(
					(unitIsTarget and inRangeAlpha or powerBar_rangeCheckAlpha)
						* (notTheTarget and alphaMultiplier or 1)
				)
			end
			buffFrame1:SetAlpha(
...
function Plater.GetSpellForRangeCheck()
	if tryingToUpdateRangeChecker then
		return
	end
	...
	if specIndex then
		...
		if specID and specID ~= 0 then
			...
			Plater.RangeCheckRangeFriendly = PlaterDBChr.spellRangeCheckRangeFriendly[specID]
				or Plater.DefaultSpellRangeListF[specID]
				or 40
			Plater.RangeCheckRangeEnemy = PlaterDBChr.spellRangeCheckRangeEnemy[specID]
				or Plater.DefaultSpellRangeList[specID]
				or 40
...
function Plater.RunScheduledUpdate(timerObject) --private
	...
	if plateFrame then
		...
		if
			customHealthBarWidth
			or customHealthBarHeight
			or customCastBarWidth
			or customCastBarHeight
			or customPowerBarWidth
			or customPowerBarHeight
		then
...
function Plater:RefreshConfigProfileReset() --private
	...
	DF:ShowPromptPanel("Plater profile reset, do you want /reload now (recommended)?", function()
		ReloadUI()
	end, function() end, true, 500)
end
...
function Plater:RefreshConfigProfileChanged() --private
	...
	--call the user to /reload his UI
	DF:ShowPromptPanel("Plater profile changed, do you want /reload now (recommended)?", function()
		ReloadUI()
	end, function() end, true, 500)
end
	...
	if not cvar then -- store all
		...
		if cvars_to_store[cvar] then
			...
			if callstack then
				...
				cvarLastChangedTable[cvar] = not isCVarUtil and (caller and (caller .. ":" .. line))
					or callstack
					or "N/A"
			end
		end
		...
		if cvars_to_store[cvar] then
			print(
				"CVar info:\nName: '"
					.. cvar
					.. "'\nCurrent Value: "
					.. (Plater.ParseCVarValue(GetCVar(cvar)) or "<not set>")
					.. "\nStored Value: "
					.. (Plater.db.profile.saved_cvars[cvar] or "<not stored>")
					.. "\nLast changed by: "
					.. (
						Plater.db.profile.saved_cvars_last_change[cvar]
							and ("\n" .. Plater.db.profile.saved_cvars_last_change[cvar])
						or "<no info>"
					)
			)
...
local eventFunctions = {
	...
	UNIT_FLAGS = function(_, unit)
		...
		if not string.match(unit, "nameplate%d%d?$") then
			return
		end
		...
		if plateFrame then
			...
			elseif curReaction ~= newReaction then
				if curReaction == Plater.UnitReaction.UNITREACTION_NEUTRAL and newReaction ~= curReaction then
					reactionChanged = true
				elseif
					curReaction < Plater.UnitReaction.UNITREACTION_NEUTRAL
					and newReaction >= Plater.UnitReaction.UNITREACTION_NEUTRAL
				then
					reactionChanged = true
				elseif
					curReaction > Plater.UnitReaction.UNITREACTION_NEUTRAL
	...
	GROUP_ROSTER_UPDATE = function()
		if
			(platerInternal.last_GROUP_ROSTER_UPDATE + 2) > GetTime()
			or platerInternal.has_GROUP_ROSTER_UPDATE_Scheduled
		then
	...
	PLAYER_REGEN_ENABLED = function()
		...
		if platerInternal.OpenOptionspanelAfterCombat then
			...
			C_Timer.NewTimer(1.5, function()
				Plater.OpenOptionsPanel(unpack(OpenOptionspanelAfterCombat))
			end)
		end
	end,
	...
	UNIT_NAME_UPDATE = function(_, unitID)
		if unitID then
			...
			if plateFrame and plateFrame.unitFrame.PlaterOnScreen then
				...
				if
					DB_USE_NAME_TRANSLIT
					and (not IS_WOW_PROJECT_MIDNIGHT or (IS_WOW_PROJECT_MIDNIGHT and not issecretvalue(unitName)))
				then
					unitNameTranslit = LibTranslit:Transliterate(unitName, TRANSLIT_MARK)
				end
				plateFrame[MEMBER_NAME] = unitNameTranslit
				plateFrame[MEMBER_NAMELOWER] = (IS_WOW_PROJECT_MIDNIGHT and plateFrame[MEMBER_NAME] or "")
					or lower(plateFrame[MEMBER_NAME])
	...
	PLAYER_LOGIN = function()
		...
		C_Timer.After(5, function()
			if not IS_WOW_PROJECT_MIDNIGHT then
				...
				if petGUID then
					local entry = {
						ownerGUID = Plater.PlayerGUID,
						ownerName = UnitName("player"),
						petName = UnitName("playerpet"),
						time = time(),
					}
		...
		C_Timer.After(15, function()
			if (C_AddOns.IsAddOnLoaded or C_AddOns.IsAddOnLoaded)("ElvUI") then
				if
					ElvUI[1]
					and ElvUI[1].private
					and ElvUI[1].private.nameplates
					and ElvUI[1].private.nameplates.enable
				then
					Plater:Msg(
						"'ElvUI Nameplates' and 'Plater Nameplates' are enabled and both nameplates won't work together."
					)
					Plater:Msg(
						"You may disable ElvUI Nameplates at /elvui > Nameplates section or you may disable Plater at the addon control panel."
					)
				end
			end
		end)
	...
	NAME_PLATE_CREATED = function(event, plateFrame)
		...
		if DB_USE_UIPARENT then
			...
			newUnitFrame = DF:CreateUnitFrame(
				UIParent,
				plateFrame:GetName() .. "PlaterUnitFrame",
				unitFrameOptions,
				healthBarOptions,
				castBarOptions,
				powerBarOptions
		...
		plateFrame.PlaterAnchorFrame:SetSize(
			Plater.db.profile.plate_config.enemynpc.health[1] or 112,
			Plater.db.profile.plate_config.enemynpc.health[2] or 12
		)
		...
		local buffFrame2 = CreateFrame(
			"frame",
			unitFrame:GetName() .. "BuffFrame2",
			unitFrame,
			BackdropTemplateMixin and "BackdropTemplate"
		)
		buffFrame2 = CreateFrame(
			"frame",
			unitFrame:GetName() .. "BuffFrame2",
			unitFrame,
			BackdropTemplateMixin and "BackdropTemplate"
		...
		plateFrame.debugAreaTexture:SetColorTexture(0.1, 0.1, 0.1, 0.834)
		...
		plateFrame.debugAreaText:SetTextColor(0.7, 0.7, 0.7)
		...
		healthBar.FrameOverlay =
			CreateFrame("frame", "$parentOverlayFrame", healthBar, BackdropTemplateMixin and "BackdropTemplate")
		...
		local cutoffAnimationOnStop = function()
			healthCutOff:SetAlpha(0.5)
		end

		local healthCutOffShowAnimation =
			DF:CreateAnimationHub(healthCutOff, cutoffAnimationOnPlay, cutoffAnimationOnStop)
		DF:CreateAnimation(healthCutOffShowAnimation, "Scale", 1, 0.2, 0.3, 0.3, 1.2, 1.2)
		DF:CreateAnimation(healthCutOffShowAnimation, "Scale", 2, 0.2, 1.2, 1.2, 1, 1)
		DF:CreateAnimation(healthCutOffShowAnimation, "Alpha", 1, 0.2, 0.2, 1)
		DF:CreateAnimation(healthCutOffShowAnimation, "Alpha", 2, 0.2, 1, 0.5)
		healthCutOff.ShowAnimation = healthCutOffShowAnimation

		local healthCutOffUpperShowAnimation =
			DF:CreateAnimationHub(healthCutOffUpper, cutoffAnimationOnPlay, cutoffAnimationOnStop)
		DF:CreateAnimation(healthCutOffUpperShowAnimation, "Scale", 1, 0.2, 0.3, 0.3, 1.2, 1.2)
		DF:CreateAnimation(healthCutOffUpperShowAnimation, "Scale", 2, 0.2, 1.2, 1.2, 1, 1)
		DF:CreateAnimation(healthCutOffUpperShowAnimation, "Alpha", 1, 0.2, 0.2, 1)
		DF:CreateAnimation(healthCutOffUpperShowAnimation, "Alpha", 2, 0.2, 1, 0.5)
		...
		local executeGlowAnimationOnStop = function() end

		executeGlowUp.ShowAnimation =
			DF:CreateAnimationHub(executeGlowUp, executeGlowAnimationOnPlay, executeGlowAnimationOnStop)
		DF:CreateAnimation(executeGlowUp.ShowAnimation, "Scale", 1, 0.2, 1, 0.1, 1, 1.2, "bottom", 0, 0)
		DF:CreateAnimation(executeGlowUp.ShowAnimation, "Scale", 1, 0.2, 1, 1, 1, 1)

		executeGlowDown.ShowAnimation =
			DF:CreateAnimationHub(executeGlowDown, executeGlowAnimationOnPlay, executeGlowAnimationOnStop)
		DF:CreateAnimation(executeGlowDown.ShowAnimation, "Scale", 1, 0.2, 1, 0.1, 1, 1.2, "top", 0, 0)
		DF:CreateAnimation(executeGlowDown.ShowAnimation, "Scale", 1, 0.2, 1, 1.1, 1, 1)

		executeGlowUpperUp.ShowAnimation =
			DF:CreateAnimationHub(executeGlowUpperUp, executeGlowAnimationOnPlay, executeGlowAnimationOnStop)
		DF:CreateAnimation(executeGlowUpperUp.ShowAnimation, "Scale", 1, 0.2, 1, 0.1, 1, 1.2, "bottom", 0, 0)
		DF:CreateAnimation(executeGlowUpperUp.ShowAnimation, "Scale", 1, 0.2, 1, 1, 1, 1)

		executeGlowUpperDown.ShowAnimation =
			DF:CreateAnimationHub(executeGlowUpperDown, executeGlowAnimationOnPlay, executeGlowAnimationOnStop)
		DF:CreateAnimation(executeGlowUpperDown.ShowAnimation, "Scale", 1, 0.2, 1, 0.1, 1, 1.2, "top", 0, 0)
		DF:CreateAnimation(executeGlowUpperDown.ShowAnimation, "Scale", 1, 0.2, 1, 1.1, 1, 1)
		...
		unitFrame.PlaterRaidTargetFrame =
			CreateFrame("frame", nil, unitFrame, BackdropTemplateMixin and "BackdropTemplate")
		...
		DF:CreateAnimation(raidMarkAnimation, "Scale", 1, 0.075, 0.1, 0.1, 1.2, 1.2)
		DF:CreateAnimation(raidMarkAnimation, "Scale", 2, 0.075, 1.2, 1.2, 1, 1)
		...
		castBar2.FrameOverlay =
			CreateFrame("frame", "$parentOverlayFrame", castBar2, BackdropTemplateMixin and "BackdropTemplate")
		...
		--> 3D model frame
		plateFrame.Top3DFrame =
			CreateFrame("playermodel", plateFrame:GetName() .. "3DFrame", plateFrame, "ModelWithControlsTemplate")
		...
		if Plater.Masque and not castIconFrame.Masqued then
			...
			local dummyMasqueIconButton = CreateFrame(
				"Button",
				castBar:GetName() .. "dummyMasqueIconButton",
				castBar,
				BackdropTemplateMixin and "BackdropTemplate"
			)
					...
					if not v then
						...
						if type(v) == "function" then
							return function(self, ...)
								return castIconFrame[k](castIconFrame, ...)
							end
						end
					end
					return v
		...
		--this is ideal for adding borders and other overlays
		castBar.FrameOverlay =
			CreateFrame("frame", "$parentOverlayFrame", castBar, BackdropTemplateMixin and "BackdropTemplate")
		...
		castBar.FrameDownlayer =
			CreateFrame("frame", "$parentDownlayerFrame", castBar, BackdropTemplateMixin and "BackdropTemplate")
		...
		--> soft-interact icon
		unitFrame.softInteractIconFrame = CreateFrame(
			"frame",
			unitFrame:GetName() .. "softInteractIconFrame",
			plateFrame,
			BackdropTemplateMixin and "BackdropTemplate"
		)
		...
		PixelUtil.SetPoint(
			unitFrame.softInteractIconFrame.Mask,
			"CENTER",
			unitFrame.softInteractIconFrame,
			"CENTER",
			0,
			0
		)
	...
	FORBIDDEN_NAME_PLATE_UNIT_ADDED = function(event, unitBarId)
		...
		if plateFrame then -- and plateFrame.template == "ForbiddenNamePlateUnitFrameTemplate"
			if not IS_WOW_PROJECT_MAINLINE then
				...
				if
					GetCVarBool("nameplateShowOnlyNames")
					or Plater.db.profile.saved_cvars.nameplateShowOnlyNames == "1"
				then
	...
	NAME_PLATE_UNIT_ADDED = function(event, unitBarId)
					...
					if not IS_WOW_PROJECT_MAINLINE then
						...
						then
							TextureLoadingGroupMixin.RemoveTexture(
								{ textures = plateFrame.UnitFrame.CastBar },
								"showCastbar"
							)
						else
		...
		reaction = reaction
			or isSoftInteract and Plater.UnitReaction.UNITREACTION_NEUTRAL
			or Plater.UnitReaction.UNITREACTION_HOSTILE
		reaction = reaction <= Plater.UnitReaction.UNITREACTION_HOSTILE and Plater.UnitReaction.UNITREACTION_HOSTILE
			or reaction >= Plater.UnitReaction.UNITREACTION_FRIENDLY and Plater.UnitReaction.UNITREACTION_FRIENDLY
			or Plater.UnitReaction.UNITREACTION_NEUTRAL
		...
		local isPlateEnabled = not isSoftInteractObject
				and (DB_PLATE_CONFIG[actorType].module_enabled and not isWidgetOnlyMode)
			or (not Plater.db.profile.ignore_softinteract_objects and isSoftInteractObject)
		isPlateEnabled = (isPlayer or not Plater.ForceBlizzardNameplateUnits[plateFrame[MEMBER_NPCID]])
			and isPlateEnabled
		...
		if isPlateEnabled then
			...
			local onlyNames = GetCVarBool("nameplateShowOnlyNames")
				or Plater.db.profile.saved_cvars.nameplateShowOnlyNames == "1"
				or GetCVarBool("nameplateShowOnlyNameForFriendlyPlayerUnits")
				or Plater.db.profile.saved_cvars.nameplateShowOnlyNameForFriendlyPlayerUnits == "1"
		...
		local castBarWasEnabled = (unitFrame.Settings.ShowCastBar and (plateFrame.PreviousUnitType == actorType))
			or (unitFrame.unit ~= unitID)
		...
		if Plater.PerformanceUnits[plateFrame[MEMBER_NPCID]] then
			...
			unitFrame.isPerformanceUnitAura = bit.band(perfUnitData, platerInternal.PERF_UNIT_OVERRIDES_BIT.AURA)
						== 0
					and true
				or false
			unitFrame.isPerformanceUnitThreat = bit.band(perfUnitData, platerInternal.PERF_UNIT_OVERRIDES_BIT.THREAT)
						== 0
					and true
				or false
			unitFrame.isPerformanceUnitCast = bit.band(perfUnitData, platerInternal.PERF_UNIT_OVERRIDES_BIT.CAST)
						== 0
					and true
				or false
		...
		unitFrame.InCombat = UnitAffectingCombat(unitID)
			or (Plater.ForceInCombatUnits[unitFrame[MEMBER_NPCID]] and PLAYER_IN_COMBAT)
			or false
				...
				if isPlayer then
					...
					elseif isBattlePet then
						...
						if
							not isObject
							and (
								Plater.ZoneInstanceType == "raid"
								or Plater.ZoneInstanceType == "party"
								or Plater.ZoneInstanceType == "scenario"
							)
						then
							if plateFrame[MEMBER_NPCID] and plateFrame[MEMBER_NAME] ~= UNKNOWN then --UNKNOWN is the global string from blizzard
								...
								if not npcCacheInfo then
									DB_NPCIDS_CACHE[plateFrame[MEMBER_NPCID]] = {
										plateFrame.unitNameInternal,
										Plater.ZoneName or "UNKNOWN",
										Plater.Locale or "enUS",
									}
								else
						...
						local isTanking, threatStatus, threatpct, threatrawpct, threatValue =
		...
		if IS_WOW_PROJECT_MAINLINE then
			...
			if plateFrame.unitFrame.WidgetContainer then
				...
				Plater.SetAnchor(
					plateFrame.unitFrame.WidgetContainer,
					Plater.db.profile.widget_bar_anchor,
					plateFrame.unitFrame
				)
			end
		end

		--can check aggro
		unitFrame.CanCheckAggro = unitFrame.displayedUnit == unitID
			and actorType == ACTORTYPE_ENEMY_NPC
			and not unitFrame.isPerformanceUnitThreat
		...
		if
			DB_HOVER_HIGHLIGHT
			and (not plateFrame.PlayerCannotAttack or (plateFrame.PlayerCannotAttack and DB_SHOW_HEALTHBARS_FOR_NOT_ATTACKABLE))
			and (actorType == ACTORTYPE_ENEMY_PLAYER or actorType == ACTORTYPE_ENEMY_NPC)
		then
	...
	NAME_PLATE_UNIT_REMOVED = function(event, unitBarId)
		...
		plateFrame.PlaterAnchorFrame:ClearAllPoints()
		plateFrame.PlaterAnchorFrame:SetParent(plateFrame)
		local enemyHealthSize = Plater.db.profile.plate_config.enemynpc
				and Plater.db.profile.plate_config.enemynpc.health
			or { 112, 12 }
		...
		if
			plateFrame.unitFrame.healthBar.HealthFlashFrame
			and plateFrame.unitFrame.healthBar.HealthFlashFrame.animation
		then
...
function Plater.OnRetailNamePlateShow(self) --private
	...
	if self:IsProtected() then
		...
		if self.HealthBarsContainer then
			self.HealthBarsContainerOrigParent = self.HealthBarsContainer:GetParent()
				or self.HealthBarsContainerOrigParent
...
function Plater.OnInit() --private --~oninit ~init
	...
	SUPPORT_BLIZZARD_PLATEFRAMES = not DB_PLATE_CONFIG[ACTORTYPE_PLAYER].module_enabled
		or not DB_PLATE_CONFIG[ACTORTYPE_FRIENDLY_PLAYER].module_enabled
		or not DB_PLATE_CONFIG[ACTORTYPE_ENEMY_PLAYER].module_enabled
		or not DB_PLATE_CONFIG[ACTORTYPE_FRIENDLY_NPC].module_enabled
	...
	Plater.db.profile.plate_config.global_health_width = Plater.db.profile.plate_config.global_health_width
		or Plater.db.profile.plate_config.enemynpc.health[1]
	Plater.db.profile.plate_config.global_health_height = Plater.db.profile.plate_config.global_health_height
		or Plater.db.profile.plate_config.enemynpc.health[2]
	...
	LibRangeCheck.RegisterCallback(Plater, LibRangeCheck.CHECKERS_CHANGED, function()
		Plater.GetSpellForRangeCheck()
	end)
			...
			for _, plateFrame in ipairs(Plater.GetAllShownPlates()) do
				...
				if plateFrame then
					for i = 1, HOOK_PLAYER_POWER_UPDATE.ScriptAmount do
						...
						local scriptContainer = unitFrame:ScriptGetContainer()
						local scriptInfo =
							unitFrame:HookGetInfo(globalScriptObject, scriptContainer, "Player Power Update")
						--run
	...
	function Plater.UpdateResourceFrameAnchor(buffFrame)
		if Plater.CurrentTargetResourceFrame then
			...
			if buffFrame.unitFrame[MEMBER_TARGET] then
				...
				if
					buffFrame.amountAurasShown > 0
					or (DB_AURA_SEPARATE_BUFFS and buffFrame.BuffFrame2.amountAurasShown > 0)
				then
					Plater.CurrentTargetResourceFrame:SetPoint(
						"bottom",
						buffFrame.healthBar,
						"top",
						0,
						Plater.db.profile.resources.y_offset_target
							+ Plater.db.profile.resources.y_offset_target_withauras
					)
				else
					Plater.CurrentTargetResourceFrame:SetPoint(
						"bottom",
	...
	function Plater.UpdatePersonalBar(self)
		...
		if plateFrame then
			...
			if
				NamePlateDriverFrame.classNamePlatePowerBar and NamePlateDriverFrame.classNamePlatePowerBar:IsShown()
			then
			...
			if DB_PLATE_CONFIG.player.power_enabled then
				...
				if plateConfigs.power_percent_text_enabled then
					...
					Plater.SetFontOutlineAndShadow(
						textString,
						plateConfigs.power_percent_text_outline,
						plateConfigs.power_percent_text_shadow_color,
						plateConfigs.power_percent_text_shadow_color_offset[1],
						plateConfigs.power_percent_text_shadow_color_offset[2]
					)
				else
			...
			if onCurrentTarget then
				...
				if targetPlateFrame then
					...
					resourceFrame:SetPoint(
						"bottom",
						targetPlateFrame.unitFrame.healthBar,
						"top",
						0,
						Plater.db.profile.resources.y_offset_target
					)
				...
				if personalPlateFrame then
					...
					if personalPlateFrame.unitFrame.powerBar:IsShown() then
						resourceFrame:SetPoint(
							"top",
							personalPlateFrame.unitFrame.powerBar,
							"bottom",
							0,
							-3 + Plater.db.profile.resources.y_offset
						)
					else
						resourceFrame:SetPoint(
							"top",
							personalPlateFrame.unitFrame.healthBar,
							"bottom",
							0,
							-3 + Plater.db.profile.resources.y_offset
						)
	...
	function Plater.SetNamePlatePreferredClickInsets(nameplateType, left, right, top, bottom)
		if not InCombatLockdown() then
			...
			C_Timer.After(1, function()
				Plater.SetNamePlatePreferredClickInsets(nameplateType, left, right, top, bottom)
			end)
		end
	end
	if not IS_WOW_PROJECT_MIDNIGHT then
	...
	function platerInternal.CastBar.StartTestCastBarForNameplate(plateFrame)
		...
		if plateFrame.unitFrame.PlaterOnScreen then
			...
			if castBar.channeling then
				Plater.CastBarOnEvent_Hook(
					castBar,
					"UNIT_SPELLCAST_CHANNEL_START",
					plateFrame.unitFrame.unit,
					plateFrame.unitFrame.unit
				)
			else
				Plater.CastBarOnEvent_Hook(
	...
	function Plater.UpdateCastbarTargetText(castBar, targetFontString)
		...
		if profile.castbar_target_show then
			...
			Plater.SetFontOutlineAndShadow(
				textString,
				profile.castbar_target_outline,
				profile.castbar_target_shadow_color,
				profile.castbar_target_shadow_color_offset[1],
				profile.castbar_target_shadow_color_offset[2]
			)
			...
			if profile.castbar_icon_show then
				...
				if profile.castbar_icon_attach_to_side == "left" then
					...
					elseif profile.castbar_icon_size == "same as castbar plus healthbar" then
						...
						local healthBarHeight = unitFrame.customHealthBarHeight
							or (plateConfigs and plateConfigs[healthBarConfigKey][2])
							or 0
		...
		if self.isNamePlate then
			...
			if event == "UNIT_SPELLCAST_START" or event == "UNIT_SPELLCAST_CHANNEL_START" then
				...
				if not IS_WOW_PROJECT_MIDNIGHT or (IS_WOW_PROJECT_MIDNIGHT and not issecretvalue(self.spellID)) then
					...
					if customColor then
						...
						if color and isEnabled then
							...
							if originalCastColor then
								...
								self.castColorTexture:SetHeight(
									self:GetHeight() + profile.cast_color_settings.height_offset
								)
							end
						end
					...
					if
						(self.SpellNameRenamed == self.SpellName or Plater.db.profile.bossmod_castrename_priority)
						and Plater.db.profile.bossmod_support_enabled
						and Plater.db.profile.bossmod_castrename_enabled
					then
						local bmSpellName = (
							(BigWigsAPI and BigWigsAPI.GetSpellRename and BigWigsAPI.GetSpellRename(self.spellID))
							or (DBM and DBM.GetAltSpellName and DBM:GetAltSpellName(self.spellID))
						) or nil
						if bmSpellName then
				...
				if
					unitFrame.ActorType == "enemynpc"
					and event == "UNIT_SPELLCAST_CHANNEL_START"
					and (
						not DB_CAPTURED_SPELLS[spellID]
						or DB_CAPTURED_SPELLS[spellID].isChanneled == nil
						or not DB_CAPTURED_CASTS[spellID]
						or DB_CAPTURED_CASTS[spellID].isChanneled == nil
					)
				then
					parserFunctions.SPELL_CAST_SUCCESS(
						nil,
						"SPELL_CAST_SUCCESS",
						nil,
						unitFrame[MEMBER_GUID],
						unitFrame.unitNameInternal,
						0x00000040,
						nil,
						nil,
						nil,
			...
			if shouldRunCastStartHook then
				if HOOK_CAST_START.ScriptAmount > 0 then
					for i = 1, HOOK_CAST_START.ScriptAmount do
						...
						scriptEnv._Duration = not IS_WOW_PROJECT_MIDNIGHT and (self.SpellEndTime - self.SpellStartTime)
							or nil
						...
						scriptEnv._RemainingTime = not IS_WOW_PROJECT_MIDNIGHT and max(self.SpellEndTime - GetTime(), 0)
							or nil
		...
		if self.percentText then --check if is a plater cast bar
			...
			if self.ThrottleUpdate < 0 then
						...
						if targetName then
							...
							if canShowTargetName then
								if
									DB_USE_NAME_TRANSLIT
									and (
										not IS_WOW_PROJECT_MIDNIGHT
										or (IS_WOW_PROJECT_MIDNIGHT and not issecretvalue(targetName))
									)
								then
								...
								if class then
									...
									self.FrameOverlay.TargetName:SetText(targetName)
									DF:SetFontColor(
										self.FrameOverlay.TargetName,
										Plater.db.profile.castbar_target_color
									)
								end
							else
				...
				if not IS_WOW_PROJECT_MIDNIGHT or (IS_WOW_PROJECT_MIDNIGHT and not issecretvalue(self.SpellName)) then
					...
					if
						globalScriptObject
						and self.SpellEndTime
						and GetTime() < self.SpellEndTime
						and (self.casting or self.channeling)
	...
	function Plater.QuickHealthUpdate(unitFrame)
		...
		unitFrame.healthBar:SetMinMaxValues(
			0,
			unitHealthMax,
			IS_WOW_PROJECT_MIDNIGHT and Enum.StatusBarInterpolation.ExponentialEaseOut
		)
		unitFrame.healthBar:SetValue(
			unitHealth,
			IS_WOW_PROJECT_MIDNIGHT and Enum.StatusBarInterpolation.ExponentialEaseOut
		)
		...
		if IS_WOW_PROJECT_MIDNIGHT then
			...
			unitFrame.healthBar.currentHealthPercent =
				UnitHealthPercent(unitFrame.unit, true, CurveConstants.ScaleTo100)
		else
	...
	local run_on_health_change_hook = function(unitFrame)
		if unitFrame.isPerformanceUnit then
			return
		end -- don't run health update hooks on performance units
	...
	function Plater.OnUpdateHealth(self) --self is unitFrame.healthBar
		...
		if plateFrame.IsSelf then
			...
			if plateFrame.PlateConfig.healthbar_color_by_hp then
				...
				local r, g, b = DF:LerpLinearColor(
					math.abs(currentHealth / currentHealthMax - 1),
					1,
					originalColor[1],
					originalColor[2],
					originalColor[3],
			...
			if DB_USE_QUICK_HIDE and IS_WOW_PROJECT_MAINLINE then
				if
					(not UnitExists(unitFrame.unit) and not UnitIsVisible(unitFrame.unit))
					or (not IS_WOW_PROJECT_MIDNIGHT and self.CurrentHealth < 1)
				then
					--the unit died!
					unitFrame:Hide()
	...
	function Plater.OnProfileCreated()
		C_Timer.After(0.5, function()
			...
			DF:ShowPromptPanel("Plater profile created, do you want /reload now (recommended)?", function()
				ReloadUI()
			end, function() end, true, 500)
		end)
	end
	...
	if IS_WOW_PROJECT_MAINLINE then
		for classID = 1, MAX_CLASSES do
			...
			local GetNumSpecializationsForClassID = GetNumSpecializationsForClassID
				or C_SpecializationInfo.GetNumSpecializationsForClassID --10.0.5
...
function Plater.SetQuestColorByReaction(unitFrame)
	...
	if unitFrame.ActorType and DB_PLATE_CONFIG[unitFrame.ActorType].quest_color_enabled then
		if unitFrame[MEMBER_REACTION] == Plater.UnitReaction.UNITREACTION_NEUTRAL then
			Plater.ChangeHealthBarColor_Internal(
				unitFrame.healthBar,
				unpack(DB_PLATE_CONFIG[unitFrame.ActorType].quest_color_neutral)
			)
		elseif unitFrame[MEMBER_REACTION] < Plater.UnitReaction.UNITREACTION_NEUTRAL then
			Plater.ChangeHealthBarColor_Internal(
				unitFrame.healthBar,
				unpack(DB_PLATE_CONFIG[unitFrame.ActorType].quest_color_enemy)
			)
		else
			...
			Plater.ChangeHealthBarColor_Internal(
				unitFrame.healthBar,
				unpack(DB_PLATE_CONFIG[unitFrame.ActorType].quest_color or { 0.5, 1, 0, 1 })
			)
		end
	end
end
...
function Plater.FindAndSetNameplateColor(unitFrame, forceRefresh)
	...
	if unitFrame.IsSelf or not unitFrame.PlaterOnScreen then
		...
		elseif
			Plater.db.profile.unit_type_coloring_enabled
			and (Plater.ZoneInstanceType == "party" or Plater.ZoneInstanceType == "raid")
			and unitFrame.isGoodAggroState
		then
...
function Plater.FullRefreshAllPlates() --private
	for _, plateFrame in ipairs(Plater.GetAllShownPlates()) do
		...
		Plater.RunScheduledUpdate({
			unitId = plateFrame[MEMBER_UNITID] or plateFrame.unitFrame[MEMBER_UNITID],
			GUID = plateFrame[MEMBER_GUID],
		})
	end
end
...
function Plater.UpdateSelfPlate() --private
	...
	if InCombatLockdown() then
		return C_Timer.After(0.3, re_update_self_plate)
	end
...
function Plater.UpdatePlateSize(plateFrame)
	...
	if plateFrame.actorType == ACTORTYPE_ENEMY_PLAYER then
		if
			(Plater.ZoneInstanceType == "pvp" or Plater.ZoneInstanceType == "arena")
			and DB_PLATE_CONFIG.player.pvp_always_incombat
		then
			isInCombat = true
		end
	end
	...
	local healthBarWidth, healthBarHeight =
		unitFrame.customHealthBarWidth or plateConfigs[healthBarConfigKey][1],
		unitFrame.customHealthBarHeight or plateConfigs[healthBarConfigKey][2]
	local castBarWidth, castBarHeight =
		unitFrame.customCastBarWidth or plateConfigs[castBarConfigKey][1],
		unitFrame.customCastBarHeight or plateConfigs[castBarConfigKey][2]
	local powerBarWidth, powerBarHeight =
		unitFrame.customPowerBarWidth or plateConfigs[manaConfigKey][1],
		unitFrame.customPowerBarHeight or plateConfigs[manaConfigKey][2]
	...
	if DB_USE_UIPARENT then
		...
		PixelUtil.SetPoint(
			...
			unitFrame,
			"topleft",
			xOffSet + profile.global_offset_x,
			-yOffSet + profile.global_offset_y
		)
		PixelUtil.SetPoint(
			healthBar,
			"bottomright",
	...
	PixelUtil.SetPoint(
		...
		healthBar,
		"bottomright",
		-castBarOffSetXRel + castBarOffSetX,
		castBarOffSetY
	)
	...
	castBar._points = {
		{ "topleft", healthBar, "bottomleft", castBarOffSetXRel + castBarOffSetX, castBarOffSetY },
	...
	Plater.SetAnchor(
		buffFrame1,
		{ side = bf1Anchor.side, x = bf1Anchor.x, y = bf1Anchor.y + plateConfigs.buff_frame_y_offset },
		unitFrame.healthBar,
		(Plater.db.profile.aura_grow_direction or 2) == 2
	)

	local bf2Anchor = Plater.db.profile.aura_frame2_anchor
	Plater.SetAnchor(
		buffFrame2,
		{ side = bf2Anchor.side, x = bf2Anchor.x, y = bf2Anchor.y + plateConfigs.buff_frame_y_offset },
		unitFrame.healthBar,
		(Plater.db.profile.aura2_grow_direction or 2) == 2
	)

	if
		(Plater.db.profile.show_health_prediction or Plater.db.profile.show_shield_prediction)
		and healthBar.displayedUnit
	then
...
function Plater.SetPlateBackground(plateFrame)
	plateFrame.unitFrame:SetBackdrop({
...
function Plater.UpdateBaseNameplateOptions()
	...
	if GetCVarBool("nameplateShowOnlyNames") or Plater.db.profile.saved_cvars.nameplateShowOnlyNames == "1" then
		...
		TextureLoadingGroupMixin.AddTexture(
			{ textures = DefaultCompactNamePlateFrameSetUpOptions },
		...
		TextureLoadingGroupMixin.AddTexture(
			{ textures = DefaultCompactNamePlateFriendlyFrameOptions },
			"colorNameBySelection"
		)
		TextureLoadingGroupMixin.AddTexture(
			{ textures = DefaultCompactNamePlateFriendlyFrameOptions },
		...
		TextureLoadingGroupMixin.AddTexture(
			{ textures = DefaultCompactNamePlateEnemyFrameOptions },
			"colorNameBySelection"
		)
		TextureLoadingGroupMixin.AddTexture(
			{ textures = DefaultCompactNamePlateEnemyFrameOptions },
...
function Plater.UpdatePlateClickSpace(needReorder, isDebug) --private
	...
	if IS_WOW_PROJECT_MIDNIGHT then
		...
		widthScale, heightScale =
			Plater.db.profile.click_space_scale_friendly[1], Plater.db.profile.click_space_scale_friendly[2]
...
function Plater.NameplateTick(tickFrame, deltaTime) --private
	...
	if shouldUpdate then
		...
		if HEALTHCUTOFF_AT_DATA.healthCutOffActive and not unitFrame.IsSelf and not unitFrame.PlayerCannotAttack then
			-- setup
			if
				HEALTHCUTOFF_AT_DATA.healthCutOffValue
				and HEALTHCUTOFF_AT_DATA.healthCutOffValue ~= healthBar.healthCutOffValue
				and HEALTHCUTOFF_AT_DATA.healthCutOffValue > 0
			then
				healthBar.healthCutOff:ClearAllPoints()
				healthBar.healthCutOff:SetSize(healthBar:GetHeight(), healthBar:GetHeight())
				healthBar.healthCutOff:SetPoint(
					"center",
					healthBar,
					"left",
					healthBar:GetWidth() * HEALTHCUTOFF_AT_DATA.healthCutOffValue,
					0
				)

				healthBar.executeRange:SetTexCoord(0, HEALTHCUTOFF_AT_DATA.healthCutOffValue, 0, 1)
				healthBar.executeRange:SetAlpha(0.2)
				healthBar.executeRange:SetVertexColor(0.3, 0.3, 0.3)
			...
			if
				HEALTHCUTOFF_AT_DATA.healthCutOffUpperValue
				and HEALTHCUTOFF_AT_DATA.healthCutOffUpperValue ~= healthBar.healthCutOffUpperValue
				and HEALTHCUTOFF_AT_DATA.healthCutOffUpperValue < 1
			then
				healthBar.healthCutOffUpper:ClearAllPoints()
				healthBar.healthCutOffUpper:SetSize(healthBar:GetHeight(), healthBar:GetHeight())
				healthBar.healthCutOffUpper:SetPoint(
					"center",
					healthBar,
					"right",
					-(healthBar:GetWidth() * (1 - HEALTHCUTOFF_AT_DATA.healthCutOffUpperValue)),
					0
				)
				...
				healthBar.executeRangeUpper:SetVertexColor(0.3, 0.3, 0.3)
			...
			if IS_WOW_PROJECT_MIDNIGHT then
				if HEALTHCUTOFF_AT_DATA.healthCutOffValue and HEALTHCUTOFF_AT_DATA.healthCutOffValue > 0 then
					local _, _, _, lowerAlpha = UnitHealthPercent(
						unitFrame[MEMBER_UNITID],
						true,
						HEALTHCUTOFF_AT_DATA.healthCutOffValueCurve
					):GetRGBA()
					--lower
				...
				if HEALTHCUTOFF_AT_DATA.healthCutOffUpperValue and HEALTHCUTOFF_AT_DATA.healthCutOffUpperValue < 1 then
					--upper
					local _, _, _, upperAlpha = UnitHealthPercent(
						unitFrame[MEMBER_UNITID],
						true,
						HEALTHCUTOFF_AT_DATA.healthCutOffUpperValueCurve
					):GetRGBA()
		...
		if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(isSoftInteract) then
		...
		unitFrame.PlateFrame.isSoftInteractObject = isSoftInteract
			and (unitFrame.PlateFrame.isObject or unitFrame.isObject)
		...
		if
			actorTypeDBConfig.percent_text_enabled
			and (
				(profile.use_player_combat_state and PLAYER_IN_COMBAT or unitFrame.InCombat)
				or actorTypeDBConfig.percent_text_ooc
			)
		then
			Plater.UpdateLifePercentText(
				healthBar,
				unitFrame.unit,
				actorTypeDBConfig.percent_show_health,
				actorTypeDBConfig.percent_show_percent,
				actorTypeDBConfig.percent_text_show_decimals
			)
		...
		if not IS_WOW_PROJECT_MIDNIGHT then
			...
			local globalScriptObject = SCRIPT_UNIT_TRIGGER_CACHE[tickFrame.PlateFrame[MEMBER_NAMELOWER]]
				or SCRIPT_UNIT_TRIGGER_CACHE[unitFrame[MEMBER_NPCID]]
			--check if this aura has a custom script
		...
		--details! integration
		if
			IS_USING_DETAILS_INTEGRATION
			and not tickFrame.PlateFrame.IsSelf
			and PLAYER_IN_COMBAT
			and unitFrame.InCombat
		then
			local detailsPlaterConfig = Details.plater
		...
		if UnitGetTotalAbsorbs then
			if profile.indicator_shield then
				local amountAbsorb = UnitGetTotalAbsorbs(
					tickFrame.PlateFrame[MEMBER_UNITID] or tickFrame.PlateFrame.unitFrame[MEMBER_UNITID]
				)
				if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(amountAbsorb) then
					amountAbsorb = 0
				end --MIDNIGHT!!
	...
	if DB_LERP_COLOR and not unitFrame.isPerformanceUnit then
		...
		local r, g, b = DF:LerpLinearColor(
			...
			currentG,
			currentB,
			healthBar.R or currentR,
			healthBar.G or currentG,
			healthBar.B or currentB
		)
...
local set_aggro_color = function(self, r, g, b, a) --self = unitName
	if DB_AGGRO_CHANGE_HEALTHBAR_COLOR then
		if not self.DenyColorChange then --tagged from a script
			if
				not self.isGoodAggroState and Plater.db.profile.unit_type_coloring_enabled
				or not Plater.db.profile.unit_type_coloring_enabled
			then
...
function Plater.UpdateNameplateThread(self) --self = unitFrame
	...
	local isTanking, threatStatus, threatpct, threatrawpct, threatValue =
		UnitDetailedThreatSituation("player", self.displayedUnit)
				...
				if IsInRaid() then
					...
					for tank, _ in pairs(TANK_CACHE) do
						if UnitExists(tank) and not UnitIsUnit("player", tank) then
							otherIsTanking, otherThreatStatus, otherThreatpct =
								UnitDetailedThreatSituation(tank, self.displayedUnit)
				...
				elseif threatStatus == 2 then --player is tanking the mob with low aggro
					...
					if Plater.ZoneInstanceType == "party" or Plater.ZoneInstanceType == "raid" then
						...
						if DB_AGGRO_CAN_CHECK_NOTANKAGGRO then
							...
							if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(unitTarget) then
								unitTarget = "N/A"
							end --MIDNIGHT!!
...
function Plater.UpdateSoftInteractTarget(plateFrame, updateText)
	...
	if isSoftInteract and Plater.db.profile.show_softinteract_icons then
		--re-anchor
		Plater.SetAnchor(
			unitFrame.softInteractIcon,
			unitFrame.softInteractIcon.anchor or { side = 8, x = 0, y = 18 },
			plateFrame
		)
		...
		local hasTexture = SetUnitCursorTexture(
			unitFrame.softInteractIcon,
			plateFrame[MEMBER_UNITID] or unitFrame[MEMBER_UNITID],
			nil,
			true
		)
...
function Plater.UpdateTarget(plateFrame) --private
	...
	if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(isFocus) then
		isFocus = false
	end
	if isFocus then
	...
	if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(isTarget) then
		isTarget = false
	end
	if isTarget then
		...
		if profile.target_highlight then
			if
				plateFrame.actorType ~= ACTORTYPE_FRIENDLY_PLAYER
				and plateFrame.actorType ~= ACTORTYPE_FRIENDLY_NPC
				and not plateFrame.PlayerCannotAttack
				and unitFrame.healthBar:IsShown()
			then
		...
		if
			unitFrame.IsTarget
			or unitFrame.TargetTextures2Sides[1]:IsShown()
			or unitFrame.TargetTextures4Sides[1]:IsShown()
		then
			for i = 1, 2 do
		...
		if
			DB_TARGET_SHADY_ENABLED
			and (not DB_TARGET_SHADY_COMBATONLY or (profile.use_player_combat_state and PLAYER_IN_COMBAT or unitFrame.InCombat))
			and not plateFrame.IsSelf
		then
...
function Plater.OnPlayerTargetChanged() --private
	...
	if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(selfTarget) then
		selfTarget = false
	end --MIDNIGHT!!
	...
	if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(selfFocus) then
		selfFocus = false
	end --MIDNIGHT!!
...
function Plater.UpdatePlateText(plateFrame, plateConfigs, needReset) --private
	...
	if needReset then
		...
		Plater.SetFontOutlineAndShadow(
			spellnameString,
			plateConfigs.spellname_text_outline,
			plateConfigs.spellname_text_shadow_color,
			plateConfigs.spellname_text_shadow_color_offset[1],
			plateConfigs.spellname_text_shadow_color_offset[2]
		)
	...
	if plateConfigs.spellpercent_text_enabled then
		...
		if needReset then
			...
			Plater.SetFontOutlineAndShadow(
				spellPercentString,
				plateConfigs.spellpercent_text_outline,
				plateConfigs.spellpercent_text_shadow_color,
				plateConfigs.spellpercent_text_shadow_color_offset[1],
				plateConfigs.spellpercent_text_shadow_color_offset[2]
			)
	...
	elseif plateFrame.IsFriendlyPlayerWithoutHealthBar then --not critical code
		...
		Plater.SetFontOutlineAndShadow(
			nameFontString,
			plateConfigs.actorname_text_outline,
			plateConfigs.actorname_text_shadow_color,
			plateConfigs.actorname_text_shadow_color_offset[1],
			plateConfigs.actorname_text_shadow_color_offset[2]
		)
		...
		if
			Plater.db.profile.plate_config[ACTORTYPE_FRIENDLY_PLAYER].actorname_use_guild_color
			and plateFrame.playerGuildName == Plater.PlayerGuildName
		then
			--is a guild friend?
			DF:SetFontColor(
				nameFontString,
				unpack(Plater.db.profile.plate_config[ACTORTYPE_FRIENDLY_PLAYER].actorname_guild_color)
			)
			plateFrame.isFriend = true
		elseif
			Plater.db.profile.plate_config[ACTORTYPE_FRIENDLY_PLAYER].actorname_use_friends_color
			and (IS_WOW_PROJECT_MIDNIGHT and not issecretvalue(plateFrame.unitNameInternal) or not IS_WOW_PROJECT_MIDNIGHT)
			and Plater.FriendsCache[plateFrame.unitNameInternal]
		then
			--is regular friend
			DF:SetFontColor(
				nameFontString,
				unpack(Plater.db.profile.plate_config[ACTORTYPE_FRIENDLY_PLAYER].actorname_friend_color)
			)
			--DF:SetFontOutline (nameFontString, plateConfigs.actorname_text_shadow)
			Plater.SetFontOutlineAndShadow(
				nameFontString,
	...
	elseif plateFrame.IsNpcWithoutHealthBar then --not critical code
		...
		if
			plateConfigs.all_names or (plateFrame.isSoftInteract and Plater.db.profile.show_healthbars_on_softinteract)
		then
			...
			if plateFrame[MEMBER_REACTION] <= 4 then
				...
				Plater.SetFontOutlineAndShadow(
					...
					plateConfigs.big_actorname_text_outline,
					plateConfigs.big_actorname_text_shadow_color,
					plateConfigs.big_actorname_text_shadow_color_offset[1],
					plateConfigs.big_actorname_text_shadow_color_offset[2]
				)
				...
				if subTitle and subTitle ~= "" then
					...
					Plater.SetFontOutlineAndShadow(
						plateFrame.ActorTitleSpecial,
						plateConfigs.big_actortitle_text_outline,
						plateConfigs.big_actortitle_text_shadow_color,
						plateConfigs.big_actortitle_text_shadow_color_offset[1],
						plateConfigs.big_actortitle_text_shadow_color_offset[2]
					)
				else
					plateFrame.ActorTitleSpecial:Hide()
				end
	...
	elseif
		plateFrame.actorType == ACTORTYPE_FRIENDLY_PLAYER
		and Plater.db.profile.plate_config[ACTORTYPE_FRIENDLY_PLAYER].actorname_use_class_color
	then
	...
	elseif
		plateFrame.actorType == ACTORTYPE_ENEMY_PLAYER
		and Plater.db.profile.plate_config[ACTORTYPE_ENEMY_PLAYER].actorname_use_class_color
	then
		--class colors should be used, if possible, because this is enabled
		plateFrame.isFriend = nil
	...
	if plateConfigs.level_text_enabled then
		...
		if needReset then
			...
			Plater.SetFontOutlineAndShadow(
				levelString,
				plateConfigs.level_text_outline,
				plateConfigs.level_text_shadow_color,
				plateConfigs.level_text_shadow_color_offset[1],
				plateConfigs.level_text_shadow_color_offset[2]
			)
	...
	if plateConfigs.percent_text_enabled then
		...
		Plater.UpdateLifePercentText(
			plateFrame.unitFrame.healthBar,
			plateFrame.unitFrame[MEMBER_UNITID],
			plateConfigs.percent_show_health,
			plateConfigs.percent_show_percent,
			plateConfigs.percent_text_show_decimals
		)
	else
		lifeString:Hide()
	end
...
function platerInternal.UpdatePercentTextLayout(fontString, plateConfigs)
	...
	Plater.SetFontOutlineAndShadow(
		fontString,
		plateConfigs.percent_text_outline,
		plateConfigs.percent_text_shadow_color,
		plateConfigs.percent_text_shadow_color_offset[1],
		plateConfigs.percent_text_shadow_color_offset[2]
	)
...
function Plater.UpdateLifePercentVisibility(self)
	...
	if plateConfigs.percent_text_enabled then
		...
		if
			(Plater.db.profile.use_player_combat_state and PLAYER_IN_COMBAT or self.unitFrame.InCombat)
			or plateConfigs.percent_text_ooc
		then
...
function Plater.UpdateUnitName(plateFrame)
	...
	Plater.UpdateUnitNameTextSize(
		plateFrame[MEMBER_NAME] or plateFrame.unitFrame[MEMBER_NAME] or "",
		nameString,
		nil,
		DB_PLATE_CONFIG[plateFrame.actorType].actorname_text_max_length or 99
	)
...
function Plater.UpdateUnitNameTextSize(name, nameString, maxWidth, maxLength)
	if not name or not nameString then
		return
	end
	if maxWidth then
...
function Plater.UpdateLevelTextAndColor(levelString, unitId)
	...
	--level color
	local color = GetRelativeDifficultyColor(UnitLevel("player") or 120, UnitLevel(unitId) or 120)
		or Plater.DefaultLevelColor
...
function Plater.UpdateBlizzardNameplateFonts(updateFont)
	local profile = Plater.db.profile
	if
		profile.blizzard_nameplate_font_override_enabled
		and (
			updateFont == true
			or profile.blizzard_nameplate_font == updateFont
			or profile.blizzard_nameplate_large_font == updateFont
		)
	then
...
function Plater.UpdatePlateFrame(plateFrame, actorType, forceUpdate, justAdded, regenDisabled)
	...
	if IS_IN_OPEN_WORLD and actorType == ACTORTYPE_ENEMY_NPC and DB_PLATE_CONFIG[actorType].quest_enabled then --actorType == ACTORTYPE_FRIENDLY_NPC or
		...
		if
			isQuestMob
			and DB_PLATE_CONFIG[actorType].quest_color_enabled
			and not Plater.IsUnitTapDenied(plateFrame.unitFrame.unit)
		then
			if plateFrame[MEMBER_REACTION] == Plater.UnitReaction.UNITREACTION_NEUTRAL then
	...
	if actorType == ACTORTYPE_FRIENDLY_NPC then
		...
		if
			(IS_WOW_PROJECT_MIDNIGHT and UnitIsUnit(unitFrame[MEMBER_UNITID], "playerpet"))
			or (not IS_WOW_PROJECT_MIDNIGHT and Plater.PlayerPetCache[unitFrame[MEMBER_GUID]])
				and not plateFrame.isBattlePet
		then
		...
		elseif
			DB_PLATE_CONFIG[actorType].follow_blizzard_npc_option
			and not (unitFrame.isSoftInteract or plateFrame[MEMBER_TARGET])
			and not UnitShouldDisplayName(plateFrame[MEMBER_UNITID] or unitFrame[MEMBER_UNITID])
		then
		...
		elseif
			IS_IN_OPEN_WORLD
			and DB_PLATE_CONFIG[actorType].quest_enabled
			and Plater.IsQuestObjective(plateFrame)
		then
	...
	elseif actorType == ACTORTYPE_ENEMY_PLAYER then
		...
		if plateFrame.PlayerCannotAttack and not DB_SHOW_HEALTHBARS_FOR_NOT_ATTACKABLE then
			...
			if DB_PLATE_CONFIG[actorType].use_playerclass_color then
				...
				if class then
					...
					Plater.ChangeHealthBarColor_Internal(
						healthBar,
						unpack(DB_PLATE_CONFIG[actorType].fixed_class_color)
					)
				end
			else
				Plater.ChangeHealthBarColor_Internal(healthBar, unpack(DB_PLATE_CONFIG[actorType].fixed_class_color))
		...
		if
			(plateFrame.PlayerCannotAttack and not DB_SHOW_HEALTHBARS_FOR_NOT_ATTACKABLE and not unitFrame.IsSelf)
			or unitFrame.isSoftInteractObject
		then
			healthBar:Hide()
			buffFrame:Hide()
			buffFrame2:Hide()
	...
	Plater.UpdatePlateText(
		plateFrame,
		DB_PLATE_CONFIG[actorType],
		shouldForceRefresh or plateFrame.PreviousUnitType ~= actorType or unitFrame.RefreshID < PLATER_REFRESH_ID
	)
	...
	if plateFrame.IsSelf then
		...
		if plateFrame.PlateConfig.healthbar_color_by_hp then
			...
			local originalColor = plateFrame.PlateConfig.healthbar_color
			local r, g, b = DF:LerpLinearColor(
				abs(currentHealth / currentHealthMax - 1),
				1,
				originalColor[1],
				originalColor[2],
				originalColor[3],
	...
	if unitFrame.ExtraIconFrame.RefreshID < PLATER_REFRESH_ID then
		...
		unitFrame.ExtraIconFrame:SetOption(
			"texcoord",
			Plater.db.profile.extra_icon_wide_icon and Plater.WideIconCoords or Plater.BorderLessIconCoords
		)
...
function Plater.UpdatePlateBorderThickness(plateFrame)
	...
	plateFrame.unitFrame.healthBar.border:SetBorderSizes(
		DB_BORDER_THICKNESS,
		DB_BORDER_THICKNESS,
		DB_BORDER_THICKNESS,
		DB_BORDER_THICKNESS
	...
	plateFrame.unitFrame.powerBar.border:SetBorderSizes(
		DB_BORDER_THICKNESS,
		DB_BORDER_THICKNESS,
		DB_BORDER_THICKNESS,
		DB_BORDER_THICKNESS
...
function Plater.UpdatePlateRaidMarker(plateFrame)
	...
	if index and not plateFrame.IsSelf then
		...
		if Plater.db.profile.indicator_extra_raidmark then
			...
			plateFrame.RaidTarget:SetAlpha(0.4)
		end
	else
...
function Plater.UpdateIndicators(plateFrame, actorType, regenDisabled)
	...
	if actorType == ACTORTYPE_ENEMY_PLAYER then
		...
		if
			config.indicator_spec and (config.indicator_spec_always or (not InCombatLockdown() and not regenDisabled))
		then
...
function Plater.AddIndicator(plateFrame, indicator, ...)
	...
	elseif indicator == "elite" then
		...
		thisIndicator:SetVertexColor(1, 0.8, 0)
		thisIndicator:SetSize(12, 12)
		...
		if combat or InCombatLockdown() then -- update this separately and only if needed
			if onlyNamesEnabled ~= profile.auto_toggle_combat.blizz_healthbar_ic then
				if IS_WOW_PROJECT_MIDNIGHT then
					SetCVar(
						...
						profile.auto_toggle_combat.blizz_healthbar_ic and CVAR_ENABLED or CVAR_DISABLED
					)
				else
					SetCVar(
			...
			if onlyNamesEnabled ~= profile.auto_toggle_combat.blizz_healthbar_ooc then
				if IS_WOW_PROJECT_MIDNIGHT then
					...
					SetCVar(
						"nameplateShowOnlyNames",
						profile.auto_toggle_combat.blizz_healthbar_ooc and CVAR_ENABLED or CVAR_DISABLED
					)
					--Plater.UpdateBaseNameplateOptions()
				end
			end
			if alwaysShow ~= profile.auto_toggle_combat.always_show_ooc then
				SetCVar(
					"nameplateShowAll",
					profile.auto_toggle_combat.always_show_ooc and CVAR_ENABLED or CVAR_DISABLED
				)
			end
		end
		...
		local function setStacking(state, stateFriendly)
			if not IS_WOW_PROJECT_MIDNIGHT then
				...
				C_CVar.SetCVarBitfield(
					...
					stateFriendly and true or false
				)
			end
		end
		...
		elseif zoneType == "arena" or zoneType == "pvp" then
			...
			if IsResting() then
				SetCVar(
					...
					profile.auto_toggle_friendly["cities"] and CVAR_ENABLED or CVAR_DISABLED
				)
			else
...
function Plater.ParseHealthSettingForPlayer(plateFrame, force) --private
	...
	if
		DB_PLATE_CONFIG[ACTORTYPE_FRIENDLY_PLAYER].only_thename
		and not DB_PLATE_CONFIG[ACTORTYPE_FRIENDLY_PLAYER].only_damaged
	then
...
function Plater.CheckHighlight(self)
	...
	if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(isMouseOver) then
		isMouseOver = false
	end
	if isMouseOver then
...
function Plater.CreateHighlightNameplate(plateFrame) --private
	local highlightOverlay = CreateFrame(
		"frame",
		"$parentHighlightOverlay",
		plateFrame.unitFrame.healthBar,
		BackdropTemplateMixin and "BackdropTemplate"
	) --why this was parented to UIParent (question mark)
...
function Plater.CreateHealthFlashFrame(plateFrame) --private
	local f_anim =
		CreateFrame("frame", nil, plateFrame.unitFrame.healthBar, BackdropTemplateMixin and "BackdropTemplate")
...
function Plater.AnimateLeftWithAccel(self, deltaTime)
	...
	self.AnimationStart = self.CurrentHealthMax == 0 and 1
		or self.AnimationStart - (self.CurrentHealthMax * calcAnimationSpeed)
...
function Plater.AnimateRightWithAccel(self, deltaTime)
	if self.AnimationEnd > self.CurrentHealthMax then
		self.AnimationEnd = self.CurrentHealthMax
	end
	...
	for animationIndex, animationTable in ipairs(frameAnimations) do
		if (animationTable.animationCooldown[plateFrame] or 0) < GetTime() then
			...
			if animationTable.animation_type == "scale" then
				...
				(animationHub.ScaleUp.SetToScale or animationHub.ScaleUp.SetScaleTo)(
					animationHub.ScaleUp,
					scaleUpX,
					scaleUpY
				);
				(animationHub.ScaleDown.SetFromScale or animationHub.ScaleDown.SetScaleFrom)(
					animationHub.ScaleDown,
					1,
					1
				);
				(animationHub.ScaleDown.SetToScale or animationHub.ScaleDown.SetScaleTo)(
					animationHub.ScaleDown,
					scaleDownX,
					scaleDownY
				)
			...
			elseif animationTable.animation_type == "frameshake" then
				...
				if not frameShake then
					...
					frameShake = DF:CreateFrameShake(
						shakeTargetFrame,
						animationTable.duration,
						animationTable.amplitude,
						animationTable.frequency,
						animationTable.absolute_sineX,
						animationTable.absolute_sineY,
						animationTable.scaleX,
						animationTable.scaleY,
						animationTable.fade_in,
						animationTable.fade_out,
						points
					)
				...
				if IS_EDITING_SPELL_ANIMATIONS then
					shakeTargetFrame:SetFrameShakeSettings(
						frameShake,
						animationTable.duration,
						animationTable.amplitude,
						animationTable.frequency,
				...
				if isCritical and animationTable.critical_scale then
					...
					shakeTargetFrame:PlayFrameShake(
						frameShake,
						animationScale,
						animationScale,
						animationScale,
						DF:Clamp(0.75, 1.75, animationScale)
					) --, animationScale
				else
...
function Plater.GetNpcIDFromGUID(guid) --private
	if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(guid) then
		return 0
	end --MIDNIGHT!!
...
function Plater.GetNpcID(plateFrame) --private
	...
	if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(npcId) then
		return
	end --MIDNIGHT!!
...
function Plater.GetUnitType(plateFrame)
	...
	elseif IS_WOW_PROJECT_MIDNIGHT then
		local unitID = plateFrame.unitFrame[MEMBER_UNITID]
		local isPet = UnitIsMinion(unitID) or UnitIsOtherPlayersPet(unitID) or UnitIsUnit("pet", unitID)
		if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(isPet) then
			return "normal"
		end --MIDNIGHT!!
		if isPet then
			return "pet"
...
function Plater.TranslateNPCCache()
	if not Plater.db.profile.auto_translate_npc_names then
		return
	end
	if Plater.TranslateNPCCacheIsRunning then
		return
	end
	...
	local function GetCreatureNameFromID(npcID)
		if C_TooltipInfo then
			...
			local tooltipFrame = GetCreatureNameFromIDFinderTooltip
				or CreateFrame("GameTooltip", "GetCreatureNameFromIDFinderTooltip", nil, "GameTooltipTemplate")
...
local PlaterCLEUParser =
	CreateFrame("frame", "PlaterCLEUParserFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate")
...
parserFunctions = {
		...
		for _, plateFrame in ipairs(Plater.GetAllShownPlates()) do
			...
			if unitFrame.PlaterOnScreen and castBar:IsShown() then
				if plateFrame[MEMBER_GUID] == targetGUID then
					...
					if
						DB_USE_NAME_TRANSLIT
						and (not IS_WOW_PROJECT_MIDNIGHT or (IS_WOW_PROJECT_MIDNIGHT and not issecretvalue(name)))
					then
					...
					if Plater.db.profile.cast_statusbar_interrupt_anim then
						...
						if not interruptAnim then
							...
							animationHub.ScaleDown =
								DF:CreateAnimation(animationHub, "scale", 2, duration, 1, 1.05, 1, 0.95)
							...
							local frameShakeObject = DF:CreateFrameShake(
								castBar,
								duration,
								amplitude,
								frequency,
	...
	SPELL_CAST_SUCCESS = function(
		...
		if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(spellID) then
			return
		end --MIDNIGHT!!
		if
			(tonumber(spellID) or 0) > 0
			and (
				not DB_CAPTURED_SPELLS[spellID]
				or DB_CAPTURED_SPELLS[spellID].isChanneled == nil
				or not DB_CAPTURED_CASTS[spellID]
		...
		then -- check isChanneled to ensure update of already existing data
...
function Plater.ForceFindPetOwner(serial) --private
	...
	if IS_WOW_PROJECT_MAINLINE then
		if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(serial) then
			return
		end
		...
		local tooltipFrame = PlaterPetOwnerFinder
			or CreateFrame("GameTooltip", "PlaterPetOwnerFinder", nil, "GameTooltipTemplate")
		tooltipFrame:SetOwner(WorldFrame, "ANCHOR_NONE")
	...
	if text1 and text1 ~= "" then
		...
		if text1:find(playerName) then
			...
			ownerName = (
				string.match(text1, string.gsub(UNITNAME_TITLE_PET, "%%s", "(%.*)"))
				or string.match(text1, string.gsub(UNITNAME_TITLE_MINION, "%%s", "(%.*)"))
				or string.match(text1, string.gsub(UNITNAME_TITLE_GUARDIAN, "%%s", "(%.*)"))
			)
			if ownerName then
				isOtherPet = true
			end
...
function Plater.CreatePlaterButtonAtInterfaceOptions()
	if not InterfaceOptionsFrame then
		return
	end --DF does not has this anymore TODO
...
function Plater.GetActorSubName(plateFrame) --private
	...
	if IS_WOW_PROJECT_MAINLINE then
		...
		local GameTooltipFrame = PlaterScanTooltip
			or CreateFrame("GameTooltip", "PlaterScanTooltip", nil, "GameTooltipTemplate")
		...
		if IS_WOW_PROJECT_MAINLINE then
			...
			if tooltipData then
				for _, line in ipairs(tooltipData.lines or {}) do
					if IS_WOW_PROJECT_MIDNIGHT and issecretvalue(line.type) then
						break
					end
					if
						line.type == Enum.TooltipDataLineType.QuestObjective
						or line.type == Enum.TooltipDataLineType.QuestTitle
						or line.type == Enum.TooltipDataLineType.QuestPlayer
					then
			...
			local GameTooltipScanQuest = PlaterScanQuestTooltip
				or CreateFrame("GameTooltip", "PlaterScanQuestTooltip", nil, "GameTooltipTemplate")
...
local update_quest_cache = function()
	...
	local numEntries, numQuests = C_QuestLog.GetNumQuestLogEntries and C_QuestLog.GetNumQuestLogEntries()
	...
	for questLogId = 1, numEntries do
		if IS_WOW_PROJECT_MAINLINE then
			...
			if
				...
				and type(questDetails.questID) == "number"
				and questDetails.questID > 0
			then
			...
			local title, level, suggestedGroup, isHeader, isCollapsed, isComplete, frequency, questId, startEvent, displayQuestID, isOnMap, hasLocalPOI, isTask, isStory =
				GetQuestLogTitle(questLogId)
		...
		if mapId then
			...
			if type(worldQuests) == "table" then
				for i, questTable in ipairs(worldQuests) do
					local x, y, floor, numObjectives, questId, inProgress =
						questTable.x,
						questTable.y,
						questTable.floor,
						questTable.numObjectives,
						questTable.questId,
						questTable.inProgress
	...
	if IS_WOW_PROJECT_MAINLINE then
		if Plater.ZoneInstanceType == "pvp" then
			...
			for i = 1, curNumScores do
				...
				if info then
					local name, faction, race, class, classToken, talentSpec =
						info.name, info.faction, info.raceName, info.className, info.classToken, info.talentSpec
					if name then
						BG_PLAYER_CACHE[name] = {
							faction = faction,
							race = race,
							class = class,
							...
							specID = (
								CLASS_INFO_CACHE[classToken]
								and CLASS_INFO_CACHE[classToken][talentSpec]
								and CLASS_INFO_CACHE[classToken][talentSpec].specID
							),
							name = name,
						}
					end
...
function Plater.ShowIndicator(unitFrame, texture, width, height, color, L, R, T, B)
	tinsert(
		unitFrame.CustomIndicators,
		{ texture or "", width or 12, height or 12, color or "white", L or 0, R or 1, T or 0, B or 1 }
	)
...
function Plater.SetExecuteRange(isExecuteEnabled, healthAmountLower, healthAmountUpper)
	...
	if IS_WOW_PROJECT_MIDNIGHT then
		...
		HEALTHCUTOFF_AT_DATA.healthCutOffValueCurve:AddPoint(
			tonumber(healthAmountLower) or -0.1,
			CreateColor(0, 0, 0, 0)
		) -- Invisible 20% - 100%
		...
		HEALTHCUTOFF_AT_DATA.healthCutOffUpperValueCurve:AddPoint(
			tonumber(healthAmountUpper) or 1.1,
			CreateColor(0, 0, 0, 0)
		) -- Invisible 0% - 89%
	else
		HEALTHCUTOFF_AT_DATA.healthCutOffValue = tonumber(healthAmountLower) or -0.1
...
function Plater.NameplateHasAura(unitFrame, aura)
	return unitFrame.BuffFrame.AuraCache[aura]
		or unitFrame.BuffFrame2.AuraCache[aura]
		or unitFrame.ExtraIconFrame.AuraCache[aura]
end
...
function Plater.RefreshNameplateColor(unitFrame)
	if unitFrame.unit then
		...
		elseif DB_UNITCOLOR_CACHE[unitFrame[MEMBER_NPCID] or -1] then
			Plater.ChangeHealthBarColor_Internal(
				unitFrame.healthBar,
				unpack(DB_UNITCOLOR_CACHE[unitFrame[MEMBER_NPCID]])
			)
			unitFrame.UsingCustomColor = true --exposed to scripts
		else
			if InCombatLockdown() then
...
function Plater.HideHealthBar(unitFrame, showPlayerName, showNameNpc)
	...
	showPlayerName = showPlayerName
		and (unitFrame.ActorType == ACTORTYPE_FRIENDLY_PLAYER or unitFrame.ActorType == ACTORTYPE_ENEMY_PLAYER)
	showNameNpc = showNameNpc
		and (unitFrame.ActorType == ACTORTYPE_FRIENDLY_NPC or unitFrame.ActorType == ACTORTYPE_ENEMY_NPC)
...
local ErrorHandler = function(errorMessage)
	local secretError = errorMessage
			and (string.find(lower(errorMessage), "secret value") or string.find(errorMessage, "index is secret"))
			and true
		or false
	...
	if BugGrabber then
		if not Plater.fullVersionInfoContainsName then
			Plater.GetVersionInfo()
		end
		geterrorhandler()(
...
Plater.ScriptMetaFunctions = {
	...
	HookGetInfo = function(self, globalScriptObject, scriptContainer)
		...
		if
			not scriptInfo
			or scriptInfo.GlobalScriptObject.NeedHotReload
			or (scriptInfo.GlobalScriptObject.Build and scriptInfo.GlobalScriptObject.Build < PLATER_HOOK_BUILD)
		then
			...
			if globalScriptObject.HasConstructor and (not scriptInfo.Initialized or forceHotReload) then
				...
				local okay, errortext = xpcall(
					globalScriptObject.Constructor,
					GetErrorHandler("Plater Mod |cFFAAAA22" .. modName .. "|r Constructor error: ", globalScriptObject),
					self,
					self.displayedUnit or self.unit or self:GetParent()[MEMBER_UNITID],
					self,
					scriptInfo.Env,
					PLATER_GLOBAL_MOD_ENV[scriptInfo.GlobalScriptObject.DBScriptObject.scriptId]
				)
	...
	ScriptHotReload = function(self, scriptInfo)
		...
		if scriptInfo.HotReload < scriptInfo.GlobalScriptObject.HotReload then
			...
			local okay, errortext = xpcall(
				scriptInfo.GlobalScriptObject["ConstructorCode"],
				GetErrorHandler(
					"Plater Script |cFFAAAA22" .. scriptName .. "|r Constructor error: ",
					scriptInfo.GlobalScriptObject
				),
				self,
				unitFrame.displayedUnit or unitFrame.unit or unitFrame.PlateFrame[MEMBER_UNITID],
				unitFrame,
				scriptInfo.Env,
				PLATER_GLOBAL_SCRIPT_ENV[scriptInfo.GlobalScriptObject.DBScriptObject.scriptId]
			)
	...
	ScriptRunOnUpdate = function(self, scriptInfo)
		if scriptInfo.GlobalScriptObject.tmpDisabled then
			return
		end
		...
		local okay, errortext = xpcall(
			scriptInfo.GlobalScriptObject["UpdateCode"],
			GetErrorHandler(
				"Plater Script |cFFAAAA22" .. scriptName .. "|r OnUpdate error: ",
				scriptInfo.GlobalScriptObject
			),
			self,
			unitFrame.displayedUnit or unitFrame.unit or unitFrame.PlateFrame[MEMBER_UNITID],
	...
	ScriptRunOnShow = function(self, scriptInfo)
		...
		local okay, errortext = xpcall(
			func,
			GetErrorHandler(
				"Plater Script |cFFAAAA22" .. scriptName .. "|r OnShow error: ",
				scriptInfo.GlobalScriptObject.DBScriptObject
			),
			self,
			unitFrame.displayedUnit or unitFrame.unit or unitFrame.PlateFrame[MEMBER_UNITID],
			unitFrame,
	...
	ScriptRunOnHide = function(self, scriptInfo)
		...
		local okay, errortext = xpcall(
			scriptInfo.GlobalScriptObject["OnHideCode"],
			GetErrorHandler(
				"Plater Script |cFFAAAA22" .. scriptName .. "|r OnHide error: ",
				scriptInfo.GlobalScriptObject
			),
			self,
			unitFrame.displayedUnit or unitFrame.unit or unitFrame.PlateFrame[MEMBER_UNITID],
	...
	ScriptRunInitialization = function(globalScriptObject)
		if globalScriptObject.tmpDisabled then
			return
		end
		...
		local okay, errortext = xpcall(
			globalScriptObject["Initialization"],
			GetErrorHandler("Plater Script |cFFAAAA22" .. scriptName .. "|r Initialization error: ", globalScriptObject),
			PLATER_GLOBAL_SCRIPT_ENV[globalScriptObject.DBScriptObject.scriptId]
		)
	...
	ScriptRunCommMessageHook = function(globalScriptObject, hookName, source, ...)
		...
		local okay, errortext = xpcall(
			globalScriptObject[hookName],
			GetErrorHandler(
				"Plater Mod |cFFAAAA22" .. modName .. "|r code for |cFFBB8800" .. hookName .. "|r error: ",
				globalScriptObject
			),
			PLATER_GLOBAL_MOD_ENV[globalScriptObject.DBScriptObject.scriptId],
			source,
			...
		)
	...
	ScriptRunHook = function(self, scriptInfo, hookName, frame, ...)
		...
		Plater.StartLogPerformance("Mod-RunHooks", modName, hookName)
		local okay, errortext = xpcall(
			scriptInfo.GlobalScriptObject[hookName],
			GetErrorHandler(
				"Plater Mod |cFFAAAA22" .. modName .. "|r code for |cFFBB8800" .. hookName .. "|r error: ",
				scriptInfo.GlobalScriptObject
			),
	...
	ScriptRunNoAttach = function(hookInfo, hookName)
		...
		local okay, errortext = xpcall(
			...
			PLATER_GLOBAL_MOD_ENV[hookInfo.DBScriptObject.scriptId]
		)
...
local function buildShadowTable(privateFunctionsTable, tableKey, shadowTable)
	if not privateFunctionsTable then
		return
	end
			...
			for _, plateFrame in ipairs(Plater.GetAllShownPlates()) do
				if plateFrame then
					...
					local okay, errortext = xpcall(
						...
						GetErrorHandler(
							"Mod: |cFFAAAA22"
								.. scriptInfo.GlobalScriptObject.DBScriptObject.Name
								.. "|r "
								.. scriptObject.Name
								.. " error: ",
							scriptObject
...
function Plater.CompileHook(scriptObject, noHotReload)
	...
	do --check integrity
		...
		if
			not scriptObject.LoadConditions.class
			or not scriptObject.LoadConditions.spec
			or not scriptObject.LoadConditions.race
			or not scriptObject.LoadConditions.talent
			or not scriptObject.LoadConditions.pvptalent
			or not scriptObject.LoadConditions.group
			or not scriptObject.LoadConditions.role
			or not scriptObject.LoadConditions.affix
			or not scriptObject.LoadConditions.encounter_ids
			or not scriptObject.LoadConditions.map_ids
		then
			Plater:Msg("fail to load mod: " .. (scriptObject.Name or "") .. ".")
			Plater.EndLogPerformanceCore("Plater-Core", "Mod/Script", "CompileHook")
			return
	...
	for i = 1, #scriptOptions do
		...
		if options_for_config_table[thisOption.Type] then
			if type(scriptOptionsValues[thisOption.Key]) == "boolean" then
				PLATER_GLOBAL_MOD_ENV[scriptObject.scriptId].config[thisOption.Key] =
					scriptOptionsValues[thisOption.Key]
	...
	for hookName, code in pairs(scriptCode) do
		...
		if hookName == "Send Comm Message" then
			code = string.gsub(
				...
				"Plater.SendComm%s*%(",
				"Plater.SendComm("
					.. globalScriptContainer.ScriptAmount + 1
					.. ', "'
			...
			if foundSendComm then
				Plater:Msg(
					"failed to compile "
						...
						.. scriptObject.Name
						.. ": "
						.. "Usage of 'Plater.SendComm' is only allowed in 'Send Comm Message' hook."
				)
			end
		end
...
function Plater.CompileScript(scriptObject, noHotReload, ...)
	...
	for i = 1, #scriptOptions do
		...
		if options_for_config_table[thisOption.Type] then
			if type(scriptOptionsValues[thisOption.Key]) == "boolean" then
				PLATER_GLOBAL_SCRIPT_ENV[scriptObject.scriptId].config[thisOption.Key] =
					scriptOptionsValues[thisOption.Key]
	...
	if HOOK_COMM_RECEIVED_MESSAGE.ScriptAmount > 0 then
		for i = 1, HOOK_COMM_RECEIVED_MESSAGE.ScriptAmount do
			...
			if globalScriptObject.DBScriptObject.UID == scriptUID then
				--run
				Plater.ScriptMetaFunctions.ScriptRunCommMessageHook(
					globalScriptObject,
...
function Plater.VerifyScriptIdForComm(scriptIndex, scriptId, uniqueId)
	if not scriptIndex or not scriptId or not uniqueId then
		return
	end
	...
	if
		globalScriptObject
		and globalScriptObject.DBScriptObject
		and globalScriptObject.DBScriptObject.scriptId
		and globalScriptObject.DBScriptObject.scriptId == scriptId
		and globalScriptObject.DBScriptObject.UID
		and globalScriptObject.DBScriptObject.UID == uniqueId
	then
		return true
	end
		...
		for _, plateFrame in ipairs(Plater.GetAllShownPlates()) do
			if plateFrame and plateFrame.unitFrame.PlaterOnScreen then
				for i = 1, HOOK_PLAYER_TALENT_UPDATE.ScriptAmount do
					...
					local scriptContainer = unitFrame:ScriptGetContainer()
					local scriptInfo =
						unitFrame:HookGetInfo(globalScriptObject, scriptContainer, "Player Talent Update")
					--run
