...
function ele:CastStop(event, f, _, guid)
	if not f.state.casting or guid ~= f.cast_state.guid then
		return
	end
	f.handler:CastBarHide(((event == 'UNIT_SPELLCAST_INTERRUPTED' or event == 'UNIT_SPELLCAST_FAILED') and ele.HIDE_INTERRUPT) or (event == 'UNIT_SPELLCAST_SUCCEEDED' and ele.HIDE_SUCCESS) or ele.HIDE_STOP)
end
...
function ele:UNIT_SPELLCAST_CHANNEL_STOP(_, f)
	if not f.state.casting or not f.cast_state.channel then
		return
	end
	f.handler:CastBarHide(ele.HIDE_STOP)
end
