...
function DDBL:Create(addonName, defaults)
	...
	do -- Validation
		...
		assert(select(2, _G.C_AddOns.IsAddOnLoaded(addonName)), consts.ADDON_NOT_LOADED_MSG:format(addonName))

		-- database for addon does not already exist
		assert(db_cache[addonName] == nil, ('attempt to create existing database for "%s"'):format(addonName))
...
local frame = _G.CreateFrame("Frame", ("%s_%s_EventFrame"):format(DDBL.metadata.name, DDBL.metadata.version))
