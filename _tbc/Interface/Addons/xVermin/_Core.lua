...
xVermin.IsSoD = xVermin.IsClassic
	and C_Seasons.HasActiveSeason()
	and (C_Seasons.GetActiveSeason() ~= Enum.SeasonID.Hardcore)
xVermin.MaxPlayerLevel = xVermin.IsSoD and 60 or MAX_PLAYER_LEVEL_TABLE[GetExpansionLevel()]
...
xVermin.IfUnitIsCastingInteruptible = function(unit)
	local name_casting, text, texture, startTimeMS, endTimeMS, isTradeSkill, castID, notInterruptible_casting, spellId =
		CastingInfo(unit)
	local name_channeling, text, texture, startTimeMS, endTimeMS, isTradeSkill, notInterruptible_chanelling, spellId =
		ChannelInfo(unit)

	if
		(name_casting ~= nil and not notInterruptible_casting)
		or (name_channeling ~= nil and not notInterruptible_chanelling)
	then
		return true
	end
	return false
...
xVermin.IfUnitIsCastingNonInteruptible = function(unit)
	...
	local name_channeling, text, texture, startTimeMS, endTimeMS, isTradeSkill, notInterruptible_chanelling, spellId =
		ChannelInfo(unit)
	if
		(name_casting ~= nil and notInterruptible_casting) or (name_channeling ~= nil and notInterruptible_chanelling)
	then
		return true
	end
	return false
