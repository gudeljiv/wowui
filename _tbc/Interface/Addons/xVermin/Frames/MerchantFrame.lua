...
local function SkinItem(item, itemLink)
	...
	local itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemIcon, itemSellPrice, itemClassID, itemSubClassID, bindType, expacID, itemSetID, isCraftingReagent =
	...
	for bag = 0, 4, 1 do
		for slot = 1, C_Container.GetContainerNumSlots(bag), 1 do
			...
			if
				name
				and (
					string.find(name, "Balanced Shadow Crystal")
					or string.find(name, "Perfect Balanced Shadow Crystal")
					or string.find(name, "Dazzling Dark Jade")
					or string.find(name, "Perfect Dazzling Dark Jade")
				)
			then
