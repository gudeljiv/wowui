...
local function UpdateBarVisibility() end
...
local function UpdateBarValueAndColor()
	for key, value in pairs(bars) do
		if not value.hidden then
			...
			if
				_G[value.frameStatusBar].FactionName:IsVisible() and _G[value.frameStatusBar].FactionName2:IsVisible()
			then
			...
			_G[value.frameStatusBar .. "_wrap"]:SetScript("OnEnter", function()
			...
			_G[value.frameStatusBar .. "_wrap"]:SetScript("OnLeave", function()
				_G[value.frameStatusBar].Value:Show()
				_G[value.frameStatusBar].Value2:Hide()
				_G[value.frameStatusBar].FactionName:Show()
				_G[value.frameStatusBar].FactionName2:Hide()
...
local function UpdateBars(self, event)
	...
	while factionIndex <= numFactions do
		...
		if hasRep or not isHeader then
			...
			bars[name].FactionInfo.earnedValue_old = bars[name].FactionInfo.earnedValue
					and bars[name].FactionInfo.earnedValue
				or earnedValue
			...
			if not bars[name].isWatched then
				if bars[name].FactionInfo.earnedValue_old ~= bars[name].FactionInfo.earnedValue then
					...
					bars[name].timer = C_Timer.NewTimer(30, function()
...
f:SetScript("OnEvent", function(self, event, unit)
	UpdateBars()
	if event == "UNIT_LEVEL" and unit ~= "player" and unit ~= "pet" then
		return
	end
	CheckPet()
end)
