...
function Move(src, dst)
	...
	if
		srcContainerInfo
		and not srcContainerInfo.isLocked
		and (not dstContainerInfo or not dstContainerInfo.isLocked)
	then
		ClearCursor()
...
function Item(container, position)
	...
	if link then
		local _, _, itemID, enchantID, suffixID, uniqueID =
			strfind(link, "item:(%d+):(%d*):%d*:%d*:%d*:%d*:(%-?%d*):(%-?%d*)")
