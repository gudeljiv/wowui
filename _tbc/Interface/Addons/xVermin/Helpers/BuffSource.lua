	...
	if ok and caster then
		...
		if caster then
			tooltip:AddDoubleLine(
				" ",
				(pet == caster and "|cffffc000Source:|r %s" or "|cffffc000Source:|r %s (%s)"):format(
					UnitName(caster),
					UnitName(pet)
				),
				1,
...
hooksecurefunc(GTT_Meta, "SetUnitAura", function(self, ...)
	return ProcessAura(self, UnitAura, ...)
end)
hooksecurefunc(GTT_Meta, "SetUnitBuff", function(self, ...)
	return ProcessAura(self, UnitBuff, ...)
end)
hooksecurefunc(GTT_Meta, "SetUnitDebuff", function(self, ...)
