			...
			if unitcasting then
				if not targetcasting then
					...
					if xVermin.Class == "WARRIOR" then
						...
						if
							defensive
							or (battle and IsEquippedItemType("Shields")) and IsSpellKnownOrOverridesKnown(72)
						then
							if
								IsSpellInRange("Shield Bash", "target") == 0
								or select(2, GetSpellCooldown("Shield Bash")) > 0
							then
								rangecasting = rangecasting + 1
							end
						end
					...
					if xVermin.Class == "DEATHKNIGHT" then
						if
							IsSpellInRange("Mind Freeze", "target") == 0
							or select(2, GetSpellCooldown("Mind Freeze")) > 0
						then
							rangecasting = rangecasting + 1
						end
					end
					if xVermin.Class == "SHAMAN" and IsSpellKnownOrOverridesKnown(8042) then
						if
							IsSpellInRange("Earth Shock", "target") == 0
							or select(2, GetSpellCooldown("Earth Shock")) > 0
						then
							rangecasting = rangecasting + 1
						end
					end
