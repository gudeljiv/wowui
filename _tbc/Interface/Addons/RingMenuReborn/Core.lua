...
local function GetHoveredSlot(container, profile)
	...
	while angleDifference >= TWO_PI do
		angleDifference = angleDifference - TWO_PI
	end
...
function Addon:BuildWheel(wheelIndex)
		...
		SecureHandlerWrapScript(
            ...
            if down then
                ...
                if wheel:IsShown() then
					...
					wheel:SetPoint("CENTER",gameParent,"BOTTOMLEFT",gameParent:GetWidth() / 2 + 400,gameParent:GetHeight() / 2 + 250)
                    wheel:Show()
                end
	...
	for slotIndex = 1, profile.slotCount do
		if not container.actions[slotIndex] then
			local actionButton = CreateFrame(
				"CheckButton",
				container:GetName() .. "Slot" .. slotIndex,
				container,
		...
		actionButton:SetPoint(
			"CENTER",
			container,
			"CENTER",
			profile.size * math.cos(buttonAngle),
			profile.size * math.sin(buttonAngle)
		)
