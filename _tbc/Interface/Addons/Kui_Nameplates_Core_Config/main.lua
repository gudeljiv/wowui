...
local function C_command(command, option)
	return C(3) .. '/knp ' .. ((command and ((option and command .. ' ' .. C(4) .. option .. '|r') or command)) or (option and C(4) .. option .. '|r' or '')) .. '|r'
end
...
local command_doc = {
	['help'] = format('This message. Use  %s  for more.', C_command('help', 'command')),
	...
	['config'] = {
		...
		format('%sUsage|r  %s', C(2), C_command('config', 'page')),
		'Enter the name of a page, full or partial, to open it',
		'This command is run by default if no other is matched',
		format('%sExample|r  %s  Opens the auras page', C(2), C_command(nil, 'aur'))
	},
	['profile'] = {
		'Switch to named profile',
		format('%sUsage|r  %s', C(2), C_command('profile', '! profile name')),
		format('%sExample|r  %s  Switch to profile %s if it exists', C(2), C_command('profile', 'mine'), 'mine'),
		format('%sExample|r  %s  Switch to profile %s, creating it if it does not exist', C(2), C_command('profile', '! new'), 'new')
	},
	['get'] = {
		'Show current value of configuration key',
		format('%sUsage|r  %s', C(2), C_command('get', 'key')),
		format('Use  %s  to search available configuration keys', C_command('find'))
	},
	['set'] = {
		'Set configuration key to value',
		format('%sUsage|r  %s', C(2), C_command('set', 'key value')),
		format('Enter  %s  as  %s  to reset the key to default', 'nil', 'value'),
		format('Use  %s  to search available configuration keys', C_command('find')),
		format('%sSupported values|r  bool  %s%s|r  colour  %s%s|r  number  text', C(2), C(2), 'true/false', C(2), 'r,g,b,a (0-1)'),
		format('%sExample|r  %s', C(2), C_command('set', 'frame_width 132')),
		format('%sExample|r  %s', C(2), C_command('set', 'target_glow_colour .8,.25,1,.8'))
	},
	['find'] = {
		'Search available configuration keys',
		format('%sUsage|r  %s', C(2), C_command('find', 'search text')),
		format('%sExample|r  %s', C(2), C_command('find', 'cvar always show'))
	},
	['locale'] = {
		"Switch KNP's config language",
		format('%sUsage|r  %s', C(2), C_command('locale', 'new locale')),
		format("Enter  %s  as  %s  to reset to WoW's default", 'nil', 'new locale')
	},
...
function command_func.config(...)
	...
	for i, f in ipairs(opt.pages) do
		if f.name then
			local name = f.name
			local locale = L.page_names[name] and strlower(L.page_names[name])

			if msg == name or msg == locale then
				-- exact match
				found = i
				break
			elseif not found and (name:match('^' .. msg) or locale:match('^' .. msg)) then
...
function command_func.dump()
	...
	d:AddText(format('%s %d.%d%s%s%s%s', '2.27.3a', knp.MAJOR, knp.MINOR, debug, custom, barauras, extras))
...
function command_func.set(key, value)
	...
	if value == 'nil' then
		...
		if type(extant_v) ~= type(value) then
			knp:ui_print(format('Invalid value for key (expected %s, got %s)', type(extant_v), type(value)))
			return
		end
		if type(value) == 'table' and #value ~= #extant_v then
			knp:ui_print(format('Invalid table length (expected %d, got %d)', #extant_v, #value))
			return
		end
	end
	...
	local d =
		...
		function(input)
			...
			if config.gsv.profiles[profile_name] and not create then
				...
				knp:ui_print(format('Import failed (profile  %s  exists - use  %s  to ignore)', profile_name, C_command('import', '!')))
				return
			end
