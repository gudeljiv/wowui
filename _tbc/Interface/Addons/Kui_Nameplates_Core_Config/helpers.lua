...
local function GetLocaleString(common_key, name, fallback)
	...
	if name then
		if L.titles[name] then
			return L.titles[name]
		end
		return name
	end
	return fallback
...
do
	...
	function opt.CreateSlider(parent, name, min, max, small, common_name)
		...
		local label = slider:CreateFontString(this_name and this_name .. 'Label', 'ARTWORK', (small and 'GameFontNormalSmall' or 'GameFontNormal'))
...
do
	local function OnValueChanged(self, value, text)
		if value and value == 'new_profile' then
			opt.Popup:ShowPage('text_entry', L.titles['new_profile_label'], nil, self.new_profile_callback)
		else
...
do
	...
	function opt:CurrentPage_Name()
		...
		if not self.active_page or not self.active_page.name then
			return
		end
	...
	function opt:CurrentPage_Paste()
		...
		if not self:CurrentPage_CanPaste() then
			return
		end
	...
	function opt:CurrentPage_CanPaste()
		...
		return (self.active_page and clipboard and clipboard_page and clipboard_profile and clipboard_page == self.active_page.name)
	end
	...
	function opt:CurrentPage_ClipboardButtonClick(button)
		if button == 'RightButton' or not self:CurrentPage_CanPaste() then
			...
			-- confirm paste
			self.Popup:ShowPage('confirm_dialog', format(L.titles['paste_page_label'], opt:CurrentPage_Name(), clipboard_profile, self.config.csv.profile), callback)
		end
	end
...
local function page_reset_callback(_, accept)
	if accept then
		opt:CurrentPage_Reset()
	end
end
local function page_reset_OnClick(self)
	opt.Popup:ShowPage('confirm_dialog', format(L.titles['reset_page_label'], opt:CurrentPage_Name()), self.callback)
end
...
local function profile_copy_OnClick(self)
	opt.Popup:ShowPage('text_entry', L.titles['copy_profile_label'], nil, self.callback)
end
...
local function profile_rename_OnClick(self)
	opt.Popup:ShowPage('text_entry', string.format(L.titles['rename_profile_label'], opt.config.csv.profile), opt.config.csv.profile, self.callback)
end
...
local function profile_reset_OnClick(self)
	opt.Popup:ShowPage('confirm_dialog', string.format(L.titles.reset_profile_label, opt.config.csv.profile), self.callback)
end
...
local function profile_delete_OnClick(self)
	opt.Popup:ShowPage('confirm_dialog', string.format(L.titles.delete_profile_label, opt.config.csv.profile), self.callback)
end
function opt:Initialise()
	...
	version:SetText(format(L.titles.version, 'Kui Nameplates', 'Kesavaa', 'Twitch', '2.27.3a'))
