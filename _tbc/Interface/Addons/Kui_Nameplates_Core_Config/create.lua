...
local function MovablePopupButton_OnClick(button)
	opt.Popup:ShowPage('movable', button.movable_prefix, button.movable_title or button.env, button.movable_keys, button.movable_minmax)
end
...
function general:Initialise()
	...
	quest_icon_position.enabled = function(p)
		return p.show_quest_icon
	end
	...
	raid_icon_position.enabled = function(p)
		return p.show_raid_icon
	end

	use_blizzard_personal.require_reload = true
	use_blizzard_powers.require_reload = true

	target_glow_colour.enabled = function(p)
		return p.target_glow or p.target_arrows
	end
	mouseover_glow_colour.enabled = function(p)
		return p.mouseover_glow
	end
	target_arrows_size.enabled = function(p)
		return p.target_arrows
	end
	use_blizzard_powers.enabled = function(p)
		return not p.use_blizzard_personal
	end
...
function fade_rules:Initialise()
	...
	avoid_ci.enabled = function(p)
		return p.fade_avoid_casting_friendly or p.fade_avoid_casting_hostile
	end
	avoid_cu.enabled = avoid_ci.enabled
	avoid_c.enabled = function(p)
		return not p.fade_avoid_tracked
	end
...
function healthbars:Initialise()
	...
	absorb_striped.enabled = function(p)
		return p.absorb_enable
	end
	colour_absorb.enabled = function(p)
		return p.absorb_enable
	end

	colour_self.enabled = function(p)
		return not p.colour_self_class
	end
	colour_player.enabled = function(p)
		return not p.colour_player_class
	end
	colour_enemy_player.enabled = function(p)
		return not p.colour_enemy_class
	end

	execute_auto.enabled = function(p)
		return p.execute_enabled
	end
	execute_colour.enabled = execute_auto.enabled
	execute_percent.enabled = function(p)
		return p.execute_enabled and not p.execute_auto
	end
...
function text:Initialise()
	...
	bot_vertical_offset.enabled = function(p)
		return p.level_text or p.health_text
	end
	...
	hidenamesCheck.enabled = function(p)
		return p.name_text
	end
	...
	health_text_friend_max.enabled = function(p)
		return p.health_text
	end
...
function nameonly:Initialise()
	...
	nameonly_no_font_style.enabled = function(p)
		return p.nameonly
	end
...
function framesizes:Initialise()
	...
	frame_height_personal.enabled = function(p)
		return not p.use_blizzard_personal
	end

	frame_width_target.enabled = function(p)
		return p.frame_target_size
	end
	frame_height_target.enabled = frame_width_target.enabled
	frame_width_minus.enabled = function(p)
		return p.frame_minus_size
	end
	frame_height_minus.enabled = frame_width_minus.enabled
	glow_size_shadow.enabled = function(p)
		return p.glow_as_shadow
	end
	glow_size_target.enabled = function(p)
		return p.target_glow or p.mouseover_glow
	end
	glow_size_target.enabled = function(p)
		return p.target_glow or p.mouseover_glow
	end
...
function auras:Initialise()
	...
	purge_size.enabled = function(p)
		return p.auras_show_purge
	end
	purge_opposite.enabled = function(p)
		return p.auras_show_purge
	end
...
function castbars:Initialise()
	...
	castbar_colour.enabled = function(p)
		return p.castbar_enable
	end
	...
	castbar_height.enabled = function(p)
		return p.castbar_enable and not p.castbar_detach
	end
	castbar_friend.enabled = function(p)
		return p.castbar_enable and p.castbar_showall
	end
	castbar_enemy.enabled = castbar_friend.enabled
	name_v_offset.enabled = function(p)
		return p.castbar_enable and p.castbar_name
	end
	animate.enabled = castbar_colour.enabled
	animate_cc.enabled = function(p)
		return p.castbar_animate and p.castbar_enable
	end

	castbar_detach.enabled = castbar_colour.enabled
	castbar_detach_width.enabled = function(p)
		return p.castbar_enable and p.castbar_detach
	end
...
function classpowers:Initialise()
	...
	local function classpowers_enabled(p)
		return p.classpowers_enable
	end
...
function bossmod:Initialise()
	...
	local function bossmod_enabled(p)
		return p.bossmod_enable
	end
	...
	bossmod_clickthrough.enabled = function(p)
		return p.bossmod_enable and p.bossmod_control_visibility
	end
...
function cvars:Initialise()
	...
	sfn.enabled = function(p)
		return p.cvar_enable
	end
