...
function LeaPlusLC:ShowSystemEditBox(word, focuschat)
	...
	LeaPlusLC.FactoryEditBox.b:SetScript("OnChar", function()
...
function LeaPlusLC:LoadVarNum(var, def, valmin, valmax)
	if
		LeaPlusDB[var]
		and type(LeaPlusDB[var]) == "number"
		and LeaPlusDB[var] >= valmin
		and LeaPlusDB[var] <= valmax
	then
		LeaPlusLC[var] = LeaPlusDB[var]
	else
		LeaPlusLC[var] = def
...
function LeaPlusLC:LoadVarAnc(var, def)
	if
		LeaPlusDB[var] and type(LeaPlusDB[var]) == "string" and LeaPlusDB[var] == "CENTER"
		or LeaPlusDB[var] == "TOP"
		or LeaPlusDB[var] == "BOTTOM"
		or LeaPlusDB[var] == "LEFT"
		or LeaPlusDB[var] == "RIGHT"
		or LeaPlusDB[var] == "TOPLEFT"
		or LeaPlusDB[var] == "TOPRIGHT"
		or LeaPlusDB[var] == "BOTTOMLEFT"
		or LeaPlusDB[var] == "BOTTOMRIGHT"
	then
		LeaPlusLC[var] = LeaPlusDB[var]
	else
		LeaPlusLC[var] = def
...
function LeaPlusLC:ShowMemoryUsage(frame, anchor, x, y)
	...
	memframe:SetScript("OnUpdate", function(self, elapsed)
		if memtime > 2 or memtime == -1 then
			...
			memtext = math.floor(memtext + 0.5) .. " KB"
...
function LeaPlusLC:FriendCheck(name, guid)
	...
	if LeaPlusLC["FriendlyGuild"] == "On" then
		...
		for i = 1, gCount do
			local gName, void, void, void, void, void, void, void, gOnline, void, void, void, void, gMobile, void, void, gGUID =
...
function LeaPlusLC:ReloadCheck()
	-- Chat
	if
		(LeaPlusLC["UseEasyChatResizing"] ~= LeaPlusDB["UseEasyChatResizing"]) -- Use easy resizing
...
function LeaPlusLC:Player()
		...
		if LeaPlusLC["NoFriendRequests"] == "On" then
			for i = BNGetNumFriendInvites(), 1, -1 do
				...
				if id and player then
					...
					C_Timer.After(0.1, function()
						LeaPlusLC:Print(
							L["A friend request from"] .. " " .. player .. " " .. L["was automatically declined."]
						)
					end)
				end
			end
	...
	do
		...
		frame:SetScript("OnEvent", function(self, event, arg1, arg2, ...)
			if
				...
				and strlower(strtrim(arg1)) == strlower(LeaPlusLC["InvKey"])
			then
						...
						if LeaPlusLC:FriendCheck(arg2, guid) or LeaPlusLC["InviteFriendsOnly"] == "Off" then
							...
							if theWhisperRealm then
								...
								if theCharRealm then
									if theWhisperRealm == theCharRealm then
										arg2 = theWhisperName
									end
								end
							end
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, group invites will only be sent to friends.|n|nIf unchecked, group invites will be sent to everyone."
		)
		...
		local function SetKeywordTip()
			LeaPlusCB["InviteFromWhisper"].tiptext = gsub(
				LeaPlusCB["InviteFromWhisper"].tiptext,
				"(|cffffffff)[^|]*(|r)",
				"%1" .. LeaPlusLC["InvKey"] .. "%2"
			)
		end
			...
			if not UnitAffectingCombat("player") then
				...
				LeaPlusLC:Print(
					L["The summon from"]
						...
						.. sLocation
						.. ") "
						.. L["will be automatically accepted in 10 seconds unless cancelled."]
				)
	...
	do
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, clock hourly chimes will be muted."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteFizzle",
			...
			-112,
			false,
			"If checked, the spell fizzle sounds will be muted."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteInterface",
			...
			-132,
			false,
			"If checked, the interface button sound, the chat frame tab click sound and the game menu toggle sound will be muted."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteLogin",
		...
		LeaPlusLC:MakeCB(
			...
			-192,
			false,
			"If checked, holiday event sounds will be muted.|n|nThis applies to Headless Horseman."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteReady",
			...
			-212,
			false,
			"If checked, the ready check sound will be muted."
		)
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, gyrocopters will be muted.|n|nThis applies to the engineering flying machine mounts."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteMechSteps",
			...
			-112,
			false,
			"If checked, footsteps for mechanical mounts will be muted."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteStriders",
			...
			-132,
			false,
			"If checked, mechanostriders will be quieter."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteHorsesteps",
			...
			-152,
			false,
			"If checked, footsteps for horse mounts will be muted."
		)
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, Screech will be muted.|n|nThis is a spell used by some flying pets."
		)
		LeaPlusLC:MakeCB(
			SoundPanel,
			"MuteYawns",
			...
			-112,
			false,
			"If checked, yawns from hunter pet cats will be muted."
		)
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, A'dal in Shattrath City will be muted."
		)
		...
		if LeaPlusLC["MuteGameSounds"] == "On" then
			SetupMute()
		end
	...
	if LeaPlusLC["ShowWowheadLinks"] == "On" then
		...
		LeaPlusLC:MakeCB(
			WowheadPanel,
			"WowheadLinkComments",
			"Links go directly to the comments section",
			16,
			-92,
			false,
			"If checked, Wowhead links will go directly to the comments section."
		)
		...
		if GameLocale == "deDE" then
			wowheadLoc = "de.tbc.wowhead.com"
		elseif GameLocale == "esMX" then
			wowheadLoc = "mx.tbc.wowhead.com"
		elseif GameLocale == "esES" then
			wowheadLoc = "es.tbc.wowhead.com"
		elseif GameLocale == "frFR" then
			wowheadLoc = "fr.tbc.wowhead.com"
		elseif GameLocale == "itIT" then
			wowheadLoc = "it.tbc.wowhead.com"
		elseif GameLocale == "ptBR" then
			wowheadLoc = "pt.tbc.wowhead.com"
		elseif GameLocale == "ruRU" then
			wowheadLoc = "ru.tbc.wowhead.com"
		elseif GameLocale == "koKR" then
			wowheadLoc = "ko.tbc.wowhead.com"
		elseif GameLocale == "zhCN" then
			wowheadLoc = "cn.tbc.wowhead.com"
		elseif GameLocale == "zhTW" then
			wowheadLoc = "tw.tbc.wowhead.com"
		else
			wowheadLoc = "tbc.wowhead.com"
		end
		...
		gossipFrame:SetScript("OnEvent", function()
			...
			if npcGuid and not IsShiftKeyDown() then
				...
				if npcID then
					...
					if
						npcID == "999999999" -- Reserved for future use
	...
	do
		...
		LeaPlusLC:MakeCB(
			QuestPanel,
			"AutoQuestAvailable",
			"Accept available quests automatically",
			16,
			-92,
			false,
			"If checked, available quests will be accepted automatically."
		)
		LeaPlusLC:MakeCB(
			QuestPanel,
			"AutoQuestCompleted",
			"Turn-in completed quests automatically",
			16,
			-112,
			false,
		...
		LeaPlusLC:MakeCB(
			QuestPanel,
			"AutoQuestShift",
			"Require override key for quest automation",
			16,
			-132,
			false,
			"If checked, you will need to hold the override key down for quests to be automated.|n|nIf unchecked, holding the override key will prevent quests from being automated."
		)

		LeaPlusLC:CreateDropdown(
			"AutoQuestKeyMenu",
			"Override key",
			146,
			"TOPLEFT",
			...
			356,
			-92,
			{ { L["SHIFT"], 1 }, { L["ALT"], 2 }, { L["CONTROL"], 3 }, { L["CMD (MAC)"], 4 } }
		)
		...
		local function IsOverrideKeyDown()
			if
				LeaPlusLC["AutoQuestKeyMenu"] == 1 and IsShiftKeyDown()
		...
		local function isNpcBlocked(actionType)
			...
			if npcGuid then
				...
				if npcID then
					...
					if
						npcID == "15192" -- Anachronos (Caverns of Time)
					...
					if actionType == "Accept" then
						...
						if
							npcID == "467" -- The Defias Traitor (The Defias Brotherhood)
					...
					if actionType == "Select" then
						if
							npcID == "12944" -- Lokhtos Darkbargainer (Thorium Brotherhood, Blackrock Depths)
							or npcID == "19401" -- Wing Commander Brack (Return to the Abyssal Shelf) (Horde)
							or npcID == "20235" -- Gryphoneer Windbellow (Return to the Abyssal Shelf) (Alliance)
		...
		local function DoesQuestHaveRequirementsMet(title)
			if title and title ~= "" then
				...
				elseif title == L["Gurubashi, Vilebranch, and Witherbark Coins"] then
					...
					if
						C_Item.GetItemCount(19701) >= 1
						and C_Item.GetItemCount(19702) >= 1
						and C_Item.GetItemCount(19703) >= 1
					then
						return true
					end
				elseif title == L["Sandfury, Skullsplitter, and Bloodscalp Coins"] then
					...
					if
						C_Item.GetItemCount(19704) >= 1
						and C_Item.GetItemCount(19705) >= 1
						and C_Item.GetItemCount(19706) >= 1
					then
						return true
					end
				...
				elseif title == L["Small Furry Paws"] then
					if C_Item.GetItemCount(5134) >= 5 then
						return true
					end
				elseif title == L["Evil Bat Eyes"] then
					if C_Item.GetItemCount(11404) >= 10 then
						return true
					end
				elseif title == L["Glowing Scorpid Blood"] then
					if C_Item.GetItemCount(19933) >= 10 then
						return true
					end
		...
		qFrame:SetScript("OnEvent", function(self, event, arg1)
			...
			if LeaPlusLC["AutoQuestShift"] == "On" and not IsOverrideKeyDown() then
				return
			elseif LeaPlusLC["AutoQuestShift"] == "Off" and IsOverrideKeyDown() then
				return
			end
			...
			if event == "QUEST_PROGRESS" and IsQuestCompletable() then
				if LeaPlusLC["AutoQuestCompleted"] == "On" then
					...
					if QuestRequiresBlockedItem() then
						return
					end
							...
							for titleIndex, questInfo in ipairs(gossipQuests) do
								if
									...
									and (
										...
										or questInfo.questID and IsQuestComplete(questInfo.questID)
									)
								then
	...
	do
		...
		LeaPlusLC:MakeCB(
			SellJunkFrame,
			"AutoSellShowSummary",
			"Show vendor summary in chat",
			16,
			-92,
			false,
			"If checked, a vendor summary will be shown in chat when junk is automatically sold."
		)
		...
		-- Reset button handler
		SellJunkFrame.r.tiptext = SellJunkFrame.r.tiptext
			.. "|n|n"
			.. L["Note that this will not reset your exclusions list."]
		...
		LeaPlusLC:CreateHelpButton(
			"SellJunkExcludeHelpButton",
			SellJunkFrame,
			titleTX,
				...
				for i = 1, #tipList do
					if tipList[i] then
						...
						if tipList[i] and tipList[i] > 0 and tipList[i] < 999999999 then
							local void, tLink, Rarity, void, void, void, void, void, void, void, ItemPrice =
								C_Item.GetItemInfo(tipList[i])
			...
			for BagID = 0, 4 do
				for BagSlot = 1, C_Container.GetContainerNumSlots(BagID) do
					...
					if CurrentItemLink then
						void, void, Rarity, void, void, void, void, void, void, void, ItemPrice =
							C_Item.GetItemInfo(CurrentItemLink)
	...
	do
		...
		LeaPlusLC:MakeCB(
			RepairPanel,
			"AutoRepairShowSummary",
			"Show repair summary in chat",
			16,
			-92,
			false,
			"If checked, a repair summary will be shown in chat when your gear is automatically repaired."
		)
	...
	if LeaPlusLC["ShowPlayerChain"] == "On" and not LeaLockList["ShowPlayerChain"] then
		...
		LeaPlusLC:CreateDropdown(
			...
			16,
			-92,
			{ { L["RARE"], 1 }, { L["ELITE"], 2 }, { L["RARE ELITE"], 3 } }
		)
		...
		local function SetChainStyle()
			...
			if
				EasyFramesDB
				and EasyFramesDB.profiles
				and EasyFramesDB.profiles.Default
				and EasyFramesDB.profiles.Default.general
				and EasyFramesDB.profiles.Default.general.lightTexture
			then
	...
	if LeaPlusLC["ShowRaidToggle"] == "On" and not LeaLockList["ShowRaidToggle"] then
		...
		if CompactRaidFrameManagerDisplayFrameHiddenModeToggle then
			...
			local cBackdrop =
				CreateFrame("Frame", nil, CompactRaidFrameManagerDisplayFrameHiddenModeToggle, "BackdropTemplate")
			cBackdrop:SetAllPoints()
			cBackdrop.backdropInfo = {
				edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
				tile = false,
				tileSize = 0,
				edgeSize = 16,
			...
			hooksecurefunc("CompactRaidFrameManager_UpdateOptionsFlowContainer", function()
				if CompactRaidFrameManager and CompactRaidFrameManagerDisplayFrameHiddenModeToggle then
					...
					CompactRaidFrameManagerDisplayFrameHiddenModeToggle:SetPoint(
						"TOPLEFT",
						UIParent,
						"TOPLEFT",
						0,
	...
	if LeaPlusLC["ClassColFrames"] == "On" and not LeaLockList["ClassColFrames"] then
		...
		LeaPlusLC:MakeCB(
			ClassFrame,
			"ClassColPlayer",
			"Show player frame in class color",
			16,
			-92,
			false,
			"If checked, the player frame background will be shown in class color."
		)
		LeaPlusLC:MakeCB(
			ClassFrame,
			"ClassColTarget",
			"Show target frame and focus frame in class color",
			16,
			-112,
			false,
			"If checked, the target frame background and focus frame background will be shown in class color."
		)
	...
	if LeaPlusLC["QuestFontChange"] == "On" then
		...
		GossipFrame.GreetingPanel.ScrollBar:SetPoint(
			"BOTTOMLEFT",
		...
		LeaPlusLC:MakeSL(
			QuestTextPanel,
			"LeaPlusQuestFontSize",
			"Drag to set the font size of quest text.",
			10,
			30,
			1,
			16,
	...
	if LeaPlusLC["MailFontChange"] == "On" then
		...
		LeaPlusLC:MakeSL(
			MailTextPanel,
			"LeaPlusMailFontSize",
			"Drag to set the font size of mail text.",
			10,
			30,
			1,
			16,
	...
	if LeaPlusLC["BookFontChange"] == "On" then
		...
		LeaPlusLC:MakeSL(
			BookTextPanel,
			"LeaPlusBookFontSize",
			"Drag to set the font size of book text.",
			10,
			30,
			1,
			16,
				...
				if GetInventorySlotInfo(slotName) then
					...
					if duraval ~= nil then
						...
						if where == "tip" then
							...
							valcol = (durapercent >= 80 and "|cff00FF00")
								or (durapercent >= 60 and "|cff99FF00")
								or (durapercent >= 40 and "|cffFFFF00")
								or (durapercent >= 20 and "|cffFF9900")
								or (durapercent >= 0 and "|cffFF2000")
								or "|cffFFFFFF"
							_G["GameTooltipTextLeft1"]:SetText(L["Durability"])
							_G["GameTooltipTextLeft2"]:SetText(
								_G["GameTooltipTextLeft2"]:GetText() .. SlotsFriendly[k] .. "|n"
							)
							_G["GameTooltipTextRight2"]:SetText(
								_G["GameTooltipTextRight2"]:GetText() .. valcol .. durapercent .. "%" .. "|n"
							)
						end
			...
			if event == "UI_ERROR_MESSAGE" then
				...
				if LeaPlusLC["ShowErrorsFlag"] == 1 then
					if
						...
						or err == ERR_FEIGN_DEATH_RESISTED
			...
			if StaticPopup1EditBox:IsShown() then
				...
				if link then
					StaticPopup1Text:SetText(
						gsub(StaticPopup1Text:GetText(), gsub(TypeDeleteLine, "@", ""), "") .. link
					)
				end
			else
				...
				if link then
					StaticPopup1Text:SetText(
						gsub(StaticPopup1Text:GetText(), gsub(TypeDeleteLine, "@", ""), "") .. "|n|n" .. link .. "|n|n"
					)
				end
			end
		end)
	...
	if LeaPlusLC["EnhanceFlightMap"] == "On" then
		...
		hooksecurefunc(TaxiFrame, "SetPoint", function(self, ...)
			...
			if a ~= LeaPlusLC["FlightMapA"] or r ~= LeaPlusLC["FlightMapR"] or x ~= xb or y ~= yb then
				...
				TaxiFrame:SetPoint(
					LeaPlusLC["FlightMapA"],
					UIParent,
					LeaPlusLC["FlightMapR"],
					LeaPlusLC["FlightMapX"],
					LeaPlusLC["FlightMapY"]
				)
			end
		end)
			...
			for i = 1, NUM_TAXI_BUTTONS do
				...
				if button and button:IsVisible() then
					...
					if button:GetHighlightTexture() then
						button
							:GetHighlightTexture()
							:SetSize(LeaPlusLC["LeaPlusTaxiIconSize"] * 2, LeaPlusLC["LeaPlusTaxiIconSize"] * 2)
					end
		...
		LeaPlusLC:MakeSL(
			TaxiPanel,
			"LeaPlusTaxiMapScale",
			"Drag to set the scale of the flight map.",
			1,
			3,
			0.05,
			356,
		...
		LeaPlusLC:MakeSL(
			TaxiPanel,
			"LeaPlusTaxiIconSize",
			"Drag to set the size of the icons.",
			5,
			30,
			1,
			356,
		...
		TaxiFrame:SetScript("OnDragStop", function()
			...
			LeaPlusLC["FlightMapA"], void, LeaPlusLC["FlightMapR"], LeaPlusLC["FlightMapX"], LeaPlusLC["FlightMapY"] =
				TaxiFrame:GetPoint()
		end)
	...
	do
		...
		-- Add mute button
		local MuteCustomNowButton = LeaPlusLC:CreateButton(
			"MuteCustomNowButton",
			MuteCustomPanel,
			"Mute",
			"BOTTOMLEFT",
			...
			25,
			true,
			"Click to mute sounds in the list."
		)
		LeaPlusCB["MuteCustomNowButton"]:SetScript("OnClick", function()
			MuteCustomListFunc(false, true)
		end)

		-- Add unmute button
		local UnmuteCustomNowButton = LeaPlusLC:CreateButton(
			"UnmuteCustomNowButton",
			MuteCustomPanel,
			"Unmute",
			"BOTTOMLEFT",
			16,
			...
			25,
			true,
			"Click to unmute sounds in the list."
		)
		...
		LeaPlusCB["UnmuteCustomNowButton"]:SetScript("OnClick", function()
			MuteCustomListFunc(true, true)
		end)

		-- Add play sound file editbox
		local willPlay, musicHandle
		local MuteCustomSoundsStopButton = LeaPlusLC:CreateButton(
			"MuteCustomSoundsStopButton",
			MuteCustomPanel,
			"Stop",
			"TOPRIGHT",
			-18,
		...
		local MuteCustomSoundsPlayButton = LeaPlusLC:CreateButton(
			"MuteCustomSoundsPlayButton",
			MuteCustomPanel,
			"Play",
			"TOPRIGHT",
			-18,
		...
		local MuteCustomSoundsSoundBox = LeaPlusLC:CreateEditBox(
		...
		LeaPlusLC:CreateHelpButton(
			"MuteGameSoundsCustomPlayHelpButton",
			MuteCustomPanel,
			titlePlayer,
			"If you want to listen to a sound file, enter the sound file ID into the editbox and click the play button.|n|nYou can scroll the mousewheel over the editbox to play neighbouring sound files."
		)
	end
				...
				if LeaPlusDB["ChatHistoryName"] == name .. "-" .. realm and timeDiff and timeDiff < 10 then -- reload must be done within 15 seconds
					...
					for i = 1, 50 do
						if
							i ~= 2
							and _G["ChatFrame" .. i]
							and LeaPlusDB["ChatHistory" .. i]
							and FCF_IsChatWindowIndexActive(i)
						then
							LeaPlusDB["ChatHistory" .. i .. "Count"] = 0
							-- Add previous session messages to chat
							for k = 1, #LeaPlusDB["ChatHistory" .. i] do
								if
									LeaPlusDB["ChatHistory" .. i][k]
									~= string.match(
										LeaPlusDB["ChatHistory" .. i][k],
										"|cffffd800" .. L["Restored"] .. " " .. ".*" .. " " .. L["message"] .. ".*.|r"
									)
								then
									_G["ChatFrame" .. i]:AddMessage(LeaPlusDB["ChatHistory" .. i][k])
									LeaPlusDB["ChatHistory" .. i .. "Count"] = LeaPlusDB["ChatHistory" .. i .. "Count"]
										+ 1
								end
							end
							...
							if LeaPlusDB["ChatHistory" .. i .. "Count"] == 1 then
								_G["ChatFrame" .. i]:AddMessage(
									"|cffffd800"
										.. L["Restored"]
										.. " "
										.. LeaPlusDB["ChatHistory" .. i .. "Count"]
										.. " "
										.. L["message from previous session"]
										.. ".|r"
								)
							else
								_G["ChatFrame" .. i]:AddMessage(
									"|cffffd800"
										...
										.. LeaPlusDB["ChatHistory" .. i .. "Count"]
										.. " "
										.. L["messages from previous session"]
										.. ".|r"
								)
							end
						else
	...
	if LeaPlusLC["ManageTimer"] == "On" and not LeaLockList["ManageTimer"] then
		...
		dragframe:SetScript("OnMouseUp", function()
			...
			LeaPlusLC["TimerA"], void, LeaPlusLC["TimerR"], LeaPlusLC["TimerX"], LeaPlusLC["TimerY"] =
				MirrorTimer1:GetPoint()
			MirrorTimer1:SetMovable(true)
			MirrorTimer1:ClearAllPoints()
			MirrorTimer1:SetPoint(
				LeaPlusLC["TimerA"],
		...
		-- Toggle grid button
		local TimerToggleGridButton = LeaPlusLC:CreateButton(
			"TimerToggleGridButton",
			TimerPanel,
			"Toggle Grid",
			"TOPLEFT",
			16,
			...
			25,
			true,
			"Click to toggle the frame alignment grid."
		)
		...
		-- Help button tooltip
		TimerPanel.h.tiptext =
			L["Drag the frame overlay with the left button to position it freely or with the right button to position it using snap-to-grid."]
	...
	if LeaPlusLC["ShowFlightTimes"] == "On" then
		...
		local introMsg = L["Leatrix Plus needs to be updated with the flight details.  Press CTRL/C to copy the flight details below then paste them into an email to flight@leatrix.com.  When your report is received, Leatrix Plus will be updated and you will never see this window again for this flight."]
			.. "|n|n"
			...
			for i = 1, NumTaxiNodes() do
				...
				if nodeType == "CURRENT" then
					...
					flightFrame:SetScript("OnEvent", function()
						...
						local flightMsg = L["Flight details"]
							.. " ("
							.. gameTitle
							.. "): "
							.. nodeName
						...
						if
							destination
							and data[faction]
							and data[faction][continent]
							and data[faction][continent][routeString]
						then
							local savedDuration = data[faction][continent][routeString]
							if savedDuration then
								if
									timeTaken > (savedDuration + timeBuffer)
									or timeTaken < (savedDuration - timeBuffer)
								then
									local editMsg = introMsg
										.. flightMsg
										.. L["This flight's actual time of"]
										.. " "
										.. string.format("%0.0f", timeTaken)
										.. " "
										.. L["seconds does not match the saved flight time of"]
										.. " "
										.. savedDuration
										.. " "
										.. L["seconds"]
									...
									if LeaPlusLC["FlightBarContribute"] == "On2" then
										editFrame:Show()
									end -- LeaPlusLC.NewPatch: Currently disabled, change to On to enable again
								end
							else
								local editMsg = introMsg
									.. flightMsg
									.. L["This flight does not have a saved duration in the database."]
								editBox:SetText(editMsg)
								if LeaPlusLC["FlightBarContribute"] == "On2" then
									editFrame:Show()
								end -- LeaPlusLC.NewPatch: Currently disabled, change to On to enable again
					...
					if data[faction] and data[faction][continent] and data[faction][continent][routeString] then
						...
						if duration then
							...
							mybar:SetPoint(
								LeaPlusLC["FlightBarA"],
								UIParent,
								LeaPlusLC["FlightBarR"],
								LeaPlusLC["FlightBarX"],
								LeaPlusLC["FlightBarY"]
							)
							...
							if LeaPlusLC["FlightBarSpeech"] == "On" then
								C_Timer.After(1, function()
									C_VoiceChat.SpeakText(
										...
										destination,
										speed,
										GetCVar("Sound_MasterVolume") * 100
									)
								end)
								mybar:AddUpdateFunction(function(bar)
									mt = bar.remaining
									if mt > 600 and mt < 601 and not Seconds600 then
										Seconds600 = true
										C_VoiceChat.SpeakText(
											0,
											L["Ten minutes"],
									...
									elseif mt > 540 and mt < 541 and not Seconds540 then
										Seconds540 = true
										C_VoiceChat.SpeakText(
											0,
											L["Nine minutes"],
									...
									elseif mt > 480 and mt < 481 and not Seconds480 then
										Seconds480 = true
										C_VoiceChat.SpeakText(
											0,
											L["Eight minutes"],
									...
									elseif mt > 420 and mt < 421 and not Seconds420 then
										Seconds420 = true
										C_VoiceChat.SpeakText(
											0,
											L["Seven minutes"],
									...
									elseif mt > 360 and mt < 361 and not Seconds360 then
										Seconds360 = true
										C_VoiceChat.SpeakText(
											0,
											L["Six minutes"],
									...
									elseif mt > 300 and mt < 301 and not Seconds300 then
										Seconds300 = true
										C_VoiceChat.SpeakText(
											0,
											L["Five minutes"],
									...
									elseif mt > 240 and mt < 241 and not Seconds240 then
										Seconds240 = true
										C_VoiceChat.SpeakText(
											0,
											L["Four minutes"],
									...
									elseif mt > 180 and mt < 181 and not Seconds180 then
										Seconds180 = true
										C_VoiceChat.SpeakText(
											0,
											L["Three minutes"],
									...
									elseif mt > 120 and mt < 121 and not Seconds120 then
										Seconds120 = true
										C_VoiceChat.SpeakText(
											0,
											L["Two minutes"],
									...
									elseif mt > 060 and mt < 061 and not Seconds060 then
										Seconds060 = true
										C_VoiceChat.SpeakText(
											0,
											L["One minute"],
									...
									elseif mt > 030 and mt < 031 and not Seconds030 then
										Seconds030 = true
										C_VoiceChat.SpeakText(
											0,
											L["Thirty seconds"],
									...
									elseif mt > 020 and mt < 021 and not Seconds020 then
										Seconds020 = true
										C_VoiceChat.SpeakText(
											0,
											L["Twenty seconds"],
									...
									elseif mt > 010 and mt < 011 and not Seconds010 then
										Seconds010 = true
										C_VoiceChat.SpeakText(
											0,
											L["Ten seconds"],
							...
							if LeaPlusLC["FlightBarBackground"] == "On" then
								if LeaPlusLC.ElvUI then
									...
									if faction == "Alliance" then
										LeaPlusLC.ElvUI
											:GetModule("Skins")
											:HandleStatusBar(_G.LeaPlusGlobalFlightBar, { 0, 0.5, 1, 0.5 })
									else
										LeaPlusLC.ElvUI
											:GetModule("Skins")
											:HandleStatusBar(_G.LeaPlusGlobalFlightBar, { 1, 0.0, 0, 0.5 })
									end
								else
		...
		tempFrame:SetScript("OnMouseUp", function()
			...
			LeaPlusLC["FlightBarA"], void, LeaPlusLC["FlightBarR"], LeaPlusLC["FlightBarX"], LeaPlusLC["FlightBarY"] =
				tempFrame:GetPoint()
			-- Position actual flight progress bar if one exists
			if LeaPlusLC.FlightProgressBar then
				LeaPlusLC.FlightProgressBar:ClearAllPoints()
				LeaPlusLC.FlightProgressBar:SetPoint(
					LeaPlusLC["FlightBarA"],
					UIParent,
					LeaPlusLC["FlightBarR"],
					LeaPlusLC["FlightBarX"],
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, the flight progress bar background texture will be shown."
		)
		LeaPlusLC:MakeCB(
			FlightPanel,
			"FlightBarDestination",
			"Show destination",
			16,
			-112,
			false,
			"If checked, the flight progress bar destination will be shown."
		)
		LeaPlusLC:MakeCB(
			FlightPanel,
			"FlightBarFillBar",
			"Fill instead of drain",
			16,
			-132,
			false,
			"If checked, the flight progress bar background will fill instead of drain."
		)
		LeaPlusLC:MakeCB(
			FlightPanel,
			"FlightBarSpeech",
			...
			-152,
			false,
			"If checked, the remaining flight time will be spoken using text to speech.|n|nChanges to this setting will take effect on the next flight you take."
		)
		...
		LeaPlusLC:MakeCB(
			FlightPanel,
			"FlightBarContribute",
			"Help contribute flight times",
			16,
			-212,
			false,
			"If checked, you will be prompted to submit missing flight times."
		)
		...
		LeaPlusLC:MakeSL(
			FlightPanel,
			"FlightBarScale",
			"Drag to set the flight progress bar scale.",
			1,
			5,
			0.1,
			356,
		...
		LeaPlusLC:MakeSL(
			FlightPanel,
			"FlightBarWidth",
			"Drag to set the flight progress bar width.",
			40,
			460,
			10,
			356,
		...
		local CloseFlightBarButton = LeaPlusLC:CreateButton(
			"CloseFlightBarButton",
			FlightPanel,
			"Close Bar",
			"TOPLEFT",
			16,
			...
			25,
			true,
			"Click to close the currently active flight progress bar."
		)
		...
		if LeaPlusLC.ElvUI then
			...
			LeaPlusCB["FlightBarBackground"].tiptext = LeaPlusCB["FlightBarBackground"].tiptext
				.. "|n|n|cff00AAFF"
				.. L["The background is always shown with ElvUI."]
		else
			-- Set progress bar background
		...
		FlightPanel.r:SetScript("OnClick", function()
			-- Reset controls
			LeaPlusLC["FlightBarA"], LeaPlusLC["FlightBarR"], LeaPlusLC["FlightBarX"], LeaPlusLC["FlightBarY"] =
				"TOP", "TOP", 0, -66
			tempFrame:ClearAllPoints()
			tempFrame:SetPoint(
				LeaPlusLC["FlightBarA"],
	...
	if LeaPlusLC["MinimapModder"] == "On" and not LeaLockList["MinimapModder"] then
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, the zoom buttons will be hidden.  You can use the mousewheel to zoom regardless of this setting."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"HideMiniClock",
		...
		LeaPlusLC:MakeCB(
			...
			-132,
			false,
			"If checked, the zone text bar will be hidden."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"HideMiniTracking",
			...
			-152,
			true,
			"If checked, the tracking button will be hidden while the pointer is not over the minimap."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"HideMiniLFG",
			"Hide the Looking for Group button",
			16,
			-172,
			true,
			"If checked, the Looking for Group button will be hidden while you are not queued.|n|nThis only applies to game realms with a Looking for Group feature."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"HideMiniAddonButtons",
			"Hide addon buttons",
			16,
			-192,
			false,
			"If checked, addon buttons will be hidden while the pointer is not over the minimap."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"MinimapButtonBag",
			...
			-212,
			true,
			"If checked, minimap buttons for addons will be collected and placed in a bag which you can toggle by right-clicking the minimap.|n|nThis setting will help you declutter the minimap without the need to install a separate addon to do that."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"SquareMinimap",
			...
			-232,
			true,
			"If checked, the minimap shape will be square."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"ShowWhoPinged",
			...
			-252,
			false,
			"If checked, when someone pings the minimap, their name will be shown.  This does not apply to your pings."
		)

		-- Add excluded button
		local MiniExcludedButton = LeaPlusLC:CreateButton(
			"MiniExcludedButton",
			SideMinimap,
			"Buttons",
			"TOPLEFT",
			...
			25,
			true,
			"Click to toggle the addon buttons editor."
		)
		...
		LeaPlusLC:MakeSL(
			SideMinimap,
			"MinimapSize",
			"Drag to set the square minimap size.|n|nAdjusting this slider makes the minimap bigger but keeps the elements the same size.",
			140,
			560,
			1,
			356,
		...
		LeaPlusLC:MakeTx(SideMinimap, "Border width", 356, -122)
		LeaPlusLC:MakeSL(
			SideMinimap,
			"MinimapBorderWidth",
			"Drag to set the square minimap border width.",
			1,
			10,
			1,
			356,
		...
		do
			...
			local titleTX = LeaPlusLC:MakeTx(
				ExcludedButtonsPanel,
				"Buttons for the addons listed below will remain visible.",
				16,
				-72
			)
			titleTX:SetWidth(534)
			...
			local MiniExcludedButton2 = LeaPlusLC:CreateButton(
				"MiniExcludedButton2",
				ExcludedButtonsPanel,
				"Buttons",
				"TOPLEFT",
		...
		if LeaPlusLC["SquareMinimap"] == "On" then
			...
			LeaPlusCB["MinimapSize"].tiptext = LeaPlusCB["MinimapSize"].tiptext
				.. "|cff00AAFF|n|n"
				.. L["This slider requires 'Square minimap' to be enabled."]
				.. "|r"
		end
		...
		if LeaPlusLC["MinimapButtonBag"] == "On" then
			...
			LeaPlusCB["HideMiniAddonButtons"].tiptext = LeaPlusCB["HideMiniAddonButtons"].tiptext
				.. "|n|n|cff00AAFF"
				.. L["Cannot be used with Minimap button bag."]
			...
			bFrame:HookScript("OnShow", function()
				if ButtonFrameTicker then
					ButtonFrameTicker:Cancel()
				end
				ButtonFrameTicker = C_Timer.NewTicker(2, function()
					if ItemRackMenuFrame and ItemRackMenuFrame:IsShown() and ItemRackMenuFrame:IsMouseOver() then
						return
					end
			...
			for i = 1, #buttons do
				...
				elseif
					strfind(strlower(LeaPlusDB["MiniExcludeList"]), buttonName)
					and LeaPlusLC["SquareMinimap"] == "On"
				then
			...
			Minimap:SetScript("OnMouseUp", function(frame, button)
				if button == "RightButton" then
					if bFrame:IsShown() then
						...
						if totalButtons > 36 then
							buttonsPerRow = 10
						elseif totalButtons > 32 then
							buttonsPerRow = 9
						elseif totalButtons > 28 then
							buttonsPerRow = 8
						elseif totalButtons > 24 then
							buttonsPerRow = 7
						elseif totalButtons > 20 then
							buttonsPerRow = 6
						elseif totalButtons > 16 then
							buttonsPerRow = 5
						elseif totalButtons > 12 then
							buttonsPerRow = 4
						elseif totalButtons > 8 then
							buttonsPerRow = 3
						elseif totalButtons > 4 then
							buttonsPerRow = 2
						else
							buttonsPerRow = 1
						end
						...
						for i = 1, totalButtons do
							...
							if not strfind(strlower(LeaPlusDB["MiniExcludeList"]), buttonName) then
								local button = LibDBIconStub:GetMinimapButton(buttons[i])
								if button.db then
									if buttonName == "armory" then
										button.db.hide = false
									end -- Armory addon sets hidden to true
									if not button.db.hide then
		...
		if LeaPlusLC["SquareMinimap"] == "On" then
			...
			LeaPlusCB["MinimapBorderWidth"].tiptext = LeaPlusCB["MinimapBorderWidth"].tiptext
				.. "|cff00AAFF|n|n"
				.. L["This slider requires 'Square minimap' to be enabled."]
				.. "|r"
		end
		...
		do
			...
			local function CreateBadButton(name)
				...
				if not finalTex then
					finalTex = "Interface\\HELPFRAME\\HelpIcon-KnowledgeBase"
				end
				...
				local function ReanchorTooltip(tip, myButton)
					...
					if LeaPlusLC["MinimapButtonBag"] == "On" then
						...
						if
							Minimap:GetCenter() * Minimap:GetEffectiveScale()
							> (GetScreenWidth() * UIParent:GetEffectiveScale() / 2)
						then
					...
					if ZGV_Notification_Entry_Template_Mixin then
						...
						hooksecurefunc(ZGV_Notification_Entry_Template_Mixin, "UpdateHeight", function(self)
							...
							if
								...
								and ZGV.db
								and ZGV.db.profile
								and ZGV.db.profile.nc_size
								and ZGV.db.profile.nc_size == 1
							then
								height = 36
							end
							...
							height = height
								+ (
									self.SpecialButton
										and self.SpecialButton:IsVisible()
										and self.SpecialButton:GetHeight() + 8
									or 0
								)
				...
				elseif
					name == "TomCats-MinimapButton" -- TomCat's Tours
				...
				then
					...
					myButton:HookScript("OnEnter", function()
						...
						GameTooltip:AddLine(name)
						GameTooltip:AddLine(
							L["This is a custom button.  Please ask the addon author to use the standard LibDBIcon library instead."],
							1,
							1,
							1,
							true
				...
				for i = 1, #temp do
					if temp[i] then
						...
						if
							name
							and btype == "Button"
							and not CustomAddonTable[name]
							and (btn:GetNumRegions() >= 3 or tContains(LowRegionCountButtons, name))
							and not issecurevariable(name)
							and btn:IsShown()
						then
							if not strfind(strlower(LeaPlusDB["MiniExcludeList"]), strlower("##" .. name)) then
								if
									not string.find(name, "LibDBIcon") and not tContains(BypassButtonTable, name)
									or tContains(customButtonTable, name)
								then
		...
		local function SetZoneTextBar()
			if LeaPlusLC["HideMiniZoneText"] == "On" then
				...
				MinimapZoneTextButton:SetPoint("CENTER", MinimapCluster.BorderTop, "CENTER", -1, 3)
		...
		-- Help button tooltip
		SideMinimap.h.tiptext =
			L["To move the minimap, use Edit Mode.|n|nIf you toggle an addon minimap button, you may need to reload your UI for the change to take effect.  This only affects a few addons that use custom buttons.|n|nThis panel will close automatically if you enter combat."]
		...
		-- Reset button handler
		SideMinimap.r.tiptext = SideMinimap.r.tiptext
			.. "|n|n"
			.. L["Note that this will not reset settings that require a UI reload."]
		SideMinimap.r:HookScript("OnClick", function()
			LeaPlusLC["HideMiniZoomBtns"] = "Off"
			ToggleZoomButtons()
			LeaPlusLC["HideMiniClock"] = "Off"
			SetMiniClock()
			...
			-- Reset map position
			LeaPlusLC["MinimapA"], LeaPlusLC["MinimapR"], LeaPlusLC["MinimapX"], LeaPlusLC["MinimapY"] =
				"TOPRIGHT", "TOPRIGHT", -17, -22
			-- Refresh panel
			SideMinimap:Hide()
			SideMinimap:Show()
		end)
			...
			if LeaPlusLC["MinimapButtonBag"] == "On" then
				...
				if not strfind(strlower(LeaPlusDB["MiniExcludeList"]), buttonName) then
					if button.db and not button.db.hide then
						button:Hide()
						button:SetScript("OnShow", function()
							if not LeaPlusLC.bFrame:IsShown() then
								button:Hide()
							end
						end)
					end
	...
	if LeaPlusLC["FilterChatMessages"] == "On" and not LeaLockList["FilterChatMessages"] then
		...
		LeaPlusLC:MakeCB(
			ChatFilterPanel,
			"BlockSpellLinks",
			"Block spell links during combat",
			16,
			-92,
			false,
			"If checked, messages containing spell links will be blocked while you are in combat.|n|nThis is useful for blocking spell interrupt spam.|n|nThis applies to the say, party, raid, emote and yell channels."
		)
		LeaPlusLC:MakeCB(
			ChatFilterPanel,
			"BlockDrunkenSpam",
			...
			-112,
			false,
			"If checked, drunken messages will be blocked unless they apply to your character.|n|nThis applies to the system channel."
		)
		LeaPlusLC:MakeCB(
			ChatFilterPanel,
			"BlockDuelSpam",
			...
			-132,
			false,
			"If checked, duel victory and retreat messages will be blocked unless your character took part in the duel.|n|nThis applies to the system channel."
		)
		...
		if GameLocale == "zhTW" then
			...
			LeaPlusCB["BlockDrunkenSpam"].tiptext = LeaPlusCB["BlockDrunkenSpam"].tiptext
				.. "|n|n|cff00AAFF"
				.. L["Cannot use this with your locale."]
		end
		...
		local function ChatFilterFunc(self, event, msg)
			...
			if LeaPlusLC["BlockDuelSpam"] == "On" then
				...
				if
					msg:match(DUEL_WINNER_KNOCKOUT:gsub("%%1$s", "%.+"):gsub("%%2$s", "%.+"))
					or msg:match(DUEL_WINNER_RETREAT:gsub("%%1$s", "%.+"):gsub("%%2$s", "%.+"))
				then
	...
	do
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, resurrection requests will not be automatically accepted if the player resurrecting you is in combat."
		)
		...
		AcceptResPanel:SetScript("OnEvent", function(self, event, arg1)
			if event == "RESURRECT_REQUEST" then
				...
				elseif GameLocale == "zhTW" then
					chainLoc = "禁錮之魂"
				elseif GameLocale == "ruRU" then
					chainLoc = "Скованный дух"
				elseif GameLocale == "koKR" then
					chainLoc = "구속된 영혼"
				elseif GameLocale == "esMX" then
					chainLoc = "Espíritu encadenado"
				elseif GameLocale == "ptBR" then
					chainLoc = "Espírito Acorrentado"
				elseif GameLocale == "deDE" then
					chainLoc = "Angeketteter Geist"
				elseif GameLocale == "esES" then
					chainLoc = "Espíritu encadenado"
				elseif GameLocale == "frFR" then
					chainLoc = "Esprit enchaîné"
				elseif GameLocale == "itIT" then
					chainLoc = "Spirito Incatenato"
				end
				if arg1 == chainLoc then
					return
				end
	...
	if LeaPlusLC["ShowDruidPowerBar"] == "On" and not LeaLockList["ShowDruidPowerBar"] then
		...
		if class == "DRUID" then
			...
			LeaPlusLC:MakeCB(
				DruidBarPanel,
				"ShowDruidStatusText",
				"Show druid power bar status text",
				16,
				-92,
				true,
				"If checked, status text will be shown in the druid power bar as long as status text is enabled in the game settings interface display panel."
			)
			...
			-- Reset button handler
			DruidBarPanel.r.tiptext = DruidBarPanel.r.tiptext
				.. "|n|n"
				.. L["Note that this will not reset settings that require a UI reload."]
			...
			local ALT_MANA_BAR_PAIR_DISPLAY_INFO =
				{ DRUID = { [Enum.PowerType.Rage] = true, [Enum.PowerType.Energy] = true } }
			...
			local function AlternatePowerBar_UpdatePowerType(self)
				...
				local void, class = UnitClass(unit)
				local show = (
					UnitPowerMax(unit, self.powerIndex) > 0
					and ALT_MANA_BAR_PAIR_DISPLAY_INFO[class]
					and ALT_MANA_BAR_PAIR_DISPLAY_INFO[class][UnitPowerType(unit)]
				)
	...
	if LeaPlusLC["EnhanceDressup"] == "On" then
		...
		LeaPlusLC:MakeCB(
			...
			-92,
			false,
			"If checked, item buttons will be shown in the dressing room.  You can click the item buttons to remove individual items from the model."
		)
		LeaPlusLC:MakeCB(
			DressupPanel,
			"DressupAnimControl",
			...
			-112,
			false,
			"If checked, an animation slider will be shown in the dressing room."
		)
		...
		LeaPlusCB["DressUpTargetBtn"]:SetScript("OnClick", function()
			if UnitIsPlayer("target") then
				...
				if playerActor then
					...
					playerActor:SetAnimation(0)
					C_Timer.After(0.1, function()
						playerActor:SetAnimation(
							animTable[math.floor(LeaPlusCB["DressupAnim"]:GetValue() + 0.5)],
							0,
							1,
							1
						)
		...
		LeaPlusCB["DressUpButonsBtn"]:SetScript("OnClick", function()
			if LeaPlusLC["DressupItemButtons"] == "On" then
				LeaPlusLC["DressupItemButtons"] = "Off"
			else
				LeaPlusLC["DressupItemButtons"] = "On"
			end
			LeaPlusLC:ToggleItemButtons()
			if DressupPanel:IsShown() then
				DressupPanel:Hide()
				DressupPanel:Show()
			end
		end)
		...
		local BtnStrata, BtnLevel =
			SideDressUpModelResetButton:GetFrameStrata(), SideDressUpModelResetButton:GetFrameLevel()
	...
	do
		...
		LeaPlusLC:MakeCB(
			ReleasePanel,
			"AutoReleaseNoAlterac",
			"Exclude Alterac Valley",
			16,
			-92,
			false,
			"If checked, you will not release automatically in Alterac Valley."
		)
		...
		LeaPlusLC:MakeSL(
			ReleasePanel,
			"AutoReleaseDelay",
			"Drag to set the number of milliseconds before you are automatically released.|n|nYou can hold down shift as the timer is ending to cancel the automatic release.",
			200,
			3000,
			100,
			356,
	...
	if LeaPlusLC["EnhanceTrainers"] == "On" then
		...
		LeaPlusLC:MakeCB(
			TrainerPanel,
			"ShowTrainAllBtn",
			"Show train all skills button",
			16,
			-92,
			false,
			"If checked, a train all skills button will be shown in the skill trainer frame allowing you to train all available skills instantly."
		)
		...
		EventUtil.ContinueOnAddOnLoaded("Blizzard_TrainerUI", function()
			...
			do
				...
				for i = 1 + 1, CLASS_TRAINER_SKILLS_DISPLAYED do
					...
					_G["ClassTrainerSkill" .. i]:SetPoint(
						"TOPLEFT",
						_G["ClassTrainerSkill" .. (i - 1)],
						"BOTTOMLEFT",
						0,
						1
					)
			...
			LeaPlusCB["TrainAllButton"]:SetScript("OnEnter", function(self)
				...
				if count > 0 then
					...
					if count > 1 then
						GameTooltip:AddLine(
							L["Train"] .. " " .. count .. " " .. L["skills for"] .. " " .. GetCoinTextureString(cost)
						)
					else
						GameTooltip:AddLine(
							L["Train"] .. " " .. count .. " " .. L["skill for"] .. " " .. GetCoinTextureString(cost)
						)
					end
					GameTooltip:Show()
				end
			...
			if LeaPlusLC.ElvUI then
				...
				if E.private.skins.blizzard.enable and E.private.skins.blizzard.trainer then
					...
					_G["ClassTrainerTrainButton"]:ClearAllPoints()
					_G["ClassTrainerTrainButton"]:SetPoint(
						"BOTTOMRIGHT",
						_G["ClassTrainerFrame"],
						"BOTTOMRIGHT",
						-42,
						78
					)
	...
	do
		...
		LeaPlusLC:MakeSL(
			weatherPanel,
			"WeatherLevel",
			"Drag to set the density of weather effects.",
			0,
			3,
			1,
			16,
		...
		local function SetWeatherFunc()
			LeaPlusCB["WeatherLevel"].f:SetText(
				LeaPlusLC["WeatherLevel"] .. "  (" .. weatherSliderTable[LeaPlusLC["WeatherLevel"] + 1] .. ")"
			)
	...
	if LeaPlusLC["EnhanceProfessions"] == "On" and not LeaLockList["EnhanceProfessions"] then
		...
		EventUtil.ContinueOnAddOnLoaded("Blizzard_TradeSkillUI", function()
			...
			for i = oldTradeSkillsDisplayed + 1, TRADE_SKILLS_DISPLAYED do
				local button =
					CreateFrame("Button", "TradeSkillSkill" .. i, TradeSkillFrame, "TradeSkillSkillButtonTemplate")
				button:SetID(i)
				button:Hide()
				button:ClearAllPoints()
			...
			if
				C_AddOns.IsAddOnLoaded("ClassicProfessionFilter")
				and TradeSkillFrame.SearchBox
				and TradeSkillFrame.HaveMats
				and TradeSkillFrame.HaveMats.text
			then
				...
				TradeSkillFrame.SearchMats:SetHitRectInsets(
					0,
					-TradeSkillFrame.SearchMats.text:GetStringWidth() + 2,
					0,
					0
				)
		...
		EventUtil.ContinueOnAddOnLoaded("Blizzard_CraftUI", function()
			...
			CraftFrame.Dropdown:SetPoint("TOPLEFT", CraftFrame, "TOPLEFT", 520, -42)
			...
			if
				C_AddOns.IsAddOnLoaded("ClassicProfessionFilter")
				and CraftFrame.SearchBox
				and CraftFrame.HaveMats
				and CraftFrame.HaveMats.text
				and CraftFrame.SearchMats
				and CraftFrame.SearchMats.text
			then
	...
	if LeaPlusLC["EnhanceQuestLog"] == "On" then
		if LeaPlusLC["EnhanceQuestTaller"] == "On" then
			...
			-- Add map button
			local logMapButton =
				CreateFrame("Button", "LeaPlusGlobalQuestLogMapButton", QuestLogFrame, "UIPanelButtonTemplate")
		...
		LeaPlusLC:MakeCB(
			EnhanceQuestPanel,
			"EnhanceQuestTaller",
			"Larger quest log frame",
			16,
			-92,
			true,
			"If checked, the quest log frame will be larger."
		)
		...
		LeaPlusLC:MakeCB(
			...
			-152,
			false,
			"If checked, quest levels will be shown."
		)
		LeaPlusLC:MakeCB(
			EnhanceQuestPanel,
			"EnhanceQuestDifficulty",
			"Show quest difficulty in quest log list",
			16,
			-172,
			false,
			"If checked, the quest difficulty will be shown next to the quest level in the quest log list.|n|nThis will indicate whether the quest requires a group (+), dungeon (D), raid (R) or PvP (P).|n|nThe quest difficulty will always be shown in the quest log detail pane regardless of this setting."
		)
		...
		-- Reset button handler
		EnhanceQuestPanel.r.tiptext = EnhanceQuestPanel.r.tiptext
			.. "|n|n"
			.. L["Note that this will not reset settings that require a UI reload."]
	...
	if LeaPlusLC["ShowVendorPrice"] == "On" then
		...
		local function ShowSellPrice(tooltip, tooltipObject)
			...
			local void, ilink, void, void, void, void, void, void, void, void, sellPrice, classID =
		...
		hooksecurefunc(GameTooltip, "SetHyperlink", function(tip)
			ShowSellPrice(tip, GameTooltip)
		end)
		hooksecurefunc(ItemRefTooltip, "SetHyperlink", function(tip)
			ShowSellPrice(tip, ItemRefTooltip)
		end)
	end
	...
	if LeaPlusLC["StandAndDismount"] == "On" then
		...
		eFrame:SetScript("OnEvent", function(self, event, messageType, msg)
			-- Auto dismount
			if
				msg == ERR_OUT_OF_RAGE and LeaPlusLC["DismountNoResource"] == "On"
		...
		LeaPlusLC:MakeCB(
			DismountFrame,
			"DismountNoResource",
			"Dismount when not enough rage, mana or energy",
			16,
			-92,
			false,
			"If checked, you will be dismounted when you attempt to cast a spell but don't have the rage, mana or energy to cast it."
		)
		LeaPlusLC:MakeCB(
			DismountFrame,
			"DismountNoMoving",
			"Dismount when casting a spell while moving",
			16,
			-112,
			false,
			"If checked, you will be dismounted when you attempt to cast a non-instant cast spell while moving."
		)
		LeaPlusLC:MakeCB(
			DismountFrame,
			"DismountNoTaxi",
			"Dismount when the flight map opens",
			16,
			-132,
			false,
			"If checked, you will be dismounted when you instruct a flight master to open the flight map."
		)
		LeaPlusLC:MakeCB(
			DismountFrame,
			"DismountShowFormBtn",
			"Show cancel form button on flight map",
			16,
			-152,
			false,
			"If checked, a cancel form button will be shown on the flight map while you are playing as a shapeshifted druid or shaman."
		)

		-- Help button hidden
		DismountFrame.h.tiptext =
			L["The game will dismount you if you successfully cast a spell without addons.  These settings let you set some additional dismount rules."]
	...
	if LeaPlusLC["AhExtras"] == "On" then
		EventUtil.ContinueOnAddOnLoaded("Blizzard_AuctionUI", function()
			...
			AuctionsCreateAuctionButton:HookScript("OnEnable", function()
				...
				if AuctionsWowTokenAuctionFrame:IsShown() then
					return
				end
	...
	if LeaPlusLC["ManageWidget"] == "On" and not LeaLockList["ManageWidget"] then
		...
		dragframe:SetScript("OnMouseUp", function()
			...
			LeaPlusLC["WidgetA"], void, LeaPlusLC["WidgetR"], LeaPlusLC["WidgetX"], LeaPlusLC["WidgetY"] =
				topCenterHolder:GetPoint()
			topCenterHolder:SetMovable(true)
			topCenterHolder:ClearAllPoints()
			topCenterHolder:SetPoint(
				LeaPlusLC["WidgetA"],
		...
		titanFrame.txt = LeaPlusLC:MakeWD(
			titanFrame,
			"Titan Panel screen adjust needs to be disabled for the frame to be saved correctly.",
			16,
			-192,
			500
		)
		...
		titanFrame.btn = LeaPlusLC:CreateButton(
			"fixTitanBtn",
			titanFrame,
			"Okay, disable screen adjust for me",
			"TOPLEFT",
			16,
			-212,
			0,
			25,
			true,
			"Click to disable Titan Panel screen adjust.  Your UI will be reloaded."
		)
		...
		-- Toggle grid button
		local WidgetToggleGridButton = LeaPlusLC:CreateButton(
			"WidgetToggleGridButton",
			WidgetPanel,
			"Toggle Grid",
			"TOPLEFT",
			16,
			...
			_G[chtfrm .. "Tab"]:SetScript("OnSizeChanged", function()
				for j = 1, 50 do
					...
					if _G["ChatFrame" .. j .. "ButtonFrameBottomButton"] then
						_G["ChatFrame" .. j .. "ButtonFrameBottomButton"]:SetWidth(
							_G["ChatFrame" .. j .. "Tab"]:GetWidth() - 10
						)
					end
				end
		...
		local function ShowChatbox(chtfrm)
			...
			for iMsg = StartMsg, NumMsg do
				...
				if chatMessage then
					-- Handle Battle.net
					if
						string.match(chatMessage, "k:(%d+):(%d+):BN_WHISPER:")
						...
						for friendIndex = 1, totalBNFriends do
							...
							if id == bnetAccountID then
								...
								chatMessage = chatMessage:gsub(
									"(|HBNplayer%S-|k)(%d-)(:%S-" .. ctype .. "%S-|h)%[(%S-)%](|?h?)(:?)",
									"[" .. battleTag .. "]:"
								)
							end
						end
	...
	if LeaPlusLC["ShowCooldowns"] == "On" then
		...
		local function MakeSpellEB(num, x, y, tab, shifttab)
			...
			SpellEB[num] = LeaPlusLC:CreateEditBox(
				"Spell" .. num,
				CooldownPanel,
				80,
				8,
		...
		LeaPlusLC:MakeCB(
			CooldownPanel,
			"ShowCooldownID",
			"Show the spell ID in buff icon tooltips",
			16,
			-92,
			false,
			"If checked, spell IDs will be shown in buff icon tooltips located in the buff frame and under the target frame."
		)
		LeaPlusLC:MakeCB(
			CooldownPanel,
			"NoCooldownDuration",
			"Hide cooldown duration numbers (if enabled)",
			16,
			-112,
			false,
			"If checked, cooldown duration numbers will not be shown over the cooldowns.|n|nIf unchecked, cooldown duration numbers will be shown over the cooldowns if they are enabled in the game options panel ('ActionBars' menu)."
		)
		LeaPlusLC:MakeCB(
			CooldownPanel,
			"CooldownsOnPlayer",
			"Show cooldowns above the player frame",
			16,
			-132,
			false,
			"If checked, cooldown icons will be shown above the player frame instead of the target frame.|n|nIf unchecked, cooldown icons will be shown above the target frame."
		)
		...
		local function SavePanelControls()
			for i = 1, iCount do
				...
				LeaPlusDB["Cooldowns"][PlayerClass]["S" .. activeSpec .. "R" .. i .. "Pet"] =
					LeaPlusCB["Spell" .. i .. "Pet"]:GetChecked()
		...
		for i = 1, iCount do
			...
			LeaPlusCB["Spell" .. i .. "Pet"]:SetChecked(
				LeaPlusDB["Cooldowns"][PlayerClass]["S" .. activeSpec .. "R" .. i .. "Pet"]
			)
		...
		-- Add help button
		LeaPlusLC:CreateHelpButton(
			"ShowCooldownsHelpButton",
			CooldownPanel,
			classTagBanner,
			"Enter the spell IDs for the cooldown icons that you want to see.|n|nIf a cooldown icon normally appears under the pet frame, check the pet checkbox.|n|nCooldown icons are saved to your class."
		)
	...
	if LeaPlusLC["TipModEnable"] == "On" and not LeaLockList["TipModEnable"] then
			...
			if LeaPlusLC["TooltipAnchorMenu"] ~= 1 then
				...
				if LeaPlusLC["TooltipAnchorMenu"] == 2 or not WorldFrame:IsMouseMotionFocus() then
					...
					tooltip:SetPoint(
						...
						UIParent,
						"BOTTOMRIGHT",
						LeaPlusLC["TipOffsetX"],
						LeaPlusLC["TipOffsetY"]
					)
					return
				else
					...
					elseif LeaPlusLC["TooltipAnchorMenu"] == 5 then
						tooltip:SetOwner(
							parent,
							"ANCHOR_CURSOR_RIGHT",
							LeaPlusLC["TipCursorX"],
							LeaPlusLC["TipCursorY"]
						)
						return
					end
				end
		...
		LeaPlusLC:MakeCB(
			SideTip,
			"TipShowRank",
			"Show guild ranks for your guild",
			16,
			-92,
			false,
			"If checked, guild ranks will be shown for players in your guild."
		)
		LeaPlusLC:MakeCB(
			SideTip,
			"TipShowOtherRank",
			"Show guild ranks for other guilds",
			16,
			-112,
			false,
			"If checked, guild ranks will be shown for players who are not in your guild."
		)
		LeaPlusLC:MakeCB(
			SideTip,
			"TipShowTarget",
			...
			-132,
			false,
			"If checked, unit targets will be shown."
		)
		LeaPlusLC:MakeCB(
			SideTip,
			"TipNoHealthBar",
			...
			-152,
			true,
			"If checked, the health bar will not be shown."
		)
		...
		LeaPlusLC:MakeCB(
			SideTip,
			"TipHideShiftOverride",
			"Show tooltips with shift key",
			16,
			-232,
			false,
			"If checked, you can hold shift while tooltips are hidden to show them temporarily."
		)
		...
		LeaPlusLC:CreateDropdown(
			...
			356,
			-92,
			{ { L["None"], 1 }, { L["Overlay"], 2 }, { L["Cursor"], 3 }, { L["Cursor Left"], 4 }, {
				L["Cursor Right"],
				5,
			} }
		)
		...
		local function SetAnchorControls()
			...
			if
				...
				or LeaPlusLC["TooltipAnchorMenu"] == 2
				or LeaPlusLC["TooltipAnchorMenu"] == 3
			then
		...
		-- Reset button handler
		SideTip.r.tiptext = SideTip.r.tiptext
			.. "|n|n"
			.. L["Note that this will not reset settings that require a UI reload."]
		...
		local function SetTipScale()
			...
			if LibDBIconTooltip then
				LibDBIconTooltip:SetScale(LeaPlusLC["LeaPlusTipSize"])
			end
		...
		local function ShowTip()
			...
			elseif UnitIsDeadOrGhost(LT["Unit"]) then
				...
				_G["GameTooltipTextLeft1"]:SetText(
					"|c88888888" .. (_G["GameTooltipTextLeft1"]:GetText() or "") .. "|cffffffff|r"
				)
				return
			end
			...
			if LT["TipIsPlayer"] then
				if GameLocale == "ruRU" then
					...
					-- Show class
					LT["InfoText"] = LT["InfoText"] .. " " .. LT["LpTipClassColor"] .. LT["Class"] .. "|r "
						or LT["InfoText"] .. "|r "

					-- Show level
					if LT["Reaction"] < 5 then
						if LT["UnitLevel"] == -1 then
							LT["InfoText"] = LT["InfoText"]
								.. ("|cffff3333" .. "??-ro" .. " " .. ttLevel .. "|cffffffff")
						else
							LT["LevelColor"] = GetCreatureDifficultyColor(LT["UnitLevel"])
							LT["LevelColor"] = string.format(
								"%02x%02x%02x",
								LT["LevelColor"].r * 255,
								LT["LevelColor"].g * 255,
								LT["LevelColor"].b * 255
							...
							LT["InfoText"] = LT["InfoText"]
								.. ("|cff" .. LT["LevelColor"] .. LT["UnitLevel"] .. LT["LevelLocale"] .. "|cffffffff")
						end
					else
			...
			if
				UnitCanAttack(LT["Unit"], "player")
				and not LT["TipIsPlayer"]
				and LT["Reaction"] < 5
				and not LT["PlayerControl"]
			then
				-- Find the level line
				LT["MobInfoLine"] = 0
				...
				if LT["MobInfoLine"] > 1 then
					...
					if LT["Special"] then
						if LT["Special"] == "elite" then
							if
								strfind(_G["GameTooltipTextLeft" .. LT["MobInfoLine"]]:GetText(), "(" .. ttBoss .. ")")
							then
								LT["Special"] = "(" .. ttBoss .. ")"
							else
								LT["Special"] = "(" .. ttElite .. ")"
						...
						elseif
							LT["UnitLevel"] == -1
							and LT["Special"] == "normal"
							and strfind(_G["GameTooltipTextLeft" .. LT["MobInfoLine"]]:GetText(), "(" .. ttBoss .. ")")
						then
							LT["Special"] = "(" .. ttBoss .. ")"
						else
							LT["Special"] = nil
	...
	if LeaPlusLC["ShowBorders"] == "On" then
		...
		local function RefreshBorders()
			...
			BordTop:SetHeight(LeaPlusLC["BordersTop"])
			BordTop:SetAlpha(1 - LeaPlusLC["BordersAlpha"])
			BordBot:SetHeight(LeaPlusLC["BordersBottom"])
			BordBot:SetAlpha(1 - LeaPlusLC["BordersAlpha"])
			BordLeft:SetWidth(LeaPlusLC["BordersLeft"])
			BordLeft:SetAlpha(1 - LeaPlusLC["BordersAlpha"])
			BordRight:SetWidth(LeaPlusLC["BordersRight"])
			BordRight:SetAlpha(1 - LeaPlusLC["BordersAlpha"])
		...
		LeaPlusLC:MakeSL(
			bordersPanel,
			"BordersTop",
			"Drag to set the size of the top border.",
			0,
			300,
			5,
			16,
		...
		LeaPlusLC:MakeSL(
			bordersPanel,
			"BordersBottom",
			"Drag to set the size of the bottom border.",
			0,
			300,
			5,
			16,
		...
		LeaPlusLC:MakeSL(
			bordersPanel,
			"BordersLeft",
			"Drag to set the size of the left border.",
			0,
			300,
			5,
			186,
		...
		LeaPlusLC:MakeSL(
			bordersPanel,
			"BordersRight",
			"Drag to set the size of the right border.",
			0,
			300,
			5,
			186,
		...
		LeaPlusLC:MakeSL(
			bordersPanel,
			"BordersAlpha",
			"Drag to set the transparency of the borders.",
			0,
			0.9,
			0.1,
			356,
	...
	function LeaPlusLC:MediaFunc()
		...
		-- Create scroll bar
		scrollFrame =
			CreateFrame("ScrollFrame", nil, LeaPlusLC["Page9"], "LeaPlusConfigurationPanelScrollFrameTemplate")
		...
		-- Add stop button
		local stopBtn =
			LeaPlusLC:CreateButton("StopMusicBtn", LeaPlusLC["Page9"], "Stop", "BOTTOMLEFT", 146, 53, 0, 25, true, "")
		stopBtn:Hide()
		stopBtn:Show()
		...
		local sBox = LeaPlusLC:CreateEditBox(
			"MusicSearchBox",
			LeaPlusLC["Page9"],
			78,
			10,
		...
		local function ShowSearchResults()
			...
			local searchText =
				gsub(strlower(sBox:GetText()), "([" .. ("%^$().[]*+-?"):gsub("(.)", "%%%1") .. "])", "%%%1")
						...
						for a, b in pairs(ZoneList[e]) do
							if b.tracks then
								for k, v in pairs(b.tracks) do
									if
										(strfind(v, "#") or strfind(v, "|r"))
										and (
											strfind(strlower(v), word1)
											or strfind(strlower(b.zone), word1)
											or strfind(strlower(b.category), word1)
										)
									then
										if
											not word2
											or word2 ~= ""
												and (strfind(strlower(v), word2) or strfind(strlower(b.zone), word2) or strfind(
													strlower(b.category),
													word2
												))
										then
											if
												not word3
												or word3 ~= ""
													and (strfind(strlower(v), word3) or strfind(strlower(b.zone), word3) or strfind(
														strlower(b.category),
														word3
													))
											then
												if
													not word4
													or word4 ~= ""
														and (strfind(strlower(v), word4) or strfind(
															strlower(b.zone),
															word4
														) or strfind(strlower(b.category), word4))
												then
													if
														not word5
														or word5 ~= ""
															and (strfind(strlower(v), word5) or strfind(
																strlower(b.zone),
																word5
															) or strfind(strlower(b.category), word5))
													then
														-- Show category
		...
		local function ShowRandomList()
			...
			while #ListData < 50 do
				...
				if rTrack and rTrack ~= "" and strfind(rTrack, "#") and not tContains(dupCheck, rTrack) then
					if
						not tContains(randomBannedList, L[ZoneList[rCategory][rZone].zone])
		...
		LeaPlusLC["PageF"]:HookScript("OnMouseUp", function(self, btn)
			if
				LeaPlusLC["Page9"]:IsShown()
				and LeaPlusLC["Page9"]:IsMouseOver(0, 0, 0, -440) == false
				and LeaPlusLC["Page9"]:IsMouseOver(-(LeaPlusLC.MainPanelHeight - 47), 0, 0, 0) == false
			then
				if btn == "RightButton" then
					BackClick()
				end
	...
	do
		...
		local function SetPlusScale()
			...
			LeaPlusLC["MainPanelA"], LeaPlusLC["MainPanelR"], LeaPlusLC["MainPanelX"], LeaPlusLC["MainPanelY"] =
				"CENTER", "CENTER", 0, 0
			if LeaPlusLC["PageF"]:IsShown() then
				LeaPlusLC["PageF"]:Hide()
				LeaPlusLC["PageF"]:Show()
	...
	if not LeaPlusDB["FirstRunMessageSeen"] then
		C_Timer.After(1, function()
			LeaPlusLC:Print(
				L["Enter"] .. " |cff00ff00" .. "/ltp" .. "|r " .. L["or click the minimap button to open Leatrix Plus."]
			)
			...
			do
				...
				local function Lock(option, reason, optmodule)
					...
					if
						...
						and temp[1]
						and temp[1].t
						and temp[1].t:GetTexture() == "Interface\\WorldMap\\Gear_64.png"
					then
				...
				if LeaPlusLC.ElvUI then
					...
					if E and E.private then
						...
						if
							E.private.unitframe.disabledBlizzardFrames.player
							or E.private.unitframe.disabledBlizzardFrames.target
						then
...
function LeaPlusLC:PlayerLogout(wipe)
	...
	if
		...
		and LeaPlusDB["SquareMinimap"] == "On"
		and not LeaLockList["MinimapModder"]
	then
...
function LeaPlusLC:CreatePanel(title, globref, scrolling)
	...
	Side.r = LeaPlusLC:CreateButton(
		...
		25,
		true,
		"Click to reset the settings on this page."
	)
	Side.h = LeaPlusLC:CreateButton(
		"HelpButton",
		Side,
		"Help",
		"BOTTOMLEFT",
	...
	local reloadb = LeaPlusLC:CreateButton(
		"ConfigReload",
		Side,
		"Reload",
		"BOTTOMRIGHT",
		...
		25,
		true,
		LeaPlusCB["ReloadUIButton"].tiptext
	)
	...
	Side:SetScript("OnDragStop", function()
		...
		-- Save panel position
		LeaPlusLC["MainPanelA"], void, LeaPlusLC["MainPanelR"], LeaPlusLC["MainPanelX"], LeaPlusLC["MainPanelY"] =
			Side:GetPoint()
	end)
	...
	Side:SetScript("OnShow", function()
		Side:ClearAllPoints()
		Side:SetPoint(
			LeaPlusLC["MainPanelA"],
			UIParent,
			LeaPlusLC["MainPanelR"],
			LeaPlusLC["MainPanelX"],
			LeaPlusLC["MainPanelY"]
		)
	...
	if scrolling then
		...
		-- Create scroll frame
		Side.scrollFrame =
			CreateFrame("ScrollFrame", nil, Side.backFrame, "LeaPlusConfigurationPanelScrollFrameTemplate")
...
function LeaPlusLC:CreateMainPanel()
	...
	PageF.v:SetText(L["BCA"] .. " " .. LeaPlusLC["AddonVer"])
	...
	-- Add web link Button
	local PageFAlertButton = LeaPlusLC:CreateButton(
		"PageFAlertButton",
		PageF,
		"You should keybind web link!",
		"BOTTOMLEFT",
		16,
		10,
		0,
		25,
		true,
		"You should set a keybind for the web link feature.  It's very useful.|n|nOpen the key bindings window (accessible from the game menu) and click Leatrix Plus.|n|nSet a keybind for Show web link.|n|nNow when your pointer is over an item, NPC or spell (and more), press your keybind to get a web link.",
		true
	)
	...
	if str and str ~= "" then
		...
		elseif str == "rest" then
			...
			LeaPlusLC:Print(
				L["Rested bubbles"]
					.. ": |cffffffff"
					.. (math.floor(20 * (GetXPExhaustion() or 0) / UnitXPMax("player") + 0.5))
			)
			return
		...
		elseif str == "id" then
			...
			if not LeaPlusLC.WowheadLock then
				...
				if GameLocale == "deDE" then
					LeaPlusLC.WowheadLock = "de.tbc.wowhead.com"
				elseif GameLocale == "esMX" then
					LeaPlusLC.WowheadLock = "mx.tbc.wowhead.com"
				elseif GameLocale == "esES" then
					LeaPlusLC.WowheadLock = "es.tbc.wowhead.com"
				elseif GameLocale == "frFR" then
					LeaPlusLC.WowheadLock = "fr.tbc.wowhead.com"
				elseif GameLocale == "itIT" then
					LeaPlusLC.WowheadLock = "it.tbc.wowhead.com"
				elseif GameLocale == "ptBR" then
					LeaPlusLC.WowheadLock = "pt.tbc.wowhead.com"
				elseif GameLocale == "ruRU" then
					LeaPlusLC.WowheadLock = "ru.tbc.wowhead.com"
				elseif GameLocale == "koKR" then
					LeaPlusLC.WowheadLock = "ko.tbc.wowhead.com"
				elseif GameLocale == "zhCN" then
					LeaPlusLC.WowheadLock = "cn.tbc.wowhead.com"
				elseif GameLocale == "zhTW" then
					LeaPlusLC.WowheadLock = "tw.tbc.wowhead.com"
				else
					LeaPlusLC.WowheadLock = "tbc.wowhead.com"
				end
			end
			...
			local tooltip
			if ItemRefTooltip:IsMouseMotionFocus() then
			...
			if tooltip:IsShown() then
					...
					if _G["BuffButton" .. i] and mouseFocus == _G["BuffButton" .. i] then
						...
						if BuffData then
							...
							if spellName and spellID then
								LeaPlusLC:ShowSystemEditBox(
									"https://" .. LeaPlusLC.WowheadLock .. "/spell=" .. spellID,
									false
								)
								LeaPlusLC.FactoryEditBox.f:SetText(
									L["Spell"] .. ": " .. spellName .. " (" .. spellID .. ")"
								)
							end
						end
						return
				...
				if tipTitle then
					...
					if not unitFocus or not UnitIsPlayer(unitFocus) then
						...
						LeaPlusLC:ShowSystemEditBox(
							"https://" .. LeaPlusLC.WowheadLock .. "/search?q=" .. tipTitle,
							false
						)
		...
		elseif str == "tooltip" then
			...
			pcall(function()
				...
				while enumf do
					if
						(enumf:GetObjectType() == "GameTooltip" or strfind((enumf:GetName() or ""):lower(), "tip"))
						and enumf:IsVisible()
						and enumf:GetPoint()
					then
						print(enumf:GetName())
			...
			if GameTooltipTextLeft1:IsShown() then
				...
				LeaPlusLC:Print(
					L["Tooltip title color"]
						.. ": "
						.. strupper(string.format("%02x%02x%02x", r * 255, g * 255, b * 255) .. ".")
				)
			else
		...
		elseif str == "mk" then
			...
			print(
				'"'
					.. mod(floor(key / 2 ^ 36), 2 ^ 12)
					.. ":"
					.. mod(floor(key / 2 ^ 24), 2 ^ 12)
					.. ":"
					.. mod(floor(key / 2 ^ 12), 2 ^ 12)
					.. ":"
					.. mod(key, 2 ^ 12)
					.. '"'
			)
		...
		elseif str == "help" then
			...
			if not LeaPlusLC.HelpFrame then
				...
				LeaPlusLC:MakeWD(
					frame,
					"Block target markers (toggle) (requires assistant or leader in raid).",
					col2,
					-250
				)
					...
					if pos.x and pos.x ~= "0" and pos.y and pos.y ~= "0" then
						...
						if uHealth and uHealth > 0 and uHealthMax and uHealthMax > 0 then
							...
							C_ChatInfo.SendChatMessage(
								format(
									"%%t " .. unitTag .. "(%d%%)%s",
									uHealth / uHealthMax * 100,
									" "
										.. string.format("%.0f", pos.x * 100)
										.. ":"
										.. string.format("%.0f", pos.y * 100)
								),
								"CHANNEL",
								nil,
								index
		...
		elseif str == "col" then
			...
			if r and g and b then
				...
				if r > 1 and g > 1 and b > 1 then
					-- RGB to Hex
					LeaPlusLC:Print(
						"Hex: |cffffffff" .. strupper(string.format("%02x%02x%02x", r, g, b)) .. " (from RGB)"
					)
				else
					-- Wow to Hex
					LeaPlusLC:Print(
			...
			elseif arg1 and strlen(arg1) == 6 and strmatch(arg1, "%x") and arg2 == nil and arg3 == nil then
				...
				if strmatch(rhex, "%x") and strmatch(ghex, "%x") and strmatch(bhex, "%x") then
					...
					LeaPlusLC:Print(
						"Wow: |cffffffff"
							.. string.format("%.1f", tonumber(rhex, 16) / 255)
							.. "  "
							.. string.format("%.1f", tonumber(ghex, 16) / 255)
							.. "  "
							.. string.format("%.1f", tonumber(bhex, 16) / 255)
					)
					LeaPlusLC:Print(
						"RGB: |cffffffff"
							.. tonumber(rhex, 16)
			...
			f:SetScript("OnUpdate", function()
				...
				if x and y and x > 0 and y > 0 then
					if MouseIsOver(f, -31, 31, 31, -31) then
						ChatFrame1:Clear()
						print(
							('{"Arrow", ' .. floor(x * 1000 + 0.5) / 10) .. ",",
							(floor(y * 1000 + 0.5) / 10)
								.. ', L["Step 1"], L["Start here."], arTex, nil, nil, nil, nil, nil, '
								.. f.f:GetText()
								.. "},"
						)
		...
		elseif str == "limit" then
			...
			if not LeaPlusLC.MuteFrame then
				...
				local endBox = LeaPlusLC:CreateEditBox(
					"SoundEndBox",
					frame,
					116,
					10,
				...
				-- Set limit button
				frame.btn = LeaPlusLC:CreateButton(
					"muteRangeButton",
					frame,
					"SET LIMIT",
					"TOPLEFT",
					...
					25,
					true,
					"Click to set the sound file limit.  Use the mousewheel on the editbox along with the step buttons below to adjust the sound limit.  Acceptable range is from 1 to 9000000.  Sound files higher than this limit will be muted."
				)
				...
				-- Mute all button
				frame.MuteAllBtn = LeaPlusLC:CreateButton(
					"muteMuteAllButton",
					frame,
					"MUTE ALL",
					"TOPLEFT",
					...
					25,
					true,
					"Click to mute every sound in the game."
				)
				...
				-- Unmute all button
				frame.UnmuteAllBtn = LeaPlusLC:CreateButton(
					"muteUnmuteAllButton",
					frame,
					"UNMUTE ALL",
					"TOPLEFT",
					...
					25,
					true,
					"Click to unmute every sound in the game."
				)
				...
				-- Step buttons
				frame.millionBtn = LeaPlusLC:CreateButton(
					"SoundMillionButton",
					frame,
					"1000000",
					"TOPLEFT",
					...
					25,
					true,
					"Set the editbox step value to 1000000."
				)
				frame.millionBtn:SetScale(0.5)

				frame.hundredThousandBtn = LeaPlusLC:CreateButton(
					"SoundHundredThousandButton",
					frame,
					"100000",
					"TOPLEFT",
					16,
					...
					25,
					true,
					"Set the editbox step value to 100000."
				)
				...
				frame.tenThousandBtn = LeaPlusLC:CreateButton(
					"SoundTenThousandButton",
					frame,
					"10000",
					"TOPLEFT",
					16,
					...
					25,
					true,
					"Set the editbox step value to 10000."
				)
				...
				frame.thousandBtn = LeaPlusLC:CreateButton(
					"SoundThousandButton",
					frame,
					"1000",
					"TOPLEFT",
					...
					25,
					true,
					"Set the editbox step value to 1000."
				)
				...
				frame.hundredBtn = LeaPlusLC:CreateButton(
					"SoundHundredButton",
					frame,
					"100",
					"TOPLEFT",
				...
				frame.oneBtn = LeaPlusLC:CreateButton(
					...
					25,
					true,
					"Set the editbox step value to 1."
				)
		...
		elseif str == "svars" then
			...
			LeaPlusLC:Print(
				L["The following list shows option label, setting name and currently saved value.  Enable |cffffffffIncrease chat history|r (chat) and |cffffffffRecent chat window|r (chat) to make it easier."]
					.. "|n"
			)
			LeaPlusLC:Print(
				L["Modifying saved variables must start with |cffffffff/ltp nosave|r to prevent your changes from being reverted during reload or logout."]
					.. "|n"
			)
			LeaPlusLC:Print(
				L["Syntax is |cffffffff/run LeaPlusDB[" .. '"' .. "setting name" .. '"' .. "] = " .. '"' .. "value" .. '" |r(case sensitive).']
			)
			...
			for key, value in pairs(LeaPlusDB) do
				if LeaPlusCB[key] and LeaPlusCB[key].f then
					if LeaPlusCB[key]:GetObjectType() ~= "Slider" and LeaPlusCB[key]:GetObjectType() ~= "Button" then
						LeaPlusLC:Print(
							string.gsub(LeaPlusCB[key].f:GetText(), "%*$", "")
								.. ": |cffffffff"
								.. key
								.. "|r |cff1eff0c("
								.. value
			...
			LeaPlusLC:Print(
				L["Sliders can be set to a numeric value which must be in the range supported by the slider."] .. "|n"
			)
			for key, value in pairs(LeaPlusDB) do
				if LeaPlusCB[key] and LeaPlusCB[key].f then
			...
			LeaPlusLC:Print(
				L["Dropdowns can be set to a numeric value which must be in the range supported by the dropdown."]
					.. "|n"
			)
			for key, value in pairs(LeaPlusDB) do
		...
		elseif str == "admin" then
			...
			-- Set chat font sizes
			RunScript(
				'for i = 1, 50 do if _G["ChatFrame" .. i] then FCF_SetChatWindowFontSize(self, _G["ChatFrame" .. i], 20) end end'
			)
...
function LeaPlusLC:MakePage(name, title, menu, menuname, menuparent, menuanchor, menux, menuy, menuwidth, menuheight)
	...
	if menu then
		LeaPlusLC[menu], LeaPlusLC[menu .. ".s"] =
			LeaPlusLC:MakeMN(menu, menuname, menuparent, menuanchor, menux, menuy, menuwidth, menuheight)
...
LeaPlusLC["Page0"] =
	LeaPlusLC:MakePage("Page0", "Home", "LeaPlusNav0", "Home", LeaPlusLC["PageF"], "TOPLEFT", 16, -72, 112, 20)
LeaPlusLC["Page1"] = LeaPlusLC:MakePage(
	"Page1",
	"Automation",
	"LeaPlusNav1",
	"Automation",
...
LeaPlusLC["Page2"] =
	LeaPlusLC:MakePage("Page2", "Social", "LeaPlusNav2", "Social", LeaPlusLC["PageF"], "TOPLEFT", 16, -132, 112, 20)
LeaPlusLC["Page3"] =
	LeaPlusLC:MakePage("Page3", "Chat", "LeaPlusNav3", "Chat", LeaPlusLC["PageF"], "TOPLEFT", 16, -152, 112, 20)
LeaPlusLC["Page4"] =
	LeaPlusLC:MakePage("Page4", "Text", "LeaPlusNav4", "Text", LeaPlusLC["PageF"], "TOPLEFT", 16, -172, 112, 20)
LeaPlusLC["Page5"] = LeaPlusLC:MakePage(
	"Page5",
	"Interface",
	"LeaPlusNav5",
	"Interface",
...
LeaPlusLC["Page6"] =
	LeaPlusLC:MakePage("Page6", "Frames", "LeaPlusNav6", "Frames", LeaPlusLC["PageF"], "TOPLEFT", 16, -212, 112, 20)
LeaPlusLC["Page7"] =
	LeaPlusLC:MakePage("Page7", "System", "LeaPlusNav7", "System", LeaPlusLC["PageF"], "TOPLEFT", 16, -232, 112, 20)
LeaPlusLC["Page8"] =
	LeaPlusLC:MakePage("Page8", "Settings", "LeaPlusNav8", "Settings", LeaPlusLC["PageF"], "TOPLEFT", 16, -272, 112, 20)
LeaPlusLC["Page9"] =
	LeaPlusLC:MakePage("Page9", "Media", "LeaPlusNav9", "Media", LeaPlusLC["PageF"], "TOPLEFT", 16, -292, 112, 20)
...
LeaPlusLC:MakeCB(
	...
	-92,
	false,
	"If checked, quests will be selected, accepted and turned-in automatically.|n|nQuests which have a gold requirement will not be turned-in automatically."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"AutomateGossip",
	...
	-112,
	false,
	"If checked, you can hold down the alt key while opening a gossip window to automatically select a single gossip item.|n|nIf the gossip item type is banker, taxi, trainer, vendor or battlemaster, gossip will be skipped without needing to hold the alt key.  You can hold the shift key down to prevent this."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"AutoAcceptSummon",
...
LeaPlusLC:MakeCB(
	...
	-172,
	false,
	"If checked, you will release automatically after you die in a battleground.|n|nYou will not release automatically if you have the ability to self-resurrect."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	false,
	"If checked, all grey items in your bags will be sold automatically when you visit a merchant.|n|nYou can hold the shift key down when you talk to a merchant to override this setting."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"AutoRepairGear",
	...
	-112,
	false,
	"If checked, your gear will be repaired automatically when you visit a suitable merchant.|n|nYou can hold the shift key down when you talk to a merchant to override this setting."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	false,
	"If checked, duel requests will be blocked unless the player requesting the duel is a friend."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoPartyInvites",
	...
	-112,
	false,
	"If checked, party invitations will be blocked unless the player inviting you is a friend."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoFriendRequests",
	...
	-132,
	false,
	"If checked, BattleTag and Real ID friend requests will be automatically declined.|n|nEnabling this option will automatically decline any pending requests."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoSharedQuests",
	...
	-152,
	false,
	"If checked, shared quests will be declined unless the player sharing the quest is a friend."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	false,
	"If checked, party invitations from friends will be automatically accepted unless you are queued for a battleground or the Looking for Group feature."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"InviteFromWhisper",
...
local FriendlyGuildFooter = LeaPlusLC:MakeFT(
	LeaPlusLC[pg],
	"For all of the social options above, you can treat guild members as friends too.",
	146,
	380
)
LeaPlusLC:MakeCB(
	...
	-282,
	false,
	"If checked, members of your guild will be treated as friends for all of the options on this page."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, dragging the General chat tab while the chat frame is locked will expand the chat frame upwards.|n|nIf the chat frame is unlocked, dragging the General chat tab will move the chat frame."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoCombatLogTab",
	...
	-112,
	true,
	"If checked, the combat log will be hidden.|n|nThe combat log must be docked in order for this option to work.|n|nIf the combat log is undocked, you can dock it by dragging the tab (and reloading your UI) or by resetting the chat windows (from the chat menu)."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoChatButtons",
	...
	-132,
	true,
	"If checked, chat frame buttons will be hidden.|n|nClicking chat tabs will automatically show the latest messages.|n|nUse the mouse wheel to scroll through the chat history.  Hold down SHIFT for page jump or CTRL to jump to the top or bottom of the chat history."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"UnclampChat",
...
LeaPlusLC:MakeCB(
	...
	-192,
	true,
	"If checked, additional font sizes will be available in the chat frame font size menu."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, sticky chat will be disabled.|n|nNote that this does not apply to temporary chat windows."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"UseArrowKeysInChat",
	...
	-112,
	true,
	"If checked, you can press the arrow keys to move the insertion point left and right in the chat frame.|n|nIf unchecked, the arrow keys will use the default keybind setting."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoChatFade",
...
LeaPlusLC:MakeCB(
	...
	-152,
	false,
	"If checked, raid chat will be colored blue (to match the default party chat color)."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ClassColorsInChat",
...
LeaPlusLC:MakeCB(
	...
	-192,
	true,
	"If checked, you can hold down the control key and click a chat tab to view recent chat in a copy-friendly window."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"MaxChatHstory",
	...
	-212,
	true,
	"If checked, your chat history will increase to 4096 lines.  If unchecked, the default will be used (128 lines).|n|nEnabling this option may prevent some chat text from showing during login."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"FilterChatMessages",
	...
	-232,
	true,
	"If checked, you can block drunken spam and duel spam."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"RestoreChatMessages",
	...
	-252,
	true,
	"If checked, recent chat will be restored when you reload your interface."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, most error messages (such as 'Not enough rage') will not be shown.  Some important errors are excluded.|n|nIf you have the minimap button enabled, you can hold down the alt key and click it to toggle error messages without affecting this setting."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoHitIndicators",
...
LeaPlusLC:MakeCB(
	...
	-152,
	true,
	"If checked, keybind text will not be shown on action buttons."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"HideMacroText",
	...
	-172,
	true,
	"If checked, macro text will not be shown on action buttons."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"HideRaidGroupLabels",
	...
	-192,
	true,
	"If checked, the player frame group indicator and the group labels displayed above the compact raid frames and the pullout raid frames will be hidden."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, you will be able to change the font size of standard mail text.|n|nThis does not affect mail created using templates (such as auction house invoices)."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"QuestFontChange",
	...
	-112,
	true,
	"If checked, you will be able to change the font size of quest text."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"BookFontChange",
	...
	-132,
	true,
	"If checked, you will be able to change the font size of book text."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, you will be able to customise the minimap."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"TipModEnable",
...
LeaPlusLC:MakeCB(
	...
	-152,
	true,
	"If checked, the quest log frame will be larger and feature a world map button and quest levels."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"EnhanceProfessions",
	...
	-172,
	true,
	"If checked, the professions frame will be larger."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"EnhanceTrainers",
	...
	-192,
	true,
	"If checked, the skill trainer frame will be larger and feature a train all skills button."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"EnhanceFlightMap",
	...
	-212,
	true,
	"If checked, you will be able to customise the flight map."
)
...
LeaPlusLC:MakeCB(
	...
	-272,
	true,
	"If checked, a master volume slider will be shown in the character frame."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"AhExtras",
	...
	-292,
	true,
	"If checked, additional functionality will be added to the auction house.|n|nBuyout only - create buyout auctions without filling in the starting price.|n|nGold only - set the copper and silver prices at 99 to speed up new auctions.|n|nFind item - search the auction house for the item you are selling.|n|nIn addition, the auction duration setting will be saved account-wide."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, you will be able to place up to five beneficial cooldown icons above the target frame."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"DurabilityStatus",
	...
	-112,
	true,
	"If checked, a button will be added to the character frame which will show your equipped item durability when you hover the pointer over it.|n|nIn addition, an overall percentage will be shown in the chat frame when you die."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowVanityControls",
	...
	-132,
	true,
	"If checked, helm and cloak toggle checkboxes will be shown in the character frame.|n|nYou can hold shift and right-click the checkboxes to switch layouts."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowBagSearchBox",
...
LeaPlusLC:MakeCB(
	...
	-192,
	true,
	"If checked, the button to toggle the raid container frame will be shown just above the raid management frame (left side of the screen) instead of in the raid management frame itself.|n|nThis allows you to toggle the raid container frame without needing to open the raid management frame."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowBorders",
	...
	-212,
	true,
	"If checked, you will be able to show customisable borders around the edges of the screen.|n|nThe borders are placed on top of the game world but under the UI so you can place UI elements over them."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowPlayerChain",
...
LeaPlusLC:MakeCB(
	...
	-252,
	true,
	"If checked, a power bar will be shown in the player frame when you are playing a shapeshifted druid."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowReadyTimer",
	...
	-272,
	true,
	"If checked, a timer will be shown under the PvP encounter ready frame so that you know how long you have left to click the enter button."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowWowheadLinks",
	...
	-292,
	true,
	"If checked, Wowhead links will be shown above the quest log frame."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, you will be able to change the position and scale of the widget frame.|n|nThe widget frame is commonly used for showing PvP scores and tracking objectives."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ManageTimer",
	...
	-112,
	true,
	"If checked, you will be able to change the position and scale of the timer bar.|n|nThe timer bar is used for showing remaining breath when underwater as well as other things."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ClassColFrames",
	...
	-132,
	true,
	"If checked, class coloring will be used in the player frame and target frame."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	false,
	"If checked, the screen glow will be disabled.|n|nEnabling this option will also disable the drunken haze effect."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoScreenEffects",
	...
	-112,
	false,
	"If checked, the grey screen of death and the netherworld effect will be disabled."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"SetWeatherDensity",
	...
	-132,
	false,
	"If checked, you will be able to set the density of weather effects."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"MaxCameraZoom",
...
LeaPlusLC:MakeCB(
	...
	-172,
	true,
	"If checked, emote sounds will be silenced while your character is resting or at the Grim Guzzler.|n|nEmote sounds will be enabled at all other times."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"KeepAudioSynced",
...
LeaPlusLC:MakeCB(
	...
	-212,
	false,
	"If checked, you will be able to mute a selection of game sounds."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"MuteCustomSounds",
	...
	-232,
	false,
	"If checked, you will be able to mute your own choice of sounds."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	true,
	"If checked, your bags will not be opened or closed automatically when you interact with a merchant, bank or mailbox."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"NoConfirmLoot",
	...
	-112,
	false,
	"If checked, confirmations will no longer appear when you choose a loot roll option or attempt to sell or mail a tradable item."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"FasterLooting",
	...
	-132,
	true,
	"If checked, the amount of time it takes to auto loot creatures will be significantly reduced."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"FasterMovieSkip",
	...
	-152,
	true,
	"If checked, you will be able to cancel cinematics without being prompted for confirmation."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"StandAndDismount",
	...
	-172,
	true,
	"If checked, you will be able to set some additional rules for when your character is automatically dismounted."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowVendorPrice",
	...
	-192,
	true,
	"If checked, the vendor price will be shown in item tooltips."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"CombatPlates",
	...
	-212,
	true,
	"If checked, enemy nameplates will be shown during combat and hidden when combat ends."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"EasyItemDestroy",
	...
	-232,
	true,
	"If checked, you will no longer need to type delete when destroying a superior quality item.|n|nIn addition, item links will be shown in all item destroy confirmation windows."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"ShowFlightTimes",
	...
	-252,
	true,
	"If checked, flight times will be shown in the flight map and when you take a flight."
)
...
LeaPlusLC:MakeCB(
	...
	-92,
	false,
	"If checked, a minimap button will be available.|n|nClick - Toggle options panel.|n|nSHIFT-click - Toggle music.|n|nALT-click - Toggle errors (if enabled).|n|nCTRL/SHIFT-click - Toggle windowed mode.|n|nCTRL/ALT-click - Toggle Zygor (if installed)."
)
LeaPlusLC:MakeCB(
	LeaPlusLC[pg],
	"UseEnglishLanguage",
	...
	-112,
	true,
	"If checked, text used throughout the addon will be shown in English regardless of your game locale."
)
