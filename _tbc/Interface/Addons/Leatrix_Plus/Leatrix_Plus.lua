----------------------------------------------------------------------
-- 	Leatrix Plus 2.5.000 (14th January 2026)
----------------------------------------------------------------------
...
function LeaPlusLC:Player()
	...
	do
		...
		local npcTable = {
			...
			-- Vendor (https://www.wowhead.com/tbc/npcs?filter=29;1;0)
			12794,
			18525,
			12795,
			12784,
	...
	if LeaPlusLC["ShowPlayerChain"] == "On" and not LeaLockList["ShowPlayerChain"] then
		...
		PlayerFrameTexture:SetPoint("CENTER", -16, -3)
	...
	if LeaPlusLC["ClassColFrames"] == "On" and not LeaLockList["ClassColFrames"] then
		...
		LeaPlusLC:MakeCB(
			ClassFrame,
			"ClassColTarget",
			"Show target frame in class color",
			16,
			-112,
			false,
			"If checked, the target frame background will be shown in class color."
		)
			...
			if StaticPopup1EditBox:IsShown() then
				...
				if link then
					StaticPopup1Text:SetText(
						gsub(StaticPopup1Text:GetText(), gsub(TypeDeleteLine, "@", ""), "") .. "|n" .. link
					)
				end
			else
				...
				if link then
					StaticPopup1Text:SetText(
						gsub(StaticPopup1Text:GetText(), gsub(TypeDeleteLine, "@", ""), "") .. "|n|n" .. link
					)
				end
			end
		end)
	...
	if LeaPlusLC["MinimapModder"] == "On" and not LeaLockList["MinimapModder"] then
		...
		LeaPlusLC:MakeCB(
			...
			-212,
			true,
			"If checked, minimap buttons for addons will be collected and placed in a frame which you can toggle by right-clicking the minimap.|n|nThis setting will help you declutter the minimap without the need to install a separate addon just to do that."
		)
		LeaPlusLC:MakeCB(
			SideMinimap,
			"SquareMinimap",
		...
		if LeaPlusLC["CombineAddonButtons"] == "On" then
			...
			LeaPlusCB["HideMiniAddonButtons"].tiptext = LeaPlusCB["HideMiniAddonButtons"].tiptext
				.. "|n|n|cff00AAFF"
				.. L["Cannot be used with Combine addon buttons."]
