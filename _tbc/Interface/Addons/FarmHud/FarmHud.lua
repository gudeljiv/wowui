...
ns.QuestArrowToken = {}
ns.modules = {}
local modEvents, events =
	{},
	{ VARIABLES_LOADED = true, PLAYER_ENTERING_WORLD = true, PLAYER_LOGIN = true, PLAYER_LOGOUT = true, MODIFIER_STATE_CHANGED = true }
...
local minimapScripts, cardinalTicker, coordsTicker = { --[["OnMouseUp",]]
	OnMouseDown = "Dummy",
	OnDragStart = "nil",
}
...
local foreignObjects, anchoredFrames =
	{}, { -- <name[string]>
		-- Blizzard
...
function ns.GetTrackingTypes()
	...
	if numTrackingTypes ~= num then
		...
		wipe(trackingTypes)
		for i = 1, num do
			local name, textureId, active, objType, objLevel, objId = (
				C_Minimap and C_Minimap.GetTrackingInfo or GetTrackingInfo
			)(i)
...
do
	...
	replacements = {
		GetWidth = function()
			return MinimapOrDummy("GetWidth")
		end,
		GetHeight = function()
			return MinimapOrDummy("GetHeight")
		end,
		GetSize = function()
			return MinimapOrDummy("GetSize")
		end,
		GetCenter = function()
			return Dummy:GetCenter()
		end,
		GetEffectiveScale = function()
			return Dummy:GetEffectiveScale()
		end,
		GetLeft = function()
...
local function objectToDummy(object, enable, debugStr)
	-- == ignore == --
	if
		HBDPins and HBDPins.minimapPins[object] -- ignore herebedragons pins
	then
		return
	end
	...
	for i = 1, #FarmHud.TextFrame.cardinalPoints do
		...
		if bearing then
			v:SetPoint(
				...
				FarmHud,
				"CENTER",
				math.sin(v.rot + bearing) * scaledRadius,
				math.cos(v.rot + bearing) * scaledRadius
			)
		end
	end
end
...
function FarmHudMixin:RegisterForeignAddOnObject(object, byAddOn)
	...
	assert(
		arg1Type == "table" and object.GetObjectType,
...
function FarmHudMixin:Toggle(force)
	if #knownProblematicAddOnsDetected > 0 then
		ns:print(
			"|cffffee00" .. L["KnownProblematicAddOnDetected"] .. "|r",
...
function FarmHudMixin:RegisterModule(name, module)
	assert(
		type(name) == "string" and type(module) == "table",
...
function FarmHudMixin:OnEvent(event, ...)
	...
	elseif event == "MODIFIER_STATE_CHANGED" and self:IsShown() then
		...
		if
			not mouseOnKeybind
			and modifiers[FarmHudDB.holdKeyForMouseOn]
			and modifiers[FarmHudDB.holdKeyForMouseOn][key] == 1
		then
