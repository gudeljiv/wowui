...
function LeaMapsLC:MainFunc()
	...
	if
		LeaMapsLC["UseDefaultMap"] == "On" and LeaMapsLC["ShowZoneMenu"] == "On"
		or LeaMapsLC["UseDefaultMap"] == "Off"
	then
	...
	if LeaMapsLC["UseDefaultMap"] == "Off" then
		...
		WorldMapFrameCloseButton.SetPoint = function()
			return
		end
	...
	if LeaMapsLC["ShowZoneMenu"] == "On" and not LeaLockList["ShowZoneMenu"] then
		...
		local nodd = LeaMapsLC:CreateDropdown(
			"ZoneMapNoneMenu",
			nil,
			184,
			"TOPLEFT",
		...
		table.sort(mapEasternString, function(k, v)
			return k[1] < v[1]
		end)
		...
		local ekdd = LeaMapsLC:CreateDropdown(
			"ZoneMapEasternMenu",
			nil,
			184,
			"TOPLEFT",
		...
		table.sort(mapKalimdorString, function(k, v)
			return k[1] < v[1]
		end)
		...
		local kmdd = LeaMapsLC:CreateDropdown(
			"ZoneMapKalimdorMenu",
			nil,
			184,
			"TOPLEFT",
		...
		table.sort(mapOutlandString, function(k, v)
			return k[1] < v[1]
		end)
		...
		local otdd = LeaMapsLC:CreateDropdown(
			"ZoneMapOutlandMenu",
			nil,
			184,
			"TOPLEFT",
		...
		local cond = LeaMapsLC:CreateDropdown(
			"ZoneMapContinentMenu",
			nil,
			184,
			"TOPLEFT",
			outerFrame,
		...
		local function SetMapControls()
			...
			for k, v in pairs(mapKalimdorTable) do
				if v.mapid == WorldMapFrame.mapID then
					...
					kmdd:Show()
					LeaMapsLC["ZoneMapContinentMenu"] = 2
					cond:Show()
					return
				end
			end
			...
			for k, v in pairs(mapOutlandTable) do
				if v.mapid == WorldMapFrame.mapID then
					...
					otdd:Show()
					LeaMapsLC["ZoneMapContinentMenu"] = 3
					cond:Show()
					return
				end
			end
			...
			if WorldMapFrame.mapID == 947 then
				nodd:Show()
				LeaMapsLC["ZoneMapContinentMenu"] = 4
				cond:Show()
				return
			end
		end
		...
		if LeaMapsLC.ElvUI then
			...
			if E.private.skins.blizzard.enable and E.private.skins.blizzard.worldmap then
				...
				ekdd:ClearAllPoints()
				ekdd:SetPoint("LEFT", cond, "RIGHT", 4, 0)
				S:HandleDropDownBox(kmdd)
				kmdd:ClearAllPoints()
				kmdd:SetPoint("LEFT", cond, "RIGHT", 4, 0)
				S:HandleDropDownBox(nodd)
				nodd:ClearAllPoints()
				nodd:SetPoint("LEFT", cond, "RIGHT", 4, 0)
			end
		end
	end
	...
	if LeaMapsLC["EnhanceBattleMap"] == "On" then
		...
		LeaMapsLC:MakeCB(
			battleFrame,
			"UnlockBattlefield",
			"Unlock battlefield map",
			16,
			-92,
			false,
			"If checked, you can move the battlefield map by dragging any of its borders.|n|nYou can resize the battlefield map by dragging the bottom-right corner."
		)
		LeaMapsLC:MakeCB(
			battleFrame,
			"BattleCenterOnPlayer",
			"Center map on player",
			16,
			-112,
			false,
			"If checked, the battlefield map will stay centered on your location as long as you are not in a dungeon.|n|nYou can hold shift while panning the map to temporarily prevent it from centering."
		)
		...
		LeaMapsLC:MakeSL(
			...
			"BattleGroupIconSize",
			"Group Icons",
			"Drag to set the group icon size.",
			12,
			48,
			1,
			206,
		...
		LeaMapsLC:MakeSL(
			battleFrame,
			"BattlePlayerArrowSize",
			"Player Arrow",
			"Drag to set the player arrow size.",
			12,
			48,
			1,
			36,
		...
		LeaMapsLC:MakeSL(
			...
			"BattleMapSize",
			"Map Size",
			"Drag to set the battlefield map size.|n|nIf the map is unlocked, you can also resize the battlefield map by dragging the bottom-right corner.",
			150,
			1200,
			1,
			36,
		...
		LeaMapsLC:MakeSL(
			...
			"BattleMapOpacity",
			"Map Opacity",
			"Drag to set the battlefield map opacity.",
			0.1,
			1,
			0.1,
			206,
		...
		LeaMapsLC:MakeSL(
			...
			"BattleMaxZoom",
			"Max Zoom",
			"Drag to set the maximum zoom level.|n|nOpen the battlefield map to see the maximum zoom level change as you drag the slider.",
			1,
			6,
			0.1,
			36,
		...
		BattlefieldMapFrame:SetPoint(
			LeaMapsLC["BattleMapA"],
			UIParent,
			LeaMapsLC["BattleMapR"],
			LeaMapsLC["BattleMapX"],
			LeaMapsLC["BattleMapY"]
		)
		...
		eFrame:SetScript("OnMouseUp", function()
			...
			LeaMapsLC["BattleMapA"], void, LeaMapsLC["BattleMapR"], LeaMapsLC["BattleMapX"], LeaMapsLC["BattleMapY"] =
				BattlefieldMapFrame:GetPoint()
			BattlefieldMapFrame:SetMovable(true)
			BattlefieldMapFrame:ClearAllPoints()
			BattlefieldMapFrame:SetPoint(
				LeaMapsLC["BattleMapA"],
		...
		battleFrame:HookScript("OnShow", function()
			if BattlefieldMapFrame:IsShown() then
				LeaMapsLC.BFMapWasShown = true
			else
				LeaMapsLC.BFMapWasShown = false
			end
			RunScript("BattlefieldMapFrame:Show()")
		end)
		battleFrame:HookScript("OnHide", function()
			if not LeaMapsLC.BFMapWasShown then
				RunScript("BattlefieldMapFrame:Hide()")
			end
		end)
		...
		hooksecurefunc(BattlefieldMapFrame.ScrollContainer, "CreateZoomLevels", function(self)
			if LeaMapsLC["BattleMaxZoom"] == 1 then
				return
			end
			...
			for layerIndex, layerInfo in ipairs(layers) do
				...
				for zoomLevelIndex = 0, numZoomLevels - 1 do
					currentScale =
						math.max(layerInfo.minScale + zoomDeltaPerStep * zoomLevelIndex, currentScale + MIN_SCALE_DELTA)
		...
		do
			...
			scaleMouse:SetScript("OnEnter", function()
				scaleHandle.t:SetDesaturated(false)
			end)
			scaleMouse:SetScript("OnLeave", function()
				scaleHandle.t:SetDesaturated(true)
			end)
			...
			scaleMouse:SetScript("OnMouseDown", function(frame)
				...
				frame:SetScript("OnUpdate", function(self, elapsed)
					if BattlefieldMapFrame:GetWidth() > 1200 then
						BattlefieldMapFrame:SetWidth(1200)
					end
					if BattlefieldMapFrame:GetWidth() < 150 then
						BattlefieldMapFrame:SetWidth(150)
					end
					...
					if mapTime > 0.5 or mapTime == -1 then
						...
						LeaMapsCB["BattleMapSize"]:Hide()
						LeaMapsCB["BattleMapSize"]:Show()
						mapTime = 0
					end
		...
		do
			...
			local function cUpdate(self, elapsed)
				if cTime > 2 or cTime == -1 then
					if BattlefieldMapFrame.ScrollContainer:IsPanning() then
						return
					end
					...
					if position then
						...
						if x then
							local minX, maxX, minY, maxY =
								BattlefieldMapFrame.ScrollContainer:CalculateScrollExtentsAtScale(
									BattlefieldMapFrame.ScrollContainer:GetCanvasScale()
								)
		...
		local function SetPlayerArrow()
			for pin in BattlefieldMapFrame:EnumerateAllPins() do
				if pin.UpdateAppearanceData then
					BattlefieldMapFrame.groupMembersDataProvider:SetUnitPinSize(
						"player",
						LeaMapsLC["BattlePlayerArrowSize"]
					)
		...
		local function SetIconSize()
			LeaMapsCB["BattleGroupIconSize"].f:SetText(
				LeaMapsLC["BattleGroupIconSize"]
					.. " ("
					.. string.format("%.0f%%", LeaMapsLC["BattleGroupIconSize"] / 12 * 100)
					.. ")"
			)
			FixGroupPin()
		...
		battleFrame.r:HookScript("OnClick", function()
			...
			LeaMapsLC["BattleMapA"], LeaMapsLC["BattleMapR"], LeaMapsLC["BattleMapX"], LeaMapsLC["BattleMapY"] =
				"BOTTOMRIGHT", "BOTTOMRIGHT", -47, 83
			BattlefieldMapFrame:ClearAllPoints()
			BattlefieldMapFrame:SetPoint(
				LeaMapsLC["BattleMapA"],
				UIParent,
	...
	if LeaMapsLC["AutoChangeZones"] == "On" then
		...
		zoneEvent:SetScript("OnEvent", function()
			...
			if
				...
				and newPlayerZone > 0
				and constPlayerZone
				and constPlayerZone > 0
				and newMapID == constPlayerZone
			then
	...
	do
		...
		local function cUpdate(self, elapsed)
			if cTime > 2 or cTime == -1 then
				if WorldMapFrame.ScrollContainer:IsPanning() then
					return
				end
				if IsShiftKeyDown() then
					cTime = -2000
				...
				if position then
					local x, y = position.x, position.y
					if x then
						local minX, maxX, minY, maxY = WorldMapFrame.ScrollContainer:CalculateScrollExtentsAtScale(
							WorldMapFrame.ScrollContainer:GetCanvasScale()
						)
						local cx = Clamp(x, minX, maxX)
						local cy = Clamp(y, minY, maxY)
	...
	if LeaMapsLC["UseClassIcons"] == "On" then
		...
		local function SetIconSize()
			LeaMapsCB["ClassIconSize"].f:SetText(
				LeaMapsLC["ClassIconSize"]
					.. " ("
					.. string.format("%.0f%%", LeaMapsLC["ClassIconSize"] / 20 * 100)
					.. ")"
			)
	...
	if LeaMapsLC["UseDefaultMap"] == "Off" then
		...
		LeaMapsLC:MakeSL(
			...
			"MapScale",
			"Map Scale",
			"Drag to set the world map scale.|n|nYou can also rescale the world map by dragging the bottom-right corner.",
			0.2,
			3,
			0.001,
			16,
		...
		LeaMapsCB["UnlockMapFrameBtn"]:HookScript("OnClick", function()
			if IsShiftKeyDown() and IsControlKeyDown() then
				...
				WorldMapFrame:SetScale(LeaMapsLC["MapScale"])
				if UnlockMapPanel:IsShown() then
					UnlockMapPanel:Hide()
					UnlockMapPanel:Show()
				end
			else
		...
		scaleMouse:SetScript("OnMouseDown", function(frame)
			...
			frame:SetScript("OnUpdate", function()
				...
				LeaMapsCB["MapScale"]:Hide()
				LeaMapsCB["MapScale"]:Show()
			end)
		...
		scaleMouse:SetScript("OnMouseUp", function(frame)
			...
			LeaMapsLC["MapScale"] = WorldMapFrame:GetScale()
			WorldMapFrame:SetScale(LeaMapsLC["MapScale"])
			LeaMapsLC["MapPosA"], void, LeaMapsLC["MapPosR"], LeaMapsLC["MapPosX"], LeaMapsLC["MapPosY"] =
				WorldMapScreenAnchor:GetPoint()
			WorldMapFrame:OnFrameSizeChanged()
		end)
	...
	if LeaMapsLC["EnlargePlayerArrow"] == "On" then
		...
		local function SetArrowSize()
			LeaMapsCB["PlayerArrowSize"].f:SetText(
				LeaMapsLC["PlayerArrowSize"]
					.. " ("
					.. string.format("%.0f%%", LeaMapsLC["PlayerArrowSize"] / 16 * 100)
					.. ")"
			)
	...
	do
		...
		local mapTable = {
			...
			-- Outland
			--[[Blade's Edge Mntains]]
			[1949] = { minLevel = 65, maxLevel = 70 },
			--[[Hellfire Peninsula]]
			[1944] = { minLevel = 58, maxLevel = 70, minFish = "280" },
			--[[Nagrand]]
			[1951] = { minLevel = 64, maxLevel = 70, minFish = "280 (380) (395)" },
			--[[Netherstorm]]
			[1953] = { minLevel = 66, maxLevel = 70, minFish = "380" },
			--[[Shadowmoon Valley]]
			[1948] = { minLevel = 67, maxLevel = 70, minFish = "280" },
			--[[Terokkar Forest]]
			[1952] = { minLevel = 62, maxLevel = 70, minFish = "355 (405)" },
			--[[Zangarmarsh]]
			[1946] = { minLevel = 60, maxLevel = 63, minFish = "305 (355)" },
		}
		...
		LeaMapsLC:MakeCB(
			levelFrame,
			"ShowFishingLevels",
			"Show minimum fishing skill levels",
			16,
			-92,
			false,
			"If checked, the minimum fishing skill levels will be shown."
		)
	...
	do
		...
		LeaMapsLC:MakeSL(
			...
			"IncreaseZoomMax",
			"Maximum",
			"Drag to set the maximum zoom level.|n|nOpen the map to see the maximum zoom level change as you drag the slider.",
			1,
			6,
			0.1,
			36,
		...
		hooksecurefunc(WorldMapFrame.ScrollContainer, "CreateZoomLevels", function(self)
			if LeaMapsLC["IncreaseZoom"] == "Off" then
				return
			end
			local layers = C_Map.GetMapArtLayers(self.mapID)
			local widthScale = self:GetWidth() / layers[1].layerWidth
	...
	if LeaMapsLC["SetMapOpacity"] == "On" and not LeaLockList["SetMapOpacity"] then
		if LeaMapsLC["UseDefaultMap"] == "Off" then
			...
			LeaMapsLC:MakeSL(
				...
				"stationaryOpacity",
				"Stationary",
				"Drag to set the map opacity for when your character is stationary.",
				0.1,
				1,
				0.1,
				36,
			...
			LeaMapsLC:MakeSL(
				...
				"movingOpacity",
				"Moving",
				"Drag to set the map opacity for when your character is moving.",
				0.1,
				1,
				0.1,
				206,
			...
			LeaMapsLC:MakeCB(
				alphaFrame,
				"NoFadeCursor",
				"Use stationary opacity while pointing at map",
				16,
				-182,
				false,
				"If checked, pointing at the map while your character is moving will cause the stationary opacity setting to be applied."
			)
			...
			local function SetMapOpacity()
				...
				PlayerMovementFrameFader.AddDeferredFrame(
					WorldMapFrame,
					LeaMapsLC["movingOpacity"],
					LeaMapsLC["stationaryOpacity"],
					0.5,
					function()
						return not WorldMapFrame:IsMouseOver() or LeaMapsLC["NoFadeCursor"] == "Off"
					end
				)
			end
	...
	if LeaMapsLC["ShowPointsOfInterest"] == "On" then
				...
				for i = 1, count do
					...
					if
						LeaMapsLC["ShowDungeonIcons"] == "On"
							and (pinInfo[1] == "Dungeon" or pinInfo[1] == "Raid" or pinInfo[1] == "Dunraid")
		...
		function LeaMapsGlobalPinMixin:OnMouseUp(btn)
			if btn == "LeftButton" then
				if self.ZoneCrossing then
					WorldMapFrame:SetMapID(self.ZoneCrossing)
				end
		...
		LeaMapsLC:MakeCB(
			poiFrame,
			"ShowDungeonIcons",
			"Show dungeons and raids",
			16,
			-92,
			false,
			"If checked, dungeons and raids will be shown."
		)
		LeaMapsLC:MakeCB(
			poiFrame,
			"ShowTravelPoints",
			"Show travel points for same faction",
			16,
			-112,
			false,
			"If checked, travel points for the same faction will be shown.|n|nThis includes flight points, boat harbors, zeppelin towers and tram stations."
		)
		LeaMapsLC:MakeCB(
			poiFrame,
			"ShowTravelOpposing",
			"Show travel points for opposing faction",
			16,
			-132,
			false,
			"If checked, travel points for the opposing faction will be shown.|n|nThis includes flight points, boat harbors, zeppelin towers and tram stations."
		)
		LeaMapsLC:MakeCB(
			poiFrame,
			"ShowSpiritHealers",
			...
			-152,
			false,
			"If checked, spirit healers will be shown."
		)
		LeaMapsLC:MakeCB(
			poiFrame,
			"ShowZoneCrossings",
			...
			-172,
			false,
			"If checked, zone crossings will be shown.|n|nThese are clickable arrows that indicate the zone exit pathways."
		)
	...
	if LeaMapsLC["RevealMap"] == "On" then
		...
		local function MapExplorationPin_RefreshOverlays(pin, fullUpdate)
			...
			if not artID or not Leatrix_Maps["Reveal"][artID] then
				return
			end
			...
			if exploredMapTextures then
				for i, exploredTextureInfo in ipairs(exploredMapTextures) do
					local key = exploredTextureInfo.textureWidth
						.. ":"
						.. exploredTextureInfo.textureHeight
						.. ":"
						.. exploredTextureInfo.offsetX
						.. ":"
				...
				if not TileExists[key] then
					...
					for j = 1, numTexturesTall do
						...
						for k = 1, numTexturesWide do
							...
							texture:SetTexture(
								tonumber(fileDataIDs[((j - 1) * numTexturesWide) + k]),
								nil,
								nil,
								"TRILINEAR"
							)
		...
		local function bfMapExplorationPin_RefreshOverlays(pin, fullUpdate)
			...
			bfoverlayTextures = {}
			local mapID = BattlefieldMapFrame.mapID
			if not mapID then
				return
			end
			local artID = C_Map.GetMapArtID(mapID)
		...
		LeaMapsLC:MakeCB(
			tintFrame,
			"RevTint",
			"Tint unexplored areas",
			16,
			-92,
			false,
			"If checked, unexplored areas will be tinted."
		)
		...
		LeaMapsLC:MakeSL(
			...
			"tintGreen",
			"Green",
			"Drag to set the amount of green.",
			0,
			1,
			0.1,
			36,
		...
		local prvTitle = LeaMapsLC:MakeWD(tintFrame, "Preview", 386, -130)
		prvTitle:Hide()
		...
		local function SetTintCol()
			tintFrame.preview:SetColorTexture(
				LeaMapsLC["tintRed"],
				LeaMapsLC["tintGreen"],
				LeaMapsLC["tintBlue"],
				LeaMapsLC["tintAlpha"]
			...
			if LeaMapsLC["RevTint"] == "On" then
				...
				for i = 1, #overlayTextures do
					overlayTextures[i]:SetVertexColor(
						LeaMapsLC["tintRed"],
						LeaMapsLC["tintGreen"],
						LeaMapsLC["tintBlue"],
						LeaMapsLC["tintAlpha"]
				...
				for i = 1, #bfoverlayTextures do
					bfoverlayTextures[i]:SetVertexColor(
						LeaMapsLC["tintRed"],
						LeaMapsLC["tintGreen"],
						LeaMapsLC["tintBlue"],
						LeaMapsLC["tintAlpha"]
				...
				-- Enable controls
				LeaMapsCB["tintRed"]:Enable()
				LeaMapsCB["tintRed"]:SetAlpha(1.0)
				LeaMapsCB["tintGreen"]:Enable()
				LeaMapsCB["tintGreen"]:SetAlpha(1.0)
				LeaMapsCB["tintBlue"]:Enable()
				LeaMapsCB["tintBlue"]:SetAlpha(1.0)
				LeaMapsCB["tintAlpha"]:Enable()
				LeaMapsCB["tintAlpha"]:SetAlpha(1.0)
				prvTitle:SetAlpha(1.0)
				tintFrame.preview:SetAlpha(1.0)
			else
				-- Disable tint
				for i = 1, #overlayTextures do
				...
				-- Disable controls
				LeaMapsCB["tintRed"]:Disable()
				LeaMapsCB["tintRed"]:SetAlpha(0.3)
				LeaMapsCB["tintGreen"]:Disable()
				LeaMapsCB["tintGreen"]:SetAlpha(0.3)
				LeaMapsCB["tintBlue"]:Disable()
				LeaMapsCB["tintBlue"]:SetAlpha(0.3)
				LeaMapsCB["tintAlpha"]:Disable()
				LeaMapsCB["tintAlpha"]:SetAlpha(0.3)
				prvTitle:SetAlpha(0.3)
				tintFrame.preview:SetAlpha(0.3)
			end
		end
	...
	if LeaMapsLC["UseDefaultMap"] == "On" then
		...
		LeaMapsCB["SetMapOpacity"].tiptext = LeaMapsCB["SetMapOpacity"].tiptext
			.. "|n|n|cff00AAFF"
			.. L["Cannot be used with Use default map."]

		LeaMapsLC:LockItem(LeaMapsCB["UnlockMapFrame"], true)
		LeaMapsCB["UnlockMapFrame"].tiptext = LeaMapsCB["UnlockMapFrame"].tiptext
			.. "|n|n|cff00AAFF"
			.. L["Cannot be used with Use default map."]
	...
	do
		LeaMapsLC:CreateDropdown(
			...
			16,
			-352,
			{ { L["Never"], 1 }, { L["Battlegrounds"], 2 }, { L["Always"], 3 } },
			L["Choose where the zone map should be shown."]
		)
	...
	if not LeaMapsDB["FirstRunMessageSeen"] then
		LeaMapsLC:Print(
			L["Enter"] .. " |cff00ff00" .. "/ltm" .. "|r " .. L["or click the minimap button to open Leatrix Maps."]
		)
...
function LeaMapsLC:CreatePanel(title, globref)
	...
	local reloadb = LeaMapsLC:CreateButton(
		"ConfigReload",
		Side,
		"Reload",
		"BOTTOMRIGHT",
		...
		10,
		25,
		LeaMapsCB["ReloadUIButton"].tiptext
	)
	...
	Side:SetScript("OnDragStop", function()
		...
		-- Save panel position
		LeaMapsLC["MainPanelA"], void, LeaMapsLC["MainPanelR"], LeaMapsLC["MainPanelX"], LeaMapsLC["MainPanelY"] =
			Side:GetPoint()
	end)
	...
	Side:SetScript("OnShow", function()
		Side:ClearAllPoints()
		Side:SetPoint(
			LeaMapsLC["MainPanelA"],
			UIParent,
			LeaMapsLC["MainPanelR"],
			LeaMapsLC["MainPanelX"],
			LeaMapsLC["MainPanelY"]
		)
	end)
...
function LeaMapsLC:LoadVarNum(var, def, valmin, valmax)
	if
		LeaMapsDB[var]
		and type(LeaMapsDB[var]) == "number"
		and LeaMapsDB[var] >= valmin
		and LeaMapsDB[var] <= valmax
	then
		LeaMapsLC[var] = LeaMapsDB[var]
	else
		LeaMapsLC[var] = def
...
function LeaMapsLC:LoadVarAnc(var, def)
	if
		LeaMapsDB[var] and type(LeaMapsDB[var]) == "string" and LeaMapsDB[var] == "CENTER"
		or LeaMapsDB[var] == "TOP"
		or LeaMapsDB[var] == "BOTTOM"
		or LeaMapsDB[var] == "LEFT"
		or LeaMapsDB[var] == "RIGHT"
		or LeaMapsDB[var] == "TOPLEFT"
		or LeaMapsDB[var] == "TOPRIGHT"
		or LeaMapsDB[var] == "BOTTOMLEFT"
		or LeaMapsDB[var] == "BOTTOMRIGHT"
	then
		LeaMapsLC[var] = LeaMapsDB[var]
	else
		LeaMapsLC[var] = def
...
function LeaMapsLC:ReloadCheck()
	if
		(LeaMapsLC["ShowZoneMenu"] ~= LeaMapsDB["ShowZoneMenu"]) -- Show zone menu
...
LeaMapsLC:MakeWD(
	stopFrame,
	"A stop error has occurred but no need to worry. It can happen from time to time. Click the reload button to resolve it.",
	16,
	-32,
	338
)
...
local function SlashFunc(str)
	...
	if str and str ~= "" then
		...
		if str == "reset" then
			...
			LeaMapsLC["MainPanelA"], LeaMapsLC["MainPanelR"], LeaMapsLC["MainPanelX"], LeaMapsLC["MainPanelY"] =
				"CENTER", "CENTER", 0, 0
			if LeaMapsLC["PageF"]:IsShown() then
				LeaMapsLC["PageF"]:Hide()
				LeaMapsLC["PageF"]:Show()
...
PageF.v:SetText(L["BCA"] .. " " .. LeaMapsLC["AddonVer"])
...
LeaMapsLC:MakeCB(
	...
	-92,
	true,
	"If checked, zone and continent dropdown menus will be shown in the map frame."
)
LeaMapsLC:MakeCB(
	PageF,
	"SetMapOpacity",
	...
	-112,
	true,
	"If checked, you will be able to set the opacity of the map."
)
...
LeaMapsLC:MakeCB(
	...
	-172,
	true,
	"If checked, you will be able to enlarge the player arrow."
)
LeaMapsLC:MakeCB(
	PageF,
	"UseClassIcons",
	...
	-192,
	true,
	"If checked, group icons will use a modern, class-colored design."
)
...
LeaMapsLC:MakeCB(
	...
	-252,
	false,
	"If checked, opening the map will use the same zoom level from when you last closed it as long as the map zone has not changed."
)
LeaMapsLC:MakeCB(
	PageF,
	"IncreaseZoom",
	...
	-272,
	false,
	"If checked, you will be able to zoom further into the world map."
)
LeaMapsLC:MakeCB(
	PageF,
	"CenterMapOnPlayer",
	...
	-292,
	false,
	"If checked, the map will stay centered on your location as long as you are not in a dungeon.|n|nYou can hold shift while panning the map to temporarily prevent it from centering."
)
...
LeaMapsLC:MakeCB(
	...
	-92,
	false,
	"If checked, you will be able to scale and move the map.|n|nScale the map by dragging the scale handle in the bottom-right corner.|n|nMove the map by dragging the border and frame edges."
)
LeaMapsLC:MakeCB(
	PageF,
	"AutoChangeZones",
	...
	-112,
	true,
	"If checked, when your character changes zones, the map will automatically change to the new zone."
)
LeaMapsLC:MakeCB(
	PageF,
	"UseDefaultMap",
	...
	-132,
	true,
	"If checked, the default fullscreen map will be used.|n|nNote that enabling this option will lock out some of the other options."
)
...
LeaMapsLC:MakeCB(
	...
	-192,
	true,
	"If checked, unexplored areas of the map will be shown on the world map and the battlefield map."
)
LeaMapsLC:MakeCB(
	PageF,
	...
	-212,
	true,
	"If checked, points of interest will be shown."
)
LeaMapsLC:MakeCB(
	PageF,
	"ShowZoneLevels",
	...
	-232,
	false,
	"If checked, zone, dungeon and fishing skill levels will be shown."
)
...
LeaMapsLC:MakeCB(
	...
	-272,
	true,
	"If checked, town and city icons will not be shown on the continent maps."
)
...
LeaMapsLC:MakeCB(
	...
	-332,
	true,
	"If checked, you will be able to customise the battlefield map."
)
LeaMapsLC:MakeCB(
	PageF,
	"ShowMinimapIcon",
	...
	-352,
	false,
	"If checked, the minimap button will be shown."
)
LeaMapsLC:MakeCB(
	PageF,
	"UseEnglishLanguage",
...
local resetMapPosBtn = LeaMapsLC:CreateButton(
	"resetMapPosBtn",
	PageF,
	"Reset Map Layout",
	"BOTTOMLEFT",
	...
	10,
	25,
	"Click to reset the position and scale of the map frame."
)
resetMapPosBtn:HookScript("OnClick", function()
	if LeaMapsDB["UseDefaultMap"] == "On" then -- Check global in case use default map option reload is pending
		...
		if not WorldMapFrame:IsMaximized() then
			-- Reset map position
			LeaMapsLC["MapPosA"], LeaMapsLC["MapPosR"], LeaMapsLC["MapPosX"], LeaMapsLC["MapPosY"] =
				"TOPLEFT", "TOPLEFT", 0, 0
			WorldMapScreenAnchor:ClearAllPoints()
			WorldMapScreenAnchor:SetPoint(
				LeaMapsLC["MapPosA"],
