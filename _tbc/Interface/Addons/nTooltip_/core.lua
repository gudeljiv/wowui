...
local function GetFormattedUnitString(unit, specIcon)
	if UnitIsPlayer(unit) then
		...
		return GetFormattedUnitLevel(unit)
			...
			.. (cfg.showSpecializationIcon and specIcon or "")
	else
...
local function GetUnitPVPIcon(unit)
	...
	elseif factionGroup and UnitIsPVP(unit) then
		if cfg.showPVPIcons then
			return CreateTextureMarkup(
				[[Interface\AddOns\nTooltip\media\UI-PVP-]] .. factionGroup,
				32,
				32,
				14,
				14,
	...
	if UnitExists(unit .. "target") then
		if UnitName("player") == unitTargetName then
			...
			if UnitIsPlayer(unit .. "target") then
				self:AddLine(
					format(
						"   " .. GetUnitRaidIcon(unit .. "target") .. "|cff%02x%02x%02x%s|r",
						unitTargetClassColor.r * 255,
						unitTargetClassColor.g * 255,
						unitTargetClassColor.b * 255,
						unitTargetName:sub(1, 40)
					),
					1,
					1,
					1
				...
				self:AddLine(
					format(
						"   " .. GetUnitRaidIcon(unit .. "target") .. "|cff%02x%02x%02x%s|r",
						unitTargetReactionColor.r * 255,
						unitTargetReactionColor.g * 255,
						unitTargetReactionColor.b * 255,
						unitTargetName:sub(1, 40)
					),
					1,
					1,
