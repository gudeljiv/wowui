...
function SUI:OnInitialize()
    ...
    function self:Skin(frame, customColor, isTable)
        local function SafeRegion(regionFrame, index)
            if regionFrame and regionFrame.GetRegions then
                return select(index, regionFrame:GetRegions())
            end
        end
        ...
        local function AddForbidden(key)
            if key then
                SUI_forbiddenFrames[key] = true
            end
        end

        AddForbidden(SafeRegion(GossipFrame, 3))
        AddForbidden(SafeRegion(DressUpFrame, 3))
        AddForbidden(SafeRegion(ChatFrame1EditBox, 2))
        AddForbidden(SafeRegion(ChatFrame2EditBox, 2))
        AddForbidden(SafeRegion(ChatFrame3EditBox, 2))
        AddForbidden(SafeRegion(ChatFrame4EditBox, 2))
        AddForbidden(SafeRegion(ChatFrame5EditBox, 2))
        AddForbidden(SafeRegion(ChatFrame6EditBox, 2))
        AddForbidden(SafeRegion(ChatFrame7EditBox, 2))
        AddForbidden(SafeRegion(TradeFrame and TradeFrame.RecipientOverlay, 1))
        AddForbidden(SafeRegion(LFGListInviteDialog, 4))
