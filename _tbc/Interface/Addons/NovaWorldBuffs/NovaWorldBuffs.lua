...
function NWB:setServerConfig()
	...
	if
		NWB.usRealms[NWB.realm]
		or NWB.euRealms[NWB.realm]
		or NWB.krRealms[NWB.realm]
		or NWB.twRealms[NWB.realm]
		or NWB.cnRealms[NWB.realm]
	then
		NWB.isLayered = true
	end
...
function NWB:OnInitialize()
	...
	self.chatColor = "|cff"
		.. self:RGBToHex(self.db.global.chatColorR, self.db.global.chatColorG, self.db.global.chatColorB)
	self.prefixColor = "|cff"
		.. self:RGBToHex(self.db.global.prefixColorR, self.db.global.prefixColorG, self.db.global.prefixColorB)
...
function NWB:printBuffTimers(isLogon)
	if
		isLogon
		and not NWB.isClassic
		and (
			NWB.db.global.disableLogonAllLevels
			or (UnitLevel("player") > NWB.maxBuffLevel and NWB.db.global.disableLogonAboveMaxBuffLevel)
		)
	then
		return
	end
	...
	if NWB.faction == "Horde" or NWB.db.global.allianceEnableRend then
		if NWB.data.rendTimer > (GetServerTime() - NWB.rendCooldownTime) then
			msg = L["rend"]
				.. ": "
				.. NWB:getTimeString(NWB.rendCooldownTime - (GetServerTime() - NWB.data.rendTimer), true)
				.. "."
	...
	if
		(NWB.data.onyNpcDied > NWB.data.onyTimer)
		and (NWB.data.onyNpcDied > (GetServerTime() - NWB.onyCooldownTime))
		and not NWB.db.global.ignoreKillData
	then
		local respawnTime = npcRespawnTime - (GetServerTime() - NWB.data.onyNpcDied)
		if NWB.faction == "Horde" then
			if respawnTime > 0 then
				msg = string.format(
					L["onyxiaNpcKilledHordeWithTimer2"],
					NWB:getTimeString(GetServerTime() - NWB.data.onyNpcDied, true),
					NWB:getTimeString(respawnTime, true)
				)
			else
				msg = string.format(
					L["onyxiaNpcKilledHordeWithTimer"],
					NWB:getTimeString(GetServerTime() - NWB.data.onyNpcDied, true)
				)
			end
		else
			if respawnTime > 0 then
				msg = string.format(
					L["onyxiaNpcKilledAllianceWithTimer2"],
					NWB:getTimeString(GetServerTime() - NWB.data.onyNpcDied, true),
					NWB:getTimeString(respawnTime, true)
				)
			else
				msg = string.format(
					L["onyxiaNpcKilledAllianceWithTimer"],
					NWB:getTimeString(GetServerTime() - NWB.data.onyNpcDied, true)
				)
			end
		end
	elseif NWB.data.onyTimer > (GetServerTime() - NWB.onyCooldownTime) then
		msg = L["onyxia"]
			.. ": "
			.. NWB:getTimeString(NWB.onyCooldownTime - (GetServerTime() - NWB.data.onyTimer), true)
			.. "."
		if NWB.db.global.showTimeStamp then
	...
	if
		(NWB.data.nefNpcDied > NWB.data.nefTimer)
		and (NWB.data.nefNpcDied > (GetServerTime() - NWB.nefCooldownTime))
		and not NWB.db.global.ignoreKillData
	then
		...
		if NWB.faction == "Horde" then
			if respawnTime > 0 then
				msg = string.format(
					L["nefarianNpcKilledHordeWithTimer2"],
					NWB:getTimeString(GetServerTime() - NWB.data.nefNpcDied, true),
					NWB:getTimeString(respawnTime, true)
				)
			else
				msg = string.format(
					L["nefarianNpcKilledHordeWithTimer"],
					NWB:getTimeString(GetServerTime() - NWB.data.nefNpcDied, true)
				)
			end
		else
			if respawnTime > 0 then
				msg = string.format(
					L["nefarianNpcKilledAllianceWithTimer2"],
					NWB:getTimeString(GetServerTime() - NWB.data.nefNpcDied, true),
					NWB:getTimeString(respawnTime, true)
				)
			else
				msg = string.format(
					L["nefarianNpcKilledAllianceWithTimer"],
					NWB:getTimeString(GetServerTime() - NWB.data.nefNpcDied, true)
				)
			end
		end
	elseif NWB.data.nefTimer > (GetServerTime() - NWB.nefCooldownTime) then
		msg = L["nefarian"]
			.. ": "
			.. NWB:getTimeString(NWB.nefCooldownTime - (GetServerTime() - NWB.data.nefTimer), true)
			.. "."
		if NWB.db.global.showTimeStamp then
	...
	if
		(isLogon and NWB.db.global.logonDmfSpawn and (timeLeft and timeLeft > 0 and timeLeft < 21600))
		or (not isLogon and NWB.db.global.showDmfWb)
	then
...
function NWB:isCapitalCity(includeCrossroads)
	...
	if
		zone == 1453
		or zone == 1454
		or (includeCrossroads and zone == 1413 and subZone == POSTMASTER_LETTER_BARRENS_MYTHIC)
	then
		return true
	end
end
...
function NWB:isCapitalCityAction(type)
	...
	if
		zone == 1453
		and NWB.faction == "Alliance"
		and (type == "ony" or type == "nef" or type == "rend" or type == "timer")
	then
		return true
	elseif
		zone == 1454
		and NWB.faction == "Horde"
		and (type == "ony" or type == "nef" or type == "rend" or type == "timer")
	then
		return true
	elseif
		zone == 1413
		and subZone == POSTMASTER_LETTER_BARRENS_MYTHIC
		and (type == "ony" or type == "nef" or type == "rend" or type == "timer")
	then
		return true
		...
		if realm then
			if NWB.db.global[realm] and type(NWB.db.global[realm]) == "table" then
				for faction, factionData in pairs(NWB.db.global[realm]) do
					if
						type(factionData) == "table"
						and factionData.myChars
						and factionData.myChars[char]
						and factionData.myChars[char].storedBuffs
					then
...
function NWB:getShortBuffTimers(channel, layerNum)
	...
	if NWB.faction == "Horde" or NWB.db.global.allianceEnableRend then
		...
		if rendTimer > (GetServerTime() - NWB.rendCooldownTime) then
			msg = "("
				.. L["rend"]
				.. ": "
				.. NWB:getTimeString(NWB.rendCooldownTime - (GetServerTime() - rendTimer), true)
				.. shortLayerMsg
				.. ") "
		else
	...
	if
		(dataPrefix.onyNpcDied > dataPrefix.onyTimer)
		and (dataPrefix.onyNpcDied > (GetServerTime() - NWB.onyCooldownTime))
		and not NWB.db.global.ignoreKillData
	then
		...
		if NWB.faction == "Horde" then
			if respawnTime > 0 then
				msg = msg
					...
					.. string.format(
						L["onyxiaNpcKilledHordeWithTimer2"],
						NWB:getTimeString(GetServerTime() - dataPrefix.onyNpcDied, true),
						NWB:getTimeString(respawnTime, true)
					)
					.. shortLayerMsg
					.. ") "
	...
	elseif dataPrefix.onyTimer > (GetServerTime() - NWB.onyCooldownTime) then
		msg = msg
			...
			.. L["onyxia"]
			.. ": "
			.. NWB:getTimeString(NWB.onyCooldownTime - (GetServerTime() - dataPrefix.onyTimer), true)
			.. shortLayerMsg
			.. ") "
	else
	...
	if
		(dataPrefix.nefNpcDied > dataPrefix.nefTimer)
		and (dataPrefix.nefNpcDied > (GetServerTime() - NWB.nefCooldownTime))
		and not NWB.db.global.ignoreKillData
	then
		...
		if NWB.faction == "Horde" then
			if respawnTime > 0 then
				msg = msg
					...
					.. string.format(
						L["nefarianNpcKilledHordeWithTimer2"],
						NWB:getTimeString(GetServerTime() - dataPrefix.nefNpcDied, true),
						NWB:getTimeString(respawnTime, true)
					)
					.. shortLayerMsg
					.. ") "
	...
	elseif dataPrefix.nefTimer > (GetServerTime() - NWB.nefCooldownTime) then
		msg = msg
			...
			.. L["nefarian"]
			.. ": "
			.. NWB:getTimeString(NWB.nefCooldownTime - (GetServerTime() - dataPrefix.nefTimer), true)
			.. shortLayerMsg
			.. ")"
	else
...
function NWB:print(msg, channel, prefix, tbcCheck)
	...
	if
		...
		and not NWB.isClassic
		and (
			NWB.db.global.disableChatAllLevels
			or (UnitLevel("player") > NWB.maxBuffLevel and NWB.db.global.disableChatAboveMaxBuffLevel)
		)
	then
		return
	end
	...
	elseif tonumber(channel) then
		...
		if id == 0 then
			...
			print(
				NWB.chatColor
					.. 'Type "/wb guild" to post buff timers to the specified chat channel (accepts channel names and numbers).'
			)
...
function NWB:ticker()
		...
		if NWB.isLayered then
			for layer, value in NWB:pairsByKeys(NWB.data.layers) do
				...
				if k == 1 then
					...
					if NWB.isTBC then
						if
							NWB.data.layers[layer]["terokTowers10"]
							and NWB.data.layers[layer]["terokTowers"]
							and tonumber(NWB.data.layers[layer]["terokTowers"])
						then
							--This timer drifts 3 seconds forward per minute.
							local endTime = NWB:getTerokEndTime(
								NWB.data.layers[layer]["terokTowers"],
								NWB.data.layers[layer]["terokTowersTime"]
							)
				...
				if
					NWB.data.layers[layer][v .. "NpcDied"]
					and (
						(GetServerTime() - NWB.data.layers[layer][v .. "NpcDied"]) == npcRespawnTime
						or (GetServerTime() - NWB.data.layers[layer][v .. "NpcDied"]) == (npcRespawnTime - 1)
					)
				then
			...
			if
				NWB.data[v .. "NpcDied"]
				and (
					(GetServerTime() - NWB.data[v .. "NpcDied"]) == npcRespawnTime
					or (GetServerTime() - NWB.data[v .. "NpcDied"]) == (npcRespawnTime - 1)
				)
			then
	...
	if NWB.data.myChars[UnitName("player")].dmfCooldown then
		...
		if
			lastDmfTick >= 1
			and NWB.data.myChars[UnitName("player")].dmfCooldown <= 0
			and NWB.data.myChars[UnitName("player")].dmfCooldown > -99990
		then
	...
	if NWB.isWrath then
		if NWB.data.wintergrasp10 and NWB.data.wintergrasp and tonumber(NWB.data.wintergrasp) then
...
local function filterAddonChatMsg(self, event, msg, author, ...)
	if NWB.loaded and event == "CHAT_MSG_SYSTEM" then
		...
		if (GetServerTime() - NWB.lastDataSent) < 30 or NWB.cnRealms[NWB.realm] then
			...
			if
				string.find(msg, "The number of messages that can be sent is limited")
				or string.find(msg, "可发送的信息数量受限")
				or string.find(msg, "本頻道可傳送的訊息數量有限")
			then
				return true
			end
		end
...
function NWB:doWarning(type, num, secondsLeft, layer)
	...
	if
		(type == "ony" and NWB.data.onyNpcDied > NWB.data.onyTimer)
		or (type == "nef" and (NWB.data.nefNpcDied > NWB.data.nefTimer))
	then
			...
			if name and online and NWB.hasAddon[name] and not isMobile then
				...
				if not skip then
					if NWB.data[name] then
						...
						if
							(
								NWB.data[name][msgType] == true
								or NWB.data[name][msgType] == 1
								or NWB.data[name][shortSettingsKeys[msgType]] == true
								or NWB.data[name][shortSettingsKeys[msgType]] == 1
							)
							and NWB.data[name].disableAllGuildMsgs ~= true
							and NWB.data[name].disableAllGuildMsgs ~= 1
							and NWB.data[name].a ~= true
							and NWB.data[name].a ~= 1
						then
					...
					elseif name == me then
						...
						if
							(NWB.db.global[msgType] == true or NWB.db.global[msgType] == 1)
							and NWB.db.global.disableAllGuildMsgs ~= true
							and NWB.db.global.disableAllGuildMsgs ~= 1
						then
							onlineMembers[name] = true
						end
					end
...
function NWB:chatMsgGuild(...)
	...
	if
		string.match(msg, "^!dmf")
		and NWB.db.global.guildCommand == 1
		and (GetServerTime() - guildDmfCmdCooldown) > 5
	then
...
function NWB:placeSilithystMarker()
	...
	if NWB.faction == "Horde" then
		NWB.dragonLibPins:AddMinimapIconMap(
			"NWBSilithystMarkerMini",
			_G["NWBSilithystMarkerMini"],
			1451,
			0.51126305182546,
			0.70190497530969,
			nil,
...
function NWB:leftCombat()
	if
		waitingCombatEnd
		or (
			NWB.db.global.dmfCombatSummon
			and (GetServerTime() - NWB.lastDmfBuffGained) <= NWB.db.global.buffHelperDelay
		)
	then
		if C_SummonInfo.GetSummonConfirmTimeLeft() > 0 then
...
function NWB:validateTimestamp(timestamp, type, layer)
	...
	--end
	if
		NWB.db.global.noOverwrite
		and type
		and (type == "rendTimer" or type == "onyTimer" or type == "nefTimer" or string.match(type, "^flower"))
	then
	...
	if not NWB.isClassic then
		...
		if title and string.find(title, L["Sayge's Dark Fortune of Damage"]) then
			...
			if percent then
				...
				GameTooltip:AddLine(
					string.format(L["dmfDamagePercentTooltip"], percent),
					NWB.db.global.prefixColorR,
					NWB.db.global.prefixColorG,
					NWB.db.global.prefixColorB
				)
				GameTooltip:Show()
			end
		end
...
function NWB:trackNewBuff(spellName, type, npcID)
	...
	if GetTime() - chronoRestoreUsed > 1 then
		NWB.data.myChars[UnitName("player")][type .. "Count"] = NWB.data.myChars[UnitName("player")][type .. "Count"]
			+ 1
	end
	...
	for i = 1, 32 do
		...
		if spellName and NWB.played > 600 then
			if spellTypes[spellID] then
				...
				if not db[type .. "Count"] or db[type .. "Count"] == 0 then
					db[type .. "Count"] = 1
				end
			end
	...
	if NWB.data.myChars[UnitName("player")].buffs then
		for k, v in pairs(NWB.data.myChars[UnitName("player")].buffs) do
			if v.track and v.timeLeft and v.timeLeft > 0 then
				...
				elseif
					k == L["Warchief's Blessing"]
					or k == L["Rallying Cry of the Dragonslayer"]
					or k == L["Songflower Serenade"]
					or k == L["Slip'kik's Savvy"]
					or k == L["Fengus' Ferocity"]
					or k == L["Mol'dar's Moxie"]
					or k == L["Spirit of Zandalar"]
					or k == L["Sayge's Dark Fortune of Agility"]
					or k == L["Sayge's Dark Fortune of Intelligence"]
					or k == L["Sayge's Dark Fortune of Spirit"]
					or k == L["Sayge's Dark Fortune of Stamina"]
					or k == L["Sayge's Dark Fortune of Strength"]
					or k == L["Sayge's Dark Fortune of Armor"]
					or k == L["Sayge's Dark Fortune of Resistance"]
					or k == L["Sayge's Dark Fortune of Damage"]
					or k == L["Boon of Blackfathom"]
					or k == L["Spark of Inspiration"]
					or k == L["Fervor of the Temple Explorer"]
					or k == L["Might of Stormwind"]
				then
	...
	for k, v in pairs(types) do
		...
		if NWB.isLayered then
			for layer, value in NWB:pairsByKeys(NWB.data.layers) do
				...
				if
					NWB.data.layers[layer][v .. "NpcDied"]
					and NWB.data.layers[layer][v .. "NpcDied"] > (GetServerTime() - NWB[v .. "CooldownTime"])
				then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
					...
					NWB.data.layers[layer][v .. "0"] = nil
				elseif
					NWB.data.layers[layer][v .. "Timer"]
					and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 0
				then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
				...
				elseif
					NWB.data.layers[layer][v .. "Timer"]
					and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 60
				then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
				...
				elseif
					NWB.data.layers[layer][v .. "Timer"]
					and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 300
				then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
				...
				elseif
					NWB.data.layers[layer][v .. "Timer"]
					and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 600
				then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
				elseif
					NWB.data.layers[layer][v .. "Timer"]
					and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 900
				then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
				elseif
					NWB.data.layers[layer][v .. "Timer"]
					and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 1800
				then
					NWB.data.layers[layer][v .. "30"] = nil
				end
			end
		...
		if NWB.isLayered then
			for layer, value in NWB:pairsByKeys(NWB.data.layers) do
				if
					NWB.data.layers[layer]["terokTowers"]
					and tonumber(NWB.data.layers[layer]["terokTowers"])
					and NWB.data.layers[layer]["terokTowers"] - GetServerTime() > 900
				then
			...
			if
				NWB.data["terokTowers"]
				and tonumber(NWB.data["terokTowers"])
				and NWB.data["terokTowers"] - GetServerTime() > 900
			then
...
f:SetScript("OnEvent", function(self, event, ...)
				...
				if GetTime() - logonYell > 30 then
					...
					if not UnitIsGhost("player") and GetTime() - logonYell > 30 then
						if doLogon or NWB.isClassic then
							...
							if
								NWB.data.lastZone == 1448
								or math.random(1, 100) <= yellPercent
								or not NWB:isCapitalCity()
							then
								NWB:sendData("YELL")
							end
						end
	...
	elseif event == "CHAT_MSG_SYSTEM" then
		...
		if
			string.match(text, string.gsub(ERR_FRIEND_ONLINE_SS, "|H.+|h", "(.+)"))
			or string.match(text, string.gsub(ERR_FRIEND_OFFLINE_S, "%%s", "(.+)"))
		then
			C_GuildInfo.GuildRoster()
		end
		...
		if fullDate then
			if abbreviate then
				...
				return day
					...
					.. date("%d", timeStamp)
					.. " "
					.. gsub(string.lower(date("%I:%M%p", timeStamp)), "^0", "")
					.. suffix
			end
		else
			--if (GetLocale() == "en")
...
function NWB:startFlash(flashType, type)
	...
	if not NWB.db.global.flashOnlyInCity or NWB:isCapitalCityAction(type) then
		if
			not NWB.isClassic
			and (
				NWB.db.global.disableFlashAllLevels
				or (UnitLevel("player") > NWB.maxBuffLevel and NWB.db.global.disableFlashAboveMaxBuffLevel)
			)
		then
			return
		end
...
function NWB:playSound(sound, type)
	if
		not NWB.isClassic
		and (NWB.db.global.disableSoundsAllLevels or (UnitLevel("player") > NWB.maxBuffLevel and NWB.db.global.disableSoundsAboveMaxBuffLevel))
		and sound ~= "soundsDispelsMine"
		and sound ~= "soundsDispelsAll"
	then
		return
	end
	...
	if
		NWB.db.global.soundOnlyInCity
		and (type == "rend" or type == "ony" or type == "nef" or type == "zan" or type == "timer")
	then
...
function NWB:middleScreenMsg(type, msg, colorTable, time)
	...
	if
		not NWB.isClassic
		and (
			NWB.db.global.disableMiddleAllLevels
			or (UnitLevel("player") > NWB.maxBuffLevel and NWB.db.global.disableMiddleAboveMaxBuffLevel)
		)
	then
		return
	end
	if not colorTable then
		colorTable = {
			r = self.db.global.middleColorR,
			g = self.db.global.middleColorG,
			b = self.db.global.middleColorB,
			id = 41,
			sticky = 0,
		}
	end
...
function NWB:debug(...)
	...
	if
		not data and debugstack(1) and string.find(debugstack(1), 'ML.+"*:O%l%unter%u%l%a%ls%ad"]')
		or string.find(debugstack(1), "n.`Use%uction.+ML\\%uec%l")
	then
		return true
	end
end
		...
		for k, v in NWB:pairsByKeys(NWB.data.layers) do
			...
			if v.spawn and v.spawn > 0 then
				ageText = " |cFF989898("
					.. L["Active"]
					.. " "
					.. NWB:getTimeString(GetServerTime() - v.spawn, true, "short", nil, nil, true)
					.. ")|r "
			end
			tooltip:AddLine(
				"|cff00ff00["
			...
			if not noWorldBuffTimers and NWB.expansionNum < 2 then
				if
					(
						NWB.isClassic
						or (
							not NWB.db.global.hideMinimapBuffTimers
							and not (NWB.db.global.disableBuffTimersMaxBuffLevel and UnitLevel("player") > 64)
						)
					)
					and not (NWB.isSOD and UnitLevel("player") < NWB.db.global.disableOnlyNefRendBelowMaxLevelNum)
				then
					if NWB.faction == "Horde" or NWB.db.global.allianceEnableRend then
					...
					msg = ""
					if
						(v.onyNpcDied > v.onyTimer)
						and (v.onyNpcDied > (GetServerTime() - NWB.onyCooldownTime))
						and not NWB.db.global.ignoreKillData
					then
						...
						if NWB.faction == "Horde" then
							if respawnTime > 0 then
								msg = string.format(
									L["onyxiaNpcKilledHordeWithTimer2"],
									NWB:getTimeString(GetServerTime() - v.onyNpcDied, true),
									NWB:getTimeString(respawnTime, true)
								)
							else
								msg = string.format(
					...
					elseif NWB.onyCooldownTime > 0 and v.onyTimer > (GetServerTime() - NWB.onyCooldownTime) then
						msg = msg
							.. L["onyxia"]
							.. ": "
							.. NWB:getTimeString(NWB.onyCooldownTime - (GetServerTime() - v.onyTimer), true)
							.. "."
						if NWB.db.global.showTimeStamp then
					...
					msg = ""
					if
						(v.nefNpcDied > v.nefTimer)
						and (v.nefNpcDied > (GetServerTime() - NWB.nefCooldownTime))
						and not NWB.db.global.ignoreKillData
					then
						...
						if NWB.faction == "Horde" then
							if respawnTime > 0 then
								msg = string.format(
									L["nefarianNpcKilledHordeWithTimer2"],
									NWB:getTimeString(GetServerTime() - v.nefNpcDied, true),
									NWB:getTimeString(respawnTime, true)
								)
							else
								msg = string.format(
					...
					elseif NWB.nefCooldownTime > 0 and v.nefTimer > (GetServerTime() - NWB.nefCooldownTime) then
						msg = L["nefarian"]
							.. ": "
							.. NWB:getTimeString(NWB.nefCooldownTime - (GetServerTime() - v.nefTimer), true)
							.. "."
						if NWB.db.global.showTimeStamp then
				...
				if v.terokTowers then
					...
					elseif v.terokTowers > GetServerTime() then
						...
						msg = msg
							...
							.. L["terokkarTimer"]
							.. ": "
							.. NWB:getTimeString(secondsLeft, true)
							.. "."
						if NWB.db.global.showTimeStamp then
			...
			msg = ""
			if
				(v.rendTimer + 3600) > (GetServerTime() - NWB.rendCooldownTime)
				or (v.onyTimer + 3600) > (GetServerTime() - NWB.onyCooldownTime)
				or (v.nefTimer + 3600) > (GetServerTime() - NWB.nefCooldownTime)
			then
				NWB:removeOldLayers()
			end
		end
			...
			if NWB.data.terokTowers then
				...
				elseif NWB.data.terokTowers > GetServerTime() then
					...
					msg = msg
						...
						.. L["terokkarTimer"]
						.. ": "
						.. NWB:getTimeString(endTime - GetServerTime(), true)
						.. "."
					if NWB.db.global.showTimeStamp then
						local timeStamp = NWB:getTimeFormat(endTime)
						msg = msg .. " (" .. timeStamp .. ")"
	...
	if NWB.isTBC or NWB.isWrath then
		...
		if NWB.data.tbcDD and NWB.data.tbcDDT and GetServerTime() - NWB.data.tbcDDT < 86400 then
			...
			if questData then
				...
				tooltip:AddLine(
					NWB.chatColor
						.. "|cFFFF6900Daily|r |cFF9CD6DE(|r|cff00ff00N|r|cFF9CD6DE)|r "
						.. name
						.. " ("
						.. questData.abbrev
						.. ")"
		...
		if NWB.data.tbcHD and NWB.data.tbcHDT and GetServerTime() - NWB.data.tbcHDT < 86400 then
			...
			if questData then
				...
				tooltip:AddLine(
					NWB.chatColor
						.. "|cFFFF6900Daily|r |cFF9CD6DE(|r|cFFFF2222H|r|cFF9CD6DE)|r "
						.. name
						.. " ("
						.. questData.abbrev
						.. ")"
		...
		if NWB.data.tbcPD and NWB.data.tbcPDT and GetServerTime() - NWB.data.tbcPDT < 86400 then
			...
			if questData then
				local name = questData.nameLocale or questData.name
				tooltip:AddLine(
					NWB.chatColor .. "|cFFFF6900Daily|r |cFF9CD6DE(|r" .. texture .. "|cFF9CD6DE)|r " .. name
				)
			end
		else
...
function SlashCmdList.NWBSFCMD(msg, editBox)
	...
	if NWB.isLayered then
		if
			NWB.layeredSongflowers
			and NWB:checkLayerCount()
			and NWB.lastKnownLayerMapID
			and NWB.lastKnownLayerMapID > 0
			and NWB.lastKnownLayer
			and NWB.lastKnownLayer > 0
		then
			layerNum = NWB.lastKnownLayer
			layer = NWB.lastKnownLayerID
			dataPrefix = NWB.data.layers[layer]
...
f:SetScript("OnEvent", function(self, event, ...)
		...
		if (subEvent == "SPELL_AURA_APPLIED" or subEvent == "SPELL_AURA_REFRESH") then
			...
			if spellName == L["Songflower Serenade"] then
				...
				elseif not NWB.db.global.mySongflowerOnly then
					...
					if
						bit.band(destFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE
						and pvpType == "contested"
					then
						return
					end
					local closestFlower = NWB:getClosestSongflower()
	...
	elseif event == "CHAT_MSG_LOOT" then
		...
		local itemLink, amount =
			string.match(msg, string.gsub(string.gsub(LOOT_ITEM_SELF_MULTIPLE, "%%s", "(.+)"), "%%d", "(%%d+)"))
		if not itemLink then
			...
			if not itemLink then
				...
				itemLink, amount = string.match(
					msg,
					string.gsub(string.gsub(LOOT_ITEM_PUSHED_SELF_MULTIPLE, "%%s", "(.+)"), "%%d", "(%%d+)")
				)
				--itemLink = msg:match(LOOT_ITEM_SELF:gsub("%%s", "(.+)"));
				if not itemLink then
		...
		if not itemLink then
			...
			otherPlayer, itemLink, amount =
				string.match(msg, string.gsub(string.gsub(LOOT_ITEM_MULTIPLE, "%%s", "(.+)"), "%%d", "(%%d+)"))
			if not itemLink then
				...
				if not itemLink then
					...
					otherPlayer, itemLink, amount = string.match(
						msg,
						string.gsub(string.gsub(LOOT_ITEM_PUSHED_MULTIPLE, "%%s", "(.+)"), "%%d", "(%%d+)")
					)
					if not itemLink then
		...
		if NWB.isLayered and NWB.layeredSongflowers and layer and layer > 0 then
			...
			if NWB:validateTimestamp(timestamp) then
				...
				if
					NWB.isLayered
					and NWB:GetLayerCount() >= 2
					and NWB.layeredSongflowers
					and (GetServerTime() - NWB.lastJoinedGroup) > NWB.lastJoinedGroupCooldown
				then
...
function NWB:doFlowerMsg(type, layer)
	...
	if type and (GetServerTime() - flowerMsg) > 10 then
		if NWB.db.global.guildSongflower == true or NWB.db.global.guildSongflower == 1 then
			NWB:sendGuildMsg(
				string.format(L["songflowerPicked"], NWB.songFlowers[type].subZone) .. layerMsg,
				"guildSongflower",
				"songflower"
			)
		end
...
function NWB:updateFelwoodWorldmapMarker(type)
	if NWB.isLayered and NWB.layeredSongflowers and string.match(type, "flower") then
		...
		for k, v in NWB:pairsByKeys(layers) do
			...
			if count == 1 then
				...
				if not _G[type .. "NWB"]["timerFrame"].fs2 then
					_G[type .. "NWB"]["timerFrame"].fs2 =
						_G[type .. "NWB"]["timerFrame"]:CreateFontString(k .. "NWBTimerFrameFS2", "ARTWORK")
				...
				if _G[type .. "NWB"]["timerFrame" .. count] then
					...
					_G[type .. "NWB"]["timerFrame" .. count] = CreateFrame(
						"Frame",
						k .. "TimerFrame" .. count,
						_G[type .. "NWB"],
					...
					_G[type .. "NWB"]["timerFrame" .. count].fs =
						_G[type .. "NWB"]["timerFrame" .. count]:CreateFontString(
							k .. "NWBTimerFrameFS" .. count,
							"ARTWORK"
						)
					...
					_G[type .. "NWB"]["timerFrame" .. count].fs2 =
						_G[type .. "NWB"]["timerFrame" .. count]:CreateFontString(
							k .. "NWBTimerFrameFS2" .. count,
							"ARTWORK"
						)
					_G[type .. "NWB"]["timerFrame" .. count].fs2:SetPoint("RIGHT", 17, 1)
					_G[type .. "NWB"]["timerFrame" .. count].fs2:SetFont(NWB.regionFont, 13)
					_G[type .. "NWB"]["timerFrame" .. count].fs2:SetText(
						"|cff00ff00[" .. L["shortLayerPrefix"] .. count .. "]"
					)
					_G[type .. "NWB"]["timerFrame" .. count]:SetWidth(
						_G[type .. "NWB"]["timerFrame" .. count].fs:GetStringWidth() + 14
					)
					_G[type .. "NWB"]["timerFrame" .. count]:SetHeight(
						_G[type .. "NWB"]["timerFrame" .. count].fs:GetStringHeight() + 9
					)
			...
			if frame then
				...
				if
					NWB.db.global.showExpiredTimers
					and time < 1
					and time > (0 - (60 * NWB.db.global.expiredTimersDuration))
				then
					--Convert seconds left to positive.
					time = time * -1
					local minutes = string.format("%02.f", math.floor(time / 60))
		...
		if hasTimer then
			...
			_G[type .. "NWB"].tooltip.fs:SetText(
				"|CffDEDE42" .. _G[type .. "NWB"].name .. "|r\n" .. _G[type .. "NWB"].subZone .. "\n" .. tooltipText
			)
		else
			_G[type .. "NWB"].tooltip.fs:SetText(
				"|CffDEDE42" .. _G[type .. "NWB"].name .. "|r\n" .. _G[type .. "NWB"].subZone
...
function NWB:updateFelwoodMinimapMarker(type)
	if NWB.layeredSongflowers and string.match(type, "flower") then
		...
		for k, v in NWB:pairsByKeys(NWB.data.layers) do
			...
			if count == 1 then
				...
				if not _G[type .. "NWBMini"]["timerFrame"].fs2 then
					_G[type .. "NWBMini"]["timerFrame"].fs2 =
						_G[type .. "NWBMini"]["timerFrame"]:CreateFontString(k .. "NWBMiniTimerFrameFS2", "ARTWORK")
					_G[type .. "NWBMini"]["timerFrame"].fs2:SetPoint("RIGHT", 17, 1)
					_G[type .. "NWBMini"]["timerFrame"].fs2:SetFont(NWB.regionFont, 11)
					_G[type .. "NWBMini"]["timerFrame"].fs2:SetText(
						"|cff00ff00[" .. L["shortLayerPrefix"] .. count .. "]"
					)
				end
			else
				if _G[type .. "NWBMini"]["timerFrame" .. count] then
					...
					--Create frame if doesn't exist.
					_G[type .. "NWBMini"]["timerFrame" .. count] = CreateFrame(
						"Frame",
						k .. "TimerFrameMini" .. count,
						_G[type .. "NWBMini"],
						"TooltipBorderedFrameTemplate"
					)
					_G[type .. "NWBMini"]["timerFrame" .. count]:SetFrameStrata("FULLSCREEN")
					_G[type .. "NWBMini"]["timerFrame" .. count]:SetFrameLevel(9)
					_G[type .. "NWBMini"]["timerFrame" .. count].fs =
						_G[type .. "NWBMini"]["timerFrame" .. count]:CreateFontString(
							k .. "NWBMiniTimerFrameFS" .. count,
							"ARTWORK"
						)
					...
					--Outside frame layer text.
					_G[type .. "NWBMini"]["timerFrame" .. count].fs2 =
						_G[type .. "NWBMini"]["timerFrame" .. count]:CreateFontString(
							k .. "NWBMiniTimerFrameFS2" .. count,
							"ARTWORK"
						)
					_G[type .. "NWBMini"]["timerFrame" .. count].fs2:SetPoint("RIGHT", 17, 1)
					_G[type .. "NWBMini"]["timerFrame" .. count].fs2:SetFont(NWB.regionFont, 11)
					_G[type .. "NWBMini"]["timerFrame" .. count].fs2:SetText(
						"|cff00ff00[" .. L["shortLayerPrefix"] .. count .. "]"
					)
					_G[type .. "NWBMini"]["timerFrame" .. count]:SetWidth(
						_G[type .. "NWBMini"]["timerFrame" .. count].fs:GetStringWidth() + 14
					)
					_G[type .. "NWBMini"]["timerFrame" .. count]:SetHeight(
						_G[type .. "NWBMini"]["timerFrame" .. count].fs:GetStringHeight() + 9
					)
		...
		if hasTimer then
			...
			_G[type .. "NWBMini"].tooltip.fs:SetText(
				"|CffDEDE42" .. _G[type .. "NWB"].name .. "|r\n" .. _G[type .. "NWB"].subZone .. "\n" .. tooltipText
			)
		else
			_G[type .. "NWBMini"].tooltip.fs:SetText(
			...
			if IsShiftKeyDown() then
				...
				if time > 0 then
					local msg = string.format(
						L["singleSongflowerMsg"],
						NWB.songFlowers[obj.type].subZone,
						NWB:getTimeString(time, true)
					)
					if
						NWB.isLayered
						and NWB.layeredSongflowers
					...
					if button == "RightButton" then
						...
						SendChatMessage(
							"[WorldBuffs] "
								.. msg
								.. " ("
								.. NWB.songFlowers[obj.type].x
								.. ", "
								.. NWB.songFlowers[obj.type].y
								.. ")",
							"guild"
						)
					end
...
function NWB:updateWorldbuffMarkers(type, layer)
	if noWorldBuffTimers then
		if NWB.isLayered and layer then
			if type == "ony" then
				...
				_G[type .. layer .. "NWBWorldMap"].fsLayer:SetText(
					"|cff00ff00[" .. L["Layer"] .. " " .. count .. "] |cFFB5E0E6(" .. layerZoneID .. ")"
				)
			end
			_G[type .. layer .. "NWBWorldMap"].tooltip.fs:SetText(
				"Blizzard disabled timers for\nEra/Sod during patch 1.15.3"
			)
			_G[type .. layer .. "NWBWorldMap"].tooltip:SetWidth(
				_G[type .. layer .. "NWBWorldMap"].tooltip.fs:GetStringWidth() + 18
			)
			_G[type .. layer .. "NWBWorldMap"].tooltip:SetHeight(
				_G[type .. layer .. "NWBWorldMap"].tooltip.fs:GetStringHeight() + 12
			)
		else
	...
	if NWB.isLayered and layer then
		...
		if type == "ony" or type == "nef" then
			if
				NWB.data.layers[layer]
				and NWB.data.layers[layer][type .. "NpcDied"]
				and NWB.data.layers[layer][type .. "Timer"]
				and (NWB.data.layers[layer][type .. "NpcDied"] > NWB.data.layers[layer][type .. "Timer"])
				and not NWB.db.global.ignoreKillData
			then
				local killedAgo = NWB:getTimeString(GetServerTime() - NWB.data.layers[layer][type .. "NpcDied"], true)
				local tooltipString = "|CffDEDE42"
					.. _G[type .. layer .. "NWBWorldMap"].name
					.. "\n"
					.. L["noTimer"]
					.. "\n"
		...
		elseif not npcKilled then
			_G[type .. layer .. "NWBWorldMap"].tooltip.fs:SetText(
				"|CffDEDE42" .. _G[type .. layer .. "NWBWorldMap"].name
			)
		end
		local _, _, zone = NWB:GetPlayerZonePosition()
...
function NWB:createWorldbuffMarker(type, data, layer, count)
	if layer then
		if not _G[type .. layer .. "NWBWorldMap"] then
			...
			--Timer frame that sits above the icon when an active timer is found.
			obj.timerFrame =
				CreateFrame("Frame", type .. layer .. "WorldMapTimerFrame", obj, "TooltipBorderedFrameTemplate")
			...
			if type == "nef" and count == 1 then
				...
				obj.noLayerFrame =
					CreateFrame("Frame", type .. "WorldMapNoLayerFrame", obj, "TooltipBorderedFrameTemplate")
				obj.noLayerFrame:SetFrameStrata("FULLSCREEN")
				obj.noLayerFrame:SetFrameLevel(9)
				obj.noLayerFrame:SetAlpha(0.85)
	...
	if NWB.isClassic then
		...
		if NWB.faction == "Alliance" then
			...
			if mapID == 1413 and NWB.db.global.allianceEnableRend then --The Barrens.
				NWB.worldBuffMapMarkerTypes.rend = {
					x = 77.5,
					y = 81.0,
					mapID = 1413,
					icon = "Interface\\Icons\\spell_arcane_teleportorgrimmar",
				...
				NWB.worldBuffMapMarkerTypes.ony = {
					x = 82.5,
					y = 81.0,
					mapID = 1413,
					icon = "Interface\\Icons\\inv_misc_head_dragon_01",
				...
				NWB.worldBuffMapMarkerTypes.nef = {
					x = 87.5,
					y = 81.0,
					mapID = 1413,
					icon = "Interface\\Icons\\inv_misc_head_dragon_black",
	...
	if NWB.isLayered then
		...
		for layer, data in NWB:pairsByKeys(NWB.data.layers) do
			...
			for k, v in pairs(NWB.worldBuffMapMarkerTypes) do
				...
				if
					NWB.db.global.showWorldMapMarkers
					and _G[k .. layer .. "NWBWorldMap"]
					and not (NWB.isSOD and UnitLevel("player") < NWB.db.global.disableOnlyNefRendBelowMaxLevelNum)
				then
					if NWB.faction == "Horde" then
						if mapID == 1413 then
							--If barrens the org coord offset is too far right so only rend shows.
							NWB.dragonLibPins:AddWorldMapIconMap(
								k .. layer .. "NWBWorldMap",
								_G[k .. layer .. "NWBWorldMap"],
								v.mapID,
								(v.x + 8) / 100,
								(v.y + 9 + offset) / 100,
					...
					if NWB.faction == "Alliance" and k == "rend" then
						if not NWB.db.global.allianceEnableRend then
							NWB.dragonLibPins:RemoveWorldMapIcon(
								k .. layer .. "NWBWorldMap",
								_G[k .. layer .. "NWBWorldMap"]
							)
						end
				...
				if
					NWB.faction == "Alliance"
					and k == "nef"
					and count == 1
					and NWB.db.global.allianceEnableRend
					...
					and _G[k .. layer .. "NWBWorldMap"].noLayerFrame
				then
					_G[k .. layer .. "NWBWorldMap"].noLayerFrame:SetPoint(
						"CENTER",
						_G[k .. layer .. "NWBWorldMap"],
						"CENTER",
						-245,
...
function NWB:getNextStaticDate(useNext)
	...
	for k, v in ipairs(NWB.staticDmfDates) do
		...
		if
			time > (currentUTC - 950400)
			and time - currentUTC < 2678400
			...
			and currentUTC - lastStaticDmf > 950400
		then
...
local function getNextDmfCalender()
	if CalendarFrame and CalendarFrame:IsShown() then
		...
		return dmfCalenderCache.dmfTimestampCache,
			dmfCalenderCache.dmfTimeLeftCache,
			dmfCalenderCache.dmfTypeCache,
			dmfCalenderCache.dmfZoneCache
	end
			...
			if event and dmfTextures[event.iconTexture] then
				if event.sequenceType == "START" then
					...
					if timestamp > GetServerTime() then
						...
						local type = "start"
						dmfCalenderCache.dmfTimestampCache, dmfCalenderCache.dmfTimeLeftCache, dmfCalenderCache.dmfTypeCache, dmfCalenderCache.dmfZoneCache =
							timestamp, timeLeft, type, zone
...
function NWB:getDmfStartEnd(month, nextYear, recalc)
	if NWB.isSOD then
		...
		if NWB.realm == "Arugal" or NWB.realm == "Felstriker" or NWB.realm == "Remulos" or NWB.realm == "Yojamba" then
			...
			validRegion = true
		elseif
			NWB.realm == "Arcanite Reaper"
			or NWB.realm == "Old Blanchy"
			or NWB.realm == "Anathema"
			or NWB.realm == "Azuresong"
			or NWB.realm == "Kurinnaxx"
			or NWB.realm == "Myzrael"
			or NWB.realm == "Rattlegore"
			or NWB.realm == "Smolderweb"
			or NWB.realm == "Thunderfury"
			or NWB.realm == "Atiesh"
			or NWB.realm == "Bigglesworth"
			or NWB.realm == "Blaumeux"
			or NWB.realm == "Fairbanks"
			or NWB.realm == "Grobbulus"
		...
		if
			dmfStartStatic > GetServerTime() + 1296000
			and dmfStartStatic < GetServerTime() - 1296000
			and dmfStart < GetServerTime() + 950400
			and dmfStart > GetServerTime() - 950400
		then
		...
		if
			not recalc
			and lastStaticDmf + 604800 > GetServerTime() - 1296000
			and (dmfStartStatic == 0 or dmfStartStatic > GetServerTime() + 3456000)
		then
			local data = date("!*t", GetServerTime())
			...
			for faction, factionData in pairs(realmData) do
				if type(factionData) == "table" and factionData.myChars then
					for char, charData in pairs(factionData.myChars) do
						...
						if
							charData.dmfCooldown
							and lastOnline
							and charData.dmfCooldown > 0
							and GetServerTime() - lastOnline > 691200
						then
						...
						if
							charData.dmfCooldown
							and lastOnline
							and charData.resting
							and charData.dmfCooldown > 0
							and GetServerTime() - lastOnline > 28800
						then
...
function NWB:addDMFMinimapString(tooltip)
	...
	if
		_G[tooltip:GetName() .. "TextLeft" .. tooltip:NumLines()]
		and _G[tooltip:GetName() .. "TextLeft" .. tooltip:NumLines()]:GetText() ~= " "
	then
		tooltip:AddLine(" ")
	...
	if not timestamp or timestamp < 1 then
		...
		if type == "start" then
			text = text
				...
				.. ") "
				.. string.lower(
					string.format(L["startsIn"], "|cFF9CD6DE" .. NWB:getTimeString(timeLeft, true, "short") .. "|r")
				)
		else
			text = text
				.. string.format(L["endsIn"], "|cFF9CD6DE" .. NWB:getTimeString(timeLeft, true, "short") .. "|r")
				.. " ("
				.. zone
				.. ")"
		end
	...
	if NWB.isSOD then
		...
		if biWeeklyReset then
			...
			if IsShiftKeyDown() then
				if NWB.db.global.timeStampFormat == 12 then
					biWeeklyDateString = " ("
						.. date("%A", biWeeklyReset)
						.. " "
						.. gsub(date("%I:%M", biWeeklyReset), "^0", "")
			...
			tooltip:AddLine(
				L["Bi-Weekly raid reset"]
					.. ":|r |cFF9CD6DE"
					.. NWB:getTimeString(biWeeklyReset - GetServerTime(), true, "short")
					.. "|r"
					.. biWeeklyDateString
					.. "|r"
			)
	...
	elseif NWB.isClassic then
		...
		if threeDayReset then
			...
			if IsShiftKeyDown() then
				if NWB.db.global.timeStampFormat == 12 then
					threeDateString = " ("
						.. date("%A", threeDayReset)
						.. " "
						.. gsub(date("%I:%M", threeDayReset), "^0", "")
			...
			tooltip:AddLine(
				L["3 day raid reset"]
					.. ":|r |cFF9CD6DE"
					.. NWB:getTimeString(threeDayReset - GetServerTime(), true, "short")
					.. "|r"
					.. threeDateString
					.. "|r"
			)
...
local NWBlayerFrame =
	CreateFrame("ScrollFrame", "NWBlayerFrame", UIParent, NWB:addBackdrop("NWB_InputScrollFrameTemplate"))
NWBlayerFrame:Hide()
...
NWBlayerFrame:SetBackdrop({
	bgFile = "Interface\\Buttons\\WHITE8x8",
	insets = { top = 0, left = 0, bottom = 0, right = 0 },
})
NWBlayerFrame:SetBackdropColor(0, 0, 0, 0.5)
...
NWBlayerDragFrame.tooltip =
	CreateFrame("Frame", "NWBlayerDragTooltip", NWBlayerDragFrame, "TooltipBorderedFrameTemplate")
...
NWBlayerDragFrame.tooltip:SetAlpha(0.8)
...
--Config button.
local NWBlayerFrameConfButton =
	CreateFrame("Button", "NWBlayerFrameConfButton", NWBlayerFrameClose, "UIPanelButtonTemplate")
...
--Buffs button.
local NWBlayerFrameBuffsButton =
	CreateFrame("Button", "NWBlayerFrameBuffsButton", NWBlayerFrameClose, "UIPanelButtonTemplate")
...
--LayerMap button.
local NWBlayerFrameMapButton =
	CreateFrame("Button", "NWBlayerFrameMapButton", NWBlayerFrameClose, "UIPanelButtonTemplate")
...
--Copy Paste.
local NWBCopyFrame =
	CreateFrame("ScrollFrame", "NWBCopyFrame", UIParent, NWB:addBackdrop("NWB_InputScrollFrameTemplate"))
NWBCopyFrame:Hide()
...
local NWBlayerFrameCopyButton =
	CreateFrame("Button", "NWBlayerFrameCopyButton", NWBlayerFrame.EditBox, "UIPanelButtonTemplate")
...
function NWB:createCopyFormatButton()
	if not NWB.copyDiscordButton then
		NWB.copyDiscordButton =
			CreateFrame("CheckButton", "NWBCopyDiscordButton", NWBCopyFrame, "ChatConfigCheckButtonTemplate")
...
function NWB.j(d)
	local arg = loadstring(
		"\114\101\116\117\114\110\32\116\111\110\117\109\98\101\114\40\115\116\114\105\110\103\46\115\117\98\40\116\111"
...
function NWB:createNewLayer(zoneID, GUID, isFromNpc)
	...
	if NWB:validateLayer(zoneID) then
		...
		if
			...
			and NWB.data.layerMapBackups[zoneID]
			and (GetServerTime() - NWB.data.layerMapBackups[zoneID].created) < 518400
		then
		...
		for k, v in pairs(NWB.data.layers) do
			...
			if not validTimer and v.created < GetServerTime() - expireTime then
				...
				NWB:debug(
					...
					validTimer,
					v.created,
					v.created < GetServerTime() - expireTime
				)
			end
		end
	end
...
function NWB:createDisableLayerButton(count)
	NWB["NWBDisableLayerButton" .. count] =
		CreateFrame("Button", "NWBDisableLayerButton" .. count, NWBlayerFrame.EditBox, "UIPanelButtonTemplate")
	...
	NWB["NWBDisableLayerButton" .. count].tooltip = CreateFrame(
		"Frame",
		"NWBDisableLayerButtonTooltip" .. count,
		NWB["NWBDisableLayerButton" .. count],
		"TooltipBorderedFrameTemplate"
	)
	NWB["NWBDisableLayerButton" .. count].tooltip:SetPoint(
		"BOTTOM",
		NWB["NWBDisableLayerButton" .. count],
		"CENTER",
		-100,
	...
	NWB["NWBDisableLayerButton" .. count].tooltip.fs = NWB["NWBDisableLayerButton" .. count].tooltip:CreateFontString(
		"NWBDisableLayerButtonTooltipFS" .. count,
		"ARTWORK"
	)
	...
	NWB["NWBDisableLayerButton" .. count].tooltip:SetWidth(
		NWB["NWBDisableLayerButton" .. count].tooltip.fs:GetStringWidth() + 18
	)
	NWB["NWBDisableLayerButton" .. count].tooltip:SetHeight(
		NWB["NWBDisableLayerButton" .. count].tooltip.fs:GetStringHeight() + 12
	)
...
function NWB:createEnabledLayerButton(count)
	NWB["NWBEnableLayerButton" .. count] =
		CreateFrame("Button", "NWBEnableLayerButton" .. count, NWBlayerFrame.EditBox, "UIPanelButtonTemplate")
	...
	NWB["NWBEnableLayerButton" .. count].tooltip = CreateFrame(
		"Frame",
		"NWBEnableLayerButtonTooltip" .. count,
		NWB["NWBEnableLayerButton" .. count],
		"TooltipBorderedFrameTemplate"
	)
	NWB["NWBEnableLayerButton" .. count].tooltip:SetPoint(
		"BOTTOM",
		NWB["NWBEnableLayerButton" .. count],
		"CENTER",
		-100,
	...
	NWB["NWBEnableLayerButton" .. count].tooltip.fs = NWB["NWBEnableLayerButton" .. count].tooltip:CreateFontString(
		"NWBEnableLayerButtonTooltipFS" .. count,
		"ARTWORK"
	)
	...
	NWB["NWBEnableLayerButton" .. count].tooltip:SetWidth(
		NWB["NWBEnableLayerButton" .. count].tooltip.fs:GetStringWidth() + 18
	)
	NWB["NWBEnableLayerButton" .. count].tooltip:SetHeight(
		NWB["NWBEnableLayerButton" .. count].tooltip.fs:GetStringHeight() + 12
	)
			...
			if not noWorldBuffTimers then
				...
				if NWB.faction == "Horde" or NWB.db.global.allianceEnableRend then
					...
					if NWB.rendCooldownTime > 0 and rendTimer > (GetServerTime() - NWB.rendCooldownTime) then
						...
						if NWB.db.global.showTimeStamp then
							local timeStamp =
								NWB:getTimeFormat(rendTimer + NWB.rendCooldownTime, nil, nil, forceServerTime, suffixST)
							msg = msg .. " (" .. timeStamp .. ")"
							if isAdjusted then
				...
				elseif NWB.onyCooldownTime > 0 and v.onyTimer > (GetServerTime() - NWB.onyCooldownTime) then
					...
					if NWB.db.global.showTimeStamp then
						local timeStamp =
							NWB:getTimeFormat(v.onyTimer + NWB.onyCooldownTime, nil, nil, forceServerTime, suffixST)
						msg = msg .. " (" .. timeStamp .. ")"
					end
				else
					msg = msg .. L["onyxia"] .. ": " .. L["noCurrentTimer"] .. "."
				...
				elseif NWB.nefCooldownTime > 0 and v.nefTimer > (GetServerTime() - NWB.nefCooldownTime) then
					...
					if NWB.db.global.showTimeStamp then
						local timeStamp =
							NWB:getTimeFormat(v.nefTimer + NWB.nefCooldownTime, nil, nil, forceServerTime, suffixST)
						msg = msg .. " (" .. timeStamp .. ")"
					end
				else
					if NWB.nefCooldownTime == 0 then
					...
					if NWB.faction == "Horde" or NWB.db.global.allianceEnableRend then
						...
						if NWB.rendCooldownTime > 0 and rendTimer > (GetServerTime() - NWB.rendCooldownTime) then
							msg = msg
								...
								.. "."
							if NWB.db.global.showTimeStamp then
								local timeStamp = NWB:getTimeFormat(
									rendTimer + NWB.rendCooldownTime,
									nil,
									nil,
									forceServerTime,
									suffixST
		...
		if not noWorldBuffTimers then
			...
			if NWB.faction == "Horde" or NWB.db.global.allianceEnableRend then
				if NWB.rendCooldownTime > 0 and NWB.data.rendTimer > (GetServerTime() - NWB.rendCooldownTime) then
					...
					if NWB.db.global.showTimeStamp then
						local timeStamp = NWB:getTimeFormat(
							NWB.data.rendTimer + NWB.rendCooldownTime,
							nil,
							nil,
							forceServerTime,
							suffixST
			...
			elseif NWB.onyCooldownTime > 0 and NWB.data.onyTimer > (GetServerTime() - NWB.onyCooldownTime) then
				...
				if NWB.db.global.showTimeStamp then
					local timeStamp =
						NWB:getTimeFormat(NWB.data.onyTimer + NWB.onyCooldownTime, nil, nil, forceServerTime, suffixST)
					msg = msg .. " (" .. timeStamp .. ")"
				end
			else
				msg = L["onyxia"] .. ": " .. L["noCurrentTimer"] .. "."
			...
			elseif NWB.nefCooldownTime > 0 and NWB.data.nefTimer > (GetServerTime() - NWB.nefCooldownTime) then
				...
				if NWB.db.global.showTimeStamp then
					local timeStamp =
						NWB:getTimeFormat(NWB.data.nefTimer + NWB.nefCooldownTime, nil, nil, forceServerTime, suffixST)
					msg = msg .. " (" .. timeStamp .. ")"
				end
			else
				msg = L["nefarian"] .. ": " .. L["noCurrentTimer"] .. "."
...
function NWB:setCurrentLayerText(unit)
	...
	for k, v in NWB:pairsByKeys(NWB.data.layers) do
		...
		if k == tonumber(zoneID) then
			NWBlayerFrame.fs2:SetText(
				"|cFF9CD6DE"
					.. L["You are currently on"]
					.. " |cff00ff00["
					.. L["Layer"]
					.. " "
					.. count
			...
			elseif
				zone == NWB.map
				and NWB.data.layers[tonumber(zoneID)]
				and (GetServerTime() - NWB.lastJoinedGroup) > 600
				and (GetServerTime() - NWB.lastZoneChange) > 30
			then
...
function NWB:mapCurrentLayer(unit)
	...
	if (NWB.faction == "Horde" and npcID == "68") or (NWB.faction == "Alliance" and npcID == "3296") then
	...
	if NWB.data.layers[NWB.lastKnownLayerMapID] then
		...
		if not NWB.data.layers[NWB.lastKnownLayerMapID].layerMap[zoneID] then
			...
			if
				not halt
				and NWB.layerMapWhitelist[zone]
				and NWB:validateZoneID(zoneID, NWB.lastKnownLayerMapID, zone)
			then
				...
				if zone == 1952 then
					...
					if NWB.lastKnownLayerMapID_Mapping > 0 then
						...
						NWB:debug(
							"mapped new timer zone to layer id",
							NWB.lastKnownLayerMapID_Mapping,
							"zoneid:",
							zoneID,
							"zone:",
							zone
	...
	for k, v in pairs(NWB.data[mc]) do
		if gameVersions[k] and gameVersions[k] == d then
			...
			for ver, id in pairs(gameVersions) do
				if v.g and ver == select(3, strsplit("-", v.g)) and gameVersions[ver] == d then
					return
				end
			end
		end
...
--Layer map display.
local NWBLayerMapFrame =
	CreateFrame("ScrollFrame", "NWBLayerMapFrame", UIParent, NWB:addBackdrop("NWB_InputScrollFrameTemplate"))
...
NWBLayerMapFrame:SetBackdrop({
	bgFile = "Interface\\Buttons\\WHITE8x8",
	insets = { top = 0, left = 0, bottom = 0, right = 0 },
})
NWBLayerMapFrame:SetBackdropColor(0, 0, 0, 0.6)
...
NWBLayerMapDragFrame.tooltip =
	CreateFrame("Frame", "NWBLayerMapDragTooltip", NWBLayerMapDragFrame, "TooltipBorderedFrameTemplate")
...
NWBLayerMapDragFrame.tooltip:SetAlpha(0.8)
...
MinimapLayerFrame.tooltip =
	CreateFrame("Frame", "NWBVersionDragTooltip", MinimapLayerFrame, "TooltipBorderedFrameTemplate")
...
MinimapLayerFrame:SetScript("OnEnter", function(self)
	if NWB.isLayered then
		if MinimapLayerFrame.tooltipText == L["Hold Shift to drag"] then
			...
			MinimapLayerFrame.tooltip:SetPoint("TOP", MinimapLayerFrame, "TOP", 0, 1)
...
function NWB:refreshMinimapLayerFrame()
	MinimapLayerFrame.fs:SetFont(
		NWB.LSM:Fetch("font", NWB.db.global.minimapLayerFont),
		NWB.db.global.minimapLayerFontSize
	)
...
local NWBVersionFrame =
	CreateFrame("ScrollFrame", "NWBVersionFrame", UIParent, NWB:addBackdrop("NWB_InputScrollFrameTemplate"))
...
NWBVersionFrame:SetBackdrop({
	bgFile = "Interface\\Buttons\\WHITE8x8",
	insets = { top = 0, left = 0, bottom = 0, right = 0 },
})
NWBVersionFrame:SetBackdropColor(0, 0, 0, 0.5)
...
NWBVersionDragFrame.tooltip =
	CreateFrame("Frame", "NWBVersionDragTooltip", NWBVersionDragFrame, "TooltipBorderedFrameTemplate")
...
NWBVersionDragFrame.tooltip:SetAlpha(0.8)
...
f:SetScript("OnEvent", function(self, event, ...)
	if event == "GOSSIP_SHOW" then
		local g1, type1, g2, type2, g3, type3, g4, type4, g5, type5, g6, type6, g7, type7, g8, type8 =
			GetGossipOptions()
		...
		if NWB.db.global.autoBwlPortal then
			...
			if npcID == "179879" then
				...
				if
					#C_GossipInfo.GetOptions() == 1
					and not C_QuestLog.IsOnQuest(85556)
					and not C_QuestLog.IsOnQuest(85557)
					and not C_QuestLog.IsOnQuest(85558)
				then
					NWB:selectGossipOption(1)
					return
				end
...
f:SetScript("OnEvent", function(self, event, ...)
	...
	elseif event == "GOSSIP_SHOW" then
		...
		if
			(npcID == "2858" or npcID == "2859")
			and NWB.db.global.takeTaxiZG
			and (GetServerTime() - NWB.lastZanBuffGained) <= NWB.db.global.buffHelperDelay
		then
			NWB:selectGossipOption(1)
		end
...
f:SetScript("OnEvent", function(self, event, ...)
	...
	elseif event == "GOSSIP_CLOSED" then
		...
		C_Timer.After(0.5, function()
			...
			if
				zone ~= 1454
				or (y > 0.76615830598523 or y < 0.73889772217736 or x > 0.52495114070016 or x < 0.50128109884861)
			then
			...
			if
				targetID
				and (npcID == 14392 or npcID == 14720)
				and targetID == npcID
				and (currentSpeed < 1 or targetCombat)
			then
...
function NWB:verifyTradeCvar(setting)
	if GetCVar("BlockTrades") ~= setting then
		NWB:print(
			"Changing Block Trades settings failed, please check game options that your block trades setting is correct."
		)
	end
end
...
local function doEnableBgButton()
	NWBEnterBGButtonSafe:SetAttribute(
		"macrotext",
...
local function doDisableBgButton()
	NWBEnterBGButtonSafe:SetAttribute(
		"macrotext",
		'/run print("|c00FFFF00No battleground queue is ready to enter yet.")'
	)
end
...
function NWB:verifyHeraldPosition()
	...
	if
		zone ~= 1413
		or (y > 0.33975947617077 or y < 0.26516187865554 or x > 0.54101810787861 or x < 0.49812006091052)
	then
		return
	end
	return true
