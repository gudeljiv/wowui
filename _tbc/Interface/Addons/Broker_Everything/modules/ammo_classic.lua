...
local classes = {
	HUNTER = true,
	ROGUE = true,
	WARRIOR = true,--[[WARLOCK=true]]
}
if not (ns.client_version < 5 and classes[ns.player.class]) then
	return
end
...
local function updateBroker()
	if not (ammo.sum or thrown.sum) then
		return
	end
	...
	if itemInfoInUse then
		...
		text = C(
			(itemInfoInUse.count <= 10 and "red")
				or (itemInfoInUse.count <= 25 and "orange")
				or (itemInfoInUse.count <= 50 and "yellow")
				or "green",
			itemInfoInUse.count
		)
...
function createTooltip(tt, update)
	if not (tt and tt.key and tt.key == ttName) then
		return
	end -- don't override other LibQTip tooltips...
	if tt.lines ~= nil then
		tt:Clear()
	end
	tt:SetCell(tt:AddLine(), 1, C("dkyellow", INVTYPE_AMMO .. " / " .. INVTYPE_THROWN), tt:GetHeaderFont(), "LEFT", 0)
	tt:AddSeparator(1)
	if (ammo.sum or 0) > 0 then
		...
		for id, itemInfo in pairs(ammo.itemInfo) do
			tt:AddLine(
				"    |T"
					.. itemInfo.icon
					.. ":0|t "
					.. C("quality" .. itemInfo.quality, itemInfo.name)
					.. (ammo.inUse == id and " " .. C("green", "(" .. CONTRIBUTION_ACTIVE .. ")") or ""),
				C("white", itemInfo.count)
			)
		end
	end
		...
		for id, itemInfo in pairs(thrown.itemInfo) do
			tt:AddLine(
				"    |T"
					...
					.. ":0|t "
					.. C("quality" .. itemInfo.quality, itemInfo.name)
					.. (thrown.inUse == id and " " .. C("green", "(" .. CONTRIBUTION_ACTIVE .. ")") or ""),
				C("white", itemInfo.count)
			)
		end
	end
	...
	for sharedSlot in pairs(ns.items.ammo) do
		...
		if count > 0 then
			if not items[s][item.id] then
				...
				items[s][item.id].name, _, items[s][item.id].quality, _, _, _, _, _, _, items[s][item.id].icon =
					C_Item.GetItemInfo(item.id)
			else
	...
	return {
		broker = {
			showNameBroker = {
				type = "toggle",
				order = 1,
				name = L["Show ammunition name"],
				desc = L["Display ammunition name on broker button"],
			},
		},
		tooltip = nil,
		misc = nil,
...
function module.onenter(self)
	if ns.tooltipChkOnShowModifier(false) then
		return
	end
