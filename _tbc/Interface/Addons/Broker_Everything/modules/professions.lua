...
local function unpackFactionRecipes()
	for _,skillIndex in ipairs({GetProfessions()}) do
		local skillLine = select(7, GetProfessionInfo(skillIndex));
		if skillLine then
			for exp, profs in pairs(faction_recipes) do
				if type(exp)=="number" and type(profs)=="table" and type(profs[skillLine])=="string" then
					local tmp = {};
					local entries = {strsplit(";",profs[skillLine])};
					for _, entry in ipairs(entries) do
						local t={};
						for _, value in ipairs({strsplit(",",entry)}) do
							tinsert(t,tonumber(value) or value);
						end
						tinsert(tmp,t);
					end
					faction_recipes[exp][skillLine] = tmp;
				end
			end
		end
	end
			...
			for _,recipe in ipairs(recipes) do
				if not factions[recipe[1]] then -- fill factions table
					...
					if C_Reputation.IsMajorFaction(recipe[1]) then
						local info2 = C_MajorFactions.GetMajorFactionData(recipe[1])
						if info2 then
							factions[recipe[1]].isMajor = true;
							factions[recipe[1]].standing = info2.renownLevel;
							factions[recipe[1]].standingText = MAJOR_FACTION_RENOWN_LEVEL_TOAST:format(info2.renownLevel);
						end
...
local function createTooltip(tt)
	...
	if #professions>0 then
		...
		for exp=6, (GetNumExpansions()-1) do
			if ns.toon.level>=GetMaxLevelForExpansionLevel(exp-1)+1 and ns.profile[name]["showExpansion"..exp.."FactionRecipes"] then
				...
				AddFactionRecipeLines(tt,exp,faction_recipes[exp]);
			end
		end
	else
...
module = {
	...
	config_defaults = {
		...
		--showLegionFactionRecipes = false, -- deprecated
		--showBfAFactionRecipes = false,
		--showShadowFactionRecipes = false,
		--showDFFactionRecipes = true,
		--showExpansion%dFactionRecipes -- filled after module table description
		inTitle = {},
...
do
	local max = GetNumExpansions()-1;
	for exp=6, max do
		module.config_defaults["showExpansion"..exp.."FactionRecipes"] = false;
	end
	module.config_defaults["showExpansion"..max.."FactionRecipes"] = true;
end
...
local function hideFactionRecipesEntry(info)
	local exp = tonumber((info[#info]:gsub("showExpansion(%d+)FactionRecipes","%1")));
	return not faction_recipes[exp];
end

function module.options()
	local o,tooltip = 1,{
		showCooldowns={ type="toggle", order=30, name=L["Show cooldowns"], desc=L["Show/Hide profession cooldowns from all characters."] },
		showAllFactions=31,
		showRealmNames=32,
		showCharsFrom=33,
	}
	for  exp=6, (GetNumExpansions()-1) do
		tooltip["showExpansion"..exp.."FactionRecipes"] = {
			type="toggle", order=o, width="full",
			name=L["EmissaryVendorRecipes"].." (".._G["EXPANSION_NAME"..exp]..")",
			desc=L["EmissaryVendorRecipesDesc"],
			hidden = hideFactionRecipesEntry
		};
		o = o+1;
	end
	return {
...
function module.init()
	...
	faction_recipes[8]={ -- https://www.wowhead.com/items/recipes
			-- Alchemy
			[171]="2465,5,183106,307087;2439,5,182660,307143",
			--Blacksmithing
			[164]="2407,6,183094,322590;2410,6,183095,322593",
			--Enchanting
			[333]="2465,6,183096,309644",
			--Engineering
			[202]="2407,8,183097,331007;2410,6,183858,310535",
			--Inscription
			[773]="2407,5,183098,311424;2407,7,183103,311409;2465,7,183093,311410;2410,7,183104,311411;2413,7,183102,311412",
			--Jewelcrafting
			[755]="2413,6,183099,311870",
			--Leatherworking
			[165]="2465,6,183100,324088",
			--Tailoring
			[197]="2410,6,183101,310898",
			--Cooking
			[185]="2413,7,182668,308403",
	}

	-- https://www.wowhead.com/de/items/recipes?filter=166:92;10:1;0:0
	-- [<tradeSkillId>]={ {<factionId>, <standingId>, <itemId>, <spellId>[, <recipeStars>]}, ... }
	faction_recipes[9]={
		-- Alchemy
		...
		-- Engineering
		[202]="2507,9,198781,382343;2507,9,198784,382322;2507,15,198782,382339;2507,15,198783,382340;2507,21,198785,382332;2510,11,199246,382344;2510,19,199244,382341;2510,19,199245,382342;2544,2,199290,382404;2544,3,199242,382338;2544,3,199243,382337;2544,3,199289,382403;2544,4,199285,382399;2544,4,199286,382400;2544,4,199295,384313;2544,5,199240,382335",
		-- Jewelcrafting
	...
	-- https://www.wowhead.com/de/items/recipes?filter=166:99:92;11:11:1;0:0:0
	-- https://www.wowhead.com/de/items/recipes?filter=166:92;11:1;0:0
	-- [<tradeSkillId>] = { {<factionId>, <standingId>, <itemId>, <spellId>[, <recipeStars>]}, ... }
	faction_recipes[10] = {
		-- Alchemy
		[171]="2600,4,223080,427185;2658,5,244841,1238010;2658,5,248550,1246966",
		-- Blacksmithing
		[164]="2570,4,223038,450238",
		-- Enchanting
		[333]="2570,4,223114,445368;2590,5,223127,445322;2600,4,223125,445346",
		-- Inscription
		[773]="2570,4,223064,444218;2600,4,225355,444219;2590,5,223062,444220;2594,3,223063,444217;2658,5,244334,1236908;2653,5,232535,471132;2653,5,233022,472951;2658,5,244043,1236170",
		-- Engineering
		[202]="2653,5,234274,1213620",
		-- Jewelcrafting
		--[755]="",
		-- Leatherworking
		[165]="2590,5,223103,444102;2653,5,235338,1216520",
		-- Tailoring
		[197]="2594,3,225995,447888;2658,5,239485,1228338;2658,5,239486,1228344;2658,5,239487,1228343;2658,5,239488,1228342",
		-- Cooking
		[185]="2590,22,248013,1245993;2594,4,224418,445115"
	}

	-- https://www.wowhead.com/de/items/recipes?filter=166:92;11:1;0:0
	-- [<tradeSkillId>]={ {<factionId>, <standingId>, <itemId>, <spellId>[, <recipeStars>]}, ... }
	-- Armanistamm 2696
	--vendors[2696] = {45.95, 65.92};
	faction_recipes[11] = {
		-- Alchemy
		[171]="2696,5,258305,1230864;2699,5,259459,1230859;2704,5,247500,1233133;2704,0,258141,1230873",
		-- Blacksmithing
		--[164]={},
		-- Enchanting
		[333]="2696,5,256741,1236065;2696,5,256744,1236063;2696,5,268043,1281348;2699,5,256757,1236095;2699,5,256758,1236093;2699,5,268044,1281349;2710,5,256760,1236089;2710,5,259374,1246907;2710,5,268042,1281342;2704,5,256743,1236464;2704,5,256745,1236068;2704,5,256749,1236078;2704,5,256751,1236081;2714,5,259375,1246909;2711,5,259367,1246902;2712,5,259364,1246906",
		-- Inscription
		[773]="2696,5,251629,1230052;2699,5,258500,1248621;2699,5,263445,1230054;2710,5,263444,1230051;2704,5,258509,1248630;2704,5,258510,1248631;2704,5,263443,1230053;2713,5,258505,1248626",
		-- Engineering
		--[202]={},
		-- Jewelcrafting
		[755]="2696,5,256717,1230479;2710,5,259363,1246891;2704,5,256700,1230487",
		-- Leatherworking
		[165]="2696,5,256647,1237544;2710,5,256636,1237486;2704,5,256639,1237498;2704,5,259457,1246940",
		-- Tailoring
		[197]="2699,5,258128,1229001",
		-- Cooking
		[185]="2696,5,251630,1226172",
	}
	...
	profKnowledge[9] = {
		...
		-- Alchemy
