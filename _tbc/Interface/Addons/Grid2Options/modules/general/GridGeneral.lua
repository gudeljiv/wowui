...
Grid2Options:AddGeneralOptions(
	'General',
	'Themes',
	{
		themes = {
...
do
	...
	function SetFormat(formatType, mask, tenths)
		...
		if short then
			...
			string.format(short, 1, 1)
			string.format(long, 1, 1)
			local dbx = Grid2.db.profile.formatting
...
Grid2Options:AddGeneralOptions(
	...
	'Text Formatting',
	{
		dFormat = {
			...
			desc = L['Examples:\n(%d)\n%d seconds'],
			get = function()
				return GetFormat('DecimalFormat')
			end,
			set = function(_, v)
				SetFormat('DecimalFormat', v, nil)
			end
		},
		dTenths = {
			type = 'select',
			...
			get = function()
				return GetTenths('DecimalFormat')
			end,
			set = function(_, v)
				SetFormat('DecimalFormat', nil, v)
			end,
			values = tenthsValues
		},
		...
		dsFormat = {
			...
			desc = L['Examples:\n%d/%s\n%s(%d)'],
			get = function()
				return GetFormat('DurationStackFormat')
			end,
			set = function(_, v)
				SetFormat('DurationStackFormat', v, nil)
			end
		},
		dsTenths = {
			type = 'select',
			...
			desc = L['Display tenths of a second'],
			get = function()
				return GetTenths('DurationStackFormat')
			end,
			set = function(_, v)
				SetFormat('DurationStackFormat', nil, v)
			end,
			values = tenthsValues
		},
...
Grid2Options:AddGeneralOptions(
	...
	'blink',
	{
		effect = {
			...
			order = 10,
			get = function()
				return Grid2Frame.db.shared.blinkType
			end,
			set = function(_, v)
		...
		frequency = {
			...
			disabled = function()
				return Grid2Frame.db.shared.blinkType == 'None'
			end,
			min = 1,
			max = 10,
			step = .5,
	...
	Grid2Options:AddGeneralOptions(
		...
		'Auras',
		{
			classicDurations = {
				...
				order = 115,
				get = function()
					return not Grid2.db.global.disableDurations
				end,
				set = function(_, v)
				...
				confirm = function()
					return L['UI must be reloaded to change this option. Are you sure?']
				end
			}
		}
	)
	...
	Grid2Options:AddGeneralOptions(
		...
		'Minimap Icon',
		{
			showMinimapIcon = {
				...
				order = 119,
				get = function()
					return not Grid2Layout.db.shared.minimapIcon.hide
				end,
				set = function(_, v)
	...
	Grid2Options:AddGeneralOptions(
		...
		'Blizzard Raid Frames',
		{
			hideBlizzardRaidFrames = {
				...
				order = 120,
				get = function()
					return not IsAddOnLoaded(addons[1])
				end,
				set = function(_, v)
				...
				confirm = function()
					return 'UI will be reloaded. Are your sure ?'
				end
			}
		}
	)
...
Grid2Options:AddGeneralOptions(
	...
	'Options management',
	{
		loadOnDemand = {
			...
			order = 150,
			get = function()
				return not Grid2.db.global.LoadOnDemandDisabled
			end,
			set = function(_, v)
