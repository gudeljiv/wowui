...
function Sell:CreateItemFrame(parent, lineHeight, margin)
	...
	holdingFrame:SetScript(
		'OnEnter',
		function(self)
			if AuctionFaster.db.sell.tooltips.itemEnabled then
				AuctionFaster:ShowTooltip(self, self.itemLink, true, self.item.itemId, AuctionFaster.db.sell.tooltips.itemAnchor or 'RIGHT')
			end
		end
	)
...
function Sell:DrawRightPaneItemIcon(leftMargin, topMargin, iconSize)
	...
	sellTab.itemQty = StdUi:Label(sellTab, format(L['Qty: %d, Max Stacks: %d, Remaining: %d'], 0, 0, 0), nil, nil, 250, 20)
			...
			events = {
				OnEnter = function(table, cellFrame, rowFrame, rowData, columnData, rowIndex)
					if AuctionFaster.db.sell.tooltips.enabled then
						AuctionFaster:ShowTooltip(cellFrame, rowData.itemLink, true, rowData.itemId, AuctionFaster.db.sell.tooltips.anchor)
					end
					return false
				end,
