...
function Sell:CompareSort(a, b)
	...
	if self.sortInventoryBy == 'price' then
		if valA == nil or valA == '---' then
			valA = 0
		end
		if valB == nil or valB == '---' then
			valB = 0
		end
	end
...
function Sell:GetCurrentAuctions(force)
	...
	local needRescan = not force and (auctionRecord.lastScanTime == nil or auctionRecord.lastScanTime + cacheLifetime < GetServerTime())
...
function Sell:UpdateItemQtyText()
	...
	sellTab.itemQty:SetText(format(L['Qty: %d, Max Stacks: %d, Remaining: %d'], self.selectedItem.count, maxStacks, remainingQty))
end
...
function Sell:CalculatePrice(itemRecord, auctionRecord)
	...
	local lowestBid, lowestBuy, fail, message =
...
function Sell:UpdateSellTabAuctions(itemRecord, auctionRecord)
	...
	if auctionRecord.lastScanTime then
		self.sellTab.lastScan:SetText(format(L['Last scan: %s'], AuctionFaster:FormatDuration(GetServerTime() - auctionRecord.lastScanTime)))
	end
...
function Sell:SellCurrentItem(singleStack)
	...
	local success, multisell = Auctions:SellItem(sellSettings.bidPerItem * sellSettings.stackSize, sellSettings.buyPerItem * sellSettings.stackSize, sellSettings.duration, sellSettings.stackSize, maxStacks)
...
function Sell:CheckEverythingSold()
	...
	StdUi:Confirm(L['Incomplete sell'], format(L['You still have %d of %s Do you wish to sell rest?'], qtyLeft, itemLink), buttons, 'incomplete_sell')
