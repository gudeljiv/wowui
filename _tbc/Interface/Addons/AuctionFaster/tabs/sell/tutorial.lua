...
function Sell:InitTutorial(force)
	...
	if not self.tutorials then
		...
		self.tutorials = {
			...
			{
				text = L['Your bid price '] .. C(L['per one item.'], red) .. '\n\n' .. C(L['AuctionFaster understands a lot of money formats'], green) .. L[', for example:\n\n' .. '5g 6s 19c\n5g6s86c\n999s 50c\n3000s\n9000000c'],
				anchor = 'LEFT',
				parent = sellTab.bidPerItem
			},
			{
				text = L['Your buyout price '] .. C(L['per one item.'], red) .. L[' Same money formats as bid per item.'],
				anchor = 'LEFT',
			...
			{
				text = format(L['Posts %s of selected item regardless of your\n"# Stacks" settings'], C(L['one auction'], red)),
				anchor = 'RIGHT',
				parent = sellTab.buttons.postOneButton
			},
			{
				text = format(L['Posts %s of selected item according to your\n"# Stacks" settings'], C(L['all auctions'], red)),
				anchor = 'RIGHT',
