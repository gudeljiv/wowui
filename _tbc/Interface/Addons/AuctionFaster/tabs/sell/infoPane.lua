...
function Sell:DrawInfoPane()
	...
	infoPane:SetScript(
		'OnShow',
		function()
			AuctionFaster.db.infoPaneOpened = true
		end
	)
	infoPane:SetScript(
		'OnHide',
		function()
			AuctionFaster.db.infoPaneOpened = false
		end
	)
...
function Sell:UpdateInfoPaneText()
	...
	local deposit = Auctions:CalculateDeposit(self.selectedItem.itemId, self.selectedItem.itemName, self.selectedItem.quality, self.selectedItem.level, sellSettings)
...
function Sell:ShowHistoricalWindow(historicalData)
	...
	if not self.historicalWindow then
		...
		g = Graph:CreateGraphLine(nil, historicalWindow, 'TOPLEFT', 'TOPLEFT', 100, -160, graphWidth, graphHeight)
