...
function Buy:InitTutorial(force)
	...
	if not self.tutorials then
		...
		self.tutorials = {
			...
			{
				text = L['Adds all auctions to the queue that has at least the amount of quantity entered'] .. L[' in the box on the right'],
				anchor = 'LEFT',
				parent = buyTab.addWithXButton
			},
			{
				text = L['Finds the first auction '] .. C(L['across all the pages'], red) .. L[' that meets the minimum quantity\n\n'] .. C(L['You need to enter a search query first'], orange),
				anchor = 'LEFT',
