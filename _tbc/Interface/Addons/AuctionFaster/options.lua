...
function AuctionFaster:RegisterHistoricalOptionWindow()
	...
	if self.db.historical.enabled then
		enabled:SetChecked(true)
	end

	enabled.OnValueChanged = function(_, flag)
		self.db.historical.enabled = flag
	end
	keepDays.OnValueChanged = function(_, value)
		self.db.historical.keepDays = value
	end
...
function AuctionFaster:RegisterPricingOptionWindow()
	...
	maxBidDeviation.OnValueChanged = function(_, value)
		self.db.pricing.maxBidDeviation = value
	end
...
function AuctionFaster:RegisterOptionWindow()
	...
	if self.db.fastMode then
		fastMode:SetChecked(true)
	end
	if self.db.enableToolTips then
		enableToolTips:SetChecked(true)
	end

	enabled.OnValueChanged = function(_, flag)
		AuctionFaster.db.enabled = flag
	end
	fastMode.OnValueChanged = function(_, flag)
		AuctionFaster.db.fastMode = flag
	end
	enableToolTips.OnValueChanged = function(_, flag)
		AuctionFaster.db.enableToolTips = flag
	end
	...
	auctionDuration.OnValueChanged = function(_, value)
		AuctionFaster.db.auctionDuration = value
	end
	defaultTab.OnValueChanged = function(_, value)
		AuctionFaster.db.defaultTab = value
	end

	wipeSettings:SetScript(
		'OnClick',
		function()
	...
	if self.db.buy.tooltips.enabled then
		buyTooltips:SetChecked(true)
	end
	if self.db.sell.tooltips.enabled then
		sellTooltips:SetChecked(true)
	end
	if self.db.sell.tooltips.itemEnabled then
		sellItemTooltips:SetChecked(true)
	end

	buyTooltipAnchor.OnValueChanged = function(_, value)
		self.db.buy.tooltips.anchor = value
	end
	sellTooltipAnchor.OnValueChanged = function(_, value)
		self.db.sell.tooltips.anchor = value
	end
	sellItemTooltipAnchor.OnValueChanged = function(_, value)
		self.db.sell.tooltips.itemAnchor = value
	end

	buyTooltips.OnValueChanged = function(_, flag)
		self.db.buy.tooltips.enabled = flag
	end
	sellTooltips.OnValueChanged = function(_, flag)
		self.db.sell.tooltips.enabled = flag
	end
	sellItemTooltips.OnValueChanged = function(_, flag)
		self.db.sell.tooltips.itemEnabled = flag
	end
