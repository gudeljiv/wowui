...
function Inventory:AddItemToInventory(itemId, count, link, bag, slot)
	...
	if itemId == battlePetId then
		...
		for i = 1, n do
			...
			if line then
				canSell =
					not (strfind(line, ITEM_BIND_ON_PICKUP) or strfind(line, ITEM_BIND_TO_BNETACCOUNT) or strfind(line, ITEM_BNETACCOUNTBOUND) or strfind(line, ITEM_SOULBOUND) or strfind(line, ITEM_BIND_QUEST) or
		...
		for slot = 1, GetContainerNumSlots(bag) do
			...
			if id then
				...
				if id == battlePetId then
					...
					if id == itemId and name == itemName and breedQuality == itemQuality and petLevel == itemLevel then
						return bag, slot
					end
				else
					local name, _, quality, _, _, _, _, itemStackCount = GetItemInfo(link)
					local level = GetDetailedItemLevelInfo(link)
					if id == itemId and name == itemName and quality == itemQuality and level == itemLevel then
						return bag, slot
					end
				end
			end
