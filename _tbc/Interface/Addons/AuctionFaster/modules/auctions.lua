...
function Auctions:FindAuctionIndex(auctionData, randomizeIndex)
	...
	for index = 1, 50 do
		...
		if listData.name == auctionData.name and listData.itemId == auctionData.itemId and --listData.owner == auctionData.owner and -- actually there is no need to check owner
				listData.bid == auctionData.bid and listData.buy == auctionData.buy and listData.count == auctionData.count then
...
function Auctions:SellItem(bid, buy, duration, stackSize, numStacks)
	...
	AuctionFaster:Echo(
		1,
		format(L['Posting: %s for:\nper auction: %s\nper item: %s\n# stacks: %d stack size: %d'], self.lastSoldItem, StdUi.Util.formatMoney(buy), StdUi.Util.formatMoney(buy / stackSize), numStacks, stackSize)
	)
