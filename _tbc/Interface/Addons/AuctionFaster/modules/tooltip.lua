		...
		if BPBID_Arrays.BasePetStats[speciesID] then
			...
			if BPBID_Options.Breedtip.CurrentStats and tblBreedID then
				for i = 1, #tblBreedID do
					...
					local currentStats = format('\124cFFD4A017Breed %s*:\124r %d/%d/%d', internal.RetrieveBreedName(currentBreed), (stats[1] + breedStats[1]), (stats[2] + breedStats[2]), (stats[3] + breedStats[3]))
			...
			if BPBID_Options.Breedtip.AllStats and breeds then
				if not BPBID_Options.Breedtip.CurrentStats or not extraBreeds then
					for i = 1, #breeds do
						...
						local allStatsP2 = format(':\124r %d/%d/%d', (stats[1] + breedStats[1]), (stats[2] + breedStats[2]), (stats[3] + breedStats[3]))
					...
					for i = 1, 10 do
						if (extraBreeds[i]) and (extraBreeds[i] > 2) then
							...
							local allStats = format('\124cFFD4A017Breed %s:\124r %d/%d/%d', internal.RetrieveBreedName(currentBreed), (stats[1] + breedStats[1]), (stats[2] + breedStats[2]), (stats[3] + breedStats[3]))
			...
			if BPBID_Options.Breedtip.AllStats25 and breeds then
				...
				if (rarity == 4 and BPBID_Options.Breedtip.CurrentStats25Rare ~= BPBID_Options.Breedtip.AllStats25Rare) or not BPBID_Options.Breedtip.CurrentStats25 or not extraBreeds then
					for i = 1, #breeds do
						...
						local asterisk = not extraBreeds or (extraBreeds[currentBreed - 2] and extraBreeds[currentBreed - 2] > 2)
