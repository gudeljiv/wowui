...
ttHyperlink:SetScript("OnEvent", function(self, event, ...)
	...
	if supportedHyperLinks[linkToken] then
		...
		elseif linkToken == "transmogset" then -- WardrobeSetsTransmogModelMixin:RefreshTooltip() in "Blizzard_Collections/Blizzard_Wardrobe.lua"
			...
			if waitingOnQuality then
				...
				local setQuality = (numTotalSlots > 0 and totalQuality > 0) and Round(totalQuality / numTotalSlots)
		...
		elseif linkToken == "dungeonScore" then -- GetDungeonScoreLink() + DisplayDungeonScoreLink() in "ItemRef.lua"
			...
			GameTooltip_AddColoredLine(
				gtt,
				DUNGEON_SCORE_LINK_LEVEL_CLASS_FORMAT_STRING:format(playerLevel, className),
			...
			for i = 1, #sortTable do
				...
				GameTooltip_AddColoredDoubleLine(
					gtt,
					DUNGEON_SCORE_LINK_TEXT1:format(sortTable[i].mapName),
					(
						sortTable[i].level > 0 and DUNGEON_SCORE_LINK_TEXT2:format(sortTable[i].level)
						or DUNGEON_SCORE_LINK_NO_SCORE
					),
					NORMAL_FONT_COLOR,
					textColor
				)
