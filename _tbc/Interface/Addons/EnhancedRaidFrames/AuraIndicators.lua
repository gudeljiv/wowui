...
function EnhancedRaidFrames:CreateIndicators(frame)
	...
	for i = 1, 9 do
		...
		indicatorFrame:SetScript(
			'OnEnter',
			function()
				self:Tooltip_OnEnter(indicatorFrame)
			end
		)
		indicatorFrame:SetScript(
			'OnLeave',
...
function EnhancedRaidFrames:UpdateIndicators(frame, setAppearance)
	...
	if not frame.unit or string.find(frame.unit, 'target') or string.find(frame.unit, 'nameplate') or string.find(frame.unit, 'pet') or not CompactRaidFrameContainer:IsShown() then
		return
	end
...
function EnhancedRaidFrames:ProcessIndicator(indicatorFrame, unit)
	...
	if foundAura and UnitIsConnected(unit) and not self.db.profile[i].missingOnly and (not self.db.profile[i].mineOnly or (self.db.profile[i].mineOnly and castBy == 'player')) then
		...
		if icon and self.db.profile[i].showIcon then
			...
			indicatorFrame.Icon:SetColorTexture(self.db.profile[i].indicatorColor.r, self.db.profile[i].indicatorColor.g, self.db.profile[i].indicatorColor.b, self.db.profile[i].indicatorColor.a)
		...
		indicatorFrame.Text:SetTextColor(self.db.profile[i].textColor.r, self.db.profile[i].textColor.g, self.db.profile[i].textColor.b, self.db.profile[i].textColor.a)
