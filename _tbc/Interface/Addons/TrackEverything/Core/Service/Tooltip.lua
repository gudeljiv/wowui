...
function Tooltip:OnInitialize()
	GameTooltip:SetScript(
		...
		function()
			...
			if self:CanUpdateMinimapTooltip() or self:CanUpdateWorldMapTooltip() then
				self:RedrawTooltip()
			end
		end
	)
	...
	GameTooltip:HookScript(
		'OnHide',
		function()
			-- reseting GameTooltip becouse his default formating changing in RedrawTooltip() function
	...
	GameTooltip:HookScript(
		'OnTooltipCleared',
		function()
			--handle GameTooltip updates. OnTooltipCleared event fires every time GameTooltip redrawn
			...
			if self:CanUpdateWorldTooltip() then
				self:ScheduleTimer(
					...
					0,
					00000001
				) -- need to wait before new GameTooltip drawn, becouse it apears not instantly after OnTooltipCleared event
			end
		end
	)
end
...
function Tooltip:ModifyTooltip()
	...
	if not self:IsProfessionInTooltip() then
		return
	end
...
function Tooltip:GetTooltipProfession()
	...
	for profession, lookups in pairs(professionLookups) do
		for _, lookup in ipairs(lookups) do
			for i = 1, GameTooltip:NumLines() do
				if _G['GameTooltipTextLeft' .. i]:GetText() == lookup then
					return profession, lookup, i
				end
			end
		end
	end
...
function Tooltip:IsProfessionInTooltip()
	if self:GetTooltipProfession() then
		return true
	end
...
function Tooltip:HasMatches()
	for i = 1, GameTooltip:NumLines() do
		...
		for word in rowText:gmatch('[^\n]+') do
			if GatheringInfo:IsProfessionItemName(word) then
				return true
			end
		end
	end
