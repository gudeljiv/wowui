...
function SpellSwitcher:OnEnable()
	if TE.db.profile.autoTracking.spellSwitcher.enabled and not TE.db.profile.autoTracking.spellSwitcher.onmove then
		self.trackingTimer =
			...
			2
		) -- start timer after 2 cesonds of pause. It helps to avoid GCD after reloading.
	end
end

function SpellSwitcher:OnDisable()
	if private.Sound_EnableSFX_default ~= GetCVar('Sound_EnableSFX') then
		SetCVar('Sound_EnableSFX', private.Sound_EnableSFX_default)
	end
end
...
function SpellSwitcher:GetCurrentTrackingSpellID()
	for i = 1, GetNumTrackingTypes() do
		...
		if category == 'spell' and active == true then
			return spellId
		end
	end
end
...
function SpellSwitcher:GetNextSpellID(currentSpellId)
	...
	if trackingSpellIDs then
		...
		if currentSpellId == nextSpellId then
			nextSpellId = nil
		end
	end
...
function SpellSwitcher:CanCast()
	local canCast =
		private.IS_PAUSED and not (CastingInfo() or ChannelInfo() or UnitIsDeadOrGhost('player') or IsResting()) and (TE.db.profile.autoTracking.spellSwitcher.forceInCombat or not UnitAffectingCombat('player')) and
		(not TE.db.profile.autoTracking.spellSwitcher.onmove or private.IS_MOVING) and
		(not TE.db.profile.autoTracking.spellSwitcher.onmount or (IsMounted() or self:IsFlyingForm()))

	Log:PrintD('canCast = ', canCast)
	Log:PrintfD(
		'forceInCombat = [%s], InCombatLockdown = [%s] , UnitAffectingCombat = [%s] , IsMounted or Flying = [%s] ',
		tostring(TE.db.profile.autoTracking.spellSwitcher.forceInCombat),
		tostring(InCombatLockdown()),
		tostring(UnitAffectingCombat('player')),
		tostring(IsMounted() or self:IsFlyingForm())
	)
	...
	if TE.db.profile.autoTracking.spellSwitcher.enabled then
		...
		elseif private.GAME_EVENTS.TEMP_PAUSE_EVENTS[event] then
			...
			elseif event == 'UNIT_SPELLCAST_SENT' then
				if arg1 ~= 'player' or MyLib.IndexOf(arg4, Settings:GetTrackingIDs()) then
					return
				end -- return if cast made not by player or player casted trackingSpell
			end
...
function SpellSwitcher:IsFlyingForm()
	...
	if PLAYER_CLASS == 'DRUID' then
		for i = 1, GetNumShapeshiftForms() do
			...
			if FLYING_FORM_SPELL_IDS[spellId] and active then
				return true
			end
		end
	end
