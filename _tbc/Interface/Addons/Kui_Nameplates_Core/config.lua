...
LSM:Register(LSM.MediaType.FONT, 'Roboto Condensed Bold', kui.m.f.roboto, LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
...
local DEFAULT_FONT = font_support and 'Roboto Condensed Bold' or LSM:GetDefault(LSM.MediaType.FONT)
...
local function UpdateClickboxSize()
	if kui.CLASSIC then
		return
	end -- XXX functions exist, but break display
	...
	if addon.USE_BLIZZARD_PERSONAL then
		...
		C_NamePlate.SetNamePlateSelfSize(core.profile.frame_width_personal - x_pad, 45)
	else
...
local function configChangedCombatAction()
	if core.profile.combat_hostile == 1 and core.profile.combat_friendly == 1 then
...
local function UpdateCVars()
	...
	elseif GetCVar('nameplateMinScale') == '1' and GetCVar('nameplateMaxScale') == '1' and GetCVar('nameplateLargerScale') == '1' and GetCVar('nameplateSelectedScale') == '1' and GetCVar('nameplateSelfScale') == '1' then
	...
	elseif GetCVar('nameplateMinAlpha') == '1' and GetCVar('nameplateMaxAlpha') == '1' and GetCVar('nameplateSelectedAlpha') == '1' then
		-- reset to defaults
...
function core:InitialiseConfig()
	...
	if KuiNameplatesCoreSaved then
		...
		if not KuiNameplatesCoreSaved['226_TARGET_SIZE'] then
			...
			for _, profile in pairs(KuiNameplatesCoreSaved.profiles) do
				if (profile.frame_target_size or profile.frame_target_size == nil) and ((profile.frame_width and profile.frame_width > (profile.frame_width_target or default_config.frame_width_target)) or (profile.frame_height and profile.frame_height > (profile.frame_height_target or default_config.frame_height_target))) then
