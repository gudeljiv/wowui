...
function TestMode:ToggleCastbarMovable(unitID)
	...
	if unitID == "arena-testmode" and not C_AddOns.IsAddOnLoaded("Blizzard_ArenaUI") then
	...
	if self.isTesting[unitID] then
		...
		if self:SetCastbarMovable(unitID) then
			...
			if
				(ClassicCastbars.db.nameplate.enabled and unitID == "nameplate-testmode")
				or (ClassicCastbars.db.target.enabled and unitID == "target")
			then
...
function TestMode:SetCastbarMovable(unitID, parent)
	...
	if unitID == "party-testmode" or unitID == "arena-testmode" then
		if unitID == "arena-testmode" then
			TestMode:ToggleArenaContainer(true)
		end
		if unitID == "party-testmode" then
			TestMode:TogglePartyContainer(true)
		end
...
function TestMode:ReanchorOnNameplateTargetSwitch()
	if not ClassicCastbars.db.nameplate.enabled then
		return
	end
