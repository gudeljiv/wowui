...
if MultiBarBottomRight and not MultiBarRight then
end

-- TODO: remove legacy vanilla frames no longer present in classic and frames from later game versions
...
function Jostle:Refresh(...)
    if not fullyInitted then
        ChocolateBar:Debug("Refresh Jostle  not fullyInitted")
        return
    end
    ChocolateBar:Debug("Refresh Jostle 1")
    if frame == MultiBarBottomLeft then
        ChocolateBar:Debug("MultiBarBottomLeft 1")
    end
    ...
    --ChocolateBar:Debug("Refresh Jostle 2")
    if frame == MultiBarBottomLeft then
        ChocolateBar:Debug("MultiBarBottomLeft 2")
    end
    ...
    for _, frame in ipairs(frames) do
        ...
        --ChocolateBar:Debug("Refresh Jostle 5")
        if frame == MultiBarBottomLeft then
            ChocolateBar:Debug("MultiBarBottomLeft 5")
        end

        if frame and frame.isInDefaultPosition then
            ChocolateBar:Debug("Frame is in default position: ",
                frame and frame.getName and frame:GetName() or "", frameName)
            --return
        end
        ...
        if frame and not blizzardFramesData[frame] and frame.GetTop and frame:GetCenter() and select(2, frame:GetCenter()) then
            --ChocolateBar:Debug("Refresh Jostle 6")
            if frame == MultiBarBottomLeft then
                ChocolateBar:Debug("MultiBarBottomLeft 6")
            end
    ...
    for _, frame in ipairs(frames) do
        --ChocolateBar:Debug("Refresh Jostle 7 main loop ")
        if frame == MultiBarBottomLeft then
            ChocolateBar:Debug("MultiBarBottomLeft 7")
        end
        if type(frame) == "string" then
            frame = _G[frame]
        end
        ...
        if frame then
            --ChocolateBar:Debug("Refresh Jostle 8 big gate start")
            if frame == MultiBarBottomLeft then
                ChocolateBar:Debug("MultiBarBottomLeft 8 (if frame)")
            end
            ...
            if frame.systemInfo and not frame.systemInfo.isInDefaultPosition then
                -- do nothing
            ...
            elseif frame == DurabilityFrame and DurabilityFrame:IsShown() and (DurabilityFrame:GetLeft() > GetScreenWidth() or DurabilityFrame:GetRight() < 0 or DurabilityFrame:GetBottom() > GetScreenHeight() or DurabilityFrame:GetTop() < 0) then
                ...
                if frame == MultiBarBottomLeft then
                    ChocolateBar:Debug("MultiBarBottomLeft 15.2")
                end
            elseif frame == FramerateLabel and ((frameData.lastX and not isClose(frameData.lastX, frame:GetLeft())) or not isClose(WorldFrame:GetHeight() * WorldFrame:GetScale(), UIParent:GetHeight() * UIParent:GetScale())) then
                -- do nothing
                if frame == MultiBarBottomLeft then
                    ChocolateBar:Debug("MultiBarBottomLeft 16")
                end
            elseif frame == PlayerFrame or frame == MainMenuBar or frame == MinimapCluster or frame == MultiBarBottomLeft or frame == TargetFrame or frame == ConsolidatedBuffs or frame == BuffFrame or frame == CastingBarFrame or frame == TutorialFrameParent or frame == FramerateLabel or frame == DurabilityFrame or frame == WatchFrame or not (frameData and frameData.lastScale and frame.GetScale and frameData.lastScale == frame:GetScale()) or not (frameData.lastX and frameData.lastY and (not isClose(frameData.lastX, frame:GetLeft()) or not isClose(frameData.lastY, frame:GetTop()))) then
                local anchor
                local anchorAlt
                local width, height = GetScreenWidth(), GetScreenHeight()
                local x

                --ChocolateBar:Debug("Refresh Jostle 9 starting anchor")
                if frame == MultiBarBottomLeft then
                    ChocolateBar:Debug("MultiBarBottomLeft 17 starting anchor")
                end
                if frame == MultiBarBottomLeft then
                    ChocolateBar:Debug("MainMenuBar")
                end
