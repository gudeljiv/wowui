...
local UnitName, GetRealmName, UnitRace, UnitFactionGroup, IsInRaid = UnitName, GetRealmName, UnitRace, UnitFactionGroup, IsInRaid
...
local UnitIsUnit, GetRaidRosterInfo, GetSpecialization, UnitInVehicle, UnitHasVehicleUI = UnitIsUnit, GetRaidRosterInfo, GetSpecialization, UnitInVehicle, UnitHasVehicleUI
...
local function CreateTalentCache()
  ...
  elseif WeakAuras.IsTBCOrWrathOrMists() then
    -- unused
  else
...
loadedFrame:SetScript("OnEvent", function(self, event, ...)
  if event == "ADDON_LOADED" then
    if ... == ADDON_NAME then
      ...
      elseif
        db.lastArchiveClear < time() - 2505600 --[[29 days]]
      then
        db.lastArchiveClear = time()
    ...
    if event == "PLAYER_ENTERING_WORLD" then
      ...
      callback = function()
        ...
        if remainingSquelch > 0 then
          ...
          end, remainingSquelch) -- No sounds while loading
        end
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
  ...
  if event == "PET_BATTLE_CLOSE" and C_PetBattles.IsInBattle() then
    return
  end
  ...
  for id, data in pairs(db.displays) do
    if data.parent then
      ...
      elseif not db.displays[data.parent].controlledChildren then
        if not silent then
          prettyPrint("Detected corruption in saved variables: " .. id .. " thinks " .. data.parent .. " controls it, but " .. data.parent .. " is not a group.")
        end
        data.parent = nil
      else
        children[id] = data
    ...
    if #order > 0 then
      Private.Threads:Add(
        ...
        coroutine.create(function()
          prettyPrint(
            L["WeakAuras is creating a rollback snapshot of your auras. This snapshot will allow you to revert to the current state of your auras if something goes wrong. This process may cause your framerate to drop until it is complete."]
          )
...
local function customOptionIsValid(option)
  ...
  elseif Private.author_option_classes[option.type] == "simple" then
    if not option.key or not option.name or not option.default == nil then
      return false
    end
  elseif Private.author_option_classes[option.type] == "group" then
    if not option.key or not option.name or not option.default == nil or not option.subOptions then
      return false
    end
  end
  return true
...
function Private.UpdateSoundIcon(data)
  ...
  -- tts
  if (data.actions.start.do_message and data.actions.start.message_type == "TTS") or (data.actions.finish.do_message and data.actions.finish.message_type == "TTS") then
  ...
  local function frame_monitor_callback(event, frame, unit, previousUnit)
    ...
    if type(glow_frame_monitor) == "table" then
      for region, data in pairs(glow_frame_monitor) do
        if
          region.state and type(region.state.unit) == "string" and UnitIsUnit(region.state.unit, unit) and ((data.frame ~= frame) and (FRAME_UNIT_ADDED or FRAME_UNIT_UPDATE))
    ...
    for regionData, data_frame in pairs(Private.dyngroup_unitframe_monitor) do
      if
        regionData.region.state
          and type(regionData.region.state.unit) == "string"
          and UnitIsUnit(regionData.region.state.unit, unit)
  ...
  then
    ...
    if should_glow_frame then
      ...
      if actions.glow_action == "show" then
        -- remove previous glow
        if glow_frame then
          if region.active_glows_hidefunc and region.active_glows_hidefunc[glow_frame] then
      ...
      elseif actions.glow_action == "hide" and glow_frame and region.active_glows_hidefunc and region.active_glows_hidefunc[glow_frame] then
        region.active_glows_hidefunc[glow_frame]()
      ...
      for triggernum, triggerData in ipairs(triggers) do
        for id, state in pairs(triggerData) do
          if state.progressType == "timed" then
            ...
            if expirationTime and type(expirationTime) == "number" and expirationTime < t and duration and type(duration) == "number" and duration > 0 then
...
local function startStopTimers(id, cloneId, triggernum, state)
  ...
  if record.expirationTime ~= expirationTime or record.state ~= state then
    ...
    if expirationTime and type(expirationTime) == "number" then
      record.handle = timer:ScheduleTimerFixed(function()
...
local function ValueForSymbol(symbol, region, customCache, regionState, regionStates, useHiddenStates, formatters)
  ...
  elseif regionState[symbol] then
    ...
    local value = (useHiddenStates or regionState.show) and ReplaceValuePlaceHolders(symbol, region, customCache, regionState, formatters[symbol], regionState.triggernum)
    return value or ""
  end
end
...
function Private.AnchorFrame(data, region, parent, force)
  if data.anchorFrameType == "CUSTOM" and (data.regionType == "group" or data.regionType == "dynamicgroup") and not WeakAuras.IsLoginFinished() and not force then
    ...
    if data.anchorFrameParent or data.anchorFrameParent == nil or data.anchorFrameType == "SCREEN" or data.anchorFrameType == "UIPARENT" or data.anchorFrameType == "MOUSE" then
...
do
  ...
  function WeakAuras.IsUntrackableSoftTarget(unit)
    if not Private.soft_target_cvars[unit] then
      return
    end
...
function Private.ExecEnv.ParseZoneCheck(input)
  ...
  local matcher = {
    ...
    CheckBoth = function(self, zoneId, zonegroupId, instanceId, minimapZoneText)
      return self:CheckPositive(zoneId, zonegroupId, instanceId, minimapZoneText) and self:CheckNegative(zoneId, zonegroupId, instanceId, minimapZoneText)
    end,
    ...
    CheckNegative = function(self, zoneId, zonegroupId, instanceId, minimapZoneText)
      return not (self.negZoneIds[zoneId] or self.negZoneGroupIds[zonegroupId] or (instanceId and self.negInstanceIds[instanceId]) or self.negAreaNames[minimapZoneText])
    end,
