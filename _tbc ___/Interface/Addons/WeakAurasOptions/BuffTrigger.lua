...
local function GetBuffTriggerOptions(data, triggernum)
	...
	local aura_options = {
		...
		useName = {
			...
			order = 10,
			hidden = function()
				return not (trigger.type == 'aura' and not trigger.fullscan and trigger.unit ~= 'multi')
			end,
			disabled = true,
			get = function()
				return true
		...
		use_tooltip = {
			...
			order = 13,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.fullscan and trigger.unit ~= 'multi')
			end,
			set = noop
		},
		tooltip_operator = {
		...
		use_stealable = {
			...
			name = function(input)
				...
				elseif (value == false) then
					return '|cFFFF0000 ' .. L['Negator'] .. ' ' .. L['Stealable']
				else
					return '|cFF00FF00' .. L['Stealable']
				end
			end,
			order = 16,
			hidden = function()
		...
		spellId = {
			...
			order = 18,
			disabled = function()
				return not trigger.use_spellId
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.fullscan and trigger.unit ~= 'multi')
			end,
		...
		debuffClass = {
			...
			order = 20,
			disabled = function()
				return not trigger.use_debuffClass
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.fullscan)
			end,
		...
		multiicon = {
			...
			order = 11,
			disabled = function()
				return not trigger.name and spellCache.GetIcon(trigger.name)
			end,
			hidden = function()
				return not (trigger.type == 'aura' and not trigger.fullscan and trigger.unit == 'multi')
			end,
		...
		multiname = {
			...
			get = function(info)
				return trigger.spellId and tostring(trigger.spellId) or trigger.name
			end,
			set = noop
		},
		name1icon = {
			...
			width = WeakAuras.halfWidth,
			name = function()
				return getAuraMatchesLabel(trigger.names[1])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[1])
			end,
			image = function()
			...
			order = 11,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[1])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and not trigger.fullscan and trigger.unit ~= 'multi')
			end,
		...
		name1 = {
			...
			get = function(info)
				return trigger.spellIds[1] and tostring(trigger.spellIds[1]) or trigger.names[1]
			end,
			set = noop
		},
		name2space = {
			...
			order = 13,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[1] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name2icon = {
			type = 'execute',
			width = WeakAuras.halfWidth,
			name = function()
				return getAuraMatchesLabel(trigger.names[2])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[2])
			end,
			image = function()
			...
			order = 14,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[2])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[1] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		...
		name2 = {
			...
			get = function(info)
				return trigger.spellIds[2] and tostring(trigger.spellIds[2]) or trigger.names[2]
			end,
			set = noop
		},
		name3space = {
			...
			order = 16,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[2] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name3icon = {
			type = 'execute',
			width = WeakAuras.halfWidth,
			name = function()
				return getAuraMatchesLabel(trigger.names[3])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[3])
			end,
			image = function()
			...
			order = 17,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[3])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[2] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		...
		name3 = {
			...
			get = function(info)
				return trigger.spellIds[3] and tostring(trigger.spellIds[3]) or trigger.names[3]
			end,
			set = noop
		},
		name4space = {
			...
			order = 19,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[3] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name4icon = {
			type = 'execute',
			width = WeakAuras.halfWidth,
			name = function()
				return getAuraMatchesLabel(trigger.names[4])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[4])
			end,
			image = function()
			...
			order = 20,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[4])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[3] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		...
		name4 = {
			...
			get = function(info)
				return trigger.spellIds[4] and tostring(trigger.spellIds[4]) or trigger.names[4]
			end,
			set = noop
		},
		name5space = {
			...
			order = 22,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[5])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[4] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name5icon = {
			type = 'execute',
			width = WeakAuras.halfWidth,
			name = function()
				return getAuraMatchesLabel(trigger.names[5])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[5])
			end,
			image = function()
		...
		name5 = {
			...
			get = function(info)
				return trigger.spellIds[5] and tostring(trigger.spellIds[5]) or trigger.names[5]
			end,
			set = noop
		},
		name6space = {
			...
			order = 25,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[5] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name6icon = {
			type = 'execute',
			name = function()
				return getAuraMatchesLabel(trigger.names[6])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[6])
			end,
			width = WeakAuras.halfWidth,
			image = function()
			...
			order = 26,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[6])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[5] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		...
		name6 = {
			...
			get = function(info)
				return trigger.spellIds[6] and tostring(trigger.spellIds[6]) or trigger.names[6]
			end,
			set = noop
		},
		name7space = {
			...
			order = 28,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[6] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name7icon = {
			type = 'execute',
			name = function()
				return getAuraMatchesLabel(trigger.names[7])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[7])
			end,
			width = WeakAuras.halfWidth,
			image = function()
			...
			order = 29,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[7])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[6] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		...
		name7 = {
			...
			get = function(info)
				return trigger.spellIds[7] and tostring(trigger.spellIds[7]) or trigger.names[7]
			end,
			set = noop
		},
		name8space = {
			...
			order = 31,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[7] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name8icon = {
			type = 'execute',
			name = function()
				return getAuraMatchesLabel(trigger.names[8])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[8])
			end,
			width = WeakAuras.halfWidth,
			image = function()
			...
			order = 32,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[8])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[7] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		...
		name8 = {
			...
			get = function(info)
				return trigger.spellIds[8] and tostring(trigger.spellIds[8]) or trigger.names[8]
			end,
			set = noop
		},
		name9space = {
			...
			order = 34,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[8] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		},
		name9icon = {
			type = 'execute',
			name = function()
				return getAuraMatchesLabel(trigger.names[9])
			end,
			desc = function()
				return getAuraMatchesList(trigger.names[9])
			end,
			width = WeakAuras.halfWidth,
			image = function()
			...
			order = 35,
			disabled = function()
				return not spellCache.GetIcon(trigger.names[9])
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.names[8] and not trigger.fullscan and trigger.unit ~= 'multi')
			end
		...
		name9 = {
			...
			get = function(info)
				return trigger.spellIds[9] and tostring(trigger.spellIds[9]) or trigger.names[9]
			end,
			set = noop
		},
		useUnit = {
		...
		useSpecificUnit = {
			...
			disabled = true,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.unit == 'member')
			end,
			get = function()
				return true
			end,
		...
		useGroup_count = {
			...
			disabled = true,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.unit == 'group')
			end,
			get = function()
				return true
			end,
		...
		group_role = {
			...
			disabled = function()
				return not trigger.useGroupRole
			end,
			get = function()
				return trigger.group_role
			end,
			order = 47.2,
			set = noop
		},
		...
		name_info = {
			...
			order = 47.5,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.unit == 'group' and not trigger.groupclone)
			end,
			disabled = function()
				return not CanShowNameInfo(data)
			end,
			get = function()
		...
		stack_info = {
			...
			disabled = function()
				return not CanShowStackInfo(data)
			end,
			get = function()
		...
		subcountCount = {
			...
			hidden = function()
				return not (trigger.type == 'aura' and trigger.fullscan and trigger.subcount)
			end,
			order = 55.5,
			set = noop
		},
		useRem = {
			...
			hidden = function()
				return not (trigger.type == 'aura' and trigger.unit ~= 'multi')
			end,
			order = 56,
			set = noop
		},
		...
		countOperator = {
			...
			values = OptionsPrivate.Private.operator_types,
			disabled = function()
				return not trigger.useCount
			end,
			hidden = function()
				return not (trigger.type == 'aura' and trigger.unit ~= 'multi')
			end,
			get = function()
				return trigger.useCount and trigger.countOperator or nil
			end,
			set = noop
		},
		count = {
			...
			get = function()
				return trigger.useCount and trigger.count or nil
			end,
			set = noop
		},
		ownOnly = {
			...
			desc = function()
				local value = trigger.ownOnly
				if (value == nil) then
					return L['Only match auras cast by the player']
				elseif (value == false) then
					return L['Only match auras cast by people other than the player']
				else
					return L['Only match auras cast by the player']
				end
			end,
