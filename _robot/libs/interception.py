...
class device():
    ...
    def _device_io_control(self, command, inbuffer, outbuffer) -> device_io_result:
        res = k32.DeviceIoControl(self.handle, command, inbuffer, len(bytes(inbuffer)) if inbuffer != 0 else 0, outbuffer, len(bytes(outbuffer)) if outbuffer != 0 else 0, self._bytes_returned, 0)
