...
def grab_screenshot(input, pixel=False):

    s = sct.grab(input)

    if pixel:
        return s.pixel(math.floor(c_width/2), math.floor(c_height/2))
    else:
        return s
...
# grabbing images from regions
            main_image = _thread.start_new_thread(grab_screenshot, (p_main,))
            offgcd_image = _thread.start_new_thread(grab_screenshot, (p_offgcd,))
            combat = _thread.start_new_thread(grab_screenshot, (p_combat, True,))
            interrupt = _thread.start_new_thread(grab_screenshot, (p_interrupt, True))
            behind = _thread.start_new_thread(grab_screenshot, (p_behind, True,))
            clss = _thread.start_new_thread(grab_screenshot, (p_clss, True))


# matching closest class color to define in colors
            found_class, hex = _thread.start_new_thread(get_class, (clss, color_distance,))
            if not found_class:
                continue
...
# loading skills for a class if changed
            if wow_class_loaded != wow_class:
                ...
                main_abilities = _thread.start_new_thread(load_skills_main, (wow_class,))
                secondary_abilities = _thread.start_new_thread(load_skills_secondary, (wow_class,))
                main_abilities = {**main_abilities, **healing, **global_skills}
                wow_class_loaded = wow_class
