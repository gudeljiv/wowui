...
function KT.Events.COMBAT_LOG_EVENT_UNFILTERED(self)
	...
	if not pass or d_id == nil or d_id == 0 then
		return
	end
...
function KT.Events.UPDATE_MOUSEOVER_UNIT(self, ...)
	if not self.Global.TOOLTIP then
		return
	end
...
function KT:AddKill(id, name)
	...
	if self.Global.ACHIEV_THRESHOLD <= 0 then
		return
	end
...
function KT:GetSessionStats()
	if not self.Session.Start then
		return 0, 0, 0
	end
	local now = time()
...
function KT:Delete(id, charOnly)
	id = tonumber(id)
	if not id then
		error(("Expected 'id' param to be number, got %s."):format(type(id)))
	end
	local found = false
	local name
