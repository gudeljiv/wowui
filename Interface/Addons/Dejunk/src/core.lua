...
EventManager:Once(
	...
	function()
		...
		_G.SLASH_DEJUNK2 = "/dj"
		_G.SlashCmdList.DEJUNK = function()
			UI:Toggle()
		end
	end
)
...
function Core:CanDejunk()
	...
	if ListHelper:IsParsing(Inclusions) or ListHelper:IsParsing(Exclusions) then
		return false, L.CANNOT_SELL_WHILE_LISTS_UPDATING:format(Inclusions.localeColored, Exclusions.localeColored)
	end
...
function Core:CanDestroy()
	...
	if ListHelper:IsParsing(Destroyables) or ListHelper:IsParsing(Undestroyables) then
		return false, L.CANNOT_DESTROY_WHILE_LISTS_UPDATING:format(Destroyables.localeColored, Undestroyables.localeColored)
	end
...
function Core:IsBusy()
	return Dejunker:IsDejunking() or Destroyer:IsDestroying() or ListHelper:IsParsing() or Confirmer:IsConfirming()
end
...
-- Frame
_G.CreateFrame("Frame"):SetScript(
	"OnUpdate",
	function(_, elapsed)
...
function Core:OnUpdate(elapsed)
	...
	if Repairer.OnUpdate then
		Repairer:OnUpdate(elapsed)
	end
