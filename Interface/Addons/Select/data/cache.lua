...
function s.cache:Disable(cache)
	...
	if wasEnabled then -- if it was on, then we need to unregister their event(s) and empty their cache
		if cache == "item" or cache == "equip" then
			...
			wipe(caches.item)
			for k, v in pairs(caches.equip) do
				wipe(v)
			end
		elseif cache == "spell" then
		...
		elseif cache == "mount" then
			...
			for k, v in pairs(caches.mount) do
				wipe(v)
			end
		elseif cache == "toy" then
...
--[[ toy cache ]]
if not isClassicClient then
	-- note for future me who keeps forgetting: don't take TOYS_UPDATED out. On a cold login, the toys aren't ready
	-- and a NEW_TOY_LEARNED isn't fired as the toys are added. Annoyingly, GetNumLearnedDisplayedToys() and
	-- GetNumToys() are the proper amounts when the toys aren't loaded, leaving only filtered results like
	-- GetNumFilteredToys() to grow; but this value can change as the player is playing and is awful as an indicator
	-- that toys are loaded. Solution: watch TOYS_UPDATED, throttle it slow, and don't look back.

	local toySearchString = "" -- there is no C_ToyBox.GetFilterString, so need to watch for it ourselves
	hooksecurefunc(
		C_ToyBox,
		"SetFilterString",
		function(text)
			toySearchString = text
		end
	)

	local numOwnedToys = 0 -- when this value changes, then it's time to update the toy cache

	local isToysUpdating = false

	-- when toys are filtered, not all are visible; this will expand filters, cache all collected toys, and restore filters
	function updateToyCache()
		if isToysUpdating then -- an update here triggered an event; ignore these
			return
		end
		isToysUpdating = true
		-- save filter settings; going to expand to show all toys and then restore filters
		...
		-- now change filters to show all toys
		C_ToyBox.SetCollectedShown(true)
		C_ToyBox.SetUncollectedShown(false) -- don't want to add uncollected toys
		C_ToyBox.SetAllSourceTypeFilters(true)
		C_ToyBox.SetUnusableShown(true)
		local oldSearchString = toySearchString
		C_ToyBox.SetFilterString("")
		-- next fill toyCache with itemIDs,name
		...
		s.timer:Start(DATA_LOAD_DELAY, stopUpdatingToys)
	end
end

function stopUpdatingToys()
	isToysUpdating = false
end

--[[ event handling ]]
s.cache.frame = CreateFrame("Frame")

s.cache.frame:SetScript(
	"OnEvent",
	function(self, event, ...)
...
if not isClassicClient then
	...
	-- this one is unavoidable (see comments before updateToyCache) so throttled slow
	function s.cache.frame:TOYS_UPDATED()
		if not isToysUpdating then
			s.timer:Start(DATA_LOAD_DELAY * 2, updateToyCache)
		end
	end
end
