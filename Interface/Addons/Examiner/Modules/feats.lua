...
local MAX_FEATS_OF_STRENGTH_QUERY = 20000 -- limits the Feats of Strength query feature
...
-- Dialog Func
local FeatsFilterOkayFunc = function(text)
	cfg.featsFilter = text
	mod:QueryFeats()
end

-- Options
mod:AddOption({var = "inspectFeats", default = true, label = "Request Achievement Data", tip = "Asks the server for achievement data when inspecting a player"})
mod:AddOption(
	{
		var = "featsSpecialTooltip",
		default = false,
		label = "Special Achievement Tooltip",
		tip = "Show a tooltip that displays a little more information than the default achievement tooltips\nNOTE: Recommended to keep disabled if using TipTacItemRef"
	}
)
...
function mod.MenuInit(parent, list)
	...
	list[1].text = "Filter"
	list[1].header = 1
	list[2].text = "Set Filter..."
	list[2].value = 1
	list[3].text = "Hide Complete"
	list[3].value = 2
	list[3].checked = cfg.featsHideComplete
	list[4].text = "Hide Incomplete"
	list[4].value = 3
	list[4].checked = cfg.featsHideIncomplete
	-- sort
	list[5].header = 1
	list[6].text = "Sort Method"
	list[6].header = 1
	for index, method in ipairs(featsSortMethods) do
		list[index + 6].text = "Sort by " .. method
		list[index + 6].value = method
		list[index + 6].checked = (cfg.featsSort == method)
	end
	-- sort: date
	local tbl = list[#list]
	tbl.text = "Sort by date"
	tbl.value = "date"
	tbl.checked = (cfg.featsSort == "date")
end
...
function UpdateShownItems(self)
	...
	-- Update
	FauxScrollFrame_Update(self, #feats, #buttons, BUTTON_HEIGHT)
...
local function AddFeatEntry(id, name, points, description, icon, reward)
	...
	if (not cfg.featsHideComplete or not friendCompleted) and (not cfg.featsHideIncomplete or friendCompleted) then
		...
		if (filter == "") or (name:upper():find(filter)) or (description:upper():find(filter)) or (reward and reward:upper():find(filter)) then
			...
			tbl.value = value
			tbl.completed = friendCompleted
			tbl.month = friendMonth
			tbl.day = friendDay
			tbl.year = friendYear
		end
	end
...
function mod:QueryFeats()
	...
	elseif (cfg.featsStub == STUB_FEATS) then
		...
		for i = 1, MAX_FEATS_OF_STRENGTH_QUERY do
...
-- InitFunc
local function FeatsDropDown_InitFunc(self, list)
	-- All Feats of Strength
	local tbl = list[#list + 1]
	tbl.text = "|cff00c0ffFeats of Strength Query"
	tbl.value = STUB_FEATS
	tbl.stub = STUB_FEATS
	-- Tracked Achievements
	local isPlayer = ex:ValidateUnit() and UnitIsUnit(ex.unit, "player")
	if (isPlayer) then
		local tbl = list[#list + 1]
		tbl.text = "|cff00c0ffTracked Achievements |cffc0c0c0(" .. GetNumTrackedAchievements() .. ")"
		tbl.value = STUB_TRACKED
		tbl.stub = STUB_TRACKED
	end
	-- Recent Achievements
	local tbl = list[#list + 1]
	tbl.text = "|cff00c0ffRecent Achievements"
	tbl.value = STUB_RECENT
	tbl.stub = STUB_RECENT
	-- Achievements
	local tbl = list[#list + 1]
	tbl.text = "All Achievements |cffc0c0c0(" .. GetComparisonCategoryNumAchievements(-1) .. "/" .. GetCategoryNumAchievements(-1) .. ")"
	tbl.value = -1
	tbl.stub = -1
	AddCategoryListEntries(GetCategoryList, list, -1, "|cff00ff00", true)
	-- Statistics
	local tbl = list[#list + 1]
	tbl.text = "All Statistics |cffc0c0c0(" .. GetCategoryNumAchievements(-2) .. ")"
	tbl.value = -2
	tbl.stub = -2
...
-- SelectValueFunc
local function FeatsDropDown_SelectValueFunc(self, entry)
	cfg.featsCat = entry.value
	cfg.featsStub = entry.stub
	mod:QueryFeats()
end

-- DropDown "Category" Text
local cat = mod.page:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall")
cat:SetPoint("TOPLEFT", 14, -47)
cat:SetText("Category:")

-- DropDown
catDropDown = AzDropDown:CreateDropDown(mod.page, -240, FeatsDropDown_InitFunc, FeatsDropDown_SelectValueFunc, true)
...
for i = 1, NUM_BUTTONS do
	...
	btn.category:SetWordWrap(false)
...
mod.scroll:SetScript(
	"OnVerticalScroll",
	function(self, offset)
		FauxScrollFrame_OnVerticalScroll(self, offset, BUTTON_HEIGHT, UpdateShownItems)
	end
)
