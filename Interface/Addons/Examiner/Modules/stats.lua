...
function mod.MenuInit(parent, list)
	...
	tbl.header = 1
	tbl = list[#list + 1]
	tbl.text = "Cache Player"
	tbl.value = 1
	tbl.checked = (cache[ex:GetEntryName()] ~= nil)
	-- view
	tbl = list[#list + 1]
	tbl.header = 1
	tbl = list[#list + 1]
	...
	tbl.text = "Gear Stats"
	tbl.value = 4
	tbl.checked = (cfg.statsViewType == 1)
	tbl = list[#list + 1]
	tbl.text = "Details"
	tbl.value = 5
	tbl.checked = (cfg.statsViewType == 2)
	-- compare
	tbl = list[#list + 1]
	tbl.header = 1
	tbl = list[#list + 1]
	...
	tbl.header = 1
	tbl = list[#list + 1]
	tbl.text = "Mark for Compare"
	tbl.value = 2
	tbl.checked = (ex.isComparing and ex.compareStats.entry == ex:GetEntryName())
	if (ex.isComparing) then
		tbl = list[#list + 1]
		tbl.text = "Clear Compare"
		tbl.value = 3
	end
end
...
mod.scroll:SetScript(
	...
	function(self, offset)
		FauxScrollFrame_OnVerticalScroll(self, offset, ITEM_HEIGHT, UpdateShownItems)
	end
)
