...
-- Dialog Func
local CacheFilterOkayFunc = function(text)
	cfg.cacheFilter = text
	mod:BuildCacheList()
	ex:SendModuleEvent("OnConfigChanged", "cacheFilter", text)
end
...
ex.slashCommands.clearcache = function(cmd)
	oldClearCacheFunc(cmd)
	mod:BuildCacheList()
end
...
function mod.MenuInit(parent, list)
	...
	tbl.text = "Show Alts"
	tbl.value = 10
	tbl.checked = cfg.cacheShowAlts
	-- sort
	tbl = list[#list + 1]
	tbl.header = 1
	tbl = list[#list + 1]
	tbl.text = "Sort Method"
	tbl.header = 1
	for index, method in ipairs(cacheSortMethods) do
		local tbl = list[#list + 1]
		tbl.text = method
		tbl.value = method
		tbl.checked = (cfg.cacheSort == method)
	end
	-- cache
	tbl = list[#list + 1]
	tbl.header = 1
	...
	tbl.header = 1
	tbl = list[#list + 1]
	tbl.text = "Delete All Entries"
	tbl.value = 2
	if (#activeCacheList ~= GetTableEntries(cache)) then
		tbl = list[#list + 1]
		tbl.text = "Delete Shown Entries"
		tbl.value = 3
		tbl = list[#list + 1]
		tbl.text = "Delete Hidden Entries"
		tbl.value = 4
	end
end
