...
function handlerProto:IsDisabled(info)
	return (info.option ~= options and info.option ~= options.args.core and not addon.db.profile.enabled) or
		(self.dbHolder ~= addon and not self.dbHolder:IsEnabled())
end
...
local function GetOptions()
	...
	options = {
		...
		--@end-debug@]===]
		--@non-debug@
		name = addonName .. " v1.9.20-classic",
		--@end-non-debug@
		type = "group",
		...
		args = {
			...
			muteBugGrabber = {
				...
				get = function()
					return addon.db.global.muteBugGrabber
				end,
				set = function(_, v)
					addon.db.global.muteBugGrabber = v
				end,
					...
					virtualStacks = {
						...
						args = {
							...
							notWhenTrading = {
								...
								disabled = function(info)
									return info.handler:IsDisabled(info) or
										not (addon.db.profile.virtualStacks.stackable or addon.db.profile.virtualStacks.others)
								end
							}
						}
					}
