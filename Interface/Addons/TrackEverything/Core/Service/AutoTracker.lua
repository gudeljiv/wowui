...
function AutoTracker:OnEnable()
	if not TE.db.profile.general.onmove then
		self:TempPause(2)
	end -- start timer after 2 cesonds of pause. It helps to avoid GCD after reloading.
end
...
function AutoTracker:StartTimer()
	...
	if TE.db.profile.general.onmove then
		canStart = canStart and private.IS_MOVING
	end
...
function AutoTracker:CastNextSpell()
	...
	if TE.db.profile.general.muteSpellClickSound then
...
function AutoTracker:GetNextSpellID(currentSpellId)
	...
	if not trackingSpells then
		return nil
	end -- return nil if no spells to track

	if not currentSpellId then
		currentSpellId = self:GetCurrentTrackingSpellID()
	end

	if not currentSpellId then
		return trackingSpells[1]
	end -- return first spell if nothing is being tracked yet

	local currentSpellIndex = MyLib.IndexOf(trackingSpells, currentSpellId)

	if not currentSpellIndex then
		currentSpellIndex = 0
	end -- if no spellInfex found then start index will be 0
	...
	if nextSpellId == currentSpellId then
		return nil
	end -- if next spell == current tracking spell, then nothing to switch
...
function AutoTracker:isNextSpellOnCooldown()
	...
	if not nextSpellID or GetSpellCooldown(nextSpellID) ~= 0 then
		return true
	else
		return false
	end
...
function AutoTracker:EventHandler(...)
	...
	elseif private.GAME_EVENTS.TEMP_PAUSE_EVENTS[event] then
		if event == "UI_ERROR_MESSAGE" then
			if not private.GAME_EVENTS.TEMP_PAUSE_EVENTS[event][arg2] then
				return
			end -- return if arg2 don't match the list
		elseif event == "UNIT_SPELLCAST_SENT" then
			if arg1 ~= "player" or MyLib.IndexOf(Settings:GetSpellsToTrack(), arg4) then
				return
			end -- return if casted not by player or trackingSpell casted
		end
	...
	elseif private.GAME_EVENTS.MOVE_EVENTS[event] then
		if event == "PLAYER_STARTED_MOVING" then
			...
			if TE.db.profile.general.onmove then
				self:StartTimer()
				if self:isNextSpellOnCooldown() then -- try one more time after 2 sec if spell was on CD
					self:StopTimer()
					self.trackingTimer = self:ScheduleTimer("StartTimer", 2)
				end
			end
	...
	elseif private.USER_EVENTS[event] then
		if event == "ADDON_TOGGLED" then
			if TE.db.profile.general.enable then
				self:RegisterEvents(private.GAME_EVENTS.PAUSE_TRIGGER_EVENTS, "EventHandler")
				self:RegisterEvents(private.GAME_EVENTS.TEMP_PAUSE_EVENTS, "EventHandler")
				if not TE.db.profile.general.onmove or private.IS_MOVING then
					self:StartTimer()
				end
			else
