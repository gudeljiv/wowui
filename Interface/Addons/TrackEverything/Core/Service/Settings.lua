...
local MINIMAP_ICON_DISPLAY_TYPE = {
	-- LOGO = L['Default'],
					...
					args = {
						resources = {
							...
							values = function(info)
								return Settings:GetPlayerTrackingSpells(info[#info]) or {}
							end,
		...
		minimap = {
			...
			args = {
				...
				displayType = {
					...
					type = "select",
					style = "dropdown",
					values = MINIMAP_ICON_DISPLAY_TYPE,
					order = 100
				},
				scale = {
					name = L["Icon scale"],
					desc = L["Change addon icon scale"],
					type = "range",
					min = 0.01,
					max = 3,
					step = 0.01,
...
local defaults = {
	profile = {
		...
		minimap = {
			hide = false,
			hideDefaultTrackingIcon = true,
			minimapPos = 142.432304223615,
...
function Settings:GetAddonState()
	...
	if not trackingType then
		for key, val in pairs(spellInfo) do
			if IsSpellKnown(val["spellId"]) then
				if trackingSpellNames == nil then
					trackingSpellNames = {}
				end
		...
		if not trackingSpellNames then
			return
		end
	else
		for key, val in pairs(spellInfo) do
...
function Settings:CallbackHandler(...)
	...
	elseif key == "scale" then
		TE.minimapButton:SetScale(TE.db.profile.minimap.scale)
	...
	if private.GAME_EVENTS[event] then
		if event == "MINIMAP_UPDATE_TRACKING" then
			self:ScheduleTimer(
				...
				1
			) -- MINIMAP_UPDATE_TRACKING event fires before player is dead, that's why we need to wait 1 sec to get player dead info. No need to disable addon if tracking was canceled becouse of death
		end
	end
end
...
do
	...
	local function InterfaceOptionsFrame_OpenToCategory_Fix(panel)
		...
		if not panelName then
			return
		end -- if its not part of our list return early
