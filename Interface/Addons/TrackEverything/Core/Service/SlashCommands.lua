...
local OPTIONS = {
	{
		...
		print = function()
			return L["addon is [%s]"], SlashCommands:GetColoredAddonState()
		end,
		hidden = false,
		callback = function()
			Settings:Toggle()
	...
	},
	{
		...
		print = function()
			return L["Only while moving in [%s]"], SlashCommands:GetColoredOnMoveState()
		end,
		hidden = false,
		callback = function()
			Settings:ToggleOnMove()
		end,
		order = 4
	},
	{
...
function SlashCommands:ChatCommand(input)
	...
	local option = self:GetOptionByName(args[1])
