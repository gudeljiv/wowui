...
L["Drag to set the battlefield map size.|n|nIf the map is unlocked, you can also resize the battlefield map by dragging the bottom-right corner."] = "Переместите этот ползунок, чтобы установить размер карты поля боя.|n|nЕсли карта разблокирована, вы также можете изменить размер карты, потянув за её правый нижний угол."
