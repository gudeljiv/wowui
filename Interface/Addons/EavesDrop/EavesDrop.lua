...
function EavesDrop:PlaceFrame()
	...
	frame:SetPoint("TOP", CustomContainer_2, "BOTTOM", 0, -9)
end
...
function EavesDrop:CombatEvent()
	local timestamp,
		...
		a10,
		a11,
		a12 = CombatLogGetCurrentEventInfo()
	...
	if (not fromPlayer and not toPlayer) and (toPet or fromPet) and not db["PET"] then
		return
	end
	...
	if etype == "DAMAGE" then
		...
		elseif event == "RANGE_DAMAGE" then
			spellId,
				...
				critical,
				glancing,
				crushing = a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12
			if school == SCHOOL_MASK_PHYSICAL then
				outtype, intype = "TMELEE", "PHIT"
			else
				outtype, intype = "TSPELL", "PSPELL"
		...
		if (critical) then
			text = critchar .. text .. critchar
		end
		if (crushing) then
			text = crushchar .. text .. crushchar
		end
		if (glancing) then
			text = glancechar .. text .. glancechar
		end
		if (resisted) then
	...
	elseif etype == "DRAIN" then
		...
		if (db["GAINS"]) then
			...
			elseif fromPlayer and extraAmount then
				if (extraAmount < db["MFILTER"]) then
					return
				end
...
function EavesDrop:PLAYER_REGEN_ENABLED()
	...
	if (db["SUMMARY"] == true) then
		...
		local strSummary =
			convertRGBtoHEXString(db["MISC"], duration .. " " .. L["IncombatSummary"]) ..
			"\n" ..
