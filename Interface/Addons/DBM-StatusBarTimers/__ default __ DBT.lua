...
do
	...
	function DBT:CreateDummyBar(colorType, inlineIcon)
		...
		local dummy = self:CreateBar(25, "dummy" .. dummyBars, 136116, nil, true, nil, true, colorType, inlineIcon)
...
function DBT:ShowTestBars()
	self:CreateBar(10, "Test 1", 136116)
	--"Interface\\Icons\\Spell_Nature_WispSplode"
...
function barPrototype:SetText(text, inlineIcon)
	if not self.owner.options.InlineIcons then
		inlineIcon = nil
	end
	--Force change color type 7 yo custom inlineIcon
	local forcedIcon =
		(self.colorType and self.colorType == 7 and self.owner.options.Bar7CustomInline) and DBM_CORE_IMPORTANT_ICON or
		inlineIcon or
		""
	...
	if barOptions.DynamicColor and not self.color then
		...
		if colorCount and colorCount >= 1 then
			...
			elseif colorCount == 2 then --AOE
				r =
					barOptions.StartColorAER + (barOptions.EndColorAER - barOptions.StartColorAER) * (1 - timerValue / totaltimeValue)
				g =
					barOptions.StartColorAEG + (barOptions.EndColorAEG - barOptions.StartColorAEG) * (1 - timerValue / totaltimeValue)
				b =
					barOptions.StartColorAEB + (barOptions.EndColorAEB - barOptions.StartColorAEB) * (1 - timerValue / totaltimeValue)
			...
			elseif colorCount == 7 then --Important
				...
				r =
					barOptions.StartColorUIR + (barOptions.EndColorUIR - barOptions.StartColorUIR) * (1 - timerValue / totaltimeValue)
				g =
					barOptions.StartColorUIG + (barOptions.EndColorUIG - barOptions.StartColorUIG) * (1 - timerValue / totaltimeValue)
				b =
					barOptions.StartColorUIB + (barOptions.EndColorUIB - barOptions.StartColorUIB) * (1 - timerValue / totaltimeValue)
			end
		else
...
function barPrototype:ApplyStyle()
		...
		if self.colorType then
			...
			elseif colorCount == 2 then --AOE
				barStartRed, barStartGreen, barStartBlue =
					barOptions.StartColorAER,
					barOptions.StartColorAEG,
			...
			elseif colorCount == 7 then --Important
				barStartRed, barStartGreen, barStartBlue =
					barOptions.StartColorUIR,
					barOptions.StartColorUIG,
	...
	local barTextColorRed, barTextColorGreen, barTextColorBlue =
		barOptions.TextColorR,
		barOptions.TextColorG,
	...
	-- icon1:CreateBeautyBorder(6)
	-- icon2:CreateBeautyBorder(6)
	end
	self.frame:Show()
	if sparkEnabled then
		spark:SetAlpha(1)
...
function barPrototype:SetPosition()
	...
	local Enlarged = self.enlarged
	local ExpandUpwards =
		Enlarged and self.owner.options.ExpandUpwardsLarge or not Enlarged and self.owner.options.ExpandUpwards
	...
	if ExpandUpwards then
		self.frame:SetPoint(
			...
			anchor,
			"TOP",
			self.owner.options[Enlarged and "HugeBarXOffset" or "BarXOffset"],
			self.owner.options[Enlarged and "HugeBarYOffset" or "BarYOffset"]
		)
	else
		self.frame:SetPoint(
			"TOP",
