	...
	if (_detalhes.chat_tab_embed.enabled and tabname ~= "") then
		...
		if (ChatFrame) then
			...
			if (_detalhes.chat_tab_embed.single_window) then
				...
				window1.baseframe:SetPoint(
					"bottomright",
					ChatFrameBackground,
					"bottomright",
					_detalhes.chat_tab_embed.x_offset,
					y_down
				)
...
--]]
				...
				table.wipe(window1.snap)
				table.wipe(window2.snap)
				window1.snap[3] = 2
				window2.snap[1] = 1
				window1.horizontalSnap = true
				window2.horizontalSnap = true
				...
				window1.baseframe:SetSize(
					width + (_detalhes.chat_tab_embed.x_offset / 2),
					height + _detalhes.chat_tab_embed.y_offset
				)
				window2.baseframe:SetSize(
					width + (_detalhes.chat_tab_embed.x_offset / 2),
					height + _detalhes.chat_tab_embed.y_offset
				)

				window1.baseframe:SetPoint("topleft", ChatFrameBackground, "topleft", 0, y_up + _detalhes.chat_tab_embed.y_offset)
				window2.baseframe:SetPoint(
					"topright",
					ChatFrameBackground,
					"topright",
					_detalhes.chat_tab_embed.x_offset,
					y_up + _detalhes.chat_tab_embed.y_offset
				)
...
function _detalhes:CheckForPerformanceProfile()
	...
	if (profile and profile.enabled) then
		...
		if (not _detalhes.performance_profile_lastenabled or _detalhes.performance_profile_lastenabled ~= type) then
			_detalhes:InstanceAlert(
				Loc["STRING_OPTIONS_PERFORMANCE_PROFILE_LOAD"] .. type,
				{_detalhes.PerformanceIcons[type].icon, 14, 14, false, 0, 1, 0, 1, unpack(_detalhes.PerformanceIcons[type].color)},
				5,
				{_detalhes.empty_function}
			)
		end
...
function _detalhes:RegisterBackgroundTask(name, func, priority, ...)
	...
	if (background_tasks[name]) then
		...
		background_tasks[name] = {
			...
			lastexec = time(),
			priority = priority,
			nextexec = time() + task_timers[priority] * 60,
			args = {...},
			args_amt = select("#", ...),
			object = self
		}
			...
			for encounterID, encounterTable in pairs(diffTable) do
				if (type(encounterID) == "number" and type(encounterTable) == "table") then
					for index, encounter in ipairs(encounterTable) do
						...
						if
							(type(encounter.servertime) == "number" and type(encounter.time) == "number" and
								type(encounter.guild) == "string" and
								type(encounter.date) == "string" and
								type(encounter.healing) == "table" and
								type(encounter.elapsed) == "number" and
								type(encounter.damage) == "table")
						 then
	...
	if (storageDebug or (diff == 15 or diff == 16)) then --test on raid finder  ' or diff == 17' -- normal mode: diff == 14 or
		...
		if (not IsAddOnLoaded("Details_DataStorage")) then
			...
			if (not loaded) then
				if (_detalhes.debug) then
					print(
						"|cFFFFFF00Details! Storage|r: can't save the encounter, couldn't load DataStorage, may be the addon is disabled."
					)
				end
				return
			end
			...
			if (UnitIsInMyGuild("raid" .. i)) then
				if (role == "DAMAGER" or role == "TANK") then
					...
					if (damage_actor) then
						...
						this_combat_data.damage[player_name] = {
							floor(damage_actor.total),
							_detalhes.item_level_pool[guid] and _detalhes.item_level_pool[guid].ilvl or 0,
							class or 0
						}
					end
				elseif (role == "HEALER") then
					...
					if (heal_actor) then
						local guid = UnitGUID(player_name) or UnitGUID(UnitName("raid" .. i))
						this_combat_data.healing[player_name] = {
							floor(heal_actor.total),
							_detalhes.item_level_pool[guid] and _detalhes.item_level_pool[guid].ilvl or 0,
							class or 0
						}
					end
		...
		local mybest, onencounter =
			_detalhes.storage:GetBestFromPlayer(diff, encounter_id, myrole, _detalhes.playername, true) --> get dps or hps
			...
			if (myBestDps > d_one) then
				if (not _detalhes.deny_score_messages) then
					print(
						Loc["STRING_DETAILS1"] ..
							format(
								Loc["STRING_SCORE_NOTBEST"],
								_detalhes:ToK2(d_one),
								_detalhes:ToK2(myBestDps),
								onencounter.date,
								mybest[2]
							)
					)
			...
			if (instance) then
				...
				if (not _detalhes.deny_score_messages) then
					instance:InstanceAlert(
						Loc["STRING_GUILDDAMAGERANK_WINDOWALERT"],
						icon,
						_detalhes.update_warning_timeout,
						func,
						true
					)
				end
				...
				(player or "Invalid Player Name") ..
					" | " ..
						(itemLevel or "Invalid Item Level") ..
							" | " .. (currentSpec or "Invalid Spec") .. " | " .. talents .. " | " .. (serialNumber or "Invalid Serial")
		)
	end
	...
	if (CheckInteractDistance(unitid, 1)) then
		...
		if (ilvl_core.forced_inspects[guid]) then
			if (type(ilvl_core.forced_inspects[guid].callback) == "function") then
				local okey, errortext =
					pcall(
					ilvl_core.forced_inspects[guid].callback,
					guid,
					unitid,
					ilvl_core.forced_inspects[guid].param1,
					ilvl_core.forced_inspects[guid].param2
				)
				if (not okey) then
...
local can_start_loop = function()
	if
		((_detalhes:GetZoneType() ~= "raid" and _detalhes:GetZoneType() ~= "party") or ilvl_core.loop_process or
			_detalhes.in_combat or
			not _detalhes.track_item_level)
	 then
		return false
	end
...
function Details:SendPlayerClassicInformation(targetPlayer)
	...
	if (spec and spec[2] >= MIN_SPECS) then
		...
		if (targetPlayer) then
			_detalhes:SendCommMessage(
				CONST_DETAILS_PREFIX,
				_detalhes:Serialize(
					CONST_ANSWER_TALENTS,
					UnitName("player"),
					GetRealmName(),
					_detalhes.realversion,
					UnitGUID("player"),
					0,
					compressedTalents,
					Details.playerClassicSpec.specs
				),
				"WHISPER",
				targetPlayer
			)
...
function Details:ShowTalentsPanel()
		...
		for talentTabIndex = 1, 3 do
			...
			for i = 1, MAX_NUM_TALENT_TIERS do
				...
				for o = 1, NUM_TALENT_COLUMNS do
					...
					talentButton:SetPoint(
						"topleft",
						talentTab,
						"topleft",
						((o - 1) * CONST_ICON_SIZE) + (CONST_ICON_WIDTH_SPACE_REQUIRED * o),
						(-(i - 1) * CONST_ICON_SIZE) + (-i * CONST_ICON_WIDTH_SPACE_REQUIRED)
					)
	...
	table.sort(
		DetailsTalentFrame.tabFramesSort,
	...
	for talentTabIndex = 1, 3 do
		...
		if (talentTab.maxTier > 0) then
			local height =
				max(
				min(130, heightAvailable),
				((talentTab.maxTier) * CONST_ICON_SIZE) + (talentTab.maxTier * CONST_ICON_WIDTH_SPACE_REQUIRED) + 10
			)
