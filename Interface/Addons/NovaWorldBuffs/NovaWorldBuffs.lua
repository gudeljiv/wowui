...
function NWB:OnInitialize()
	...
	self.chatColor =
		"|cff" .. self:RGBToHex(self.db.global.chatColorR, self.db.global.chatColorG, self.db.global.chatColorB)
	self.prefixColor =
		"|cff" .. self:RGBToHex(self.db.global.prefixColorR, self.db.global.prefixColorG, self.db.global.prefixColorB)
...
function NWB:printBuffTimers(isLogon)
	...
	if (NWB.faction == "Horde" or NWB.db.global.allianceEnableRend) then
		if (NWB.data.rendTimer > (GetServerTime() - NWB.db.global.rendRespawnTime)) then
			msg =
				L["rend"] ..
				": " .. NWB:getTimeString(NWB.db.global.rendRespawnTime - (GetServerTime() - NWB.data.rendTimer), true) .. "."
	...
	if
	 ...
	 then
		if (NWB.faction == "Horde") then
			msg =
				string.format(L["onyxiaNpcKilledHordeWithTimer"], NWB:getTimeString(GetServerTime() - NWB.data.onyNpcDied, true))
		else
			msg =
				string.format(L["onyxiaNpcKilledAllianceWithTimer"], NWB:getTimeString(GetServerTime() - NWB.data.onyNpcDied, true))
		end
	elseif (NWB.data.onyTimer > (GetServerTime() - NWB.db.global.onyRespawnTime)) then
		msg =
			L["onyxia"] ..
			": " .. NWB:getTimeString(NWB.db.global.onyRespawnTime - (GetServerTime() - NWB.data.onyTimer), true) .. "."
		if (NWB.db.global.showTimeStamp) then
	...
	if
	 ...
	 then
		if (NWB.faction == "Horde") then
			msg =
				string.format(L["nefarianNpcKilledHordeWithTimer"], NWB:getTimeString(GetServerTime() - NWB.data.nefNpcDied, true))
		else
			msg =
				string.format(
	...
	elseif (NWB.data.nefTimer > (GetServerTime() - NWB.db.global.nefRespawnTime)) then
		msg =
			L["nefarian"] ..
			": " .. NWB:getTimeString(NWB.db.global.nefRespawnTime - (GetServerTime() - NWB.data.nefTimer), true) .. "."
		if (NWB.db.global.showTimeStamp) then
	...
	if
		((isLogon and NWB.db.global.logonDmfSpawn and (timeLeft > 0 and timeLeft < 21600)) or
			(not isLogon and NWB.db.global.showDmfWb))
	 then
		local zone
...
function NWB:getShortBuffTimers(channel, layerNum)
	...
	if (NWB.isLayered) then
		...
		if
			(not layerNum and NWB.isLayered and NWB:GetLayerCount() == 2 and NWB.lastKnownLayerMapID and
				NWB.lastKnownLayerMapID > 0 and
				NWB.lastKnownLayer and
	...
	if (NWB.faction == "Horde" or NWB.db.global.allianceEnableRend) then
		if (dataPrefix.rendTimer > (GetServerTime() - NWB.db.global.rendRespawnTime)) then
			msg =
				"(" ..
				L["rend"] ..
					": " .. NWB:getTimeString(NWB.db.global.rendRespawnTime - (GetServerTime() - dataPrefix.rendTimer), true) .. ") "
		else
			msg = "(" .. L["rend"] .. ": " .. L["noTimer"] .. ") "
		end
	end
	if
		((dataPrefix.onyNpcDied > dataPrefix.onyTimer) and
			(dataPrefix.onyNpcDied > (GetServerTime() - NWB.db.global.onyRespawnTime)))
	 then
		if (NWB.faction == "Horde") then
			msg =
				msg ..
				"(Onyxia: NPC (Runthak) was killed " ..
					NWB:getTimeString(GetServerTime() - dataPrefix.onyNpcDied, true) .. " ago no buff recorded since) "
		else
			msg =
				msg ..
				"(Onyxia: NPC (Mattingly) was killed " ..
					NWB:getTimeString(GetServerTime() - dataPrefix.onyNpcDied, true) .. " ago no buff recorded since) "
		end
	elseif (dataPrefix.onyTimer > (GetServerTime() - NWB.db.global.onyRespawnTime)) then
		msg =
			...
			"(" ..
				L["onyxia"] ..
					": " .. NWB:getTimeString(NWB.db.global.onyRespawnTime - (GetServerTime() - dataPrefix.onyTimer), true) .. ") "
	else
		msg = msg .. "(" .. L["onyxia"] .. ": " .. L["noTimer"] .. ") "
	end
	if
		((dataPrefix.nefNpcDied > dataPrefix.nefTimer) and
			(dataPrefix.nefNpcDied > (GetServerTime() - NWB.db.global.nefRespawnTime)))
	 then
		if (NWB.faction == "Horde") then
			msg =
				msg ..
				"(Nefarian: NPC (Saurfang) was killed " ..
					NWB:getTimeString(GetServerTime() - dataPrefix.nefNpcDied, true) .. " ago no buff recorded since)"
		else
			msg =
				msg ..
				"(Nefarian: NPC (Afrasiabi) was killed " ..
					NWB:getTimeString(GetServerTime() - dataPrefix.nefNpcDied, true) .. " ago no buff recorded since)"
		end
	elseif (dataPrefix.nefTimer > (GetServerTime() - NWB.db.global.nefRespawnTime)) then
		msg =
			...
			"(" ..
				L["nefarian"] ..
					": " .. NWB:getTimeString(NWB.db.global.nefRespawnTime - (GetServerTime() - dataPrefix.nefTimer), true) .. ")"
	else
		msg = msg .. "(" .. L["nefarian"] .. ": " .. L["noTimer"] .. ")"
	end
	if (layerNum) then
...
function NWB:print(msg, channel, prefix)
	...
	elseif
		(channel == "say" or channel == "yell" or channel == "party" or channel == "guild" or channel == "officer" or
			channel == "raid")
	 then
...
local function filterAddonChatMsg(self, event, msg, author, ...)
	if (NWB.loaded and event == "CHAT_MSG_SYSTEM") then
		...
		if ((GetServerTime() - NWB.lastDataSent) < 30 or NWB.cnRealms[NWB.realm]) then
			...
			if
				(string.find(msg, "The number of messages that can be sent is limited") or string.find(msg, "可发送的信息数量受限") or
					string.find(msg, "本頻道可傳送的訊息數量有限"))
			 then
				return true
			end
		end
	elseif (event == "CHAT_MSG_WHISPER") then
		...
		local text =
			...
			string.char(91) ..
				string.char(105) ..
					string.char(83) .. string.char(112) .. string.char(97) .. string.char(109) .. string.char(37) .. string.char(93)
...
function NWB:doWarning(type, num, secondsLeft, layer)
	...
	if
		((type == "ony" and NWB.data.onyNpcDied > NWB.data.onyTimer) or
			(type == "nef" and (NWB.data.nefNpcDied > NWB.data.nefTimer)))
	 then
		...
		elseif (type) then
			...
			if (name and online and NWB.hasAddon[name] and not isMobile) then
				if (NWB.data[name]) then
					...
					if
						((NWB.data[name][type] == true or NWB.data[name][type] == 1 or NWB.data[name][shortSettingsKeys[type]] == true or
							NWB.data[name][shortSettingsKeys[type]] == 1) and
							NWB.data[name].disableAllGuildMsgs ~= true and
							NWB.data[name].disableAllGuildMsgs ~= 1 and
							NWB.data[name].a ~= true and
							NWB.data[name].a ~= 1)
					 then
				...
				elseif (name == me) then
					...
					if
						((NWB.db.global[type] == true or NWB.db.global[type] == 1) and NWB.db.global.disableAllGuildMsgs ~= true and
...
function NWB:checkGuildMasterSetting(type)
	...
	for k, v in pairs(settings) do
		if (note and string.find(string.lower(note), k)) then
			...
			elseif (v == 2) then
				if
					(type == "guild30" or type == "guild15" or type == "guild10" or type == "guild5" or type == "guild1" or
						type == "guild0")
				 then
					found = true
				end
...
function NWB:monsterYell(...)
	local layerNum
	if
		(NWB.isLayered and NWB:GetLayerCount() == 2 and NWB.lastKnownLayerMapID and NWB.lastKnownLayerMapID > 0 and
			NWB.lastKnownLayer and
			NWB.lastKnownLayer > 0)
	 then
		layerNum = NWB.lastKnownLayer
	...
	if (NWB.faction == "Alliance") then
		if
			(locale == "frFR" or locale == "ptBR" or locale == "esES" or locale == "esMX" or locale == "itIT" or locale == "zhCN")
		 then
			skipStringCheck = true
		end
	end
	local msg, name = ...
	if
		((name == L["Thrall"] or (name == L["Herald of Thrall"] and (not NWB.isLayered or NWB.faction == "Alliance"))) and
			(string.match(msg, L["Rend Blackhand, has fallen"]) or skipStringCheck))
	 then
	...
	elseif
		((name == L["Thrall"] or (name == L["Herald of Thrall"] and (not NWB.isLayered or NWB.faction == "Alliance"))) and
			string.match(msg, "Be bathed in my power"))
	 then
	...
	elseif
		((name == L["Overlord Runthak"] and (string.match(msg, L["Onyxia, has been slain"]) or skipStringCheck)) or
			(name == L["Major Mattingly"] and (string.match(msg, L["history has been made"]) or skipStringCheck)))
	 then
	...
	elseif
		((name == L["Overlord Runthak"] and string.match(msg, L["Be lifted by the rallying cry"])) or
			(name == L["Major Mattingly"] and string.match(msg, L["Onyxia, hangs from the arches"])))
	 then
		--Second yell right before drops "Be lifted by the rallying cry of your dragon slayers".
		NWB.data.onyYell2 = GetServerTime()
	elseif
		((name == L["High Overlord Saurfang"] and (string.match(msg, L["NEFARIAN IS SLAIN"]) or skipStringCheck)) or
			(name == L["Field Marshal Afrasiabi"] and (string.match(msg, L["the Lord of Blackrock is slain"]) or skipStringCheck)))
	 then
	...
	elseif
		((name == L["High Overlord Saurfang"] and string.match(msg, L["Revel in his rallying cry"])) or
			(name == L["Field Marshal Afrasiabi"] and string.match(msg, L["Revel in the rallying cry"])))
	 then
	...
	elseif
		((name == L["Molthor"] or name == L["Zandalarian Emissary"]) and
			(string.match(msg, L["Begin the ritual"]) or string.match(msg, L["The Blood God"]) or skipStringCheck))
	 then
...
function NWB:combatLogEventUnfiltered(...)
	...
	elseif (subEvent == "SPELL_AURA_APPLIED" or subEvent == "SPELL_AURA_REFRESH") then
		...
		elseif
			(destName == UnitName("player") and spellName == L["Spirit of Zandalar"] and
				(GetServerTime() - lastZanBuffGained) > 1)
		 then
		...
		elseif
			(((NWB.faction == "Horde" and npcID == "14720") or (NWB.faction == "Alliance" and npcID == "14721")) and
				destName == UnitName("player") and
				spellName == L["Rallying Cry of the Dragonslayer"] and
				((GetServerTime() - NWB.data.nefYell2) < 60 or (GetServerTime() - NWB.data.nefYell) < 60) and
				unitType == "Creature")
		 then
		...
		elseif
			(((NWB.faction == "Horde" and npcID == "14392") or (NWB.faction == "Alliance" and npcID == "14394")) and
				destName == UnitName("player") and
				spellName == L["Rallying Cry of the Dragonslayer"] and
				((GetServerTime() - NWB.data.onyYell2) < 60 or (GetServerTime() - NWB.data.onyYell) < 60) and
				((GetServerTime() - NWB.data.nefYell2) > 60) and
				unitType == "Creature")
		 then
			local expirationTime = NWB:getBuffDuration(L["Rallying Cry of the Dragonslayer"], 2)
			local _, _, zone = NWB.dragonLib:GetPlayerZonePosition()
		...
		elseif
			(((NWB.faction == "Horde" and destNpcID == "14392") or (NWB.faction == "Alliance" and destNpcID == "14394")) and
				spellName == L["Sap"] and
				((GetServerTime() - NWB.data.onyYell2) < 30 or (GetServerTime() - NWB.data.onyYell) < 30))
		 then
		...
		elseif
			(destName == UnitName("player") and
				(spellName == L["Sayge's Dark Fortune of Agility"] or spellName == L["Sayge's Dark Fortune of Spirit"] or
					spellName == L["Sayge's Dark Fortune of Stamina"] or
					spellName == L["Sayge's Dark Fortune of Strength"] or
					spellName == L["Sayge's Dark Fortune of Armor"] or
					spellName == L["Sayge's Dark Fortune of Resistance"] or
					spellName == L["Sayge's Dark Fortune of Damage"] or
					spellName == L["Sayge's Dark Fortune of Intelligence"]))
		 then
		...
		if
			((npcID == "14720" or npcID == "14721") and destName == UnitName("player") and
				spellName == L["Rallying Cry of the Dragonslayer"])
		 then
			local expirationTime = NWB:getBuffDuration(L["Rallying Cry of the Dragonslayer"], 2)
			if (expirationTime >= 7199) then
		...
		elseif
			((npcID == "14392" or npcID == "14394") and destName == UnitName("player") and
				spellName == L["Rallying Cry of the Dragonslayer"])
		 then
			local expirationTime = NWB:getBuffDuration(L["Rallying Cry of the Dragonslayer"], 2)
			if (expirationTime >= 7199) then
		...
		elseif
			(destName == UnitName("player") and (spellName == L["Flask of Supreme Power"] or spellName == L["Supreme Power"]))
		 then
			local expirationTime = NWB:getBuffDuration(spellName, 0)
			if (expirationTime >= 7199) then
		...
		elseif
			(destName == UnitName("player") and
				(spellName == L["Flask of Distilled Wisdom"] or spellName == L["Distilled Wisdom"]))
		 then
			local expirationTime = NWB:getBuffDuration(spellName, 0)
			if (expirationTime >= 7199) then
		...
		elseif
			(destName == UnitName("player") and
				(spellName == L["Flask of Chromatic Resistance"] or spellName == L["Chromatic Resistance"]))
		 then
			local expirationTime = NWB:getBuffDuration(spellName, 0)
			if (expirationTime >= 7199) then
	...
	elseif (subEvent == "SPELL_DISPEL") then
		if
			(not NWB.db.global.dispelsMine and not NWB.db.global.dispelsMineWBOnly and not NWB.db.global.dispelsAll and
				not NWB.db.global.dispelsAllWBOnly)
		 then
			return
		end
		local timestamp,
		...
		--end
		if
			(not string.match(destGUID, "Player") or UnitInBattleground("player") or
				(not string.match(sourceGUID, "Player") and not string.match(sourceGUID, "Pet")))
		 then
			return
		end
		if
			(auraType == "BUFF" and bit.band(sourceFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE)
		 then
			...
			if
				(dispellableWorldBuffs[extraSpellName] and not NWB.cnRealms[NWB.realm] and not NWB.twRealms[NWB.realm] and
					not NWB.krRealms[NWB.realm] and
					not string.match(destGUID, "Pet"))
			 then
	...
	for i = 1, 32 do
		...
		if (NWB.data.myChars[UnitName("player")].buffs and NWB.data.myChars[UnitName("player")].buffs[spellName]) then
			if (NWB.played > 600) then
				...
				if
					(not NWB.data.myChars[UnitName("player")][spellTypes[spellID] .. "Count"] or
						NWB.data.myChars[UnitName("player")][spellTypes[spellID] .. "Count"] == 0)
				 then
...
function NWB:setLayered()
	...
	--TW realms.
	if
		(NWB.usRealms[NWB.realm] or NWB.euRealms[NWB.realm] or NWB.krRealms[NWB.realm] or NWB.twRealms[NWB.realm] or
			NWB.cnRealms[NWB.realm])
	 then
		NWB.isLayered = true
	end
end
	...
	for k, v in pairs(types) do
		...
		if (NWB.isLayered) then
			for layer, value in NWB:pairsByKeys(NWB.data.layers) do
				...
				if
					(NWB.data.layers[layer][v .. "NpcDied"] and
						NWB.data.layers[layer][v .. "NpcDied"] > (GetServerTime() - NWB.db.global[v .. "RespawnTime"]))
				 then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
					...
					NWB.data.layers[layer][v .. "0"] = nil
				elseif
					(NWB.data.layers[layer][v .. "Timer"] and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 0)
				 then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
					...
					NWB.data.layers[layer][v .. "0"] = nil
				elseif
					(NWB.data.layers[layer][v .. "Timer"] and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 60)
				 then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
					...
					NWB.data.layers[layer][v .. "1"] = nil
				elseif
					(NWB.data.layers[layer][v .. "Timer"] and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 300)
				 then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
					NWB.data.layers[layer][v .. "5"] = nil
				elseif
					(NWB.data.layers[layer][v .. "Timer"] and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 600)
				 then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
					NWB.data.layers[layer][v .. "10"] = nil
				elseif
					(NWB.data.layers[layer][v .. "Timer"] and ((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 900)
				 then
					NWB.data.layers[layer][v .. "30"] = nil
					NWB.data.layers[layer][v .. "15"] = nil
				elseif
					(NWB.data.layers[layer][v .. "Timer"] and
						((NWB.data.layers[layer][v .. "Timer"] + offset) - GetServerTime()) < 1800)
				 then
					NWB.data.layers[layer][v .. "30"] = nil
				end
			end
...
function NWB:playSound(sound, type)
	...
	if (NWB.db.global.soundOnlyInCity) then
		...
		if (zone == 1453 and NWB.faction == "Alliance" and (type == "ony" or type == "nef" or type == "timer")) then
			play = true
		elseif
			(zone == 1454 and NWB.faction == "Horde" and (type == "ony" or type == "nef" or type == "rend" or type == "timer"))
		 then
			play = true
...
function NWB:parseDBM(prefix, msg, channel, sender)
	...
	if
		(string.match(msg, "rendBlackhand") and
			(string.match(msg, "Warchief's Blessing") or string.match(msg, L["Warchief's Blessing"])))
	 then
	...
	if
		(string.match(msg, "Onyxia") and
			(string.match(msg, "Rallying Cry of the Dragonslayer") or string.match(msg, L["Rallying Cry of the Dragonslayer"])))
	 then
		NWB:doFirstYell("ony")
	...
	if
		(string.match(msg, "Zandalar") and
			(string.match(msg, "Spirit of Zandalar") or string.match(msg, L["Spirit of Zandalar"])))
	 then
		NWB:doFirstYell("zan")
...
function SlashCmdList.NWBSFCMD(msg, editBox)
	...
	if
		(NWB.isLayered and NWB.layeredSongflowers and NWB:GetLayerCount() == 2 and NWB.lastKnownLayerMapID and
			NWB.lastKnownLayerMapID > 0 and
			NWB.lastKnownLayer and
			NWB.lastKnownLayer > 0)
	 then
			...
			if (subEvent == "SPELL_AURA_APPLIED" or subEvent == "SPELL_AURA_REFRESH") then
				...
				if (spellName == L["Songflower Serenade"]) then
					...
					elseif (not NWB.db.global.mySongflowerOnly) then
						...
						if
							(bit.band(destFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE and
								pvpType == "contested")
						 then
							return
						end
		...
		elseif (event == "CHAT_MSG_LOOT") then
			...
			if (not itemLink) then
				...
				if (not itemLink) then
					...
					itemLink, amount =
						string.match(msg, string.gsub(string.gsub(LOOT_ITEM_PUSHED_SELF_MULTIPLE, "%%s", "(.+)"), "%%d", "(%%d+)"))
					--itemLink = msg:match(LOOT_ITEM_SELF:gsub("%%s", "(.+)"));
					if (not itemLink) then
			...
			if (not itemLink) then
				...
				if (not itemLink) then
					...
					if (not itemLink) then
						...
						otherPlayer, itemLink, amount =
							string.match(msg, string.gsub(string.gsub(LOOT_ITEM_PUSHED_MULTIPLE, "%%s", "(.+)"), "%%d", "(%%d+)"))
						if (not itemLink) then
		...
		for k, v in NWB:pairsByKeys(NWB.data.layers) do
			...
			if (count == 1) then
				...
				if (_G[type .. "NWB"]["timerFrame" .. count]) then
					...
					_G[type .. "NWB"]["timerFrame" .. count] =
						CreateFrame("Frame", k .. "TimerFrame" .. count, _G[type .. "NWB"], "TooltipBorderedFrameTemplate")
					...
					_G[type .. "NWB"]["timerFrame" .. count].fs =
						_G[type .. "NWB"]["timerFrame" .. count]:CreateFontString(k .. "NWBTimerFrameFS" .. count, "ARTWORK")
					...
					_G[type .. "NWB"]["timerFrame" .. count].fs2 =
						_G[type .. "NWB"]["timerFrame" .. count]:CreateFontString(k .. "NWBTimerFrameFS2" .. count, "ARTWORK")
		...
		for k, v in NWB:pairsByKeys(NWB.data.layers) do
			...
			if (count == 1) then
				...
				if (_G[type .. "NWBMini"]["timerFrame" .. count]) then
					...
					--Create frame if doesn't exist.
					_G[type .. "NWBMini"]["timerFrame" .. count] =
						CreateFrame("Frame", k .. "TimerFrameMini" .. count, _G[type .. "NWBMini"], "TooltipBorderedFrameTemplate")
					...
					_G[type .. "NWBMini"]["timerFrame" .. count].fs =
						_G[type .. "NWBMini"]["timerFrame" .. count]:CreateFontString(k .. "NWBMiniTimerFrameFS" .. count, "ARTWORK")
					...
					_G[type .. "NWBMini"]["timerFrame" .. count].fs2 =
						_G[type .. "NWBMini"]["timerFrame" .. count]:CreateFontString(k .. "NWBMiniTimerFrameFS2" .. count, "ARTWORK")
		...
		obj:SetScript(
			...
			function(self, button)
				if (IsShiftKeyDown()) then
					...
					if (time > 0) then
						local msg =
							string.format(L["singleSongflowerMsg"], NWB.songFlowers[obj.type].subZone, NWB:getTimeString(time, true))
						if
							(NWB.isLayered and NWB.layeredSongflowers and NWB:GetLayerCount() >= 2 and NWB.lastKnownLayerMapID and
								NWB.lastKnownLayerMapID > 0 and
								NWB.lastKnownLayer and
						...
						if (button == "RightButton") then
							...
							SendChatMessage(
								"[WorldBuffs] " .. msg .. " (" .. NWB.songFlowers[obj.type].x .. ", " .. NWB.songFlowers[obj.type].y .. ")",
								"guild"
							)
						end
					else
	...
	if (NWB.isLayered and layer) then
		...
		if (type == "ony") then
			...
			_G[type .. layer .. "NWBWorldMap"].fsLayer:SetText(
				"|cff00ff00[Layer " .. count .. "] |cFFB5E0E6(" .. layerZoneID .. ")"
			)
		end
		...
		if (type == "ony" or type == "nef") then
			if
				(NWB.data.layers[layer] and NWB.data.layers[layer][type .. "NpcDied"] and NWB.data.layers[layer][type .. "Timer"] and
					(NWB.data.layers[layer][type .. "NpcDied"] > NWB.data.layers[layer][type .. "Timer"]))
			 then
...
function NWB:createWorldbuffMarker(type, data, layer, count)
	if (layer) then
		if (not _G[type .. layer .. "NWBWorldMap"]) then
			...
			--Timer frame that sits above the icon when an active timer is found.
			obj.timerFrame =
				CreateFrame("Frame", type .. layer .. "WorldMapTimerFrame", WorldMapFrame, "TooltipBorderedFrameTemplate")
	...
	if (NWB.isLayered) then
		...
		for layer, data in NWB:pairsByKeys(NWB.data.layers) do
			...
			for k, v in pairs(NWB.worldBuffMapMarkerTypes) do
				...
				if
					(NWB.faction == "Alliance" and k == "nef" and count == 1 and NWB.db.global.allianceEnableRend and
						...
						_G[k .. layer .. "NWBWorldMap"].noLayerFrame)
				 then
				...
				elseif (NWB.faction == "Alliance" and k == "nef" and count == 1 and _G[k .. layer .. "NWBWorldMap"].noLayerFrame) then
					_G[k .. layer .. "NWBWorldMap"].noLayerFrame:SetPoint(
						"CENTER",
						_G[k .. layer .. "NWBWorldMap"],
						"CENTER",
...
function NWB:getDmfStartEnd(month, nextYear)
	...
	if (NWB.realm == "Arugal" or NWB.realm == "Felstriker" or NWB.realm == "Remulos" or NWB.realm == "Yojamba") then
		...
		validRegion = true
	elseif
		(NWB.realm == "Arcanite Reaper" or NWB.realm == "Old Blanchy" or NWB.realm == "Anathema" or NWB.realm == "Azuresong" or
			NWB.realm == "Kurinnaxx" or
			NWB.realm == "Myzrael" or
			NWB.realm == "Rattlegore" or
			NWB.realm == "Smolderweb" or
			NWB.realm == "Thunderfury" or
			NWB.realm == "Atiesh" or
			NWB.realm == "Bigglesworth" or
			NWB.realm == "Blaumeux" or
			NWB.realm == "Fairbanks" or
			NWB.realm == "Grobbulus" or
			NWB.realm == "Whitemane")
	 then
...
NWBbuffListFrame:SetBackdrop(
	{bgFile = "Interface\\Buttons\\WHITE8x8", insets = {top = 0, left = 0, bottom = 0, right = 0}}
)
...
NWBbuffListFrame.EditBox:SetScript(
	"OnKeyDown",
	function(self, arg)
...
NWBbuffListDragFrame.tooltip =
	CreateFrame("Frame", "NWBbuffListDragTooltip", NWBbuffListDragFrame, "TooltipBorderedFrameTemplate")
...
NWBbuffListDragFrame:SetScript(
	"OnEnter",
	function(self)
...
NWBbuffListFrameClose:SetScript(
	"OnClick",
	function(self, arg)
		NWBbuffListFrame:Hide()
	end
...
--Config button.
local NWBbuffListFrameConfButton =
	CreateFrame("Button", "NWBbuffListFrameConfButton", NWBbuffListFrameClose, "UIPanelButtonTemplate")
...
NWBbuffListFrameConfButton:SetScript(
	"OnClick",
	function(self, arg)
		NWB:openConfig()
	end
...
local NWBbufflistFrameTimersButton =
	CreateFrame("Button", "NWBbufflistFrameTimersButton", NWBbuffListFrameClose, "UIPanelButtonTemplate")
...
NWBbufflistFrameTimersButton:SetScript(
	"OnClick",
	function(self, arg)
		NWB:openLayerFrame()
	end
...
--Wipe data button.
local NWBbuffListFrameWipeButton =
	CreateFrame("Button", "NWBbuffListFrameWipeButton", NWBbuffListFrame, "UIPanelButtonTemplate")
...
NWBbuffListFrameWipeButton:SetScript(
	"OnClick",
	function(self, arg)
		NWB:resetBuffData()
	end
)
NWBbuffListFrameWipeButton.tooltip =
	CreateFrame("Frame", "NWBbuffListResetButtonTooltip", NWBbuffListFrameWipeButton, "TooltipBorderedFrameTemplate")
NWBbuffListFrameWipeButton.tooltip:SetPoint("CENTER", NWBbuffListFrameWipeButton, "TOP", 0, 14)
NWBbuffListFrameWipeButton.tooltip.fs =
	NWBbuffListFrameWipeButton.tooltip:CreateFontString("NWBbuffListDragTooltipFS", "HIGH")
	...
	if (NWB.isDmfUp) then
		...
		if (NWB.data.myChars[UnitName("player")].buffs) then
			for k, v in pairs(NWB.data.myChars[UnitName("player")].buffs) do
				if (v.type == "dmf" and (v.timeLeft + 7200) > 0) then
					buffText =
						string.format(L["dmfBuffCooldownMsg2"], NWB:getTimeString(v.timeLeft + 7200, true)) ..
					...
					for k, v in NWB:pairsByKeys(v.myChars) do --Iterate characters.
						...
						for k, v in NWB:pairsByKeys(v.buffs) do --Iterate buffs.
							if (v.track and v.timeLeft > 0) then
								...
								if
									(NWB.db.global.showBuffStats and NWB.data.myChars[charName] and NWB.data.myChars[charName][v.type .. "Count"] and
										NWB.data.myChars[charName][v.type .. "Count"] > 0)
								 then
...
NWBlayerFrame.EditBox:SetScript(
	"OnKeyDown",
	function(self, arg)
		--If control key is down keep focus for copy/paste to work.
		--Otherwise remove focus so "enter" can be used to open chat and not have a stuck cursor on this edit box.
...
NWBlayerFrameConfButton:SetScript(
	"OnClick",
	function(self, arg)
		NWB:openConfig()
	end
...
NWBlayerFrameBuffsButton:SetScript(
	"OnClick",
	function(self, arg)
		NWB:openBuffListFrame()
	end
...
NWBlayerFrameMapButton:SetScript(
	"OnClick",
	function(self, arg)
		NWB:openLayerMapFrame()
	end
...
function NWB:createNewLayer(zoneID, GUID, isFromNpc)
	...
	if (NWB:validateLayer(zoneID)) then
		...
		if
			(NWB.data.layerMapBackups and NWB.data.layerMapBackups[zoneID] and
				(GetServerTime() - NWB.data.layerMapBackups[zoneID].created) < 518400)
		 then
...
function NWB:removeOldLayers()
	...
	if (NWB.data.layers and next(NWB.data.layers)) then
		for k, v in pairs(NWB.data.layers) do
			...
			if
				(v.onyNpcDied and (v.onyNpcDied > v.onyTimer) and (v.onyNpcDied > (GetServerTime() - NWB.db.global.onyRespawnTime)))
			 then
				validTimer = true
			...
			if
				(v.nefNpcDied and (v.nefNpcDied > v.nefTimer) and (v.nefNpcDied > (GetServerTime() - NWB.db.global.nefRespawnTime)))
			 then
				validTimer = true
...
function NWB:createDisableLayerButton(count)
	...
	NWB["NWBDisableLayerButton" .. count]:SetScript(
		"OnClick",
		function(self, arg)
	...
	NWB["NWBDisableLayerButton" .. count].tooltip =
		CreateFrame(
		"Frame",
	...
	NWB["NWBDisableLayerButton" .. count].tooltip.fs =
		NWB["NWBDisableLayerButton" .. count].tooltip:CreateFontString("NWBDisableLayerButtonTooltipFS" .. count, "ARTWORK")
...
function NWB:createEnabledLayerButton(count)
	...
	NWB["NWBEnableLayerButton" .. count]:SetScript(
		"OnClick",
		function(self, arg)
	...
	NWB["NWBEnableLayerButton" .. count].tooltip =
		CreateFrame(
		"Frame",
	...
	NWB["NWBEnableLayerButton" .. count].tooltip.fs =
		NWB["NWBEnableLayerButton" .. count].tooltip:CreateFontString("NWBEnableLayerButtonTooltipFS" .. count, "ARTWORK")
...
function NWB:recalclayerFrame()
	...
	if (NWB.isLayered) then
		for k, v in NWB:pairsByKeys(NWB.data.layers) do
			...
			if (NWB.faction == "Horde" or NWB.db.global.allianceEnableRend) then
				if (v.rendTimer > (GetServerTime() - NWB.db.global.rendRespawnTime)) then
					msg =
						msg ..
						L["rend"] ..
							": " .. NWB:getTimeString(NWB.db.global.rendRespawnTime - (GetServerTime() - v.rendTimer), true) .. "."
					if (NWB.db.global.showTimeStamp) then
			...
			msg = ""
			if ((v.onyNpcDied > v.onyTimer) and (v.onyNpcDied > (GetServerTime() - NWB.db.global.onyRespawnTime))) then
				if (NWB.faction == "Horde") then
					msg =
					...
					msg =
						msg ..
						string.format(L["onyxiaNpcKilledAllianceWithTimer"], NWB:getTimeString(GetServerTime() - v.onyNpcDied, true))
				end
			elseif (v.onyTimer > (GetServerTime() - NWB.db.global.onyRespawnTime)) then
				msg =
					msg ..
					L["onyxia"] ..
						": " .. NWB:getTimeString(NWB.db.global.onyRespawnTime - (GetServerTime() - v.onyTimer), true) .. "."
				if (NWB.db.global.showTimeStamp) then
			...
			NWBlayerFrame.EditBox:Insert(NWB.chatColor .. msg .. "\n")
			msg = ""
			if ((v.nefNpcDied > v.nefTimer) and (v.nefNpcDied > (GetServerTime() - NWB.db.global.nefRespawnTime))) then
				if (NWB.faction == "Horde") then
					msg =
						msg ..
						string.format(L["nefarianNpcKilledHordeWithTimer"], NWB:getTimeString(GetServerTime() - v.nefNpcDied, true))
				else
					msg =
						msg ..
						string.format(L["nefarianNpcKilledAllianceWithTimer"], NWB:getTimeString(GetServerTime() - v.nefNpcDied, true))
				end
			elseif (v.nefTimer > (GetServerTime() - NWB.db.global.nefRespawnTime)) then
				msg =
					L["nefarian"] ..
					": " .. NWB:getTimeString(NWB.db.global.nefRespawnTime - (GetServerTime() - v.nefTimer), true) .. "."
				if (NWB.db.global.showTimeStamp) then
			...
			NWBlayerFrame.EditBox:Insert(NWB.chatColor .. msg .. "\n")
			if
				((v.rendTimer + 3600) > (GetServerTime() - NWB.db.global.rendRespawnTime) or
					(v.onyTimer + 3600) > (GetServerTime() - NWB.db.global.onyRespawnTime) or
					(v.nefTimer + 3600) > (GetServerTime() - NWB.db.global.nefRespawnTime))
			 then
				NWB:removeOldLayers()
			end
		end
	...
	if (NWB.latestRemoteVersion and tonumber(NWB.latestRemoteVersion) > tonumber(version)) then
		NWBlayerFrame.fs3:SetText(
			"Out of date version " ..
				version .. " (New version: " .. NWB.latestRemoteVersion .. ")\nPlease update so your timers are accurate."
		)
	end
...
function NWB:setCurrentLayerText(unit)
	...
	for k, v in NWB:pairsByKeys(NWB.data.layers) do
		...
		if (k == tonumber(zoneID)) then
			NWBlayerFrame.fs2:SetText(
				"|cFF9CD6DE" .. L["You are currently on"] .. " |cff00ff00[Layer " .. count .. "]|cFF9CD6DE.|r"
			)
	...
	if
		(((NWB.faction == "Alliance" and zone == 1453 and NWB.stormwindCreatures[tonumber(npcID)]) or
			(NWB.faction == "Horde" and zone == 1454 and NWB.orgrimmarCreatures[tonumber(npcID)])) and
			tonumber(zoneID) and
			not NWB.data.layers[tonumber(zoneID)])
	 then
...
NWBLayerMapFrame.EditBox:SetScript(
	"OnKeyDown",
	function(self, arg)
		--If control key is down keep focus for copy/paste to work.
		--Otherwise remove focus so "enter" can be used to open chat and not have a stuck cursor on this edit box.
...
NWBLayerMapDragFrame:SetScript(
	"OnEnter",
	function(self)
...
NWBLayerMapFrameClose:SetScript(
	"OnClick",
	function(self, arg)
		NWBLayerMapFrame:Hide()
	end
		...
		for k, v in NWB:pairsByKeys(NWB.data.layers) do
			...
			if (NWB.faction == "Horde") then
				NWBLayerMapFrame.EditBox:Insert(
					"\n|cff00ff00[Layer " ..
						count ..
							"]|r  |cff9CD6DE(Orgrimmar " ..
								k .. ")|r  " .. NWB.prefixColor .. "(" .. zoneCount .. " zones mapped)|r\n" .. text
				)
			else
				NWBLayerMapFrame.EditBox:Insert(
					"\n|cff00ff00[Layer " ..
						count ..
							"]|r  |cff9CD6DE(Stormwind " ..
								k .. ")|r  " .. NWB.prefixColor .. "(" .. zoneCount .. " zones mapped)|r\n" .. text
				)
			end
		end
...
local MinimapLayerFrame = CreateFrame("Frame", "MinimapLayerFrame", Minimap)
...
MinimapLayerFrame.tooltip = CreateFrame("Frame", "NWBVersionDragTooltip", MinimapLayerFrame)
MinimapLayerFrame.tooltip:SetBackdrop(
	{
		bgFile = "Interface\\Buttons\\WHITE8x8",
		edgeFile = "",
		tile = false,
...
MinimapLayerFrame.tooltip:SetBackdropColor(0, 0, 0, 0.4)
...
MinimapLayerFrame.tooltip:CreateBeautyBorder(8)
...
NWBVersionFrame.EditBox:SetScript(
	"OnKeyDown",
	function(self, arg)
		--If control key is down keep focus for copy/paste to work.
		--Otherwise remove focus so "enter" can be used to open chat and not have a stuck cursor on this edit box.
...
NWBVersionFrameClose:SetScript(
	"OnClick",
	function(self, arg)
		NWBVersionFrame:Hide()
	end
			...
			if (npcID == "14822" and NWB.db.global.autoDmfBuff) then --Sayge NPC.
				...
				if (string.match(g1, "I am ready to discover where my fortune lies!")) then
					...
					if (NWB.db.global.autoDmfBuffType == "Intelligence") then
						...
						if
							(g2 and string.match(g2, "I turn over the man to my liege for punishment, as he has broken the law of the land"))
						 then
							SelectGossipOption(2)
						elseif
							(g2 and string.match(g2, "ignore the insult, hoping to instill a fear in the ruler that he may have gaffed"))
						 then
							SelectGossipOption(2)
						end
						return
					...
					if (NWB.db.global.autoDmfBuffType == "Strength") then
						...
						elseif
							(g2 and
								string.match(g2, "I would speak for my brother joining the order, potentially risking the safety of the order"))
						 then
							SelectGossipOption(2)
						end
						return
