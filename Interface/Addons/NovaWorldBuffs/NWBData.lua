...
function NWB:OnCommReceived(commPrefix, string, distribution, sender)
	...
	elseif
		((cmd == "npcKilled" or cmd == "npcKilled2") and not (NWB.db.global.receiveGuildDataOnly and distribution ~= "GUILD"))
	 then
...
function NWB:createData(distribution)
	...
	if
		((NWB.data.onyNpcDied > NWB.data.onyTimer) and
			(NWB.data.onyNpcDied > (GetServerTime() - NWB.db.global.onyRespawnTime)))
	 then
		data["onyNpcDied"] = NWB.data.onyNpcDied
	end
	if
		((NWB.data.nefNpcDied > NWB.data.nefTimer) and
			(NWB.data.nefNpcDied > (GetServerTime() - NWB.db.global.nefRespawnTime)))
	 then
...
function NWB:createDataLayered(distribution, noLayerMap)
	...
	if
		(not noLayerMap and (GetServerTime() - lastSendLayerMap[distribution]) > sendLayerMapDelay and distribution ~= "PARTY" and
			distribution ~= "RAID")
	 then
	...
	for layer, v in NWB:pairsByKeys(NWB.data.layers) do
		...
		if (NWB.data.layers[layer].rendTimer > (GetServerTime() - NWB.db.global.rendRespawnTime)) then
			...
			if
				(NWB.data.layers[layer].GUID and not NWB.cnRealms[NWB.realm] and not NWB.twRealms[NWB.realm] and
					not NWB.krRealms[NWB.realm])
			 then
		...
		if
			((NWB.data.layers[layer].onyNpcDied > NWB.data.layers[layer].onyTimer) and
				(NWB.data.layers[layer].onyNpcDied > (GetServerTime() - NWB.db.global.onyRespawnTime)))
		 then
			if (not data.layers) then
				data.layers = {}
			end
		...
		if
			((NWB.data.layers[layer].nefNpcDied > NWB.data.layers[layer].nefTimer) and
				(NWB.data.layers[layer].nefNpcDied > (GetServerTime() - NWB.db.global.nefRespawnTime)))
		 then
			if (not data.layers) then
				data.layers = {}
			end
		...
		if
			(not foundTimer and NWB.data.layers[layer].lastSeenNPC and
				NWB.data.layers[layer].lastSeenNPC > GetServerTime() - 3600)
		 then
...
function NWB:receivedData(dataReceived, sender, distribution)
	...
	if
		(data["rendTimer"] and tonumber(data["rendTimer"]) and
			(data["rendTimer"] < NWB.data["rendTimer"] or
				(data["rendYell"] and data["rendYell"] < (data["rendTimer"] - 120) and data["rendYell2"] and
					data["rendYell2"] < (data["rendTimer"] - 120))))
	 then
	...
	if
		(data["onyTimer"] and tonumber(data["onyTimer"]) and
			(data["onyTimer"] < NWB.data["onyTimer"] or
				(data["onyYell"] and data["onyYell"] < (data["onyTimer"] - 120) and data["onyYell2"] and
					data["onyYell2"] < (data["onyTimer"] - 120))))
	 then
	...
	if
		(data["nefTimer"] and tonumber(data["nefTimer"]) and
			(data["nefTimer"] < NWB.data["nefTimer"] or
				(data["nefYell"] and data["nefYell"] < (data["nefTimer"] - 120) and data["nefYell2"] and
					data["nefYell2"] < (data["nefTimer"] - 120))))
	 then
	...
	if (NWB.isLayered and data.layers) then
		...
		for layer, vv in NWB:pairsByKeys(data.layers) do
			...
			if
				(((not vv["rendTimer"] or vv["rendTimer"] == 0) and (not vv["onyTimer"] or vv["onyTimer"] == 0) and
					(not vv["nefTimer"] or vv["nefTimer"] == 0) and
					(not vv["onyNpcDied"] or vv["onyNpcDied"] == 0) and
					(not vv["nefNpcDied"] or vv["nefNpcDied"] == 0) and
					(not vv["lastSeenNPC"] or vv["lastSeenNPC"] == 0)) or
					NWB.data.layersDisabled[layer])
			 then
				...
				if (type(vv) == "table" and next(vv)) then
					for localLayer, localV in pairs(NWB.data.layers) do
						...
						if (vv["rendTimer"] and localV["rendTimer"] and vv["rendTimer"] == localV["rendTimer"] and layer ~= localLayer) then
						...
						if (vv["onyTimer"] and localV["onyTimer"] and vv["onyTimer"] == localV["onyTimer"] and layer ~= localLayer) then
						...
						if (vv["nefTimer"] and localV["nefTimer"] and vv["nefTimer"] == localV["nefTimer"] and layer ~= localLayer) then
					...
					if (NWB:validateLayer(layer)) then
						...
						if (NWB.data.layers[layer]) then
							...
							--NWB:debug(data);
							if
								(vv["rendTimer"] and tonumber(data["rendTimer"]) and
									(vv["rendTimer"] < (GetServerTime() - NWB.db.global.rendRespawnTime) or
										--or not vv["rendYell"] or not vv["rendYell2"]
										--or (vv["rendYell"] < (vv["rendTimer"] - 120) and vv["rendYell2"] < (vv["rendTimer"] - 120)))) then
										not vv["rendYell"] or
										(vv["rendYell"] < (vv["rendTimer"] - 120))))
							 then
								--Don't overwrite any data for this timer type if it's an old timer.
							...
							if
								(vv["onyTimer"] and tonumber(data["onyTimer"]) and
									(vv["onyTimer"] < (GetServerTime() - NWB.db.global.onyRespawnTime) or
										--or not vv["onyYell"] or not vv["onyYell2"]
										--or (vv["onyYell"] < (vv["onyTimer"] - 120) and vv["onyYell2"] < (vv["onyTimer"] - 120)))) then
										not vv["onyYell"] or
										(vv["onyYell"] < (vv["onyTimer"] - 120))))
							 then
							...
							if
								(vv["nefTimer"] and tonumber(data["nefTimer"]) and
									(vv["nefTimer"] < (GetServerTime() - NWB.db.global.nefRespawnTime) or
										--or not vv["nefYell"] or not vv["nefYell2"]
										--or (vv["nefYell"] < (vv["nefTimer"] - 120) and vv["nefYell2"] < (vv["nefTimer"] - 120)))) then
										not vv["nefYell"] or
										(vv["nefYell"] < (vv["nefTimer"] - 120))))
							 then
							...
							for k, v in pairs(vv) do
								if
									((string.match(k, "flower") and NWB.db.global.syncFlowersAll) or (not NWB.db.global.receiveGuildDataOnly) or
										(NWB.db.global.receiveGuildDataOnly and distribution == "GUILD"))
								 then
									if (validKeys[k] and tonumber(v)) then
										...
										if (v ~= nil) then
											...
											if (NWB.data.layers[layer][k] and v ~= 0 and v > NWB.data.layers[layer][k] and NWB:validateTimestamp(v)) then
												...
												if
													(NWB.isLayered and string.match(k, "flower") and NWB.data.layers[layer][k] and
														(GetServerTime() - NWB.data.layers[layer][k]) < 1500)
												 then
													--Don't overwrite songflower timers on layered realms.
												else
													if
														(string.match(k, "flower") and
															not (distribution == "GUILD" and (GetServerTime() - NWB.data.layers[layer][k]) > 15))
													 then
														newFlowerData = true
													end
