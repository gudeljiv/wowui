...
local function CreateFrames()
	...
	kctimerstartstop:SetFrameStrata("MEDIUM")
...
kc:SetScript(
	...
	function(self, event, ...)
		...
		if xKillCount and xKillCount.show then
			local _, eventType, _, sourceGUID, sourceName, _, _, destGUID, destName, ADDITIONAL = CombatLogGetCurrentEventInfo()
			...
			if UnitExists("pet") and sourceGUID == UnitGUID("pet") then
				if eventType == "SWING_DAMAGE" then
					amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing, isOffHand = ADDITIONAL
				elseif eventType == "SPELL_DAMAGE" then
					spellId, spellName, spellSchool, amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing, isOffHand = ADDITIONAL
				end
