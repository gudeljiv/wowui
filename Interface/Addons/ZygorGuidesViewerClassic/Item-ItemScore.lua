	...
	if not (ItemCache[itemlink] and ItemCache[itemlink].stats) then
		...
		for num=1,Gratuity:NumLines() do
			...
			if found_level then 
				...
				break -- no hard stats after level requirements line, only stuff that can be false matches
			end

			if line==" " then break end -- no hard stats after empty line either
