...
local BugReportFlags = {guide=1,step=1,player=1,questlog=1,inventory=1,parselog=1,itemscore=1,sis=1}
...
function ZGV:GetBugReport(maint)
	...
	if BugReportFlags.player then
		...
		s = s .. "\n\n--Skills"
		s = s .. "- Locale "..GetLocale().."\n"
		s = s .. "- Skill lines ("..GetNumSkillLines().."):\n"
		for i=1, GetNumSkillLines() do
			local skillName, _, _, skillRank = GetSkillLineInfo(i);
			local gear_skillTag = ZGV.ItemScore.SkillNamesRev[skillName]
			local trade_skillTag = ZGV.Professions.LocaleSkillsR[skillName]
			s = s .. ("%s, %s, %d\n"):format(skillName,(gear_skillTag and "gear "..gear_skillTag) or trade_skillTag or "not matched",skillRank)
		end
	...
	if BugReportFlags.sis then
		s = s .. "\n\n--- SIS CATCHUP ---\n"
		if ZGV.db.char.SISlog then
			s = s .. ("* Setup: \n")
			for i,v in pairs(ZGV.db.char.SISlog) do
				s = s .. v.. "\n"
			end
		end
		if ZGV.db.char.SISdestination then
			s = s .. ("Destination: %s %d\n"):format(ZGV.db.char.SISdestination[1],ZGV.db.char.SISdestination[2])
		end
		if ZGV.db.char.SISquests then
			s = s .. ("* Quests: \n")
			local sisquests = {}
			for i,v in pairs(ZGV.db.char.SISquests) do
				v = tostring(v)
				sisquests[v] = sisquests[v] or "\n "..v.. ": "
				sisquests[v] = sisquests[v] .. i .. ", "
			end
			for _,v in pairs(sisquests) do
				s = s .. v
			end
		end
		if ZGV.db.char.SISguides then
			s = s .. ("\n* Guides: \n")
			for i,v in pairs(ZGV.db.char.SISguides) do
				s = s .. ("%s - %s\n"):format(i,tostring(v))
			end
		end
	end
	...
	if BugReportFlags.step then
		...
		if step then
			...
			s = s .. ("  tag: %s\n"):format(ZGV.QuestDB:GetStepTag(step) or "")

			s = s .. "Goals: \n"

			for i,goal in ipairs(step.goals) do
				s = s .. ("%d. %s %s\n"):format(i,(". "):rep(goal.indent),goal.text and "\""..goal.text.."\"" or "<"..goal:GetText()..">")
				local gs,gm = ZGV.QuestDB:MaybeSkipThisGoal(goal)
				s = s .. ("    skip: %s, %s\n"):format((goal.questid or "-"),gm)	
