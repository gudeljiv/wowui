...
--	if not ZygorGuidesViewerMiniFrame then error("Zygor Guide Viewer step frame not loaded.") end
	...
	if ZGV.db.profile.register_way_slash then
		SLASH_WAY1 = "/way"
		function SlashCmdList.WAY(text)  ZGV.Pointer:SetWaypointByCommandLine(text)  end
	end
...
function ZGV:SetGuide(name,step,hack,silent) --hack used for testing
	...
	if self.CurrentGuide then
		if not self.CurrentGuide.headerdata.shared and self.db.profile.share_masterslave==2 then  -- disable slave mode if user picks another guide
...
function ZGV:FocusStep(num,forcefocus)
	...
	if ZygorGrindPopup and ZygorGrindPopup:IsVisible() then ZygorGrindPopup:Hide() end
...
function ZGV:TryToCompleteStep(force)
	...
	-- check if we need to clear sis catchup history
	if stepcomplete and ZGV.db.char.SISdestination and ZGV.db.char.SISquests then
		local end_guide,end_step = ZGV.db.char.SISdestination[1],ZGV.db.char.SISdestination[2]
		if self.CurrentGuide.title == end_guide and self.CurrentStep.num==end_step then
...
function ZGV:StopFlashAnimation()
	self:Debug("&greenlines stopflashanimation")
...
function ZGV:GetStickiesAt(stepnum,only_incomplete)
	...
	if step.stickies then
		for _,stickystep in ipairs(step.stickies) do
			if (not only_incomplete or not stickystep:IsComplete()) and (ZGV.QuestDB:GetStepTag(stickystep)~=ZGV.L["sis_skipping"]) then
...
function ZGV:DoUpdateFrame(full,onupdate)
	...
	if self.CurrentGuide and self.CurrentGuide.steps and self.CurrentGuide.fully_parsed then
		...
		for stepframenum = 1,self.StepLimit do repeat
			...
			local betatext = ((self.CurrentGuide.beta or stepdata.beta) and L['stepbeta']) or ((self.CurrentGuide.devonly or stepdata.devonly) and L['stepdev'])
			...
			if (numbertext or leveltext or reqtext or titletext or betatext or prereqtext) then
				...
				lineframe.label:SetText((numbertext or "")..(leveltext or "")..(reqtext or "")..(titletext or "")..(betatext or "")..(prereqtext or ""))
					...
					if goal then
						...
						if self.db.profile.goalbackgrounds then
							...
							if (goal.action~="goto" and goal.action~="fly") and self.db.profile.goalupdateflash and needsAnimating and self.frameNeedsResizing==0 then
								...
								anim_fade:Play()
								self:Debug("&greenlines Animating progress %d '%s'",goal.num,goal:GetText())
								goalsneedanimating[goal]=nil
							...
							elseif status=="complete" and needsAnimating and self.db.profile.goalcompletionflash and self.frameNeedsResizing==0 then
								...
								anim_fade:Play()
								goalsneedanimating[goal]=nil
								self:Debug("&greenlines Animating completion %d '%s'",goal.num,goal:GetText())
