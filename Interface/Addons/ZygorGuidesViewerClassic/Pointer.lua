...
function Pointer:Startup()
	...
	if ZGV.DEV then
		...
		:Hide()
		.__END
		self.OverlayFrame.PathDebugButton = ZGV.ChainCall(CreateFrame("BUTTON","ZGVLibRoverButton",worldMap_TargetFrame_UI,"UIPanelButtonTemplate"))
...
function Pointer.frame_minimap_functions.OnClick(self,button)
	...
	if ChatFrame1EditBox:HasFocus() then
		...
		ChatFrame1EditBox:SetText(ChatFrame1EditBox:GetText() .. ("%s %.2f,%.2f "):format(info.name,x*100,y*100))
...
function Pointer.ArrowFrame_ShowSpellArrow(self,waypoint)
	local icon = Pointer.ArrowFrame.ArrowIcon
	local safe =  not InCombatLockdown() and not (UnitIsDeadOrGhost("player") and not ZGV.Parser.ConditionEnv.hasbuff("spell:5384")) -- Feign death should not disable spell/item
...
function Pointer:PointToNextTimer(elapsed)
	...
	waypoint = (waypoint.pathnode and waypoint.pathnode.waypoint) or waypoint  -- use original waypoint if this is a LibRover result

	local set = waypoint.in_set or (waypoint.is_farm and "farm")
	local pathfollow = set and self.pointsets[set].follow
	--print("pathfollow "..pathfollow)
	--print("m "..waypoint.m)
	...
	if (pathfollow=="smart" or pathfollow=="loose" or pathfollow=="smart2" or pathfollow=="smart3" or pathfollow=="smart5")
	...
	then
		...
		local newway = self:PointToNextInPath(farm)
...
function Pointer:Debug_PathMenu()
	...
	local menu = {
		{
			text = "Path \"drainage basin\" tests",
			isNotRadio = true,
			hasArrow = true,
			notCheckable = true,
			menuList = {
				{
					text = "Show 10x10 grid",
					isNotRadio = true,
					notCheckable = true,
					func = function()  self.debug_animatedrainage=false  self.debug_animatedrainagefarm=false  if self.debug_drainageshown then self:ClearWaypoints("manual") end self:TestGetNextInPath(10) self.debug_drainageshown=true end,
				},
				{
					text = "Show 50x50 grid",
					isNotRadio = true,
					notCheckable = true,
					func = function()  self.debug_animatedrainage=false  self.debug_animatedrainagefarm=false  if self.debug_drainageshown then self:ClearWaypoints("manual") end self:TestGetNextInPath(50) self.debug_drainageshown=true end,
				},
				{
					text = "Animate simple followers",
					isNotRadio = true,
					notCheckable = true,
					func = function()  self.debug_animatedrainage=true    self.debug_animatedrainagefarm=false  end,
				},
				{
					text = "Animate farmers",
					isNotRadio = true,
					notCheckable = true,
					func = function()  self.debug_animatedrainage=true    self.debug_animatedrainagefarm=true  end,
				},
				{
					text = "Clear tests",
					isNotRadio = true,
					notCheckable = true,
					func = function() self:ClearWaypoints("manual")  self.debug_animatedrainage=false  self.debug_animatedrainagefarm=false  self.debug_animatedrainage=false  self.debug_drainageshown=false  end
				}
			}
		},
		{
...
function Pointer.Overlay_OnUpdate(frame,but,...)
	...
	if frame.ZygorCoordsDEV and frame.ZygorCoordsDEV:IsVisible() then
		...
		local s=("|cffccccccCursor: |cffffffff%s|cff888888##|cffffffff%d |cffaaaaaa(%s)|r %s |cffaaaaaa(map type: %s%s)\n"):format(
			ZGV.GetMapNameByID(mm),mm,
			mmf and mmf[1] or ZGV.GetMapNameByID(mm),
		...
		s = s .. ("|cffccccccPlayer: |cffffffff%s|cff888888##|cffffffff%d |cffaaaaaa(%s)|r %s%s |cffaaaaaa(raw: %d,%d @%d)\n"):format(
			ZGV.GetMapNameByID(pm),pm,
			pmf and pmf[1] or ZGV.GetMapNameByID(pm),
		...
		frame.ZygorCoordsDEV:SetPoint("BOTTOMLEFT",0,35)
...
function Pointer:IsCorpseArrowNeeded() -- small utility against bulky ifs, NB: waypointer-independent
	if ZGV.Parser.ConditionEnv.hasbuff("spell:5384") then return false end -- feign death should not trigger corpse arrow
...
function Pointer:GetNextInPath(pathname,testway)
	...
	elseif pathfollow == "smart" then
		...
		--== Find if player is <pathdist from the path (points or segments).
	...
	elseif pathfollow == "smart5" then
		-- subdivide into points at most <pathdist> apart
		-- - if in range of a point then arrow to very next point
		-- - if not, point towards nearest point
		...
		local map=pathpoints[1].m
		for i=1,#pathpoints do
			local next_i = i+1
			if i==#pathpoints then
				if pathloop then next_i=1 else break end
			end
			local way1=pathpoints[i]
			local way2=pathpoints[next_i]
			local segment = HBD:GetZoneDistance(way1.m,way1.x,way1.y,way2.m,way2.x,way2.y) or 9999
			local subdivs = max(1,ceil(segment/pathdist))
			local dx = way2.x-way1.x
			local dy = way2.y-way1.y
			for s=0,subdivs-1 do
				local sub = s/subdivs
				subpoints_n=subpoints_n+1
				local subpoint = smart5_points[subpoints_n]
				if not subpoint then subpoint={} tinsert(smart5_points,subpoint) end
				subpoint.num = i
				subpoint.m = way1.m
				subpoint.x = way1.x+dx*sub
				subpoint.y = way1.y+dy*sub
			end
			i=i+1
		end

		if subpoints_n==0 then return end

		--print (n,"subdivs created")

		-- find closest subpoint
		local mindist=9999
		local minsubpoint
		for i=1,subpoints_n do
			local subpoint = smart5_points[i]
			local dist = HBD:GetZoneDistance(pm,px,py,map,subpoint.x,subpoint.y) or 9999
			subpoint.dist=dist
			if dist<mindist then
				minsubpoint=i
				mindist=dist
		...
		--print("closest subdiv "..minsubpoint.." dist "..mindist)

		-- find nextest point from closest still in range, unless it loops
		local i=minsubpoint
		if false and mindist<=pathdist then
			--[[
			repeat
				subpoint=smart5_points[i]
				if subpoint.dist<=pathdist then
					i=i+1
					if i>subpoints_n then  if pathloop then i=1 else break end  end
					if i==minsubpoint then break end --looped
				else break
				end
			until false
			--]]
			...
			-- just use next
			i=minsubpoint+1
			if i>subpoints_n then  if pathloop then i=1 else i=minsubpoint end  end
		end
		--print("last-in-range "..i)

		Pointer:ClearWaypoints("temp")
		local way = Pointer:SetWaypoint(smart5_points[i].m,smart5_points[i].x,smart5_points[i].y,{
			title="Follow the farm route",
			type="temp",
			is_farm=true,
...
function Pointer:Debug_MoveManualsForward()
	for i,way in ipairs(self.waypoints) do
		if way.type=="manual" and way.angle then
			...
			if timer<1 and self.debug_animatedrainagefarm then
...
do
	...
	F:SetScript("OnUpdate",function(self,elapsed)
		...
		local antfreq = ZGV.db.profile.antspeed or 0.033

		ant_interval = (antfreq>900) and 0.001 or (antfreq==0) and 999 or (1/antfreq)
...
local function spawn_curve_ants(points,loop,phase, parentmap,xmin,xmax,ymin,ymax)
	...
	for i=1,np do  while true do
		...
		local antislingshot_dists = p1.antislingshot_dists

		local curve_dist_div = 10
		local curve_dist_divn = 1/curve_dist_div
		if not curve_accuracy or (ZGV.db.profile.ant_antislingshot and not antislingshot_dists) then
			...
			if false then -- use straight dist
				...
				local i=0
				if ZGV.db.profile.ant_antislingshot then
					antislingshot_dists={}
					antislingshot_dists[0]=0
				end
				for t=curve_dist_divn,1,curve_dist_divn do
					local t2 = t*t
					local t3 = t*t*t
					...
					local segdist = sqrt((x-lx)*(x-lx)+(y-ly)*(y-ly)*0.66667)
					i=i+1
					if ZGV.db.profile.ant_antislingshot then antislingshot_dists[i]=(antislingshot_dists[i-1] or 0) + segdist end
					dist=dist+segdist
					lx,ly=x,y
				end
				if ZGV.db.profile.ant_antislingshot then for j=1,i do antislingshot_dists[j]=antislingshot_dists[j]/dist end end
				dist=dist*widths_cache[p1.gm]*3
			end
			...
			p1.antislingshot_dists = antislingshot_dists
		end

		--print("acc",curve_accuracy)
		--for t=phase*curve_accuracy,1-(1-phase)*curve_accuracy,curve_accuracy*0.999 do
		for p=phase*curve_accuracy,0.9999,curve_accuracy*0.999 do
			local t
			if ZGV.db.profile.ant_antislingshot and antislingshot_dists then
				local phase_seg = p%curve_dist_divn  -- 0.x fraction
				local base_seg = math.floor((p-phase_seg)*curve_dist_div)  -- base for lookup in antislingshot_dists
				t = Lerp(antislingshot_dists[base_seg], antislingshot_dists[base_seg+1], phase_seg/curve_dist_divn)
				--print(("%d %.2f %.2f"):format(base_seg,t,p)) --if math.abs(p-t)>0.2 then print (p,t) end
				t = p+((p-t))
			else
				t=p  -- accept uneven curve load
			end
...
local copy_fields = {title=1,arrowtitle=1,arrowicon=1,text=1,pathnode=1,map=1,x=1,y=1,["type"]=1,icon=1,pathnode=1,radius=1,
	is_farm=1, waypoint_zone=1, waypoint_realzone=1, waypoint_subzone=1, waypoint_minizone=1, waypoint_region=1, waypoint_indoors=1, player=1, ant_icon=1,noskip=1,
...
function Pointer:AnimateAnts()
	...
	local ant_speed = ZGV.db.profile.antfast or ant_speed
	...
	local function animate_set(name,target,spacing,mastermap,xmin,ymin,xmax,ymax)
		...
		elseif target=="minimap" then
			self:SetAntSpacing(spacing * (name=="farm" and 3 or 1))
			antpoints,num = spawn_func(pointset.points,pointset.loop,phase, mastermap,xmin,xmax,ymin,ymax)
	...
	if ZGV.db.profile.antspacing>0 then
		...
		local spacing=ZGV.db.profile.antspacingmini or 30
...
function Pointer:SetWaypointByCommandLine(input,justparse)
	local map,mapid,floor,x,y,label

	if not input then return end

	local way1,way2 = input:match("^from%s*(.*)%s*to%s*(.*)$")
	if way1 then
	...
	map,floor,x,y,label = input:match("^(.-)%s*/%s*(%d+)%s+([0-9%.]+)[ ,;:]+([0-9%.]+)%s+(.+)$")
	if not map then map,floor,x,y = input:match("^(.-)%s*/%s*(%d+)%s+([0-9%.]+)[ ,;:]+([0-9%.]+)$") end
	if not map then map,x,y,label = input:match("^(.-)%s+([0-9%.]+)[ ,;:]+([0-9%.]+)%s+(.+)$") end
	if not map then map,x,y = input:match("^(.-)%s+([0-9%.]+)[ ,;:]+([0-9%.]+)$") end
	if not map then map,floor = input:match("^(.-)%s*/%s*(%d+)$") end
	if not map then x,y,label = input:match("^([0-9%.]+)[ ,;:]+([0-9%.]+)%s+(.+)") end
	...
	if map=="Shadowmoon Valley" and ZGV:GetPlayerPreciseLevel()>=90 then 
		ZGV:Print("Setting waypoint to Shadowmoon Valley in Outland (TBC). If you meant Shadowmoon Valley in Draenor (WOD), please use |cffddff00Shadowmoon Valley D|r as map name.")
	end
	if map=="Nagrand" and ZGV:GetPlayerPreciseLevel()>=90 then 
		ZGV:Print("Setting waypoint to Nagrand in Outland (TBC). If you meant Nagrand in Draenor (WOD), please use |cffddff00Nagrand D|r as map name.")
	end
	...
	label = label and (label:match("\"(.+)\"") or label:match("'(.+)'") or label)
	...
	if mapid and x and y then
		ZGV.Pointer:SetWaypoint(mapid,tonumber(x)/100,tonumber(y)/100,{findpath=true,type="manual",cleartype=true,icon=ZGV.Pointer.Icons.graydot,onminimap="always",overworld=true,showonedge=true,title=label},true)
	else
