	...
	if self.items then
		...
		for i,itemdata in ipairs(self.items) do
			...
			if not crap then
				...
				itemdata.is_lively = trend and trend.health<1.2
				...
				if trend and itemdata.is_lively then
					...
					itemdata.demand = count -- classic, we have no demand values // trend.sold or trend.q_md or (trend.q_lo + trend.q_hi)/2  -- TEMPORARY. Doubling the market is bad too, but we have to start with something.
					...
					local is = ("%d %s (%s each = %s total); demand: %d  %s%s"):format(
						...
						itemdata.gouged and (" |cff88ff00GOUGED: %s > %s|r"):format(ZGV.GetMoneyString(itemdata.price),ZGV.GetMoneyString(trend.p_hi*ZGVG.OVERPRICE))
	...
	if self.bad_items then
		for ii,item in ipairs(self.bad_items) do
			...
			elseif not item.is_lively then
				comment="Market stagnant or saturated."
			else
				comment=""
			end
			if not item.itemdata[3] then
