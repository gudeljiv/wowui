...
local protomt = {__index = function(t, k)
...
helpers.AddLoadableAura = function(data, todefault)
	...
	if data.name == nil then
		print(data.id .. " spell id missing")
		return
	end
	...
	if not Aptechka.loadedAuras then
...
helpers.AddAura = function(data, todefault)
	...
	if data.id and not data.name then
		data.name = GetSpellInfo(data.id)
	end
	if data.name == nil then
...
helpers.AddTrace = function(data)
	if not config.enableTraceHeals then
		return
	end
	...
	if data.id then
		data.name = GetSpellInfo(data.id) or data.name
	end
	...
	if not data.name then
		print((data.id or "nil") .. "id or name required")
		return
	end
...
helpers.AddDebuff = function(index, data)
	if not config.DebuffDisplay then
		config.DebuffDisplay = {}
	end
...
helpers.ClickMacro = function(macro)
	if AptechkaUserConfig then
		config = AptechkaUserConfig
	else
		config = AptechkaDefaultConfig
	end
	if not config.enableClickCasting then
		return
	end
...
function helpers.Reverse(p1)
	local p2 = ""
	local dir
	if string.find(p1, "CENTER") then
		return "CENTER"
	end
	if string.find(p1, "TOP") then
		p2 = p2 .. "BOTTOM"
	end
	if string.find(p1, "BOTTOM") then
		p2 = p2 .. "TOP"
	end
	if string.find(p1, "LEFT") then
		p2 = p2 .. "RIGHT"
	end
	if string.find(p1, "RIGHT") then
		p2 = p2 .. "LEFT"
	end
