...
if isClassic then
	local dummyFalse = function()
		return false
	end
	local dummy0 = function()
		return 0
	end
	local dummyNil = function()
		return nil
	end
...
function Aptechka.PLAYER_LOGIN(self, event, arg1)
	...
	colors =
		...
		{
			__index = function(t, k)
				return (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[k]
			end
		}
	)
	...
	if not AptechkaConfigCustom[class] then
		AptechkaConfigCustom[class] = {}
	end

	local fixOldAuraFormat = function(customConfigPart)
		if not customConfigPart then
			return
		end
	...
	if NickTag then
		NickTag.RegisterCallback(
			"Aptechka",
			"NickTag_Update",
			function()
				Aptechka:ForEachUnitFrame(
					"player",
					function(frame)
	...
	if AptechkaDB.global.useDebuffOrdering then
		...
		LibSpellLocks.RegisterCallback(
			"Aptechka",
			"UPDATE_INTERRUPT",
			function(event, guid)
	...
	if AptechkaDB.global.useCombatLogHealthUpdates then
		...
		CLH.RegisterCallback(
			self,
			"COMBAT_LOG_HEALTH",
			function(event, unit, eventType)
...
function Aptechka:ReconfigureProtected()
	...
	for groupId, header in ipairs(group_headers) do
		...
		if groupId == 9 then
			groupEnabled = self.db.profile.petGroup
		end
...
function Aptechka:COMBAT_LOG_EVENT_UNFILTERED(event)
	local timestamp, eventType, hideCaster, srcGUID, srcName, srcFlags, srcFlags2, dstGUID, dstName, dstFlags, dstFlags2, spellID, spellName, spellSchool, amount, overhealing, absorbed, critical =
...
local vehicleHack = function(self, time)
	...
	if self.OnUpdateCounter < 1 then
		return
	end
	...
	for self in pairs(Roster[unit]) do
		...
		if not state.isInVehicle then
			...
			if unit ~= vehicleUnit then
				...
				if not self.vehicleFrame then
					self.vehicleFrame = CreateFrame("Frame")
					self.vehicleFrame.parent = self
				end
				...
				if self.power then
					Aptechka:UNIT_POWER_UPDATE(nil, self.unit)
				end
				if self.absorb then
...
Aptechka.OnRangeUpdate = function(self, time)
	...
	if self.OnUpdateCounter < 0.3 then
		return
	end
	self.OnUpdateCounter = 0
...
function Aptechka:UpdateAggroConfig()
	if not config.AggroStatus then
		return
	end
...
function Aptechka.UNIT_SPELLCAST_SENT(self, event, unit, targetName, lineID, spellID)
	if unit ~= "player" or not targetName then
		return
	end
...
function Aptechka.LayoutUpdate(self)
	if not self.db.global.enableProfileSwitching then
		return
	end
	local numMembers = GetNumGroupMembers()
...
function Aptechka.RAID_TARGET_UPDATE(self, event)
	if not AptechkaDB.profile.showRaidIcons then
		return
	end
	for unit, frames in pairs(Roster) do
...
function Aptechka:UpdateTargetStatusConfig()
	if not self.db.global.enableTargetStatus then
		Aptechka:ForEachFrame(
			function(self)
				SetJob(self, config.TargetStatus, false)
			end
		)
...
function Aptechka.READY_CHECK_CONFIRM(self, event, unit)
	...
	for self in pairs(Roster[unit]) do
		...
		if not status or not rci.stackcolor[status] then
			return
		end
...
local function updateUnitButton(self, unit)
	...
	if self.OnUnitChanged then
		self:OnUnitChanged(owner)
	end
	if not unit then
		return
	end
	...
	if self.guid then
		guidMap[self.guid] = unit
	end
	for guid, gunit in pairs(guidMap) do
		if not Roster[gunit] or guid ~= UnitGUID(gunit) then
			guidMap[guid] = nil
		end
	end
	...
	if HealComm and config.enableIncomingHeals then
		Aptechka:UNIT_HEAL_PREDICTION(nil, unit, UnitGUID(unit))
	end
end

local delayedUpdateTimer =
	C_Timer.NewTicker(
	5,
	function()
...
function Aptechka:SetAnchorpoint(unitGrowth, groupGrowth)
	...
	if ud == gd then
		return rug
	elseif gd == "VERTICAL" and ud == "HORIZONTAL" then
		return rgg .. rug
	elseif ud == "VERTICAL" and gd == "HORIZONTAL" then
		return rug .. rgg
	end
end
...
local onenter = function(self)
	if self.OnMouseEnterFunc then
		self:OnMouseEnterFunc()
	end
	if AptechkaDB.global.enableMouseoverStatus then
		FrameSetJob(self, config.MouseoverStatus, true)
	end
	if AptechkaDB.global.disableTooltip or UnitAffectingCombat("player") then
		return
	end
...
local onleave = function(self)
	if self.OnMouseLeaveFunc then
		self:OnMouseLeaveFunc()
	end
...
function Aptechka.SetupFrame(header, frameName)
	...
	if config.disableManaBar or not f.power then
		...
		if f.power and f.power.OnPowerTypeChange then
			f.power:OnPowerTypeChange("none")
		end
		f.power = nil
	end
...
local AssignToSlot = function(frame, opts, status, slot, ...)
	...
	if not widget.currentJob and status == false then
		return
	end
	...
	if next(jobs) then
		...
		if widget ~= frame then
			widget:Show()
		end -- taint if we show protected unitbutton frame
...
local GetRealID = function(id)
	return type(id) == "table" and id[1] or id
end
-----------------------
-- AURAS
-----------------------
...
function Aptechka.DummyFunction()
end
...
Aptechka.Commands = {
	...
	["setpos"] = function(v)
		...
		if not next(fields) then
			print("Usage: /apt setpos point=center x=0 y=0")
			return
		end
	...
	["blacklist"] = function(v)
		...
		elseif cmd == "del" then
			...
			if spellID then
				local val = nil
				if default_blacklist[spellID] then
					val = false
				end -- if nil it'll fallback on __index
