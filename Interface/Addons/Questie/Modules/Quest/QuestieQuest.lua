	...
	if QuestieQuest.NotesHidden then
		...
		for questId, framelist in pairs(QuestieMap.questIdFrames) do
			for index, frameName in ipairs(framelist) do -- this may seem a bit expensive, but its actually really fast due to the order things are checked
				...
				if
					icon ~= nil and icon.hidden and
						not ((((not Questie.db.global.enableObjectives) and (icon.data.Type == "monster" or icon.data.Type == "object" or icon.data.Type == "event" or icon.data.Type == "item")) or ((not Questie.db.global.enableTurnins) and icon.data.Type == "complete") or ((not Questie.db.global.enableAvailable) and icon.data.Type == "available")) or
							((not Questie.db.global.enableMapIcons) and (not icon.miniMapIcon)) or
							((not Questie.db.global.enableMiniMapIcons) and (icon.miniMapIcon))) or
						(icon.data.ObjectiveData and icon.data.ObjectiveData.HideIcons) or
						(icon.data.QuestData and icon.data.QuestData.HideIcons and icon.data.Type ~= "complete")
				 then
					icon:FakeUnhide()
				end
			end
...
function QuestieQuest:UpdateHiddenNotes()
	...
	for questId, framelist in pairs(QuestieMap.questIdFrames) do
		for index, frameName in ipairs(framelist) do -- this may seem a bit expensive, but its actually really fast due to the order things are checked
			...
			if icon ~= nil and icon.data then
				if
					(QuestieQuest.NotesHidden or (((not questieGlobalDB.enableObjectives) and (icon.data.Type == "monster" or icon.data.Type == "object" or icon.data.Type == "event" or icon.data.Type == "item")) or ((not questieGlobalDB.enableTurnins) and icon.data.Type == "complete") or ((not questieGlobalDB.enableAvailable) and icon.data.Type == "available")) or
						((not questieGlobalDB.enableMapIcons) and (not icon.miniMapIcon)) or
						((not questieGlobalDB.enableMiniMapIcons) and (icon.miniMapIcon))) or
						(icon.data.ObjectiveData and icon.data.ObjectiveData.HideIcons) or
						(icon.data.QuestData and icon.data.QuestData.HideIcons and icon.data.Type ~= "complete")
				 then
					icon:FakeHide()
	...
	for id, frameList in pairs(QuestieMap.manualFrames) do
		for _, frameName in ipairs(frameList) do
			local icon = _G[frameName]
			if icon ~= nil and icon.data then
				if QuestieQuest.NotesHidden or ((not questieGlobalDB.enableMapIcons) and (not icon.miniMapIcon)) or ((not questieGlobalDB.enableMiniMapIcons) and (icon.miniMapIcon)) then
					icon:FakeHide()
				else
					icon:FakeUnhide()
				end
		...
		for id, spawnData in pairs(Objective.spawnList) do -- spawnData.Name, spawnData.Spawns
			...
			if (not Objective.AlreadySpawned[id]) and (not completed) and (not Quest.AlreadySpawned[Objective.Type .. tostring(ObjectiveIndex)][spawnData.Id]) then
				if Questie.db.global.enableObjectives then
					...
					data.GetIconScale = function()
						return spawnData:GetIconScale() or 1
					end
					data.IconScale = data:GetIconScale()
		...
		if (objective.type) then
			...
			quest.Objectives[objectiveIndex].Update = function(self)
				...
				if qObjectives and qObjectives[self.Index] then
					...
					if (obj.type) then
						-- fixes for api bug
						if not obj.numFulfilled then
							obj.numFulfilled = 0
						end
						if not obj.numRequired then
							obj.numRequired = 0
						end
						if not obj.finished then
							obj.finished = false
						end -- ensure its boolean false and not nil (hack)
...
--[[function _QuestieQuest:GetLeaderBoardDetails(objectiveIndex, questId)
    ...
    return nil;
end]] -- Link contains test bench for regex in lua.
...
function _QuestieQuest:DrawAvailableQuest(questObject, noChildren)
	...
	if not _QuestieQuest:IsDoable(questObject) then
		return false
	end
	...
	for questID, v in pairs(QuestieDB.questData) do
		...
		if
			((not Questie.db.char.complete[questID]) and -- Don't show completed quests
				...
				((not quest.Repeatable) or (quest.Repeatable and showRepeatableQuests)))
		 then -- Show repeatable quests if the quest is repeatable and the option is enabled
