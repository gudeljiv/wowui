...
function SafeQueue:ADDON_ACTION_FORBIDDEN(_, func)
	...
	if func == 'AcceptBattlefieldPort()' then
		self.popupTainted = true
	end
	if func == 'func()' then
		self.minimapTainted = true
	end
	if (not self.popupTainted) or (not self.minimapTainted) then
		return
	end
...
SafeQueue:SetScript(
	'OnShow',
	function(self)
		if (not self.battlefieldId) then
			return
		end
		...
		if color then
			self.SubText:SetTextColor(color.r, color.g, color.b)
		end
...
local function GetDropDownListEnterButton(battlefieldId)
	...
	for i = 1, GetMaxBattlefieldID() do
		...
		if i == battlefieldId then
			return 'DropDownList1Button' .. index
		end
	end
end
...
function SafeQueue:SetMacroText()
	...
	if (not issecurevariable('CURRENT_BATTLEFIELD_QUEUES')) then
		self.popupTainted = true
	end
