...
function s.filters:ExpandEntries(entries)
    ...
    for _,entry in ipairs(entries) do
        ...
        if s.filters[filter] then
            ...
            if filter~="ranks" then -- don't sort ranks, so they appear in the order picked up (so rank 10 doesn't appear before rank 9)
...
local function getFullSpellName(name)
    if (name or ""):match("%(.+%)") and GetSpellInfo(name) then -- if a subtext is explicitly defined (and known), then use it
        return name
    end
...
-- if there's a search already cached for this attribClass, then add it to "subresults" list and return true;
-- if there isn't, then return false
local function getSearchCache(attribClass,search)
    if SelectSettings.NoSearchCache then
        return false -- search cache disabled
    end
    if not attribClass or not search or not s.cache.searches[attribClass] then
        return false -- this attribClass hasn't been cached yet
    end
    local cachedResults = s.cache.searches[attribClass][search]
    if cachedResults then -- search results exist
        for _,result in ipairs(cachedResults) do
            s.list:Add("subresults",result[1],result[2],result[3],result[4])
        end
        return true
    else
        return false
...
--[[ no: search
    ...
    Removes any already-added entries that contain search term in their name. Note: It does not prevent
    spells or items with that term being added after the not filter is used.
]]

function s.filters:no(search)
    if search and search:len()>0 then
        s.list:Remove("results",search)
    end
end
...
local equipLocs = {
	...
	["INVTYPE_RANGED"] = isClassicClient and 18 or 16, -- Bows
	["INVTYPE_THROWN"] = 18, -- Ranged
	["INVTYPE_RANGEDRIGHT"] = isClassicClient and 18 or 16, -- Wands, Guns, and Crossbows
...
function s.filters:equipslot(slot)
    if getSearchCache("equipslot",slot) then
        return -- if items haven't changed, use previously cached results
    end
    ...
    s.cache:AddSearch("equipslot",slot,s.list:GetAll("subresults"))
end
...
function s.filters:item(arg)
    if getSearchCache("item",arg) then
        return -- if items haven't changed, use previously cached results
    end
    ...
    s.cache:AddSearch("item",arg,s.list:GetAll("subresults"))    
end
...
function s.filters:spell(arg)
    if getSearchCache("spell",arg) then
        return -- if spells haven't changed, use previously cached results
    end
    ...
    s.cache:AddSearch("spell",arg,s.list:GetAll("subresults"))    
end
s.filters.s = s.filters.spell

--[[ ranks:arg 

    /select ranks:text will add all ranks of spells with text in its name

    Rather than using the cache for spell names, this looks for all spells in the spellbook to
    pick up every rank. (Can also pick up variants with different subtexts)

    It's recommended that 'Show Subtext on Spell Tooltips' is enabled for this option.

    (Classic Era or The Burning Crusade Classic only)
]]

if isClassicClient then

    function s.filters:ranks(arg)
        for i=1,GetNumSpellTabs() do
            local _,_,offset,numSlots = GetSpellTabInfo(i)
            for j=offset+1,offset+numSlots do
                local _,spellID = GetSpellBookItemInfo(j,"spell")
                local name = GetSpellInfo(spellID)
                if name and name:match(arg) then
                    local rank = GetSpellSubtext(spellID)
                    if rank and rank:len()>0 then
                        s.list:Add("subresults","spell","spell","spell",format("%s(%s)",name,rank))
                    else
                        s.list:Add("subresults","spell","spell","spell",format("%s",name))
                    end
                end
            end
        end
    end
    s.filters.r = s.filters.ranks
...
function s.filters:type(arg)
    ...
    if not isClassicClient and ("quest"):match(arg) then
        for i=0,4 do
            for j=1,s.utils:GetContainerNumSlots(i) do
                local isQuestItem, questID, isActive = s.utils:GetContainerItemQuestInfo(i,j)
                if isQuestItem or questID or isActive then
                    s.list:Add("subresults","item","item","item","item:"..s.utils:GetContainerItemID(i,j),true)
                end
            end
        end
    end
...
if not isClassicClient then
    ...
    function s.filters:mount(arg)
        ...
        if getSearchCache("mount",arg) then
            return -- if mounts haven't changed, use previously cached results
        end
        ...
        if mountID then -- this is a numerical mount (like m:118089)
        ...
        s.cache:AddSearch("mount",arg,s.list:GetAll("subresults"))    
...
--[[ toy:arg
 ...
 if not isClassicClient then
    ...
    function s.filters:toy(arg)
        ...
        if getSearchCache("toy",arg) then
            return -- if toys haven't changed, use previously cached results
        end

        local any = isEqual(arg,"any")
        local favorite = isEqual(arg,"favorite")

        -- if passed argument is a number, it's likely an itemID
        local argItemID = tonumber(arg)
        if argItemID then
            for toyName,itemID in pairs(s.cache:Get("toy")) do
                if itemID==argItemID then -- found
                    s.list:Add("subresults","toy","item","item",toyName,true)
                    return -- will never add more than a single item per call if itemID is used
                end
            end
        end
        ...
        s.cache:AddSearch("toy",arg,s.list:GetAll("subresults"))    
    end
