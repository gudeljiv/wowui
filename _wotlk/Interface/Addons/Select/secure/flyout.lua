...
function s.flyout:Create(list)
    ...
    -- if rows:<count> is used, set the number of rows attribute
    local numRows = tonumber(list:match("rows:(%d+)") or "") -- can be nil
    flyout:SetAttribute("numRows",numRows)
...
function s.flyout:SaveAttributes(flyout)
...
-- on login, flyouts that need filled are inserted here into an ordered list
local queuedFlyouts = {}
-- and this will fill one flyout from the ordered list and then wait to do the next
local function delayedFill()
    local index = #queuedFlyouts
    if index > 0 then
        s.flyout:Fill(queuedFlyouts[index]) -- fill the flyout
        tremove(queuedFlyouts,index) -- then remove from queue
        s.timer:Start(0.1,delayedFill) -- and kick off timer to run next
    end
end
-- does a FillAll except one flyout per 0.1 second
function s.flyout:FillOverTime()
    -- fill queue with flyouts to fill
    for _,flyout in pairs(s.root.flyouts) do
        if not flyout.invactive then
            tinsert(queuedFlyouts,flyout)
        end
    end
    -- kick off timer to start filling
    s.timer:Start(0.1,delayedFill)
end

-- for entering combat or more immediate need to fill all flyouts:
...
function s.flyout:FillAll()
    s.timer:Stop(delayedFill) -- in case we enter combat while doing a FillOverTime
    for _,flyout in pairs(s.root.flyouts) do
...
function s.flyout:Fill(flyout)
    ...
    local numAcross = flyout:GetAttribute("numRows")
    if numAcross and numAcross>0 then -- if rows:x used, force numAcross to x
        numAcross = min(numAcross,flyout.numButtons)
    else -- otherwise shape flyout to apsect ratio
        numAcross = floor(sqrt(flyout.numButtons/aspectRatio)+.5)
    end
    ...
    elseif s.cache:IsDataLoaded() then
        ...
        if not attribClass or attribClass=="empty" then -- don't overwrite any existing attributes
            ...
            if flyout.numButtons>0 and flyout.buttons[1]:GetAttribute("attribClass")~="empty" then
                ...
                -- fake an attribute change so the new flyout default is saved to savedvar (script handler
                -- is defined after LoadDefault on a new flyout, so it missed the initial OnAttributeChanged)
                s.flyout.OnAttributeChanged(flyout,"attribValue","")
                return true
            end
        else
            return true
