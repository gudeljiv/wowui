...
function s.main:PLAYER_LOGIN()
    ...
    -- this will wait until data fully cached and then do macro updates and such
    s.main:WaitForDataToCache()
end
...
function s.main:WaitForDataToCache()
    if not s.cache:IsDataLoaded() then -- wait for data to load
        s.timer:Start(0.1,s.main.WaitForDataToCache)
    else
        ...
        s.flyout:FillOverTime(0.1) -- starts filling flyouts, one per 0.1
    end    
end
