...
local function handleItemButton(item)
	...
	-- _G[item:GetName() .. 'IconTexture']:CreateBeautyBorder(8)
	end

	if item.Icon then
		item.Icon:SetSize(32, 32)
		item.Icon:SetDrawLayer('ARTWORK')
		item.Icon:SetPoint('TOPLEFT', 4, -4)
	end
...
local function questQualityColors(frame, text, link, icon)
	if not frame.template then
		handleItemButton(frame)
	end

	icon:CreateBeautyBorder(8)
	...
	if quality and quality > 1 then
		...
		icon:SetBeautyBorderTexture('Interface\\AddOns\\xVermin\\Media\\textureWhite')
		icon:SetBeautyBorderColor(r, g, b, 1)
	else
		-- text:SetTextColor(1, 1, 1)
		-- frame:SetBackdropBorderColor(unpack(E.media.bordercolor))
		icon:SetBeautyBorderTexture('Interface\\AddOns\\xVermin\\Media\\textureNormal')
		icon:SetBeautyBorderColor(1, 1, 1, 1)
	end
end

local function BlizzardQuestFrames()
	for frame, numItems in pairs({QuestLogItem = MAX_NUM_ITEMS, QuestProgressItem = MAX_REQUIRED_ITEMS}) do
		for i = 1, numItems do
			if _G[frame .. i] then
				print(numItems, _G[frame .. i]:GetName())
			end
	...
	hooksecurefunc(
		...
		function(rewardsFrame, index)
			...
			if not button and button.template then
				return
			end
	...
	_G.QuestLogFrameCancelButton:SetPoint('BOTTOMRIGHT', _G.QuestLogFrame, 'BOTTOMRIGHT', -7, 12)
	...
	_G.QuestLogSkillHighlight:SetTexture('Interface\\AddOns\\xVermin\\Media\\Highlight')
