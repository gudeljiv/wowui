...
for id = 13, 24 do
	...
	b:SetID(id)
	-- print(b:GetSize(), b:GetSize() * 12, b:GetSize() + 5, (b:GetSize() + 5) * 12)
	if _G['ExtraBarButton' .. (id - 12 - 1)] then
		b:SetPoint('LEFT', _G['ExtraBarButton' .. (id - 12 - 1)], 'LEFT', b:GetSize() + 5, 0)
	else
		b:SetPoint('LEFT', extrabar, 'LEFT', 0, 0)
	end
	b:Show()
	b:SetParent(extrabar)
end

extrabar:SetScale(0.85)
