...
StdUi.ContextMenuMethods = {
	...
	HookRightClick = function(self)
		...
		if parent then
			...
			parent:HookScript(
				'OnMouseUp',
				function(_, button)
					ContextMenuOnMouseUp(menu, button)
				end
			)
		end
	end,
	...
	CreateItem = function(parent, data, i)
		...
		elseif data.isSeparator then
			itemFrame = parent.stdUi:Frame(parent, nil, 20)
			itemFrame.texture = parent.stdUi:Texture(itemFrame, nil, 8, [[Interface\COMMON\UI-TooltipDivider-Transparent]])
		...
		itemFrame:CreateBeautyBorder(8)
	...
	DrawOptions = function(self, options)
		...
		local _, totalHeight = self.stdUi:ObjectList(self, self.optionFrames, self.CreateItem, self.UpdateItem, options, 0, self.padding, -self.padding)
