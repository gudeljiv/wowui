...
local function IsValidItem(item)
	return item ~= nil and -- May be a favourite with no items available, ignore it.
		item.location ~= nil and -- Location may be invalid because of items being moved in the bag
...
function AuctionatorSaleItemMixin:OnUpdate()
	...
	self.TotalPrice:SetText(GetMoneyString(self:GetNumStacks() * self:GetStackSize() * self.UnitPrice:GetAmount(), true))
...
function AuctionatorSaleItemMixin:GetDeposit()
	return GetAuctionDeposit(self:GetDuration(), math.min(self:GetBidAmount(), MAXIMUM_BID_PRICE), math.min(self.StackPrice:GetAmount(), MAXIMUM_BID_PRICE), self:GetStackSize(), self:GetNumStacks())
end
...
function AuctionatorSaleItemMixin:ReceiveEvent(event, ...)
	...
	elseif event == Auctionator.Components.Events.EnterPressed then
		self:PostItem()
	elseif event == Auctionator.Buying.Events.AuctionFocussed and self.itemInfo ~= nil then
		local info = ...
		if info ~= nil then
	...
	elseif event == Auctionator.Buying.Events.HistoricalPrice and self.itemInfo ~= nil then
		local price = ...
...
function AuctionatorSaleItemMixin:UpdateForNewItem()
	...
	if not self.retryingItem then
		...
		Auctionator.Utilities.DBKeyFromLink(
			self.itemInfo.itemLink,
			function(dbKeys)
...
function AuctionatorSaleItemMixin:SetDuration()
	self.Duration:SetSelectedValue(Auctionator.Config.Get(Auctionator.Config.Options.AUCTION_DURATION))
end
	...
	item:ContinueOnItemLoad(
		function()
			...
			if multiplier ~= 0 and vendorPrice ~= 0 then
				...
				self:SetUnitPrice(vendorPrice * multiplier + self:GetDeposit())
			end
		end
	)
end
...
function AuctionatorSaleItemMixin:GetPostButtonState()
	return self.itemInfo ~= nil and self.itemInfo.count > 0 and self.clickedSellItem and self.buyViewSetup and C_Item.DoesItemExist(self.itemInfo.location) and self.StackPrice:GetAmount() <= MAXIMUM_BID_PRICE and -- Sufficient money to cover deposit
		GetMoney() > self:GetDeposit() and -- Valid quantity
		...
		self.Stacks.NumStacks:GetNumber() * self.Stacks.StackSize:GetNumber() <= self.itemInfo.count and -- Positive price
		self.UnitPrice:GetAmount() > 0 and -- Have opted to ignore the throttle or searches on the client aren't throttled
...
function AuctionatorSaleItemMixin:GetConfirmationMessage()
	...
	if Auctionator.Utilities.IsVendorable(itemInfo) and vendorPrice * self:GetStackSize() * self:GetNumStacks() + self:GetDeposit() > math.floor(self.StackPrice:GetAmount() * self:GetNumStacks() * Auctionator.Constants.AfterAHCut) then
		return AUCTIONATOR_L_CONFIRM_POST_BELOW_VENDOR
	end
end

function AuctionatorSaleItemMixin:RequiresConfirmationState()
	return Auctionator.Config.Get(Auctionator.Config.Options.SELLING_CONFIRM_LOW_PRICE) and self:GetConfirmationMessage() ~= nil
end
...
function AuctionatorSaleItemMixin:SuccessfulPost(details)
	...
	if Auctionator.Config.Get(Auctionator.Config.Options.AUCTION_CHAT_LOG) then
		Auctionator.Utilities.Message(
			Auctionator.Selling.ComposeAuctionPostedMessage(
				{
...
function AuctionatorSaleItemMixin:DoNextItem(details)
	if (Auctionator.Config.Get(Auctionator.Config.Options.SELLING_AUTO_SELECT_NEXT) and IsValidItem(details.itemInfo.nextItem)) then
		-- Option to automatically select the next item in the bag view
		Auctionator.EventBus:Fire(self, Auctionator.Selling.Events.BagItemClicked, details.itemInfo.nextItem)
	end
end
...
function AuctionatorSaleItemMixin:SkipItem()
	if self.SkipButton:IsEnabled() then
		Auctionator.EventBus:Fire(self, Auctionator.Selling.Events.BagItemClicked, self.itemInfo.nextItem)
	end
end
