...
local function SoMCheck()
	local function CheckBuff(buffId, key, value)
		value = value or true
		if RXPCData and RXPCData[key] == nil and GetTime() - buffCheckTimer < 300 then
			local id = 0
			local n = 1
			while id do
			...
			for spellLvl, spells in pairs(addon.defaultSpellList[entry]) do
				if spellLvl <= level then
					for i, spellId in ipairs(spells) do
						...
						if names and rank and not (RXPCData.hardcore and addon.HCSpellList and addon.HCSpellList[spellId]) then
		...
		for unit, elements in pairs(unitscanList) do
			...
			if enabled then
				if not _G.unitscan_targets[unit] then
					-- _G.DEFAULT_CHAT_FRAME:AddMessage(_G. '<unitscan> +' .. unit)
					_G.ChatFrame7:AddMessage('UnitScan: ' .. _G.GREEN_FONT_COLOR_CODE .. unit .. ' added')
				end
				...
				if _G.unitscan_targets[unit] then
					...
					_G.ChatFrame7:AddMessage('UnitScan: ' .. _G.RED_FONT_COLOR_CODE .. unit .. ' removed')
				end
...
function addon.IsStepShown(step)
	return not (step.daily and RXPCData.skipDailies) and (RXPCData.northrendLM or not step.questguide) and addon.AldorScryerCheck(step) and addon.PhaseCheck(step) and addon.HardcoreCheck(step) and addon.SeasonCheck(step) and addon.XpRateCheck(step)
end
