...
hooksecurefunc(
	...
	function(self, msg, warnType, keepHidden, messageType)
		if _G.ScriptErrorsFrame:IsForbidden() then
			return
		end
			...
			for spellLvl, spells in pairs(addon.defaultSpellList[entry]) do
				if spellLvl <= level then
					for i, spellId in ipairs(spells) do
						...
						if names and rank and not (addon.settings.db.profile.hardcore and addon.HCSpellList and addon.HCSpellList[spellId]) then
...
local function OnTrainer()
	if not addon.settings.db.profile.enableTrainerAutomation then
		return
	end
...
local GossipGetNumActiveQuests = C_GossipInfo.GetNumActiveQuests or _G.GetNumGossipActiveQuests
local GossipGetNumAvailableQuests = C_GossipInfo.GetNumAvailableQuests or _G.GetNumGossipAvailableQuests
local GossipGetNumOptions = C_GossipInfo.GetNumOptions or _G.GetNumGossipOptions
local GossipSelectAvailableQuest = C_GossipInfo.SelectAvailableQuest or _G.SelectGossipAvailableQuest
local GossipGetActiveQuests = C_GossipInfo.GetActiveQuests or _G.GetGossipActiveQuests
local GossipSelectActiveQuest = C_GossipInfo.SelectActiveQuest or _G.SelectGossipActiveQuest
local GossipGetAvailableQuests = C_GossipInfo.GetAvailableQuests or _G.GetGossipAvailableQuests
...
function addon:QuestAutomation(event, arg1, arg2, arg3)
	...
	if not event then
		...
		elseif _G.QuestFrameRewardPanel and _G.QuestFrameRewardPanel:IsShown() or _G.QuestFrameCompleteButton and _G.QuestFrameCompleteButton:IsShown() then
	...
	elseif event == 'GOSSIP_SHOW' then
		...
		for i = 1, nActive do
			...
			if type(quests) == 'table' then
				...
				title, level, isTrivial, isComplete = select(i * 6 - 5, GossipGetActiveQuests())
			end
			-- print(title)
			-- print(quests[i])
			if addon.QuestAutoTurnIn(title) and isComplete then
...
function addon:OnEnable()
	...
	ProcessSpells()
	addon.GetProfessionLevel()
	local guide = addon.GetGuideTable(RXPCData.currentGuideGroup, RXPCData.currentGuideName)
	if not guide and addon.settings.db.profile.autoLoadStartingGuides then
		guide = addon.defaultGuide
		if addon.game == 'TBC' and (UnitLevel('player') == 58 and not guide.boost58) then
			guide = nil
		end
	end
...
function addon:PLAYER_ENTERING_WORLD()
	addon.isHidden = addon.settings and addon.settings.db.profile.hideGuideWindow or not (addon.RXPFrame and addon.RXPFrame:IsShown())
end
...
function addon:TRAINER_SHOW(...)
	...
	if not addon.trainerFrame then
		addon.trainerFrame = CreateFrame('Frame', 'RXPGuidesTrainerFrame', UIParent)
	end
...
function addon:GROUP_LEFT()
	if not addon.settings.db.profile.hideInRaid or (RXPCData and RXPCData.GA) or (addon.guide and addon.guide.farm) then
		return
	end
...
function addon.GetGuideTable(guideGroup, guideName)
	if guideGroup and addon.guideList[guideGroup] and guideName and addon.guideList[guideGroup][guideName] then
		...
		for unit, elements in pairs(unitscanList) do
			...
			if enabled then
				if not _G.unitscan_targets[unit] then
					if ChatFrame7 then
						ChatFrame7:AddMessage('UnitScan: ' .. _G.GREEN_FONT_COLOR_CODE .. unit .. _G.YELLOW_FONT_COLOR_CODE .. ' added')
					else
						_G.DEFAULT_CHAT_FRAME:AddMessage(_G.LIGHTYELLOW_FONT_COLOR_CODE .. '<unitscan> +' .. unit)
					end
				end
				...
				if _G.unitscan_targets[unit] then
					if ChatFrame7 then
						ChatFrame7:AddMessage('UnitScan: ' .. _G.RED_FONT_COLOR_CODE .. unit .. _G.YELLOW_FONT_COLOR_CODE .. ' removed')
					else
						_G.DEFAULT_CHAT_FRAME:AddMessage(_G.LIGHTYELLOW_FONT_COLOR_CODE .. '<unitscan> -' .. unit)
					end
				end
...
function addon.IsStepShown(step)
	return not (step.daily and RXPCData.skipDailies) and (addon.settings.db.profile.northrendLM or not step.questguide) and addon.AldorScryerCheck(step) and addon.PhaseCheck(step) and addon.HardcoreCheck(step) and addon.SeasonCheck(step) and addon.XpRateCheck(step)
end

function addon.SeasonCheck(step)
	if addon.settings.db.profile.SoM and step.era or step.som and not addon.settings.db.profile.SoM or addon.settings.db.profile.SoM and addon.settings.db.profile.phase > 2 and step['era/som'] then
		return false
	end
	return true
end
