...
function oGlowClassic:RegisterFilter(name, type, filter, desc)
...
do
	...
	function oGlowClassic.IterateFilters()
		return iter, nil, nil
	end
end

-- TODO: Validate that the display we try to use actually exists.
function oGlowClassic:RegisterFilterOnPipe(pipe, filter)
	...
	if(not oGlowClassicDB.EnabledFilters[filter]) then
		oGlowClassicDB.EnabledFilters[filter] = {}
	end
	oGlowClassicDB.EnabledFilters[filter][pipe] = true

	return true
end

oGlowClassic.IterateFiltersOnPipe = function(pipe)
...
function oGlowClassic:UnregisterFilterOnPipe(pipe, filter)
	argcheck(pipe, 2, 'string')
	argcheck(filter, 3, 'string')
	...
	if(ref) then
		for k, func in next, ref do
			if(func == filterTable) then
				table.remove(ref, k)
				oGlowClassicDB.EnabledFilters[filter][pipe] = nil
...
function oGlowClassic:GetNumFilters()
	return numFilters
end
