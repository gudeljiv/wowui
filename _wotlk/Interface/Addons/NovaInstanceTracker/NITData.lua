...
f:SetScript(
	...
	function(self, event, ...)
		...
		elseif (event == 'UNIT_RANGEDDAMAGE' or event == 'LOCALPLAYER_PET_RENAMED' or event == 'UNIT_PET' or event == 'UNIT_PET_TRAINING_POINTS') then
			local unit = ...
			if (unit and (event == 'UNIT_RANGEDDAMAGE' and unit ~= 'pet') or (event == 'UNIT_PET' and unit ~= 'player') or (event == 'UNIT_PET_TRAINING_POINTS' and unit ~= 'pet')) then
...
function NIT:combatLogEventUnfiltered(...)
	local timestamp, subEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, _, spellName = CombatLogGetCurrentEventInfo()
...
function NIT:enteredInstance(isReload, isLogon)
	...
	--end
	if (instance == true and ((instanceType == 'party') or (instanceType == 'raid') or (type == 'bg') or (type == 'arena'))) then
		...
		if (isGhost) then
			if (NIT.db.global.enteredMsg) then
				...
				NIT:print('Entered ' .. instanceName .. ' as ghost, not recording new instance. ' .. 'If you would like to force record a new instance click |HNITCustomLink:deletelast|h' .. texture .. '|h')
			end
		else
			if (not isReload) then
				...
				if (t.isPvp) then
					...
					C_Timer.After(
						...
						function()
							--local hourCount, hourCount24, hourTimestamp, hourTimestamp24 = NIT:getInstanceLockoutInfo();
							NIT:print('|HNITCustomLink:instancelog|h' .. msg .. '|h' .. '|HNITCustomLink:deletelast|h' .. texture .. '|h |HNITCustomLink:instancelog|h' .. L['enteredDungeon2'] .. '|h')
						end
					)
				elseif (raid) then
					C_Timer.After(
				...
				elseif (isLogon) then
					C_Timer.After(
						...
						function()
							--local hourCount, hourCount24, hourTimestamp, hourTimestamp24 = NIT:getInstanceLockoutInfo();
							NIT:print(
								'|HNITCustomLink:instancelog|h' ..
									string.format(L['loggedInDungeon'], instanceNameMsg, countMsg) .. '|h' .. ' |HNITCustomLink:deletelast|h' .. texture .. '|h |HNITCustomLink:instancelog|h ' .. L['loggedInDungeon2'] .. '|h'
							)
						end
					)
				else
					C_Timer.After(
						...
						function()
							...
							NIT:print(
								'|HNITCustomLink:instancelog|h' ..
									string.format(L['enteredDungeon'], instanceNameMsg, countMsg) .. '|h' .. '|HNITCustomLink:deletelast|h' .. texture .. '|h |HNITCustomLink:instancelog|h' .. L['enteredDungeon2'] .. '|h'
							)
						end
					)
				end
...
function NIT:showInstanceStats(id, output, showAll)
	...
	elseif
		(data.enteredTime and data.leftTime and data.enteredTime > 0 and (GetServerTime() - data.enteredTime) < 21600 and
	...
	if (output) then
		...
		elseif (output == 'say' or output == 'yell' or output == 'party' or output == 'guild' or output == 'officer' or output == 'raid') then
	...
	elseif (not NIT.db.global.statsOnlyWhenActivity or ((data.xpFromChat and data.xpFromChat > 0) or mobCount > 0 or tonumber(money) > 0)) then
		C_Timer.After(
			0.7,
			function()
...
function NIT:removeInstanceCount(instanceID)
	local char = UnitName('player')
	if (NIT.data.myChars[char] and NIT.data.myChars[char].instances and NIT.data.myChars[char].instances[instanceID] and NIT.data.myChars[char].instances[instanceID] > 0) then
		NIT.data.myChars[char].instances[instanceID] = NIT.data.myChars[char].instances[instanceID] - 1
	end
end
...
function NIT:parseGUID(unit, GUID, source)
	...
	if (GUID and doGUID and NIT.inInstance and (not string.match(source, 'combatlog') or GetServerTime() - NIT.inInstance > 2)) then
		...
		if (zoneID and zoneID > 0 and NIT.data.instances[1]) then
			...
			if (NIT.data.instances[2] and NIT.data.instances[2]['zoneID'] and NIT.data.instances[2]['zoneID'] == zoneID and not NIT.data.instances[1].zoneID) then
...
function NIT:getInstanceLockoutInfoString(char)
	...
	local countStringColorized = NIT.chatColor .. string.format(L['countStringColorized'], NIT.prefixColor, hourCount, NIT.chatColor, NIT.prefixColor, hourCount24, NIT.chatColor)
...
f:SetScript(
	...
	function(self, event, ...)
		...
		elseif (event == 'UI_INFO_MESSAGE' or event == 'UI_ERROR_MESSAGE') then
			local type, msg = ...
			if (msg == ERR_TRADE_BAG_FULL or msg == ERR_TRADE_TARGET_BAG_FULL or msg == ERR_TRADE_CANCELLED or msg == ERR_TRADE_TARGET_MAX_LIMIT_CATEGORY_COUNT_EXCEEDED_IS) then
				NIT:resetCurrentTradeData()
...
function NIT:doTrade()
	...
	if (playerMoney > 0) then
		if (NIT.db.global.showMoneyTradedChat) then
			NIT:print(
				'|HNITCustomLink:tradelog|h|cFF9CD6DE' ..
					L['gave'] .. '|r|h |r' .. NIT:getCoinString(playerMoney) .. NIT.chatColor .. ' |HNITCustomLink:tradelog|h|cFF9CD6DE' .. L['to'] .. '|r |c' .. classColorHex .. tradeWho .. NIT.chatColor .. '.|h',
				nil,
				nil,
				true,
				true
	...
	if (targetMoney > 0) then
		if (NIT.db.global.showMoneyTradedChat) then
			NIT:print(
				'|HNITCustomLink:tradelog|h|cFF9CD6DE' ..
					L['received'] .. '|r|h |r' .. NIT:getCoinString(targetMoney) .. NIT.chatColor .. ' |HNITCustomLink:tradelog|h|cFF9CD6DE' .. L['from'] .. '|r |c' .. classColorHex .. tradeWho .. NIT.chatColor .. '.|h',
				nil,
				nil,
				true,
				true
