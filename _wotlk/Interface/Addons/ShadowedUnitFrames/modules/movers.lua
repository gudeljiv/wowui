...
local function getValue(func, unit, value)
	...
	if (unitConfig[func .. unit] == nil) then
		unitConfig[func .. unit] = value
	end
	...
	configEnv =
		setmetatable(
		{
			GetRaidTargetIndex = function(unit)
				return getValue('GetRaidTargetIndex', unit, math.random(1, 8))
			end,
			GetLootMethod = function(unit)
				return 'master', 0, 0
			end,
			...
			UnitIsUnit = function(unitA, unitB)
				return unitB == 'player' and true or false
			end,
			...
			UnitIsPlayer = function(unit)
				return unit ~= 'boss' and unit ~= 'pet' and not string.match(unit, '(%w+)pet')
			end,
			UnitHealth = function(unit)
				return getValue('UnitHealth', unit, math.random(20000, 50000))
			end,
			UnitIsQuestBoss = function(unit)
			...
			UnitPowerMax = function(unit, powerType)
				if (powerType == Enum.PowerType.Rage or powerType == Enum.PowerType.Energy or powerType == Enum.PowerType.RunicPower or powerType == Enum.PowerType.LunarPower or powerType == Enum.PowerType.Maelstrom or powerType == Enum.PowerType.Insanity or powerType == Enum.PowerType.Fury or powerType == Enum.PowerType.Pain) then
					return 100
				elseif (powerType == Enum.PowerType.Focus) then
					return 120
				elseif (powerType == Enum.PowerType.ComboPoints or powerType == Enum.PowerType.SoulShards or powerType == Enum.PowerType.HolyPower or powerType == Enum.PowerType.Chi) then
					return 5
			...
			UnitInOtherParty = function(unit)
				return getValue('UnitInOtherParty', unit, math.random(0, 1) == 1)
			end,
			...
			UnitIsFriend = function(unit)
				return unit ~= 'target' and unit ~= ShadowUF.fakeUnits[unit] and unit ~= 'arena'
			end,
			...
			UnitAura = function(unit, id, filter)
				if (type(id) ~= 'number' or id > 40) then
					return
				end
...
local function OnEnter(self)
	...
	if (additionalText) then
		GameTooltip:AddLine(additionalText, 0.90, 0.90, 0.90, 1)
	end
	GameTooltip:Show()
end
