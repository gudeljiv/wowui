...
-- Consider Cata to be equivalent to Wotlk
...
local function cacheUserInventory(unit)
    ...
    for i=1,19 do
		-- In Cata pre-patch at least, this returns the mogged item ID when inspecting others.
		-- But GetInventoryItemLink works properly.
        --inventory[i] = GetInventoryItemID(unit, i)
		local itemLink = GetInventoryItemLink(unit, i)
		if itemLink then
			inventory[i] = GetItemInfoInstant(itemLink)
		else
			inventory[i] = GetInventoryItemID(unit, i)
		end
