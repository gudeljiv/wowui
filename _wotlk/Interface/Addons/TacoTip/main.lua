	...
	function(self)
		...
		if (TacoTipConfig.show_power_bar) then
			if (not TacoTipPowerBar) then
				...
				TacoTipPowerBar.updateTicker =
					NewTicker(
					POWERBAR_UPDATE_RATE,
					function()
	...
	if (itemLink and IsEquippableItem(itemLink)) then
		...
		if (TacoTipConfig.show_gs_items) then
			...
			if (gs and gs > 1) then
				...
				if (TacoTipConfig.show_gs_items_hs or IsModifierKeyDown() or playerClass == 'HUNTER' or (InspectFrame and InspectFrame:IsShown() and InspectFrame.unit and select(2, UnitClass(InspectFrame.unit)) == 'HUNTER')) then
