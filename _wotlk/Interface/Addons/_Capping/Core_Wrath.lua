	...
	do
		...
		do
			local function ReportBar(bar, channel)
				if not activeBars[bar] then
					return
				end
				if channel == 'INSTANCE_CHAT' and not IsInGroup(2) then
					channel = 'RAID'
				end -- LE_PARTY_CATEGORY_INSTANCE = 2
	...
	do
		...
		local function ScorePredictor(widgetInfo)
			if widgetInfo and (widgetInfo.widgetID == 3116 or widgetInfo.widgetID == 3117) then
				...
				if not dataTbl or not dataTbl.text then
					return
				end
				if prevTime == 0 then
					prevTime = GetTime()
	...
	if needsLocale[loc] then
		function core:LOADING_SCREEN_DISABLED()
			...
			self:Timer(
				0,
				function()
					-- Timers aren't fully functional until 1 frame after loading is done
					self:Timer(
						15,
						function()
