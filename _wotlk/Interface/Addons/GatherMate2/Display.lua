...
function Display:PlayerZoneChanged()
	...
	if GatherMate.phasing[newZone] then
		newZone = GatherMate.phasing[newZone]
	end
...
function Display:UpdateIconPositions()
	if not db.showMinimap or not Minimap:IsVisible() or not zone then
		return
	end
...
function Display:UpdateMiniMap(force)
	if not db.showMinimap or not Minimap:IsVisible() then
		return
	end
...
function Display.WorldMapDataProvider:RefreshAllData(fromOnShow)
	...
	if GatherMate.phasing[uiMapID] then
		uiMapID = GatherMate.phasing[uiMapID]
	end
...
--[[ Handy Notes WorldMap Pin ]] GatherMate2WorldMapPinMixin = CreateFromMixins(MapCanvasPinMixin)
