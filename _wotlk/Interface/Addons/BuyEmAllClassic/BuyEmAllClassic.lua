...
function BuyEmAll:OnLoad()
	...
	StaticPopupDialogs['BUYEMALL_CONFIRM2'] = {
		...
		button2 = NO,
		OnAccept = function(dialog)
			BuyMerchantItem(self.ConfirmNoItemLink)
		end,
		timeout = 0,
		hideOnEscape = true
	}
...
function BuyEmAll:CogsFreeBagSpace(itemID)
	...
	for theBag = 0, 4 do
		...
		if (theBag > 0) then -- 0 is always the backpack.
			local bagLink = GetInventoryItemLink('player', 19 + C_Container.ContainerIDToInventoryID(theBag)) -- Bag #1 is in inventory slot 20.
			if (bagLink) then
...
function BuyEmAll:MerchantItemButton_OnModifiedClick(frame, button)
	...
	if (MerchantFrame.selectedTab == 1) and (IsShiftKeyDown()) and not (IsControlKeyDown()) and not (ChatFrame1EditBox:HasFocus()) then
...
function BuyEmAll:UpdateDisplay()
	...
	elseif (self.AltCurrencyMode == true) then
		...
		if (self.AltCurrPrice[2] == nil) then
			BuyEmAllCurrencyAmt2:SetText()
		end
