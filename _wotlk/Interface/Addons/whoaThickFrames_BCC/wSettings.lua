...
local function loadDefaults()
	...
	if cfg.useBossFrames == nil then
		cfg.useBossFrames = true
	end
	if cfg.whoaTexture == nil then
		cfg.whoaTexture = true
	end
	if cfg.whoaTextureIndex == nil then
		cfg.whoaTextureIndex = 1
	end
	if cfg.noClickFrame == nil then
		cfg.noClickFrame = false
	end
	if cfg.SBTextureName == nil then
		cfg.SBTextureName = 'whoa'
	end
	if cfg.selectedTexture == nil then
		cfg.selectedTexture = 5
	end
	if cfg.frameMode == nil then
		cfg.frameMode = 'light'
	end
	if cfg.darkFrames == nil then
		cfg.darkFrames = false
	end
	if cfg.minimapZoom == nil then
		cfg.minimapZoom = true
	end
	if cfg.playerNameHide == nil then
		cfg.playerNameHide = false
	end
	if cfg.fontFamily == nil then
		cfg.fontFamily = 'SystemFont_Outline_Small'
	end
	if cfg.fontIndex == nil then
		cfg.fontIndex = 1
	end
	if cfg.playerFrameIndex == nil then
...
local function loadSettings()
	...
	if cfg.playerNameHide == true then
		whoaCheckButton3:SetChecked(true)
	end
	if cfg.reactColor == true then
		whoaCheckButton4:SetChecked(true)
	end
	if cfg.blizzColors == true then
	...
	if cfg.showPartyPets == true then
	...
	if cfg.noClickFrame == true then
		whoaCheckButton11:SetChecked(true)
	end
end
...
function SBTexturesDo()
	for i, v in pairs(
		...
		v:SetStatusBarTexture('Interface\\Addons\\' .. whoaAddon .. '\\media\\statusbar\\' .. cfg.SBTextureName)
	end
	for i, v in pairs({PlayerFrameMyHealPredictionBar}) do
		v:SetTexture('Interface\\Addons\\' .. whoaAddon .. '\\media\\statusbar\\' .. cfg.SBTextureName)
	end
	for i = 1, 4 do
...
local function fontStyleDo()
	...
	for i, v in pairs(
		...
		if v then
			v:SetFont('Fonts\\ARIALN.TTF', 14, 'OUTLINE')
		end
	end
end
...
function disableFramesClick()
	if (cfg.noClickFrame == true) then
		for i, v in pairs({PlayerFrame, PetFrame, TargetFrame, FocusFrame}) do
			v:SetMouseClickEnabled(false)
		end
	elseif (cfg.noClickFrame == false) then
		for i, v in pairs({PlayerFrame, PetFrame, TargetFrame, FocusFrame}) do
			v:SetMouseClickEnabled(true)
		end
	end
end
