...
function ItemRackOpt.UpdateInv()
	...
	for i = 0, 19 do
		_G['ItemRackOptInv' .. i]:CreateBeautyBorder(6)
		_G['ItemRackOptInv' .. i]:SetBeautyBorderTexture('Interface\\AddOns\\xVermin\\Media\\textureNormal')
		_G['ItemRackOptInv' .. i]:SetBeautyBorderColor(1, 1, 1, 1)
		...
		if ItemRackOpt.Inv[i].id ~= 0 then
			-- print(quality, GetItemQualityColor(quality))
			...
			-- end
			-- print('item', ItemRack.GetInfoByID(ItemRackOpt.Inv[i].id))
			name, texture = ItemRack.GetInfoByID(ItemRackOpt.Inv[i].id) --pass the button's ItemRack-style ID to a function that retrieves the texture for the item
			_, _, quality = GetItemInfo(name)

			r, g, b, hex = GetItemQualityColor(quality)
			_G['ItemRackOptInv' .. i]:SetBeautyBorderTexture('Interface\\AddOns\\xVermin\\Media\\textureWhite')
			_G['ItemRackOptInv' .. i]:SetBeautyBorderColor(r, g, b, 1)
		else
