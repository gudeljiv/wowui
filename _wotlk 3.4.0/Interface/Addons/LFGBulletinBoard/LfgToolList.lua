...
local function InviteRequestWithRole(gbbName, gbbDungeon, gbbHeroic, gbbRaid)
	if not GBB.DB.InviteRole then
		GBB.DB.InviteRole = 'DPS'
	end
...
function GBB.UpdateLfgTool()
	if LFGBrowseFrame.CategoryDropDown.selectedValue == 120 then
		return
	end
	...
	if LFGBrowseFrame.searching then
		return
	end
...
local function CreateItem(yy, i, doCompact, req, forceHight)
	...
	if req and req.name ~= nil then
		...
		local FriendIcon = (req.IsFriend and string.format(GBB.TxtEscapePicture, GBB.FriendIcon) or '') .. (req.IsGuildMember and string.format(GBB.TxtEscapePicture, GBB.GuildIcon) or '') .. (req.IsPastPlayer and string.format(GBB.TxtEscapePicture, GBB.PastPlayerIcon) or '')
...
function GBB.LfgRequestShowTooltip(self)
	for id in string.gmatch(self:GetName(), 'GBB.LfgItem_(.+)') do
		...
		if GBB.DB.EnableGroup and GBB.GroupTrans and GBB.GroupTrans[req.name] then
			...
			GameTooltip:AddLine(GBB.Tool.IconClass[entry.class] .. '|c' .. GBB.Tool.ClassColor[entry.class].colorStr .. entry.name)
