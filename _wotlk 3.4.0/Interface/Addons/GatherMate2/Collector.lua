...
function Collector:GasBuffDetector(b)
	if foundTarget or (prevSpell and prevSpell ~= gasSpell) then
		return
	end
...
function Collector:addItem(skill, what)
	...
	-- TODO: garrison ids
	if GatherMate.mapBlacklist[zone] then
		return
	end
	if GatherMate.phasing[zone] then
		zone = GatherMate.phasing[zone]
	end
	...
	if node_type == fishSpell or node_type == gasSpell then
		...
		if yw == 0 or yh == 0 then
			return
		end -- No zone size data
