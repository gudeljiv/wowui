	...
	{},
	{
		__index = function(tbl, index)
			...
			if (not aura) then
				...
				tbl[index] = func
				if (not tbl[index].group) then
					tbl[index].group = 'Miscellaneous'
				end
			end
...
function Indicators:OnLayoutApplied(frame)
	if (not frame.auraIndicators) then
		return
	end
	...
	for key, indicatorConfig in pairs(ShadowUF.db.profile.auraIndicators.indicators) do
		...
		indicator:CreateBeautyBorder(2)
...
for _, key in pairs(Indicators.auraFilters) do
...
local function checkSpecificAura(frame, type, name, texture, count, auraType, duration, endTime, caster, isRemovable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff)
	-- Not relevant
	if (not ShadowUF.db.profile.auraIndicators.auras[name] and not ShadowUF.db.profile.auraIndicators.auras[tostring(spellID)]) then
		return
	end
	...
	-- Only player auras
	if (auraConfig.player and not playerUnits[caster]) then
		return
	end
	...
	if (not indicator or not indicator.enabled) then
		return
	end
	-- Missing aura only
	if (auraConfig.missing) then
		return
	end

	-- Disabled on a class level
	if (ShadowUF.db.profile.auraIndicators.disabled[playerClass][name]) then
		return
	end
	-- Disabled aura group by unit
	if (ShadowUF.db.profile.units[frame.unitType].auraIndicators[auraConfig.group]) then
		return
	end
