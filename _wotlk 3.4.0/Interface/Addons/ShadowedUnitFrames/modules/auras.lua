...
function Auras:OnEnable(frame)
	...
	if (WoWClassic and LibClassicDurations and frame.unit == 'target' and ShadowUF.db.profile.units[frame.unitType].auras.buffs.approximateEnemyData) then
		...
		LibClassicDurations.RegisterCallback(
			frame,
			'UNIT_BUFF',
			function(event, unit)
...
local function updateButton(id, group, config)
	...
	if (not button) then
		...
		button.stack:SetFont('Interface\\AddOns\\ShadowedUnitFrames\\media\\fonts\\Google Sans.ttf', 8, 'OUTLINE')
	...
	button.stack:SetFont('Interface\\AddOns\\ShadowedUnitFrames\\media\\fonts\\Google Sans.ttf', math.floor((config.size * 0.40) + 0.5), 'OUTLINE')
...
local function updateTemporaryEnchant(frame, slot, tempData, hasEnchant, enchantId, timeLeft, charges)
	...
	if (tempData.has and tempData.enchantId == enchantId and (timeLeft < tempData.time and (tempData.time - timeLeft) < 750)) then
		return false
	end
...
local function renderAura(parent, frame, type, config, displayConfig, index, filter, isFriendly, curable, name, texture, count, auraType, duration, endTime, caster, isRemovable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff)
	...
	if (not (parent.overridelist[type] and (parent.overridelist[name] or parent.overridelist[strSpellID])) and not config.show[category] and (not config.show.relevant or (type == 'debuffs') ~= isFriendly)) then
		return
	end
...
local function scan(parent, frame, type, config, displayConfig, filter)
	if (frame.totalAuras >= frame.maxAuras or not config.enabled) then
		return
	end
...
local function anchorGroupToGroup(frame, config, group, childConfig, childGroup)
	...
	if (not childGroup.buttons[1]) then
		return
	end
