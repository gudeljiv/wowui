...
function M:ErrorFrameToggle(event)
	if not E.db.general.hideErrorFrame then
		return
	end
...
function M:COMBAT_LOG_EVENT_UNFILTERED()
	...
	if not announce then
		return
	end -- No announce-able interrupt from player or pet, exit.
...
function M:COMBAT_TEXT_UPDATE(_, messagetype)
	if not E.db.general.autoTrackReputation then
		return
	end
...
function M:MERCHANT_SHOW()
	if E.db.bags.vendorGrays.enable then
		E:Delay(0.5, B.VendorGrays, B)
	end

	if E.db.general.autoRepair == 'NONE' or IsShiftKeyDown() or not CanMerchantRepair() then
		return
	end
...
function M:QUEST_COMPLETE()
	if not E.db.general.questRewardMostValueIcon then
		return
	end
...
function M:BossBanner_ConfigureLootFrame(lootFrame)
	if not lootFrame.IconHitBox then
		return
	end
