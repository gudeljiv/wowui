...
local function UpdateItemLevel(button)
	...
	itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice = GetItemInfo(button.itemLink)
...
function mod:UpdateButton(event, button)
	...
	if link then
		if button.hasItem then
			...
			itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice = GetItemInfo(button.itemLink)
			texture, r, g, b, a, x1, x2, y1, y2, blendMode = GetBorder(itemRarity, isQuestItem, questId, isQuestActive, settings)
...
hooksecurefunc(addon, "CreateContainerFrame", function(self, name, isBank, bagObject)
	if AdiBagsContainer1 then AdiBagsContainer1:CreateBeautyBorder(8) end
	if AdiBagsContainer2 then AdiBagsContainer2:CreateBeautyBorder(8) end
end)