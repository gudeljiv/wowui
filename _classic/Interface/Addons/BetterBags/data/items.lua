...
function items:findBestFit(ctx, item, stackInfo, targets, movePairs)
	...
	-- so we return here.
	if item.slotkey == rootItem.slotkey then
		return
	end

	local myTarget = targets[item.slotkey]
		or {
			item = item,
	...
	local rootItemTarget = targets[rootItem.slotkey]
		or {
			item = rootItem,
	...
	for childSlotKey in pairs(stackInfo.slotkeys) do
		...
		if childItem.slotkey ~= item.slotkey then
			local childItemTarget = targets[childItem.slotkey]
				or {
					item = childItem,
...
function items:Restack(ctx, kind, callback)
	...
	for _, item in pairs(slotInfo:GetCurrentItems()) do
		if
			not item.isItemEmpty
			and ((targets[item.slotkey] and targets[item.slotkey].newNumber > 0) or not targets[item.slotkey])
		then
...
function items:DoRefreshAll(ctx)
	if not addon.Bags.Bank or not addon.Bags.Backpack then
		return
	end
...
function items:ItemAdded(newData, oldData)
	...
	if not oldData or (oldData and oldData.isItemEmpty) then
		return true
	end
	return false
end
...
function items:ItemRemoved(newData, oldData)
	if newData.isItemEmpty and (oldData and not oldData.isItemEmpty) then
		return true
	end
	if not oldData or (oldData and oldData.isItemEmpty) then
		return false
...
function items:ItemChanged(newData, oldData)
	...
	if
		C_NewItems.IsNewItem(newData.bagid, newData.slotid)
		and oldData
		and oldData.itemInfo
		and not oldData.itemInfo.isNewItem
	then
		return true
	end
	...
	if
		oldData
		and oldData.itemInfo
		and oldData.itemInfo.category == L:G("Recent Items")
		and not self:IsNewItem(oldData)
	then
		return true
	end
...
function items:ItemHashChanged(newData, oldData)
	if newData.isItemEmpty and not oldData then
		return false
	end
...
function items:LoadItems(ctx, kind, dataCache, equipmentCache, callback)
	...
	for _, currentItem in pairs(slotInfo:GetCurrentItems()) do
		...
		if items:ItemAdded(currentItem, previousItem) then
			...
			if
				not ctx:GetBool("wipe")
				and addon.isRetail
				and database:GetMarkRecentItems(kind)
				and kind == const.BAG_KIND.BACKPACK
			then
		...
		elseif items:ItemHashChanged(currentItem, previousItem) then
			...
			if
				...
				and addon.isRetail
				and database:GetMarkRecentItems(kind)
				and currentItem.itemInfo.itemID ~= previousItem.itemInfo.itemID
				and kind == const.BAG_KIND.BACKPACK
			then
				self:MarkItemAsNew(ctx, currentItem)
			end
...
function items:IsNewItem(data)
	...
	if
		(
			...
			and time() - self._newItemTimers[data.itemInfo.itemGUID] < database:GetNewItemTime()
		) or C_NewItems.IsNewItem(data.bagid, data.slotid)
	then
		return true
	end
	self._newItemTimers[data.itemInfo.itemGUID] = nil
