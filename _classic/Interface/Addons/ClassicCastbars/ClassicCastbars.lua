	...
	if cfg and cfg.enabled then
		if unitType == "nameplate" then
			...
			if UnitIsUnit("player", unitID) then
				return
			end -- personal resource display nameplate
			if not self.db.nameplate.showForPets and UnitIsOtherPlayersPet(unitID) then
				return
			end
		end
...
function ClassicCastbars:BindCurrentCastData(castbar, unitID, isChanneled, channelSpellID, isStartEvent)
	...
	if not isChanneled then
		spellName, _, iconTexturePath, startTimeMS, endTimeMS, _, castID, notInterruptible, spellID =
			UnitCastingInfo(casterUnit)
	else
		spellName, _, iconTexturePath, startTimeMS, endTimeMS, _, notInterruptible, spellID =
			UnitChannelInfo(casterUnit)
...
function ClassicCastbars:CheckAuraModifiers(castbar, unitID)
	if not isClassicEra and not isTBC then
		return
	end
	if not castbar or not castbar.isActiveCast then
		return
	end

	if castbar.isDefaultUninterruptible then
		return
	end -- always immune, no point checking further
...
function ClassicCastbars:CastPushback(unitID, castbar)
	if not castbar or not castbar.isActiveCast or not unitID then
		return
	end
	if pushbackBlacklist[castbar.spellName] then
		return
	end
	...
	if FindAuraByName(BARKSKIN, unitID, "HELPFUL") or FindAuraByName(FOCUSED_CASTING, unitID, "HELPFUL") then
		return
	end
...
local function BlizzNameplateCastbar_OnShow(frame)
	if frame:IsProtected() or frame:IsForbidden() then
		return
	end
	if not frame.unit or not strfind(frame.unit, "nameplate") then
		return
	end
...
function ClassicCastbars:UNIT_SPELLCAST_STOP(unitID, castID)
	...
	if not castbar.isChanneled and castbar.castID ~= castID then
		return
	end -- required for player
...
function ClassicCastbars:UNIT_SPELLCAST_FAILED(unitID, castID)
	...
	if castbar.isChanneled and castID ~= nil then
		return
	end
...
function ClassicCastbars:COMBAT_LOG_EVENT_UNFILTERED()
	local _, eventType, _, _, _, _, _, dstGUID, _, dstFlags, _, _, _, _, _, _, extraSchool =
			...
			if castbar:GetAlpha() > 0 then
				if UnitGUID(unitID) == dstGUID then
					castbar.Text:SetText(
						string.format(LOSS_OF_CONTROL_DISPLAY_INTERRUPT_SCHOOL, C_Spell.GetSchoolString(extraSchool))
					)
				end
			end
		end
...
ClassicCastbars:SetScript("OnUpdate", function(self, elapsed)
	for unit, castbar in next, activeFrames do
		if castbar.isActiveCast and castbar.value ~= nil and not castbar.isTesting then
			...
			castbar:SetValue(castbar.value)
			castbar.Timer:SetFormattedText(
				"%.1f",
				castbar.isChanneled and castbar.value or not castbar.isChanneled and castbar.maxValue - castbar.value
			)
			...
			if
				(castbar.isChanneled and castbar.value <= 0)
				or (not castbar.isChanneled and castbar.value >= castbar.maxValue)
			then
