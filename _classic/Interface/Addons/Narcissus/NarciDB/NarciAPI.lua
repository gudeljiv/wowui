...
function NarciAPI_GetItemStats(itemLocation)
    ...
    if not itemLocation or not DoesItemExist(itemLocation) then
        statsTable.strength = 0;
        statsTable.agility = 0;
        statsTable.stamina = 0;
        statsTable.intellect = 0;
        statsTable.spirit = 0;
        statsTable.GemIcon = "";
        statsTable.GemPos = "";
    ...
    local stats = GetItemStats(itemLink) or {};     --it seems attribute return 0 for items with postfix (leggings of Agility)

    statsTable.strength = stats["ITEM_MOD_STRENGTH_SHORT"] or 0;
    statsTable.agility = stats["ITEM_MOD_AGILITY_SHORT"] or 0;
    statsTable.stamina = stats["ITEM_MOD_STAMINA_SHORT"] or 0;
    statsTable.intellect = stats["ITEM_MOD_INTELLECT_SHORT"] or 0;
    statsTable.spirit = stats["ITEM_MOD_SPIRIT_SHORT"] or 0;
    statsTable.ilvl = GetCurrentItemLevel(itemLocation);
...
local function PickUpItemByItemID(itemID)
    --count > 0
...
NarciAPI.PickUpItemByItemID = PickUpItemByItemID;
...
local function SmartEditBoxFont(self)
	local str = self:GetText();
	local language = LanguageDetector(str);
    if language and EditBoxFont[language] then
        local height = self.fontHeight or EditBoxFont[language][2];
        if language == "CN" or language == "KR" then
            height = height - 1;
        end
		self:SetFont(EditBoxFont[language][1] , height, "");
	end
end
...
function NarciAPI_SmartEditBoxType(self)
...
function Narci_ItemParser_OnLoad(self)
    ...
    self.ClientInfo:SetText(locale.."  "..version.."."..build);
...
local function NarciAPI_InitializeModelLight(model)
    ...
    --Not ModelScene
    NarciClassicAPI.SetModelLight(model, true, false, - 0.44699833180028 ,  0.72403680806459 , -0.52532198881773, 0.8, 172/255, 172/255, 172/255, 1, 0.8, 0.8, 0.8);
end
