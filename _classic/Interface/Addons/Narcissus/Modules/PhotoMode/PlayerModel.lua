...
local SetModelLight = NarciClassicAPI.SetModelLight;
local GetModelLight = NarciClassicAPI.GetModelLight;
local SetGradientTexture = NarciClassicAPI.SetGradientTexture;
local SetModelCameraPosition = NarciClassicAPI.SetCameraPosition;
local SetModelCameraTarget = NarciClassicAPI.SetCameraTarget;
...
function LightControl:CopyLightToNewModel(newModel)
	if self.targetModelIndex and ModelFrames[self.targetModelIndex] then
		NarciClassicAPI.SetModelLightFromModel(newModel, ModelFrames[self.targetModelIndex]);
	end
end
...
function WeaponUpdator:GetInspectItemLinks(inspecteeGUID)
	local unit;

	if UnitGUID("player") == inspecteeGUID then
		unit = "player";
	elseif UnitGUID("target") == inspecteeGUID then
		unit = "target";
	end
	...
	if unit then
		for _, slotID in ipairs(TRANSMOG_SLOT_ORRDER) do
			items[slotID] = GetInventoryItemID(unit, slotID);
		end
	end
...
function WeaponUpdator:ProcessInspect(inspecteeGUID)
	...
	if model then
		...
		elseif model.TryOn then
			if not model.originalWeapons then
				model.originalWeapons = {};
			end

			model.originalWeapons[1] = transmogInfoList[16];
			model.originalWeapons[2] = transmogInfoList[17];
			model.originalWeapons[3] = transmogInfoList[18];
...
local function InitializeModelLight(newModel)
	local model = ModelFrames[ACTIVE_MODEL_INDEX];
	if LINK_LIGHT then
		NarciClassicAPI.SetModelLightFromModel(newModel, model);
	end
end
...
function NarciModelControl_HueSlider_OnValueChanged(self, value, isUserInput)
	if value ~= self.oldValue then
		...
		SetGradientTexture(NarciModelControl_SaturationSlider.Color, "HORIZONTAL", 1, 1, 1, xR, xG, xB);
		SetGradientTexture(NarciModelControl_BrightnessSlider.Color, "HORIZONTAL", 0, 0, 0, xR + (1-xSAT), xG + (1-xSAT), xB + (1-xSAT));
...
local function SetModelActive(index)
	...
	local VirtualToggle = ActorPanel.NameFrame.HiddenFrames.VirtualToggle;
...
function NarciGenericModelMixin:ResetCameraPosition()
	...
	SetModelCameraTarget(self, 0, 0, 0.8);
...
local function CreateRaceButtonList(self, buttonTemplate, buttonNameTable, numRow)
	...
	local raceID, raceInfo;
	for i = 1, numButtons do
		button = CreateFrame("BUTTON", buttonName and (buttonName .. i) or nil, self, buttonTemplate);
		raceID = buttonNameTable[i]

		if raceID ~= -1 then
			button:SetID(raceID);
			raceInfo = GetRaceInfo(raceID);
			button.Name:SetText(raceInfo.raceName);
