...
local TransitionAPI = NarciClassicAPI;
...
local function DeleteButton_OnEnter(self)
    self.Highlight:Show();
    NarciTooltip:NewText(self, L["Remove Visual Tooltip"], nil, nil, FREQUENTLY_USED_BUTTON_TOOLTIP_DELAY);
end
...
local function FavoriteButton_OnEnter(self)
    ...
    if not self:GetParent().PopUpFrame:IsShown() then
        if self.IsFav then
            NarciTooltip:NewText(self, L["Favorites Remove"], nil, nil, FREQUENTLY_USED_BUTTON_TOOLTIP_DELAY);
        else
            NarciTooltip:NewText(self, L["Favorites Add"], nil, nil, FREQUENTLY_USED_BUTTON_TOOLTIP_DELAY);
        end
    end
end
