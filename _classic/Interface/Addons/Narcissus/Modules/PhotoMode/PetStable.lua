...
local MAX_PETS = (NUM_PET_STABLE_PAGES and NUM_PET_STABLE_SLOTS and NUM_PET_STABLE_PAGES * NUM_PET_STABLE_SLOTS) or NUM_PET_STABLE_SLOTS or 200;
local MAX_ROW = 2;
local MAX_COL = 8;
...
local function Classic_SetPetModel(model,id)
    ClickStablePet(id);
...
function DataProvider:OnDataReceived()
    if self.numPets > 0 then
        ...
        MainFrame.numPages = numPages;
        MainFrame.Header.NoPetAlert:Hide();
    else
        MainFrame.numPages = 0;
        MainFrame.Header.NoPetAlert:Show();
        MainFrame.Header.NoPetAlert:SetText(Narci.L["PetStable Alert NoData"]);
    end
    self.filteredData = self.data;
...
function NarciPetStableMixin:Init()
    ...
    NarciClassicAPI.SetModelLight(PetModel, true, false, - 0.44699833180028 ,  0.72403680806459 , -0.52532198881773, 0.8, 172/255, 172/255, 172/255, 1, 0.8, 0.8, 0.8);

    local a = 0.1;
    ModelShadow:SetFogColor(a, a, a);
    NarciClassicAPI.SetModelLight(ModelShadow, false, false);
...
function NarciPetStableMixin:SelectPet(petIndex)
    Classic_SetPetModel(PetModel, petIndex);
    Classic_SetPetModel(ModelShadow, petIndex);
    PetModel:SetAnimation(0, 0);
    ModelShadow:SetAnimation(0, 0);
...
function NarciPetStableMixin:ConfirmPet()
    ...
    if petIndex then
        ACTOR_CREATED = true;
        local _, petName = GetStablePetInfo(petIndex);
        NarciPhotoModeAPI.OverrideActorInfo(TARGET_MODEL_INDEX, petName or "Pet", nil, self.petIcon);
        local newActor = self.targetModel;
        C_Timer.After(0, function()
            Classic_SetPetModel(newActor, self.selectedPetIndex);
        end);
    end
end
