...
local function ColorByDefault(button, index)
	if index % 2 == 1 then
		SetGradientTexture(button.BackgroundColor, "HORIZONTAL", 0, 0 ,0, 0.2, 0.2, 0.2);
	else
		SetGradientTexture(button.BackgroundColor, "HORIZONTAL", 0.1, 0.1 ,0.1, 0.3, 0.3, 0.3);
	end
end
...
function NarciTitleManagerSwitchMixin:IsInBound()
	return (TitleList:IsMouseOver(0, 0, -32, 0) or self:IsMouseOver());
end
