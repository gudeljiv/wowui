...
local REDIRECT_WATCH_FRAME_LINK_BUTTON;

local RedirectFrame = {};
RedirectFrame.hasOverwritten = false;

function RedirectFrame:RestoreFunctions()
    REDIRECT_WATCH_FRAME_LINK_BUTTON = false;
...
function RedirectFrame:OverrideFunctions()
    REDIRECT_WATCH_FRAME_LINK_BUTTON = true;
    ...
    if not self.hasOverwritten then
        ...
        if not self.isHooked then
            ...
            if WatchFrameLinkButtonTemplate_OnLeftClick then
                hooksecurefunc("WatchFrameLinkButtonTemplate_OnLeftClick", function(watchFrameLinkButton, mouseButton)
                    if REDIRECT_WATCH_FRAME_LINK_BUTTON and watchFrameLinkButton.type == "ACHIEVEMENT" and (not InCombatLockdown()) then
                        if AchievementFrame then
                            HideUIPanel(AchievementFrame);
                        end
                        local clickAgainToClose = true;
                        AttemptToOpenAchievement(watchFrameLinkButton.index, clickAgainToClose);
                    end
                end);
            end
        end
