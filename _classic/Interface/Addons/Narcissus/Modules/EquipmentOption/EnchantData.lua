...
do
    local classicExtraEnchantSlot = {
        1,   --Head
        3,   --Shoulder
        7,   --Legs
    };

    for _, slotID in ipairs(classicExtraEnchantSlot) do
        if not enchantData[slotID] then
            enchantData[slotID] = {};
        end
    end
end
...
local SUBCLASSID_ITEM_ENCHANT = 6;  --Different on Classic

local function IsItemEnchant(itemClassID, itemSubclassID)
    return itemClassID == CLASSID_CONSUMABLE and itemSubclassID == SUBCLASSID_ITEM_ENCHANT
end

function DataProvider:ApplyFilter(ownedOnly)
    self.filteredData = {};

    local itemList, numData = NarciAPI.GetBagItemsByItemType(IsItemEnchant);
