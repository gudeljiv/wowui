...
local function GetItemGemSubclassID(name, fallbackID)
    return Enum.ItemGemSubclass[name] or fallbackID
end

if Enum.ItemGemSubclass then
    for name, id in pairs(GemSubclassID) do
        GemSubclassID[name] = GetItemGemSubclassID(name, id);
    end
end

local GEM_CLASSID = 3;
local MATCH_SUBCLASSID_1 = -1;
local MATCH_SUBCLASSID_2 = -1;
local MATCH_SUBCLASSID_3 = -1;

local function MatchGem(itemClassID, itemSubclassiD)
    return itemClassID == GEM_CLASSID and (itemSubclassiD == MATCH_SUBCLASSID_1 or itemSubclassiD == MATCH_SUBCLASSID_2 or itemSubclassiD == MATCH_SUBCLASSID_3)
end
...
function DataProvider:SetSubset(dataSetID)
    local subclass1, subclass2, subclass3;

    if dataSetID == 1 then  --prismatic
        subclass1 = "Prismatic";
    elseif dataSetID == 2 then
        --Domination Shard
    elseif dataSetID == 3 then
    ...
    MATCH_SUBCLASSID_1 = subclass1 and GemSubclassID[subclass1] or -1;
    MATCH_SUBCLASSID_2 = subclass2 and GemSubclassID[subclass2] or -1;
    MATCH_SUBCLASSID_3 = subclass3 and GemSubclassID[subclass3] or -1;
end
...
function DataProvider:ApplyFilter(ownedOnly)
    ...
    local itemList, numData = NarciAPI.GetBagItemsByItemType(MatchGem);
    table.sort(itemList, function(a, b) return a > b end);

    self.filteredData = itemList;
...
--Unavailable Systems in Classic
local function ReturnNothing()
end

NarciAPI.GetCrystallicSpell = ReturnNothing;
NarciAPI.IsItemDominationShard = ReturnNothing;
