...
local C_Item = C_Item;
local GetSlotVisualInfo = C_Transmog and C_Transmog.GetSlotVisualInfo; --##
...
function NarciGameTooltipMixin:OnLoad()
    GenericTooltip = self;
    NarciAPI.NineSliceUtil.SetUpBackdrop(self, "phantom", -CLASSIC_PADDING, 20/255, 24/255, 28/255);
    NarciAPI.NineSliceUtil.SetUpBorder(self, "shadowHugeR0", -CLASSIC_PADDING);
    self.leftTexts = {};
    self.rightTexts = {};

    --TooltipTextureN
    for i = 1, 4 do
        self["LineTexture"..i]:SetSize(12, 12);
    end

    --self:SetCustomLineSpacing(2); --N/A in Classic
end
...
function NarciGameTooltipMixin:UpdateAlignment()
    --Subsequent tooltip Text uses the font of TextLeft2
    --In Classic, the default JustifyH is "CENTER" (if not specified in Font.xml)
    ...
    for i = 1, numLines do
        if not self.leftTexts[i] then
            ...
            if self.leftTexts[i] then
                self.leftTexts[i]:SetJustifyH("LEFT");
            end
        end
    end
end

function NarciGameTooltipMixin:ShowTooltip()
    self:Show();
...
function NarciGameTooltipMixin:AnchorToSlotButton(slotButton, offsetX, offsetY)
    ...
    offsetX = (offsetX or 0) + CLASSIC_PADDING;
    offsetY = (offsetY or 0) - CLASSIC_PADDING;
...
function NarciEquipmentTooltipMixin:OnLoad()
    ...
    self.transmogLocation = CreateFromMixins(TransmogLocationMixin or {});
