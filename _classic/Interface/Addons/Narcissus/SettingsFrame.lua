...
function FeaturePreview:SetupPreview(previewKey)
    if self[previewKey] and self.anchorTo then
        ...
        local scale = self[previewKey][6] or 0.75;
        self.frame:SetSize(w1 * scale, h1 * scale);
...
--Classic Features
local function EquipmentManagerToggle_SetState(self, state)
    NarciAPI.SetUsePaperDollEquipmentManager(state);

    local IS_WRATH = false;

    if IS_WRATH then
        if state and C_CVar.GetCVarBool("equipmentManager") then
            self.Description:SetText(L["Equipment Manager Description"].."\n\n"..(L["Equipment Manager Replacement"] or ""));
        else
            self.Description:SetText(L["Equipment Manager Description"]);
        end
    end
...
local function PaperDollFlyoutToggle_SetState(self, state)
    ...
    if isTextObject then
        ...
        if widgetType == "header" then
            if widgetData.text then
                ...
                print(widgetData.type, widgetData.level, widgetData.key)
            end
        else
...
function ClipboardUtil.scripts.OnEvent(self)
