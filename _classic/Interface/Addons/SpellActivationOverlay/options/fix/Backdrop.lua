...
function FixBackdropTemplateMixin:OnBackdropLoaded()
...
function FixBackdropTemplateMixin:OnBackdropSizeChanged()
	if self.backdropInfo then
		self:SetupTextureCoordinates();
	end
end

function FixBackdropTemplateMixin:GetEdgeSize()
...
function FixBackdropTemplateMixin:SetupTextureCoordinates()
...
function FixBackdropTemplateMixin:SetupPieceVisuals(piece, setupInfo, pieceLayout)
...
function FixBackdropTemplateMixin:SetBorderBlendMode(blendMode)
...
function FixBackdropTemplateMixin:HasBackdropInfo(backdropInfo)
	return self.backdropInfo == backdropInfo;
end

function FixBackdropTemplateMixin:ClearBackdrop()
	if self.backdropInfo then
		for pieceName in pairs(textureUVs) do
			local region = self[pieceName];
			if region then
...
function FixBackdropTemplateMixin:ApplyBackdrop()
	...
	local layout = {
		...
		setupPieceVisualsFunction = FixBackdropTemplateMixin.SetupPieceVisuals,
	};
...
function FixBackdropTemplateMixin:SetBackdrop(backdropInfo)
...
function FixBackdropTemplateMixin:GetBackdrop()
	if self.backdropInfo then
...
function FixBackdropTemplateMixin:GetBackdropColor()
	if not self.backdropInfo then
		return;
	end
	if self.Center then
...
function FixBackdropTemplateMixin:SetBackdropColor(r, g, b, a)
	if not self.backdropInfo then
...
function FixBackdropTemplateMixin:GetBackdropBorderColor()
	if not self.backdropInfo then
		return
	end
...
function FixBackdropTemplateMixin:SetBackdropBorderColor(r, g, b, a)
	if not self.backdropInfo then
		-- Ideally this would throw an error here but the old API just failed silently
		return;
	end
