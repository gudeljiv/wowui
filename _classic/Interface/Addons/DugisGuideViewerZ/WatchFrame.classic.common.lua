...
WF.collapseHeader.MinimizeButton:SetScript("OnClick", function()
	QuestWatchFrame.collapsed = not QuestWatchFrame.collapsed
...
local function GetBottomElement()
	if QuestWatchFrame:GetAlpha() == 0 then
		return nil
	end
	...
	for i=1, MAX_QUESTWATCH_LINES do
		...
		if not line:IsVisible() then return last end
		last = line
	end
end
...
local function IsWatchFrameVisible()
	...
	for i=1, MAX_QUESTWATCH_LINES do
		local line = _G["QuestWatchLine"..i]
		if line:IsVisible() then return true end
	end
end

local function UpdateWatchFrameMinimizer()
	local smallFrame = GetSmallFrame()
	if QuestWatchFrame.collapsed or (smallFrame and smallFrame.collapsed and DGV:IsSmallFrameAnchored()) then
...
WF.OnFrameUpdate = function()
	...
	if DGV:IsSmallFrameFloating() then
		...
		ObjectiveTrackerBackground:SetHeight(GetWorldQuestRealHeight() + 40)
	else
	...
	if not DGV:IncompatibleAddonLoaded() then
		...
		if DGV:IsSmallFrameFloating() then
			...
			QuestWatchFrame:SetPoint("TOPLEFT", ObjectiveTrackerBackground, "TOPLEFT", 50 + dX,  -18)
		else
			ObjectiveTrackerBackground:SetClampedToScreen(false)
			QuestWatchFrame:SetPoint("TOPLEFT", ObjectiveTrackerBackground, "TOPLEFT", 50 + dX,  -10)
		end
	end

	--Visibility of the QuestWatchFrame background
	...
	--We only change the alpha herre instead of showing/hiding because this frame is used to drag the QuestWatchFrame
	or DGV:IsSmallFrameAnchored() 	
	or not IsWatchFrameVisible()
	or QuestWatchFrame.collapsed
	then
	...
	--Visibility of the QuestWatchFrame
	if not DGV:IncompatibleAddonLoaded() then
	...
	--Position of the QuestWatchFrame
	if DGV:IsSmallFrameAnchored() and SmallFrame then
		ObjectiveTrackerBackground:SetPoint("TOPLEFT", SmallFrame.Frame, "BOTTOMLEFT", -10,  20) 
	end
	...
	if DGV:IsSmallFrameFloating() then
		...
		WF.collapseHeader:SetPoint("BOTTOMRIGHT", ObjectiveTrackerBackground, "TOPRIGHT", -9, -25)
		WF.collapseHeader.Title:SetText("")
...
function WF:Initialize()  
	...
	function GetWorldQuestRealHeight()
		...
		if lastBlock then
			...
			local questWatchFrameTop = QuestWatchFrame:GetTop() or questWatchFrameTopCached
	...
	function WF:GetWorldQuestMaxWidth()
		...
		for i=1, MAX_QUESTWATCH_LINES do
			local line = _G["QuestWatchLine"..i]
			if line:IsShown() and line:IsVisible() then
	...
	function WF:Load()
		objectiveTrackerUpdateReaction = RegisterFunctionReaction("QuestWatch_Update", nil, function()
			UpdateWatchFrameMinimizer()
			WF.DelayUpdate()
			...
			--QuestWatchFrame relies on frame visibility to hide lines when nothing is being watched, 
			--and may leave some visible if we affect QuestWatchFrame.Show (which we do).  Clean up
	...
	local function GetLineByQuestTitle(questTitle)
		for i=1, 100 do 
			...
			if not questLine or not questLine:IsShown() then
				return 
			end

			if questLine:GetText() == questTitle then
				return questLine, i
			end
		end
	end
		  ...
		  if questIndex then
			...
			if isWatched then
				...
				if questLine then
					...
					questLine.button:SetFrameLevel(65500)
				end
			end
		  end
		end
