...
function TaxiDB:Initialize()
	...
	DugisGuideUser.flightMastersWithNoData = DugisGuideUser.flightMastersWithNoData or {}	

	hooksecurefunc(UIErrorsFrame, "AddMessage", function(_, message)
		if message == ERR_TAXINOPATHS then
			local currentFlightmasterGUID = UnitGUID("npc")
			local id = DGV:GuidToNpcId(currentFlightmasterGUID)

			DugisGuideUser.flightMastersWithNoData[id] = true

			local firstWaypoint = DugisArrowGlobal.waypoints[1]
			if firstWaypoint and firstWaypoint.getFlightMasterData == true then
				DugisArrowGlobal.RefreshRoute(1)
			end
		end
	end)	
					...
					for i=1,NumTaxiNodes() do
						...
						if loopNPC and waypoint.flightMasterID==loopNPC then
                            if WorldFlightMapFrame == nil then
                                ...
                                if cont == 8 or cont == 9 or cont == 875 or cont == 876 or cont == 1 or cont == 2 then
                                    local activePool = FlightMapFrame.pinPools["FlightMap_FlightPointPinTemplate"]
                                    for pin in activePool:EnumerateActive() do 
	...
	function DGV:PLAYER_STOPPED_MOVING()
        ...
        if DugisGuideViewer.Modules.Taxi then
            DugisGuideViewer.Modules.Taxi.currentDestinationFMx = nil
        end
	end
	...
	function DGV:TAXIMAP_OPENED()
		...
		local cont, newContId = DGV.GetCurrentMapContinent_dugi()
		...
		then
			DugisFlightmasterDataTable[cont][key] =
			{
				...
				f = (globalData and globalData.f) or DGV.GetCurrentMapDungeonLevel(),
		...
		if recalulateRoute or (DGV:IsModuleLoaded("Guides") and DGV.actions[DugisGuideUser.CurrentQuestIndex] == "f") then
			...
			DugisGuideUser.flightMastersWithNoData = {}		
	...
	--[[local function StoreZoneDataPostTeleport()
		local currentLevel = DGV.GetCurrentMapDungeonLevel()
	...
	function TaxiDB:OnModulesLoaded()
		if authorMode then
			...
			function TaxiDB:OnMapChangedOrOpen()
				...
				local mapId = DGV:GetCurrentMapID() 
				local c = DGV.GetMapContinent_dugi(mapId)
