...
function Target:UpdateMode()
  DGV.DoOutOfCombat(function()
	...
	and not DugisGuideViewer:UserSetting(DGV_TRGET_BUTTON_FIXED_MODE) and DugisGuideViewer.chardb.EssentialsMode == 1
	then
		return
	end
	...
	if DugisGuideViewer:UserSetting(DGV_TRGET_BUTTON_FIXED_MODE) and DugisGuideViewer.chardb.EssentialsMode == 0 then
...
function Target:Initialize()
    ...
    function SetModel(npcId)
		...
		if mv and mv.npcDB and mv.npcDB[npcId] then
			GUIUtils.setCreature(modelFrame, npcId)
		elseif npcId then
			GUIUtils.setCreature(modelFrame, npcId)
		end
	...
	function Target:CreateFrame( )
        ...
        Target.Frame:SetScript("OnEnter", function()    
			...
			and not DugisGuideViewer:UserSetting(DGV_TRGET_BUTTON_FIXED_MODE) or DugisGuideViewer.chardb.EssentialsMode == 1 then 
          	  Target:ShowTooltip( )    
			end
        end)     
