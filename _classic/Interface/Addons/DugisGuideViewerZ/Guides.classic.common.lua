...
--Calendar_LoadUI and C_Calendar not present.  Todo: probably remove
	...
	local function TabInfoActivate_(self, isInThread)
		...
		if self.title then
			Main.title:SetText(L[self.title])
			Main.title:Show()
		end
        ...
        function UpdateWhatsNewText()
			...
			whatsNewFrame:SetJustifyH("P", "LEFT")
        ...
        if self.text == "Recent Guides" then
            if not whatsNewFrame then
                ...
                whatsNewFrame:SetFontObject("P", whatsNewContentFont) 
                ...
                whatsNewFrame:SetJustifyH("P", "CENTER")
                whatsNewFrame:SetJustifyV("P", "TOP")    
                
                whatsNewFrame:SetSpacing("P", 2)
                whatsNewFrame:SetFrameLevel(51)
                
                whatsNewFrame_EventHandler:SetFontObject("P", whatsNewContentFont)
                ...
                whatsNewFrame_EventHandler:SetJustifyH("P", "CENTER")
                whatsNewFrame_EventHandler:SetJustifyV("P", "TOP")    
                ...
                whatsNewFrame_EventHandler:SetSpacing("P", 2)
            ...
            local text = NPCJournalFrame:ReplaceSpecialTags(whatsNewText, nil, nil, nil, true)
			whatsNewFrame_EventHandler:SetJustifyH("P", "LEFT")
...
--[[	
	...
	function DGV:getIcon(objectiveType, i)
		...
		elseif isFirstaid then
			button.validTexture = "Interface\\Icons\\spell_holy_sealofsacrifice"
			return button.validTexture
		elseif isHerb then
    ...
    function DGV:UpdateRowVisualizations(i)
        ...
        if rowObj.currentTitle ~= CurrentTitle then
			...
			local icon = DGV:getIcon(DGV.actions[i], i)	
			if icon then
				frame.Button:SetNormalTexture(icon)	
			end
