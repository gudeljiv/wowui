...
local MAJOR, MINOR = "HereBeDragons-2.0-Dugis", 9
...
if not oldversion or oldversion < 7 then
    ...
    local function processMap(id, data, parent)
		...
		if data.mapType ~= Enum.UIMapType.Orphan then
            ...
            if not HereBeDragons.mapName2MapIds[data.name] then
                HereBeDragons.mapName2MapIds[data.name] = {}
            end
            
            local mapName2MapIds_ = HereBeDragons.mapName2MapIds[data.name]
            
            mapName2MapIds_[#mapName2MapIds_ + 1] = id
...
-- @param allowOutOfBounds Allow coordinates to go beyond the current map (ie. outside of the 0-1 range), otherwise nil will be returned
-- @param useMapIdFromWorldMapFrame: if set to true currentPlayerUIMapID will not be used. Instead value from WorldMapFrame:GetMapID() will be used.
-- @return x, y, uiMapID, mapType
function HereBeDragons:GetPlayerZonePosition(allowOutOfBounds, useMapIdFromWorldMapFrame)
	local currentPlayerUIMapID_ = currentPlayerUIMapID
    ...
    --Fix for Azeroth (doesn't exist in original HBD)
	if currentPlayerUIMapID_ == 947 then
		x, y = HereBeDragons:GetAzerothWorldMapCoordinatesFromWorld(x, y, instanceID, allowOutOfBounds)
	else
		x, y = self:GetZoneCoordinatesFromWorld(x, y, currentPlayerUIMapID_, allowOutOfBounds)
	end
	
    if not x or not y then return nil, nil, nil, nil end

    if x and y then
        return x, y, currentPlayerUIMapID_, currentPlayerUIMapType
    end
