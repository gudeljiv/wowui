...
function A:SlashCommand(args)
    ...
    elseif command == "login" then
        ...
        if AO_loginMessage then
            A:Print("Login message enabled")
        else
            A:Print("Login message disabled")
        end
    elseif command == "quest" then
        AutoOpenQuestItems = not AutoOpenQuestItems
        if AutoOpenQuestItems then
            A:Print("Auto opening quest items.")
        else
            A:Print("Not auto opening quest items.")
        end
    else
        A:Print("Commands")
    ...
    for S = 1, GetContainerNumSlots(B) do
        ...
        if itemLink then
            ...
            if typeid ~= TYPEID_ARMOR and typeid ~= TYPEID_WEAPON then
                if
                    lootable and not locked and not string.find(itemLink:lower(), "lockbox") and
                        not string.find(itemLink, "Junkbox") and
                        not AOBL[itemName]
                 then -- make sure its not a lockbox
