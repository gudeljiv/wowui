...
L.Website					= "Следите за новостями и обновлениями MysticalOS - |cFF73C2FBhttps://allmylinks.com/mysticalos|r. Спасибо за годы поддержки!"
...
L.OTabTools					= "Инструменты"--Например, кнопка таймера перерыва, проверки прочности, проверки задержки, проверки ключей М+ и т.п.
...
L.Mod_Reset					= "Настройки по умолчанию"
L.Reset 					= "Сброс"
L.Import					= "Импорт"
...
L.UIGroupingOptions			= "Параметры группировки интерфейса (изменение этих параметров требует перезагрузки интерфейса для любого уже загруженного мода)"
...
L.LogDelves					= "Актуальный уровень Вылазок"--Только для актуальной версии WoW
L.LogChallenges				= "Особые испытания актуального уровня (такие как Торгаст, Видения и Башня Магов)"--Только для актуальной версии WoW
...
L.WorldBossNearAlert		= "Воспроизводить звук проверки готовности и мигать иконкой приложения, когда неподалеку атакуют нужных Вам боссов вне подземелья"
...
L.RaidWarning_Header		= "Параметры предупреждений для рейда"
...
L.ColorResetted 			= "Настройки цвета для этого поля были сброшены."
...
L.ShowAllVersions	 		= "Показывать версии Босс Мода для всех членов группы во фрейме чата при проверке версий<br/>Если отключено, все равно отображается устаревшая/текущая сводка"
...
L.CheckGear					= "Показывать предупреждение, когда во время пулла уровень Вашей экипировки намного ниже, чем максимальный (40+), а также если у Вас пустой слот оружия"
...
L.RaidDiffChangedAlert		= "Показывать сообщение об изменении сложности рейда"
L.RaidDiffChangedRaidOnly	= "Показывать сообщение об изменении сложности рейда только при нахождении в рейдовой группе<br/>Пояснение: при присоединении к рейду информация о сложности не будет отображаться, если рейд ещё не преобразован в рейдовую группу"
L.DungeonDiffChangedAlert	= "Показывать сообщение об изменении сложности подземелья"

L.Area_BugAlerts			= "Параметры оповещений об ошибках"
L.BadTimerAlert				= "Показывать сообщение в чате, когда DBM обнаруживает неверный таймер с погрешностью не менее 1 секунды"
...
L.Bar7CustomInline			= "Использовать пользовательский встроенный значок '!'"
--Timer Example Texts
L.CBTGeneric						= "Общий"
...
L.Bar_EnlargeTime			= "Увеличение полосы, когда время меньше: %d"
L.Bar_AppearTime			= "Скрывать полосу, когда время больше: %d"
L.Bar_HideLongBars			= "Скрыть полосы, пока они не станут ниже порогового значения"
...
L.Area_SpamFilter			= "Параметры фильтра спама"
...
L.SpamSpecInformationalOnly			= "Заменить все текстовые и голосовые оповещения на специальные объявления (требуется перезагрузка интерфейса). Оповещения по-прежнему будут отображаться и воспроизводить звук, но станут общими и менее директивными"
...
L.SpamSpecRoleGTFO					= "Не показывать 'GTFO' оповещения (никакого текста или звука)"
...
L.HideBlizzardTimeline				= "Скрывать временную шкалу рейда Blizzard во время сражений с боссами"
...
L.Tab_PrivacyMidnight 		= "Конфиденциальность сообщений"
...
L.EnableWBSharing 			= "Делиться, когда Вы атакуете или побеждаете мирового босса со своей гильдией или друзьями из Battle.net, которые находятся в одном игровом мире"
...
L.NPAuraText				= "Показывать текст таймера на иконках индикаторов здоровья"
L.NPAuraSize				= "Размер иконок в пикселях (в квадрате): %d"
L.NPIcon_BarOffSetX 		= "Смещение иконки X: %d"
L.NPIcon_BarOffSetY 		= "Смещение иконки Y: %d"
L.NPIcon_GrowthDirection 	= "Направление роста иконки"
L.NPIcon_Spacing		 	= "Расстояние между иконками: %d"
L.NPIcon_MaxTextLen		 	= "Максимальная длина текста: %d"
L.NPIconAnchorPoint		 	= "Место крепления иконки"
L.NPDemo					= "Тест (примечание: Вы должны находиться рядом с индикаторами здоровья)"
...
L.NPIcon_GlowBehavior 		= "Режим свечения иконки"
L.NPIcon_CastGlowBehavior 	= "Режим свечения значка произнесения заклинания"
L.NPIcon_GlowNone			= "Никогда не подсвечивать иконки"
L.NPIcon_GlowImportant		= "Подсвечивать иконки важных восстановлений заклинаний/кастов, которые заканчиваются"
L.NPIcon_GlowAll			= "Подсвечивать все иконки восстановлений заклинаний/кастов, которые заканчиваются"
L.NPIcon_GlowTypeCD			= "Тип свечения иконки восстановления"
L.NPIcon_GlowTypeCast		= "Тип свечения иконки произнесения заклинания"
