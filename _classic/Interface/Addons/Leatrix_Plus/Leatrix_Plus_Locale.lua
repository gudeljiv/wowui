...
L["Drag to set the cluster scale.|n|nNote: Adjusting the cluster scale affects the entire cluster including frames attached to it such as the buffs frame and objectives tracker.|n|nIt will also cause the default UI right-side action bars to scale when you login.  If you use the default UI right-side action bars, you may want to leave this at 100%."] = "Arrastra para establecer la escala del clúster.|n|nNota: El ajuste de la escala del clúster afecta a todo el clúster, incluidos los marcos adjuntos, como el marco de mejoras y el rastreador de objetivos.|n|nTambién hará que las barras de acción del lado derecho de la interfaz de usuario predeterminada se escalen cuando inicia sesión. Si usa las barras de acción del lado derecho de la interfaz de usuario predeterminada, es posible que desee dejar esto al 100%."
...
L["Home"] = "Principal"
L["Hover the pointer over a button."] = "Pasa el puntero sobre un botón."
...
L["If checked, addon buttons will be hidden while the pointer is not over the minimap."] = "Si está seleccionado, los botones de addon se ocultarán mientras el puntero no esté sobre el minimapa."
...
L["If checked, you will be able to change the position and scale of the player frame, target frame and timer bar.|n|nNote that enabling this option will prevent you from using the default UI to move the player and target frames."] = "Si está seleccionado, podrá cambiar la posición y la escala del marco del jugador, el marco del objetivo y la barra de tiempo.|n|nTenga en cuenta que habilitar esta opción le impedirá usar la interfaz de usuario predeterminada para mover los marcos de jugador y del objetivo."
...
L["If checked, you will be able to set some additional rules for when your character is automatically dismounted."] = "Si está seleccionado, podrá establecer algunas reglas adicionales para cuando tu personaje se desmonte automáticamente."
